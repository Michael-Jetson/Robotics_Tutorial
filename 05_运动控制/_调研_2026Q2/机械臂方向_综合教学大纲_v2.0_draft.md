...
> **数据基础**: 100+ 机械臂开源项目源码级分析 + 7 个调研 agent + 2 个审计 agent (2026-05-24);课程主线按 2026-05 前后版本锁定 Pinocchio 3.9.0 / Drake v1.52 / MoveIt2 Kilted / **cuRoboV2** / **cuMotion**,具体 API 与版本特性以各项目当前官方文档和 release notes 为准。
> **总投入**: 已完成第一篇 P/M 系列 17 章（P01-P02 + M01-M15）~24 周 + 第二篇 F 系列 10 章 ~31-33 周 + 第三篇 D 系列 10 章 ~25-27 周,合计 37 章 ~80-84 周;另有第四篇前沿专题 8 章（F11/F12/D11/D12/M16 + 新增 F11_触觉控制/M16_接触丰富操作优化/F12_端口Hamilton）~18-25 周,可按篇独立选修。

---

## 快速路径（Quick-Start Track, ~16-17 周）

不是每个人都需要完整 80-84 周核心课程（若加前沿专题约 98-109 周）。以下最小路径让你获得**端到端 pick-and-place** 能力：
...
| **F11_触觉控制** (v2.0 新增) | ❌ | ⚠️ DIGIT 360 | TACTO 仿真可替代 |
| **M16_接触丰富操作优化** (v2.0 新增) | ⚠️ GPU加速 | ❌ 仿真优先 | Drake / MuJoCo |
| **F12_端口Hamilton** (v2.0 新增) | ❌ | ⚠️ 推荐真机验证 | Pinocchio + QP |
...
| 🔬 | 研究前沿 | 规划草案 F12(可微仿真), D06(波变量), 规划草案 D12(VLA), 规划草案 M16(灵巧手), **F11_触觉控制**, **M16_接触丰富操作优化**, **F12_端口Hamilton** |
...
└──────────────────────────────────────────────────────────────────┘
         ▼
┌──────────────────────────────────────────────────────────────────┐
│ 第二篇: 力控与柔顺控制 (F系列, F01-F10, ~31-33 周)                │
│  Part 1  力控基础 (F01-F05, 9-10 周)                              │
│  Part 2  高级力控 (F06-F08, 11-12 周)                             │
│  Part 3  学习型力控 (F09-F10, 11-12 周)                           │
└──────────────────────────────────────────────────────────────────┘
         ▼
┌──────────────────────────────────────────────────────────────────┐
│ 第三篇: 双臂协调与遥操作 (D系列, D01-D10, ~25-27 周)              │
│  Part 1  双臂协同理论与规划 (D01-D04, 9-10 周)                   │
│  Part 2  遥操作理论与数据采集 (D05-D08, 8 周)                    │
│  Part 3  系统集成与综合实战 (D09-D10, 8-9 周)                    │
└──────────────────────────────────────────────────────────────────┘
         ▼
┌──────────────────────────────────────────────────────────────────┐
│ 第四篇: 前沿专题 (F11/F12/D11/D12/M16 + 新增 F11_触觉控制/M16_接触丰富操作优化/F12_端口Hamilton, 18-25 周) │
└──────────────────────────────────────────────────────────────────┘
```
...
| **F11_触觉控制** (v2.0 新增) | F01、F04、D08 | 触觉传感器原理、阻抗工程、数据采集 |
| **M16_接触丰富操作优化** (v2.0 新增) | M05、M08、F07 | 凸优化/NLP 建模、轨迹优化、WBC/QP |
| **F12_端口Hamilton** (v2.0 新增) | F02、F06 | 无源性理论、能量罐方法、端口哈密顿系统 |
...
| **SwRI 工程派** | MoveIt2 Kilted + ros2_control + **cuMotion** | 2025-26: MoveIt2/MTC 与 ros2_control 随 ROS 2 发行版持续演进;**cuMotion** GPU 规划器插件正式集成;重点关注 MoveGroup/MoveItCpp、PlanningScene、MTC、ros2_control 异步硬件组件与控制器链,具体能力以官方 release notes 为准 |
| **GPU 加速派** (新增) | **cuRoboV2** + cuMotion + pRRTC | 2025-26: cuRoboV2 B-spline+GPU TSDF 99.7% 成功率 48-DoF (arXiv 2603.05493);pRRTC GPU RRT-Connect **165x 加速** (arXiv 2503.06757);GCS 凸集图全局最优保证 (Science Robotics) |
| **触觉+安全** (新增) | **DIGIT 360** + ISO 10218-2:2025 | 2025-26: DIGIT 360 全指尖 18+ 模态传感器;ISO 10218-2:2025 29 体区 PFL/SSM 显式;OSCBF 操作空间数百约束实时 (arXiv 2503.06736) |
...
13. **HJCD-IK** (v2.0 新增): 比 cuRobo 快 1.06-1.93x 的高性能 IK 求解器 (arXiv 2510.07514)
14. **GeoFIK** (v2.0 新增): 螺旋理论 Franka 几何 IK (arXiv 2503.03992)
...
- **核心知识点:**
  ...
  11. **pRRTC** (v2.0 新增): GPU 并行 RRT-Connect **165x 加速** (arXiv 2503.06757)
  12. **SBAMP** (v2.0 新增): RRT*+Lyapunov 自适应混合采样规划 (arXiv 2511.12022)
- **关键项目:**
...
- **核心知识点:**
  ...
  11. **GCS (Graph of Convex Sets)** (v2.0 新增): 凸集图全局最优保证 (Science Robotics),构型空间凸分解 + 混合整数规划
  12. **拓扑驱动并行轨迹优化** (v2.0 新增): 多同伦类逃逸局部极小 (arXiv 2401.06021)
- **关键项目:**
...
- **核心知识点:**
  ...
  11. **cuRoboV2** (v2.0 新增): B-spline+GPU TSDF 99.7% 成功率 48-DoF (arXiv 2603.05493);cuRobo 工业扩展 <50ms 规划 (arXiv 2508.04146)
  12. **cuMotion MoveIt2 插件** (v2.0 新增): 生产级 GPU 规划器正式集成 MoveIt2,与 Jazzy/Kilted 兼容
  13. **VAMP-MR 多臂** (v2.0 新增): AAAI 2026 多臂 SIMD 加速 10-100x
- **关键项目:**
...
- **周数**：2-3 周 (**v2.0 审计修正**: v1.2 大纲 1 周,实际内容含 ros2_controllers 源码深度精读需 3 周,修正为 2-3 周)
...
**一句话**：掌握变阻抗(VIC)、能量罐/KB 条件无源性保证、动量观测器碰撞检测与 ISO 10218/15066 安全。

- **周数**：4-5 周 (**v2.0 审计修正**: v1.2 大纲 3 周,实际内容含端口 Hamilton 展开需 5 周,修正为 4-5 周)
...
- **核心知识点**：
  ...
  6. ISO 15066:2016 力/压强限值表与工程计算; **ISO 10218-1:2011 / ISO 10218-2:2025 工业机器人安全要求** — 29 体区划分、PFL(Power and Force Limiting)/SSM(Speed and Separation Monitoring) 显式规范、SRS/SLS/STO 安全额定监控功能; 风险评估方法论 (ISO 12100); **OSCBF 操作空间安全屏障** (arXiv 2503.06736) 数百约束实时; **动能限制 CBF** (arXiv 2411.02186) 最小侵入阻尼; **层级 CBF** (arXiv 2505.16055) 敏感部位更强保护; **SafePR** (arXiv 2501.17773) 72 次碰撞 <275ms 反应; DeLaN 碰撞检测 (RCM 2026) 深度拉格朗日网络
  ...
  11. **构造性稳定与 CBF 几何→能量域统一趋势** (2025-2026)
...
### F09 学习型力控——RL+阻抗 / Diffusion Policy / SERL / MPC力控 ⭐

**一句话**：将 RL/模仿学习/MPC 与阻抗控制结合——变阻抗 RL、Diffusion Policy、SERL/HIL-SERL 接触丰富操作,以及 MPC 辅助力控前沿。

- **周数**：5 周 (**v2.0 审计标注**: v1.2 大纲 5 周,实际已写内容仅覆盖约 3 周,**需扩充 MPC/优化视角**——补充 MPC 辅助力控、贝叶斯优化调阻抗、可微仿真+力控等内容以达到 5 周实质)
...
- **核心知识点**：
  ...
  11. **MPC 辅助力控** (v2.0 新增): 在线辨识→MPC 力矩前馈、MPPI 力控采样式方案、MuJoCo 全身 MPC iLQR (arXiv 2503.04613)、双臂 MPC Bilevel+Bezier+预测导纳 (arXiv 2410.22910)
  12. **贝叶斯优化调阻抗** (v2.0 新增): Bayesian Optimization 自动调节 (K_d, D_d, M_d) 参数、高效采样避免真机大量试验
  13. **可微仿真+力控** (v2.0 新增): 短视界可微展开生成力控梯度 + 长视界 RL/MPC 混合、Barrier Lyapunov 变阻抗误差有界保证 (Nonlinear Dynamics 2026)、Certified Gaussian Manifold 稳定流形采样 (arXiv 2511.16330)、扩散阻抗学习 Transformer 生成阻抗 (arXiv 2509.19696)、FILIC 力引导模仿双环 (arXiv 2509.17053)、高阶有限时间 GMO 外力估计 (IJRNC 2025)
...
- **周数**：5 周 (**v2.0 审计标注**: v1.2 大纲 5 周,实际已写内容仅覆盖约 2 周,**需扩充 capstone 内容**——补充变阻抗三模式完整实现、Sim2Real 迁移流程、YAML DSL 任务定义、性能基准量化)
- **前置依赖**：F01-F09 全部
- **核心知识点**：
  1. 系统架构：Pinocchio 动力学 + ros2_control 插件 + MuJoCo 仿真后端
  2. 控制器插件工厂：阻抗/导纳/力位混合三模式运行时切换
  3. 安全层：力矩限幅 + 动量观测器碰撞检测 + 能量罐无源性
  4. 变阻抗模块：固定参数/在线调参/RL 策略三种模式 (**需完整实现三模式切换逻辑**)
  5. 力传感器抽象：F/T 标定 + 重力补偿 + 低通滤波流水线
  6. 任务定义 DSL：YAML 配置任务空间目标/约束帧/阻抗参数 (**需完整 DSL 规范与解析器**)
  7. Sim2Real 迁移：MuJoCo → libfranka/UR 参数映射与 gap 处理 (**需完整迁移流程文档**)
  8. 性能基准：1kHz 热路径延迟分析、力跟踪带宽测量 (**需量化基准测试框架**)
...
> **F01-F10 总计 31-33 周** | Part 1 力控基础 9-10 周 (F05 由 1→2-3 周) · Part 2 高级力控 11-12 周 (F06 由 3→4-5 周) · Part 3 学习型力控 10 周 (F09/F10 需扩充 MPC 视角和 capstone)

---

# 第三篇：双臂协调与遥操作 (D系列, 25-27周)

> **完成本篇后你能做什么**：设计并部署双臂协调操作系统，掌握遥操作理论(波变量/TDPA)与现代数据采集管线(ALOHA/UMI/LeRobot)，能从零搭建双臂遥操作→模仿学习→自主执行的端到端闭环。

## Part 1: 双臂协同理论与规划 (9-10周)
...
- **周数**：3-4周 (**v2.0 审计修正**: v1.2 大纲 2 周,实际内容含 Bilevel/GCS/MODAP 等高级主题需 4 周,质量高但膨胀,修正为 3-4 周)
...
- **核心知识点**：
  ...
  7. 波变量补偿器：2025年三通道最新设计 (**v2.0 审计标注**: 需补充 2025 年三通道最新设计的完整实现——数据驱动波变量 LSTM+被动性验证 (arXiv 2512.08436)、5G 时延场景适配)
  8. 位置漂移(Position Drift)问题与补偿
  9. 多自由度波变量扩展(6-DOF力/速度)
  10. C++实现：波变量编码/解码器与网络传输模块
  11. **数据驱动波变量** (v2.0 新增): LSTM+被动性验证 (arXiv 2512.08436) 在变时延通道上的自适应波变量设计
...
- **核心知识点**：
  ...
  11. **无力反馈系统稳定性分析** (v2.0 审计标注): 需补充——无力反馈遥操作系统的稳定性边界分析、CBF 共享自主安全区→控制权切换 (arXiv 2511.19869)、BarrierIK CBF 嵌入 IK (ICRA 2026)、四通道双边无力传感器方案 (arXiv 2507.06174)
...
- **周数**：3周 (**v2.0 审计修正**: v1.2 大纲 4 周,实际内容约 3 周,已按实际收缩调整)
...
- **周数**：3周 (**v2.0 审计修正**: v1.2 大纲 4 周,实际内容约 3 周,透明度未量化——需后续补充定量透明度评测指标)
- **前置依赖**：D01-D09全部
- **核心知识点**：
...
> 原有 F11/F12/D11/D12/M16 为规划草案,尚未拆分独立文件;v2.0 新增 F11_触觉控制/M16_接触丰富操作优化/F12_端口Hamilton 三章正式规划,建议至少完成第一至三篇核心内容后再进入。
...
## F11_触觉传感与触觉控制 (v2.0 正式规划) ⭐⭐⭐⭐

> DIGIT 360 全指尖多模态传感、触觉 MPC、触觉条件化策略、ISO 10218-2:2025 触觉安全集成。

**一句话** — 从"有力觉的手"到"有触觉的手指"：DIGIT 360 驱动的触觉闭环控制与 ISO 安全合规。

| 属性 | 值 |
|------|----|
| **周数** | 2-3 周 |
| **前置依赖** | F01 (力控导论)、F04 (笛卡尔阻抗工程)、D08 (数据采集)、F11 (触觉感知基础, 推荐) |
| **共享标记** | ⚪ 部分共享（触觉传感在灵巧手、腿足着地检测中通用） |

**核心知识点**

1. **DIGIT 360 传感器深度** (2024-2025): 全指尖球形设计、18+ 感知特征通道 (法线场/温度/振动/滑动/接触面积等)、Meta AI + GelSight 联合架构
2. **触觉 MPC**: 基于触觉观测的模型预测控制——触觉状态→预测模型→最优力/位置指令; 双臂触觉 MPC (arXiv 2509.22421) 多智能体 MPC 框架
3. **触觉条件化策略**: FARM 触觉扩散 (arXiv 2510.13324) 触觉条件化 Diffusion Policy、SA-KLQR 柔性工具 Koopman+LQR (arXiv 2503.21491)
4. **ISO 10218-2:2025 触觉安全集成**: 29 体区划分下的触觉反馈安全策略、PFL(Power and Force Limiting) 触觉辅助实现、接触力实时监控与安全停止
5. **TacEva 标准化评测** (Adv. Intell. Syst. 2026): 首个触觉传感器标准化评测基准,跨传感器可比性
6. 触觉传感器对比: GelSight Mini vs DIGIT vs DIGIT 360 vs AllSight 的分辨率/频率/成本/模态谱
7. 触觉图像实时处理: 光度立体法 (Photometric Stereo) 在 GPU 上的实时实现、法线场→接触力场映射
8. 触觉-视觉-力觉三模态融合: 多流编码器 + Cross-Attention + 力传感器信号对齐
9. 触觉伺服 (Tactile Servoing): 基于触觉误差的雅可比估计与闭环控制,精密装配场景
10. 触觉 Sim-to-Real 进展: TACTO / Taxim 仿真器 + 域随机化 + real-to-sim 校准
11. 触觉在柔性物体操作中的应用: 线缆/织物/食品等非刚性物体的触觉反馈策略

**关键项目**

| 项目 | 难度 |
|------|------|
| 用 DIGIT 360 (或仿真替代) 构建触觉 MPC 抓取闭环控制器 | ⭐⭐⭐ |
| 实现触觉条件化 Diffusion Policy: FARM 框架复现 | ⭐⭐⭐⭐ |
| DIGIT 360 + ISO 10218-2:2025 安全合规演示: 29 体区触觉安全停止 | ⭐⭐⭐ |
| 触觉引导精密装配: Peg-in-Hole 触觉伺服 (仿真 + 真机) | ⭐⭐⭐⭐ |
| TacEva 基准复现: 跨传感器对比评测 | ⭐⭐ |

**关键论文**

- Yang et al., "Digit 360: Fingertip-Shaped Tactile Sensor with 18+ Sensing Features" (2024)
- FARM: "Tactile-conditioned Diffusion Policy for Contact-Rich Manipulation" (arXiv 2510.13324)
- Dual-Arm Tactile MPC (arXiv 2509.22421): Multi-agent MPC with Tactile Feedback
- SA-KLQR (arXiv 2503.21491): Koopman+LQR for Flexible Tool Manipulation
- TacEva (Advanced Intelligent Systems 2026): Standardized Tactile Evaluation
- ISO 10218-2:2025: Safety Requirements for Industrial Robot Systems and Integration

**练习**

1. 在 TACTO 仿真器中模拟 DIGIT 360 传感器,实现接触力场可视化与滑动检测
2. 用触觉 MPC 实现易碎物体 (鸡蛋) 的安全拾放,对比有/无触觉反馈的成功率
3. 设计一个满足 ISO 10218-2:2025 PFL 要求的触觉安全监控节点,集成到 ros2_control

---

## M16_接触丰富操作优化 (v2.0 正式规划) ⭐⭐⭐⭐

> CRISP 接触隐式凸优化、层次化接触轨迹优化、手内操作优化、GCS 凸集图在操作中的应用。

**一句话** — 让优化器理解接触: 从 CRISP 凸化接触约束到 GCS 全局最优, 解决接触丰富操作的轨迹优化难题。

| 属性 | 值 |
|------|----|
| **周数** | 2-3 周 |
| **前置依赖** | M05 (QP/NLP 建模)、M08 (轨迹优化)、F07 (WBC/QP 基础)、F12 (接触隐式优化, 推荐) |
| **共享标记** | ⚪ 部分共享（接触优化在腿足步态生成中通用） |

**核心知识点**

1. **CRISP 接触隐式凸优化** (arXiv 2502.01055): 将接触隐式轨迹优化凸化, 成功率从 0-20% 提升到 70-90%; 互补约束的凸松弛技巧
2. **层次化接触轨迹优化** (arXiv 2503.07963): MILP (混合整数线性规划) 上层选择接触模式 + NLP 下层优化轨迹; 大规模接触规划的可扩展性
3. **手内操作优化**: 高自由度灵巧手的接触丰富轨迹优化; 指尖滚动/滑动/枢转的模式编码
4. **GCS (Graph of Convex Sets) 在操作中的应用**: 构型-接触空间的凸集图构建; 全局最优保证下的接触丰富路径规划 (Science Robotics)
5. 接触模式枚举与剪枝: 指数级接触模式的高效搜索策略
6. 物理驱动数据生成 (arXiv 2502.20382): 轨迹优化精炼示教数据, TO→policy 蒸馏
7. 接触约束在 QP/NLP 中的建模: 互补约束松弛 (Fischer-Burmeister / log barrier)、摩擦锥线性化
8. 接触丰富操作的 MPC 实时化: ACAL-iLQR + GPU 并行化, 在线接触模式切换
9. 可微接触优化: 通过接触的梯度传播, 隐函数定理 vs 随机平滑
10. 与力控的接口: 优化输出的力/力矩参考如何送入阻抗/导纳控制器

**关键项目**

| 项目 | 难度 |
|------|------|
| 复现 CRISP: 在 Drake 中实现接触隐式凸优化推物任务 | ⭐⭐⭐ |
| 层次化接触 TO: MILP+NLP 层次化方法在灵巧手操作中的实现 | ⭐⭐⭐⭐ |
| GCS 接触规划: 构建操作场景的凸集图并求解全局最优路径 | ⭐⭐⭐⭐ |
| 接触 MPC 实时化: GPU 加速的接触隐式 iLQR 控制器 | ⭐⭐⭐⭐⭐ |
| TO→Policy 蒸馏: 用轨迹优化数据训练接触丰富操作策略 | ⭐⭐⭐ |

**关键论文**

- CRISP (arXiv 2502.01055): "Contact-Implicit Convex Optimization for Manipulation"
- Hierarchical Contact TO (arXiv 2503.07963): "MILP+NLP for Contact-Rich Trajectory Optimization"
- GCS (Science Robotics): "Motion Planning around Obstacles with Convex Optimization"
- Physical Data Generation (arXiv 2502.20382): "Physics-driven Data Generation for Contact-Rich Manipulation"
- Topology-driven Parallel TO (arXiv 2401.06021): "Multi-homotopy Trajectory Optimization"

**练习**

1. 在 Drake 中实现一个简单的 CRISP 优化: 平面推物从初始位姿到目标位姿
2. 对比 CRISP (凸化) vs 标准 NLP (Ipopt) 在同一接触场景下的成功率和求解时间
3. 构建一个 2D 操作场景的 GCS 图,可视化凸集和最优路径

---

## F12_端口 Hamilton 与被动性框架 (v2.0 正式规划) ⭐⭐⭐⭐

> 多腔能量罐、线性化能量罐+MPC、IPC 统一框架——端口哈密顿系统在力控中的现代应用。

**一句话** — 从能量罐到端口 Hamilton 统一: 用能量域语言重构力控稳定性理论, 实现构造性被动性保证。

| 属性 | 值 |
|------|----|
| **周数** | 2-3 周 |
| **前置依赖** | F02 (端口无源性理论)、F06 (能量罐/变阻抗)、F07 (WBC/QP, 推荐) |
| **共享标记** | ✅ 全方向共享（被动性框架在遥操作/腿足/人形力控中通用） |

**核心知识点**

1. **端口哈密顿系统回顾**: 能量函数 H(x)、端口变量 (effort, flow)、Dirac 结构、互连与组合
2. **多腔能量罐** (arXiv 2510.17341): IFIC (Intrinsically Flexible Interconnection Control) 阀控双腔能量储备 + 端口 Hamilton 形式化; 多腔设计实现更精细的能量预算分配
3. **线性化能量罐+MPC** (TRO 2025): 将非线性能量罐约束线性化, 使之可嵌入凸 MPC; 在线被动性保证 + 性能优化的统一求解
4. **IPC (Interconnection and Port-based Control) 统一框架**: 将阻抗控制、导纳控制、混合力位控制统一到端口哈密顿语言; 控制器设计归结为选择互连结构和阻尼注入
5. **CBF 几何→能量域统一** (2025-2026 趋势): 控制屏障函数从几何约束扩展到能量域约束; 能量感知 CBF 与端口 Hamilton 的自然结合
6. **构造性稳定方法**: 从 Lyapunov 分析到构造性设计——能量塑造 (energy shaping) + 阻尼注入 (damping injection)
7. 端口 Hamilton 在遥操作中的应用: 双端口网络的端口哈密顿建模, 波变量的能量域解释
8. 端口 Hamilton 在多体系统中的应用: Pinocchio 动力学的端口哈密顿改写, 约束动力学的能量守恒
9. P-MPCIC 被动 MPC (IEEE 2025): 形式化被动性保证的 MPC 框架, 在双臂协调中的应用
10. 实时实现: 能量罐+MPC 在 1kHz 控制循环中的计算预算、warm start 策略

**关键项目**

| 项目 | 难度 |
|------|------|
| 实现多腔能量罐: 双腔设计 + 阀控逻辑 + 被动性验证 | ⭐⭐⭐ |
| 线性化能量罐嵌入 MPC: 凸 QP 约束形式 + OSQP/ProxQP 求解 | ⭐⭐⭐⭐ |
| IPC 统一框架: 用端口 Hamilton 语言重写阻抗/导纳控制器 | ⭐⭐⭐ |
| 能量感知 CBF: 实现能量域控制屏障函数并与端口 Hamilton 集成 | ⭐⭐⭐⭐ |
| P-MPCIC 双臂应用: 被动 MPC 在双臂协调操作中的部署 | ⭐⭐⭐⭐⭐ |

**关键论文**

- IFIC Multi-Chamber Energy Tank (arXiv 2510.17341): "Valve-controlled Dual-Chamber Energy Reservoir"
- Linearized Energy Tank + MPC (TRO 2025): "Embedding Passivity Constraints in Convex MPC"
- P-MPCIC (IEEE 2025): "Passive Model Predictive Control for Cooperative Manipulation"
- Ortega et al., "Interconnection and Damping Assignment Passivity-Based Control" (European Journal of Control 2002)
- van der Schaft, "Port-Hamiltonian Systems: An Introductory Survey" (2006)

**练习**

1. 用 Pinocchio 将 7-DOF 臂的动力学改写为端口 Hamilton 形式, 验证能量守恒
2. 实现双腔能量罐并在变阻抗切换场景中验证: 对比单腔 vs 双腔的能量预算精度
3. 将能量罐约束线性化并嵌入一个简单的阻抗 MPC 求解器, 验证 1kHz 实时性
