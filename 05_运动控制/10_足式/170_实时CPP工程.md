...
**向前承接**: 足式/90_WBC分层优化与TSID 引入了 `EIGEN_RUNTIME_NO_MALLOC` 作为 WBC 的硬约束，足式/110_OCS2完整栈与双线程MPC 展示了 OCS2 的双线程 Triple Buffer 架构。本章将这些零散的实时概念统一为完整的工程方法论。

**向后指向**: 足式/230_Perceptive_MPC(Perceptive MPC)和 足式/240_legged_control精读(legged_control 精读)将在本章的实时基础设施上构建感知-控制闭环。

---

## 前置自测

📋 **答不出 $\geq$ 2 题 → 先回基础/并发编程章节和基础/C++17 pmr 章节复习**
...
6. 将 足式/110_OCS2完整栈与双线程MPC 的双线程 MPC 架构落地为生产级实时代码
...
>
> **延伸**: RL + MPC 混合架构(足式/210_RL与MPC混合范式)仍然需要实时系统支撑。
...
> **跨领域类比**：PREEMPT_RT 之于普通 Linux 内核，好比急诊室分诊系统之于普通门诊排队——普通门诊按挂号顺序看病，即使心脏骤停患者也得等前面的人看完；急诊分诊则严格按病情紧急程度排序，心脏骤停的患者（高优先级 WBC 线程）可以立刻打断正在看感冒的医生（低优先级后台任务）。

PREEMPT_RT(由 Ingo Molnar 和 Thomas Gleixner 自 2004 年开始开发)对 Linux 内核进行了三个核心改造:
...
在 足式/90_WBC分层优化与TSID 中我们已经初步接触了"控制循环的禁忌"。这里做系统性的梳理:
...
**总计最坏情况: 数百微秒到数毫秒**——对 1 ms 控制周期来说是致命的。

> **跨领域类比**：内存池的思路好比预装弹夹——士兵在战场上不会现场去弹药库领子弹（`malloc`），而是提前把弹夹装满（预分配 buffer），战斗中直接换弹夹即可（O(1) 分配）。`pmr::monotonic_buffer_resource` 就是这样的"预装弹夹"。

### `EIGEN_RUNTIME_NO_MALLOC`——开发期的安全网 ⭐⭐

足式/90_WBC分层优化与TSID 已经详细介绍了 `EIGEN_RUNTIME_NO_MALLOC` 的原理和实践(见 53.6 节)。本节在此基础上，从**工程集成**的角度进一步展开。

**回顾核心机制**: 足式/90_WBC分层优化与TSID.6 建立了 Eigen 实时安全编程的基础——通过 `EIGEN_RUNTIME_NO_MALLOC` 宏，Eigen 的内部分配函数在每次 `malloc` 前检查全局标志 `is_malloc_allowed`。在 足式/90_WBC分层优化与TSID 中我们用它保护 WBC 的 QP 求解循环；这里同样的机制被扩展到整个控制管线。具体而言: 当定义了该宏时，如果 `Eigen::internal::set_is_malloc_allowed(false)` 被调用后仍有分配请求，触发 `eigen_assert` 失败，立即暴露违规代码的位置。

**足式/90_WBC分层优化与TSID 未覆盖的工程问题——CI/CD 集成**:
...
足式/90_WBC分层优化与TSID.6.4 列出了常见违规操作。这里用表格形式补充完整对照:
...
回顾基础/C++17 内存资源章节：`std::pmr`（Polymorphic Memory Resource）是 C++17 引入的分配器框架，允许在运行时切换内存分配策略而不改变容器类型。足式/90_WBC分层优化与TSID.6.5 展示了在 WBC 中使用 `pmr::vector` 的基本模式。这里深入讲解 `monotonic_buffer_resource` 为什么特别适合实时控制的**原理**。
...
回顾 足式/110_OCS2完整栈与双线程MPC 的双线程架构:
...
**无锁**(lock-free)不等于"没有同步"——它是用**原子操作**(`std::atomic`)替代锁，保证至少一个线程总在前进(progress guarantee)。

> **跨领域类比**：Triple Buffer 的工作方式好比餐厅传菜窗口的旋转托盘——厨师（MPC 线程）把做好的菜放在托盘的一个槽位上，服务员（WBC 线程）随时从另一个槽位取最新的菜，两人互不等待、互不阻塞，托盘的原子旋转保证了服务员总能拿到最新出品。
...
足式/90_WBC分层优化与TSID.8.2 和 足式/110_OCS2完整栈与双线程MPC.7 已经从不同角度深入介绍了 Triple Buffer——足式/90_WBC分层优化与TSID 讲了 MPC+WBC 集成的概念和简化实现，足式/110_OCS2完整栈与双线程MPC 讲了 OCS2 的 `BufferedValue` 工业级实现和 `memory_order` 选择。本节聚焦于**选型决策**和**实际部署经验**。
...
**MPC→WBC 应该用 Triple Buffer**(如 足式/110_OCS2完整栈与双线程MPC 所述)，因为 WBC 只需要**最新的** MPC 解，不需要历史值。如果 MPC 在 WBC 两次读取之间产出了 2 个解，WBC 应该跳过旧的、只读最新的。

**实际部署中的 Triple Buffer 实现**: 足式/110_OCS2完整栈与双线程MPC.7 详细介绍了 OCS2 中 Triple Buffer 的架构设计——三个槽位（write / middle / read）通过原子交换实现无锁通信，写者永远不阻塞、读者总是拿到最新数据；足式/90_WBC分层优化与TSID.8.2 则从 WBC 集成的角度说明了 MPC 解如何通过 Triple Buffer 传递给实时 WBC 循环。下面给出一个简化但功能完整的 C++ 实现，供部署参考:
...
前面的内容建立了通用的实时编程技术。现在让我们把这些技术应用到一个具体的、工程意义重大的场景——足式/110_OCS2完整栈与双线程MPC 中 OCS2 的双线程 MPC 架构的实时实现。

## 61.8 OCS2 双线程 MPC 的实时工程 ⭐⭐⭐

### 动机:理论架构到生产代码的鸿沟

足式/110_OCS2完整栈与双线程MPC 详细介绍了 OCS2 的双线程架构:**MPC Node**(异步求解)和 **MRT Node**(实时查询，Motion Reference Tracking)通过 `BufferedValue` 通信。`BufferedValue` 本质是 Triple Buffer——足式/110_OCS2完整栈与双线程MPC.7.2-55.7.7 从"为什么不用 mutex"出发，推导了三槽位原子交换的设计，证明了其读写无阻塞、数据总是最新的性质。那一章聚焦于架构设计和算法层面。本节不重复 足式/110_OCS2完整栈与双线程MPC 的内容，而是补充 足式/110_OCS2完整栈与双线程MPC 未覆盖的**部署层实时工程**——线程配置、失败恢复、诊断集成。
...
//         + Triple Buffer read (61.5/足式/110_OCS2完整栈与双线程MPC.7) + clock_nanosleep (61.3)
void mrtLoop() {
  ...
  while (running_) {
    ...
    //    Uses BufferedValue from 足式/110_OCS2完整栈与双线程MPC.7 (atomic swap, ~5 ns)
...
当 MPC 求解超时或失败时，MRT 需要有 fallback。这在 足式/110_OCS2完整栈与双线程MPC 中以架构形式提到，这里给出具体的实时安全实现:
...
| ros2_control `update()` 周期性超时但无明显 CPU 负载 | 透明大页(THP)合并或 `compaction_proactiveness` 触发内核压缩 | 1. 检查 `/sys/kernel/mm/transparent_hugepage/enabled` 2. 设为 `never` 或 `madvise` 3. 用 `perf stat -e compaction-stalls` 确认 | 61.9 |

---

## 61.10 Linux 6.12+ PREEMPT_RT 新实践与 eBPF 实时诊断 ⭐⭐⭐

### Linux 6.12 主线合并后的变化 ⭐⭐

2024 年 11 月 Linux 6.12 正式将 PREEMPT_RT 合入主线，这是 20 年开发的里程碑。对腿足机器人工程师而言，这带来了三个重要变化:

**变化 1: 无需再打外部补丁**。在 6.12 之前，使用 PREEMPT_RT 需要从 `kernel.org/pub/linux/kernel/projects/rt/` 下载对应内核版本的 RT 补丁，手动应用后编译。补丁版本经常滞后于主线内核 1-3 个月，导致安全更新延迟。6.12+ 直接在 `make menuconfig` 中选择 `Fully Preemptible Kernel (Real-Time)` 即可，与主线安全补丁同步更新。

**变化 2: 发行版开始原生支持**。Ubuntu 24.04+ 提供 `linux-image-*-realtime` 包;Fedora 41+ 提供 `kernel-rt`;Debian 提供 `linux-image-rt-*`。这意味着**无需编译内核**——直接 `apt install` 即可获得 RT 内核。

**变化 3: 持续优化仍在进行**。主线合入不等于"完成"。以下优化在 6.13-6.15 中持续推进:
- Lazy preemption 模型(合并 `PREEMPT_NONE`/`VOLUNTARY`/`FULL` 为统一模型，运行时可选)
- `printk` 线程化(消除 `printk` 在 RT 路径上的最后一个不可抢占区域)
- NAPI 线程化改进(网络接收路径的实时性优化)

> **本质洞察**:PREEMPT_RT 合入主线的意义**不是**"实时 Linux 终于可用了"——它在此之前已经被 Tesla Autopilot、SpaceX Starlink 和多家腿足机器人公司用了十年。真正的意义是**降低了准入门槛**:不再需要内核编译专家来维护 RT 补丁，任何会 `apt install` 的工程师都能获得实时能力。

### eBPF 在实时诊断中的应用 ⭐⭐⭐

eBPF(extended Berkeley Packet Filter)是 Linux 内核的可编程观测框架——可以在内核中安全地运行用户自定义的代码来收集数据，**几乎不影响被观测系统的实时性**。这是 eBPF 相对 `ftrace`/`trace-cmd` 的关键优势:传统 tracing 工具的开销在微秒到毫秒级，而 eBPF 程序的开销通常在纳秒级。

**为什么腿足工程师需要 eBPF**:当 `cyclictest` 显示偶尔有 200 $\mu$s 的延迟尖峰，但 `ftrace` 无法定位具体原因时，eBPF 可以精确到**哪个内核函数、哪个中断处理程序**导致了延迟。

**工具 1: `bpftrace`——实时延迟热点定位**

```bash
# 追踪所有耗时 > 50 us 的调度延迟事件
sudo bpftrace -e '
tracepoint:sched:sched_switch {
  @start[args->prev_pid] = nsecs;
}
tracepoint:sched:sched_wakeup /args->pid == $1/ {
  if (@start[args->pid]) {
    $lat = (nsecs - @start[args->pid]) / 1000;
    if ($lat > 50) {
...
' -- $(pgrep wbc_controller)
```

**工具 2: `runqlat`——运行队列延迟分布**

```bash
# 来自 bcc-tools 包
sudo runqlat -p $(pgrep wbc_controller) -m 10
# 输出 WBC 线程在 CPU 运行队列中等待的时间分布(直方图)
# 如果 p99 > 100 us, 说明调度配置有问题
```

**工具 3: 自定义 eBPF 程序监控 `malloc` 调用**

```bash
# 检测实时线程中是否有 malloc 调用(不应该有!)
sudo bpftrace -e '
uprobe:/lib/x86_64-linux-gnu/libc.so.6:malloc /pid == $1/ {
  printf("WARNING: malloc called in RT thread! size=%d\n", arg0);
  print(ustack);  // 打印调用栈
}
' -- $(pgrep wbc_controller)
```

这比 `EIGEN_RUNTIME_NO_MALLOC` 更强大，因为它能捕获**所有** malloc 调用(不只是 Eigen 的)，包括 STL 容器、`std::string`、`std::function` 等隐式分配。

**工具 4: 自定义 eBPF 延迟归因——精确到内核函数**

当 `cyclictest` 报告偶尔出现 200 $\mu$s 延迟但无法定位根因时，以下 `bpftrace` 脚本可以追踪实时线程被抢占的具体位置:

```bash
# 追踪 WBC 线程被抢占时正在执行的内核函数
sudo bpftrace -e '
tracepoint:sched:sched_switch /args->prev_pid == $1 && args->prev_state == 0/ {
  // prev_state == 0 表示"被抢占"(而非主动让出)
  printf("preempted at: ");
  print(kstack(5));  // 打印内核调用栈(5 层)
  printf("---\n");
}
' -- $(pgrep wbc_controller)
```

这个脚本的输出可以直接定位"是哪个中断处理程序/内核路径导致了实时线程被抢占"。常见的罪魁祸首包括:网络中断处理(`net_rx_action`)、块设备 I/O 完成(`blk_mq_complete_request`)、以及 RCU 回调(`rcu_core`)。针对每种根因有对应的解决方案——网络中断用 `irqaffinity` 绑定到非实时核心，块设备 I/O 用 `ionice` 降低优先级，RCU 回调用 `rcu_nocbs` 隔离。

**eBPF 与传统工具的对比**:

| 工具 | 开销 | 精度 | 适用场景 |
|------|------|------|---------|
| `printf` 调试 | 10--1000 $\mu$s/次 | 低(改变系统行为) | 绝对禁止在 RT 路径中 |
| `ftrace` | 1--10 $\mu$s/事件 | 中 | 粗粒度调度事件追踪 |
| `trace-cmd` | 1--5 $\mu$s/事件 | 中高 | 离线分析调度延迟来源 |
| `perf` | 采样开销可控 | 统计性 | 热点分析(非单次事件) |
| `bpftrace`/eBPF | 10--100 ns/事件 | **最高** | 精确定位单次延迟事件的内核根因 |

> **跨领域类比**：eBPF 之于实时系统诊断，好比 X 光之于医学诊断——它让你**不打开系统**(不修改代码、不重新编译、不影响实时性)就能看到内部运行状态。传统的 printf 调试相当于"切开病人看一看"——本身就会改变系统行为。eBPF 的纳秒级开销意味着它可以在生产环境的 1 kHz 实时循环中持续运行而不引入可测量的延迟——这是 `ftrace` 和 `trace-cmd` 做不到的。

**工程建议:eBPF 实时诊断的最佳实践**

1. **开发阶段**:用 `bpftrace` 脚本检测所有 malloc 调用和优先级反转事件，作为 CI 的一部分
2. **压力测试阶段**:在 `cyclictest` 并行运行时用 `runqlat` 监控运行队列延迟分布，确认 p99.99 < 100 $\mu$s
3. **生产部署阶段**:保留轻量级 eBPF 监控(如 malloc 检测)作为常驻安全网，延迟开销 < 1 $\mu$s/次

### 跨章综合练习 ⭐⭐⭐

**练习 61.10.1（跨章综合: 足式/90_WBC分层优化与TSID + 足式/110_OCS2完整栈与双线程MPC + 本章）**:

设计并实现一个完整的实时控制部署方案。具体要求:

1. 回顾 足式/110_OCS2完整栈与双线程MPC 的 OCS2 双线程架构(MPC 线程 + MRT 线程)，画出完整的线程拓扑图，标注每个线程的 SCHED_FIFO 优先级、CPU 亲和性和通信机制(Triple Buffer vs SPSC Queue)
2. 回顾 足式/90_WBC分层优化与TSID 的 WBC QP 求解，确认 QP 求解器(如 qpOASES 或 OSQP)内部是否有 malloc 调用——用 `EIGEN_RUNTIME_NO_MALLOC` 或 eBPF 验证
3. 编写一个 `cyclictest` + `stress` 的自动化测试脚本，在 PREEMPT_RT 内核上运行 24 小时，验证 WBC 线程的 max latency < 100 $\mu$s
4. 分析:如果在 WBC 线程中加入 RL 策略推理(足式/200_RL的CPP部署)，推理延迟 ~100 $\mu$s 对 1 ms 控制周期的时间预算意味着什么?是否需要将 RL 推理放在单独的线程中?
...
| EIGEN_RUNTIME_NO_MALLOC | 运行时检测 Eigen 堆分配;开发期安全网(足式/90_WBC分层优化与TSID.6 建立原理，本章 61.4 补充 CI 集成) | ⭐⭐ |
| 预分配模式 | 固定大小矩阵 / 预分配动态矩阵 / 对象池 / pmr | ⭐⭐ |
| SPSC 无锁队列 | 单生产者单消费者，release/acquire 语义，cache line 对齐 | ⭐⭐⭐ |
| Triple Buffer | 读最新值，写不阻塞;MPC→WBC 的标准通信方式(足式/110_OCS2完整栈与双线程MPC.7 架构设计，本章 61.5 部署实现) | ⭐⭐⭐ |
...
本章的内容与 足式/90_WBC分层优化与TSID(WBC 的 `EIGEN_RUNTIME_NO_MALLOC` 和 MPC+WBC 集成架构)和 足式/110_OCS2完整栈与双线程MPC(OCS2 的双线程 Triple Buffer 架构)形成三角关系:足式/90_WBC分层优化与TSID 和 足式/110_OCS2完整栈与双线程MPC 从**算法和架构**角度提出了实时需求，本章从**系统和工程**角度给出了完整的实现方案。三章合起来覆盖了腿足实时控制的完整知识链。
...
2. 实现 MPC→WBC 的通信(使用 OCS2 的 `MPC_MRT_Interface`，基于 足式/110_OCS2完整栈与双线程MPC)
