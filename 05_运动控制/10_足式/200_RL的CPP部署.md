...
**练习 64.2.1**: 写一个完整的 `export.py` 脚本,加载你在 足式/190_腿足RL训练栈 训好的 Go2 checkpoint,导出为 TorchScript `.pt` 文件。用 10 个随机输入验证导出前后的输出误差均 < 1e-6。
...
**练习 64.4.1**: 把 足式/190_腿足RL训练栈 训好的策略同时导出为 TorchScript (.pt) 和 ONNX (.onnx)。在 C++ 中分别加载两者,用 10 个随机输入验证输出一致性(互相之间误差 < 1e-5)。
...
分别掌握了 MPC(足式/110_OCS2完整栈与双线程MPC)和 RL(足式/190_腿足RL训练栈-64)之后,自然的问题是:能否融合两者的优势?足式/210_RL与MPC混合范式 正是腿足控制最活跃的研究前沿——OCS2 MPC-Net、Cafe-MPC VWBC、RAMBO、残差 RL 四条 RL+MPC 混合路线都要讲透。足式/220_腿足感知数据结构 则衔接到感知的工程侧——讲清楚 Grid Map、Elevation Mapping 等数据结构和工程实现,为 足式/230_Perceptive_MPC 的 Perceptive MPC 算法做好数据层准备。
