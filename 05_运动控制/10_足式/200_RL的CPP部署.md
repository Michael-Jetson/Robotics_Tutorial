...
**向前承接**: 足式/190_腿足RL训练栈 训好了 Go2 的 RL 策略(`.pth` checkpoint)——一个包含 Actor 网络权重的 Python 对象，此时它只能在 Python + PyTorch 环境中运行。足式/170_实时CPP工程 搭好了实时 C++ 和 ros2_control 框架——其中 `update()` 以 1 kHz 运行，要求每次调用的延迟严格低于 1 ms。足式/180_腿足硬件栈 提供了硬件接口(Unitree SDK)——通过 UDP 协议以 500 Hz 与电机通信。本章把三者打通——策略从 Python 进入 C++ 实时循环。

**向后指向**: 足式/210_RL与MPC混合范式 将探索 RL+MPC 混合范式——其中 RL 部分的部署正是本章内容。足式/250_Mini-Legged综合实战(Mini-Legged 综合实战)将整合本章的部署技术完成真机验证。
...
3. **[足式/190_腿足RL训练栈]** PPO 训练后得到的 `.pth` checkpoint 文件包含什么内容?为什么不能直接在 C++ 中加载 `.pth`?
4. **[C++ 基础]** `std::memcpy` 与 `std::copy` 在连续内存上的性能差异是什么?什么时候用哪个?
5. **[足式/180_腿足硬件栈]** ros2_control 的 `update()` 函数运行在什么线程上?它的调用频率由什么决定?
...
2. **垃圾回收 (GC)**: Python 的 GC 可能在任意时刻暂停程序数十毫秒——足式/170_实时CPP工程 讲过，这对 1 kHz 控制循环是致命的(5 ms 的暂停就可能让机器人摔倒)。
...
**练习 64.2.1**: 写一个完整的 `export.py` 脚本，加载你在 足式/190_腿足RL训练栈 训好的 Go2 checkpoint，导出为 TorchScript `.pt` 文件。用 10 个随机输入验证导出前后的输出误差均 < 1e。
...
class RLPolicy {
 public:
  ...
  RLPolicy(const std::string& model_path,
    ...
    // 3. 预分配 tensor(关键的实时编程技巧,足式/170_实时CPP工程)
...
**练习 64.4.1**: 把 足式/190_腿足RL训练栈 训好的策略同时导出为 TorchScript (.pt) 和 ONNX (.onnx)。在 C++ 中分别加载两者，用 10 个随机输入验证输出一致性(互相之间误差 < 1e)。
...
**练习 64.5.1**: fork rl_sar，对照你在 足式/190_腿足RL训练栈 训练时的 legged_gym/IsaacLab 配置文件，逐项修改 rl_sar 的配置。列出你修改的每一项及其原因。
...
**阶段 1：仿真闭环验证（最重要，避免 80% 的真机问题）**

在把策略放到真机上之前，先在仿真（Gazebo + ros2_control）中验证完整的 train → export → C++ deploy pipeline。

| 检查项 | 方法 | 合格标准 |
|--------|------|---------|
| C++ 输出与 Python 一致 | 100 个随机输入交叉验证 | 最大绝对误差 < $10^{-5}$ |
| 观测归一化正确 | 打印 C++ 端的归一化观测，与 Python 端对比 | 每个维度误差 < $10^{-4}$ |
| 关节顺序映射正确 | 发送已知关节命令，检查每个关节是否正确响应 | 所有 12 个关节一一对应 |
| 推理延迟达标 | 运行 10000 次推理，记录延迟分布 | P99 < 500 $\mu$s |
| 安全降级有效 | 注入 NaN 和超限值 | 正确触发对应级别降级 |

**阶段 2：真机站立测试**

不行走，只让机器人用 RL 策略站立。这是最安全的真机验证——即使策略有问题，站立时摔倒的风险最低。

检查：关节是否在合理角度范围内？是否有异常抖动？电机温度是否正常？

**阶段 3：真机低速行走**

以 0.2-0.3 m/s 的低速在平地上行走。低速降低了摔倒风险，便于观察策略行为。

检查：步态是否稳定？是否有方向偏差？推扰后能否恢复？

**阶段 4：正常速度 + 扰动测试**

逐步提高速度到 1.0 m/s，加入推扰和方向变化。
...
```
┌──────────────────────────────────────────────┐
│ 1. 训练 (足式/190_腿足RL训练栈): IsaacLab + rsl_rl → ckpt.pth │
├──────────────────────────────────────────────┤
...
| 容易摔倒 | DR 范围不够大 | 回 足式/190_腿足RL训练栈 加大 DR(摩擦、延迟) |
...
**练习 64.8.1**: 按照 8 步 Pipeline，完成从 足式/190_腿足RL训练栈 训好的 Go2 策略到 Gazebo 仿真部署的全流程。记录每一步花费的时间和遇到的问题。
...
| Jetson Orin 上推理延迟 > 300 $\mu$s 且波动大 | CPU governor 非 performance 模式，或 Jetson 处于节能功率模式 | 1. `sudo jetson_clocks` 锁定最高频率 2. 确认 `nvpmodel -q` 为 MAXN 模式 3. 用 `taskset` 绑定推理线程到性能核心 | 64.6, 64.8 |
...
TorchScript 试图"编译 Python"——把 Python 源代码翻译为 TorchScript IR。这导致了大量限制(不支持动态类型、复杂控制流等)。`torch.export` 转变思路——不编译 Python，而是**记录计算图**(类似 trace)，但引入了 `torch.cond`、`torch.map` 等显式控制流原语来处理 TorchScript 无法处理的动态情况。

```python
# torch.export 基本用法 (PyTorch 2.x+)
import torch

class Actor(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.fc1 = torch.nn.Linear(48, 256)
...
# 使用 torch.export 导出
exported = torch.export.export(model, (torch.randn(1, 48),))

# 导出为 ExportedProgram,可以序列化
torch.export.save(exported, "policy_exported.pt2")
...
onnx_program = torch.onnx.export(
    ...
    (torch.randn(1, 48),),
    "policy.onnx",
    dynamo=True  # 使用 TorchDynamo 后端,比旧的 torch.onnx.export 更可靠
)
```

**对腿足 RL 部署的实际影响**:

| 维度 | TorchScript (旧) | torch.export (新) | 建议 |
|------|-----------------|-------------------|------|
| **纯 MLP 策略** | 完美工作 | 完美工作 | 两者皆可，但新项目用 torch.export |
| **RNN/GRU 策略** | 需要 script 模式 | 原生支持 | torch.export 更可靠 |
| **LibTorch C++ 加载** | 成熟支持 | 通过 AOTInductor 或 ONNX 间接支持 | 短期仍用 TorchScript;中期转 ONNX |
| **生态支持** | 逐步减少 | 快速完善 | 新项目应投资 torch.export |
...
| 后端 | MLP [48→256→256→12] | 含 GRU 策略 | 优化手段 | 适用场景 |
|------|---------------------|-------------|---------|---------|
| **LibTorch CPU** | ~80 $\mu$s | ~150 $\mu$s | JIT 优化 | x86 桌面 / Jetson CPU |
| **ONNX Runtime CPU** | ~60 $\mu$s | ~120 $\mu$s | 图优化 | 通用，跨平台 |
| **ONNX Runtime + TensorRT EP** | ~30 $\mu$s | ~60 $\mu$s | TRT 层融合 + FP16 | Jetson GPU |
| **TensorRT (直接)** | ~20 $\mu$s | ~45 $\mu$s | 全链路优化 + FP16 | Jetson GPU，最高性能 |

**关键注意事项**:
- TensorRT 的 **GPU 推理延迟低**，但 host-device 数据拷贝增加 ~30-100 $\mu$s 开销。对于腿足 RL 的小型 MLP,CPU 推理反而可能更快(无拷贝开销)
- TensorRT 的 FP16 优化可能引入 ~0.1% 的数值差异——必须验证策略行为不受影响
- TensorRT 引擎是**平台特定**的(在 Jetson Orin 上构建的引擎不能在 Jetson Xavier 上运行)，需要在目标平台上构建
...
完成从训练到实时部署的完整 pipeline 验证:

1. 回顾 足式/190_腿足RL训练栈 的 PPO 训练，确认训好的 `.pth` checkpoint 中包含哪些键(权重、归一化参数、训练配置)
2. 分别用 TorchScript trace 和 ONNX export 两种方式导出模型，对 100 个随机输入验证两种导出的输出误差 < 1e-5
3. 在 C++ 中分别用 LibTorch 和 ONNX Runtime 加载并推理，测量各自的 P50/P99/Max 延迟(运行 10000 次)
4. 回顾 足式/170_实时CPP工程 的实时线程模板，将推理类集成到 `clock_nanosleep` 精确定时循环中，确认在 `stress -c $(nproc)` 负载下 Max 延迟仍 < 500 $\mu$s
5. 分析:如果未来策略从 MLP 升级为 Transformer(参数量 10x)，哪些部署决策需要重新评估?(提示:考虑 GPU 推理、TensorRT 优化、延迟预算)
...
| 符号 | 含义 | 首次出现 |
|------|------|---------|
| `torch::jit::Module` | TorchScript 加载后的模型对象 | 64.3 |
| `Ort::Session` | ONNX Runtime 推理会话对象 | 64.4 |
| `obs_dim` | 观测向量维度（典型值 48） | 64.1 |
| `act_dim` | 动作向量维度（典型值 12） | 64.1 |
| `obs_scales` | 观测归一化缩放系数（训练端决定） | 64.5 |
| `action_scale` | 动作缩放系数（从仿真角度到真机角度） | 64.5 |
| P50 / P99 / Max | 推理延迟的 50% / 99% 分位数和最大值 | 64.6 |
| $R_0, k_d$ | 传感器噪声模型参数（基础方差和距离系数） | 64.8 |

## 定理与公式速查表

| 概念 | 一句话说明 | 对应节 |
|------|----------|--------|
| TorchScript trace | 录制一次前向传播的计算图，适合无控制流网络 | 64.2 |
| TorchScript script | 解析 Python 源码为 IR，支持控制流但限制多 | 64.2 |
| 零拷贝推理 | 直接引用外部内存创建 Tensor，避免 memcpy | 64.3, 64.4 |
| 4 级降级 | Hold Last → Stand → Damping → E-Stop | 64.7 |
| Double buffer | 前后缓冲交换，无锁的实时数据通信 | 64.8 |
| FP16 量化 | 将 FP32 权重转为 FP16，速度 2-3x，精度损失 < 0.1% | 64.10 |
...
```
足式/30_Pinocchio深度精读: 加载 URDF + Pinocchio 建模           [完成]
足式/50_空间向量与浮动基座动力学: 正/逆运动学 + 接触 Jacobian           [完成]
足式/60_QP_NLP建模: QP 求解器集成                         [完成]
足式/80_接触力学与约束优化: 摩擦锥约束                            [完成]
足式/90_WBC分层优化与TSID: WBC 实现 (加权 QP)                    [完成]
足式/110_OCS2完整栈与双线程MPC: OCS2 MPC 集成                         [完成]
足式/170_实时CPP工程: 实时部署框架                          [完成]
足式/180_腿足硬件栈: 硬件接口 + Unitree SDK                [完成]
足式/190_腿足RL训练栈: RL 策略训练                           [完成]
足式/200_RL的CPP部署: RL 策略 C++ 部署                      <-- 本章新增
...
**本章新增的具体任务**:
1. 将 足式/190_腿足RL训练栈 训好的 Go2 策略导出为 TorchScript `.pt` 和 ONNX `.onnx`
...
1. **先在 Python 端验证一切**：在进入 C++ 之前，先用 Python + PyTorch 验证推理正确性（对比训练检查点和导出模型的输出）。C++ 端出问题时，你至少知道 Python 端是对的
2. **rl_sar 是最好的学习材料**：精读 rl_sar 的 `rl_real.cpp` 文件，逐行理解观测构建和动作应用的逻辑。这比从零开始写部署代码高效 10 倍
3. **安全第一**：在真机上调试 RL 策略时，始终保持急停按钮在手边。SafetyGuard 模块不是可选的——它是必须的

### 给有经验者的建议

1. **关注大模型策略的部署趋势**：Transformer 策略和 Diffusion Policy 的推理延迟比 MLP 高 10-100 倍，需要异步推理架构。提前了解这些架构可以让你在技术转型时占据先机
2. **TensorRT 是 Jetson 的秘密武器**：对于需要 GPU 推理的场景，TensorRT 的加速效果远超 LibTorch CUDA。花时间学习 TensorRT 的工具链（trtexec, engine 缓存, INT8 标定）是值得的投资
3. **自动化 benchmark 管线**：建立一个自动化的"训练→导出→benchmark→报告"管线。每次训练新策略后自动测量延迟和精度，避免手动验证的遗漏
...
**向前承接**:
- 足式/170_实时CPP工程(实时 C++)→ 64.6 RL 推理的实时工程要求
- 足式/180_腿足硬件栈(硬件栈)→ 64.5 关节顺序映射、看门狗保护
- 足式/190_腿足RL训练栈(RL 训练)→ 64.2 模型导出、64.5 归一化一致性

**向后指向**:
- 足式/200_RL的CPP部署 → 足式/210_RL与MPC混合范式 RL+MPC 混合(RL 部分的部署正是本章内容)
- 足式/200_RL的CPP部署 → 足式/250_Mini-Legged综合实战 Mini-Legged 综合实战(真机部署完整验证)
- 足式/200_RL的CPP部署 安全降级 → 足式/260_研究方向与博士导引 研究方向(安全 RL、运行时验证)
...
RL 策略的训练是持续的——每次改进 reward 函数、增加域随机化或更换训练平台后，都会产生新的策略版本。在工程实践中，需要解决以下问题:

1. **版本追溯**：真机上跑的是哪个版本的策略？它对应哪个训练 checkpoint？
2. **灰度发布**：新策略能否先在 10% 的时间内运行，与旧策略对比？
3. **快速回滚**：新策略出问题时，能否在 1 秒内切回旧策略？
...
│   ├── policy.onnx        # ONNX 模型
│   ├── config.yaml        # 训练配置（obs_scales, action_scale 等）
│   ├── metadata.json      # 训练超参数、环境版本、性能指标
│   └── checksum.sha256    # 完整性校验
├── v1.1_improved_reward/
│   └── ...
├── v2.0_terrain_aware/
│   └── ...
└── current -> v1.1_improved_reward/  # 软链接指向当前使用版本
```
...
{
  "version": "v1.1",
  "training_env": "IsaacLab 3.0 + MuJoCo Warp",
  "robot": "Unitree Go2",
  "checkpoint_step": 50000000,
  "reward_total": 28.5,
  "obs_dim": 48,
  "act_dim": 12,
  "obs_scales": {"ang_vel": 0.25, "lin_vel": 2.0},
  "joint_order": ["FL_hip", "FL_thigh", "FL_calf", ...],
  "deploy_latency_p99_us": 120,
  "sim_success_rate": 0.97,
...
class PolicyManager {
 public:
  ...
  void loadPolicy(const std::string& model_path) {
    auto new_policy = std::make_shared<RLPolicy>(model_path);
    // warmup: 在非实时线程中做 100 次推理，触发 JIT 编译和内存预分配
    Eigen::VectorXd dummy_obs = Eigen::VectorXd::Zero(obs_dim_);
    for (int i = 0; i < 100; ++i) {
      new_policy->infer(dummy_obs);
    }
    // 原子交换：实时线程无感知地切换到新策略
    pending_policy_.store(new_policy);
  }

  // 在实时线程中调用
  Eigen::VectorXd infer(const Eigen::VectorXd& obs) {
    // 检查是否有新策略待切换
    auto pending = pending_policy_.exchange(nullptr);
    if (pending) {
      active_policy_ = pending;
    }
    return active_policy_->infer(obs);
  }

 private:
  std::shared_ptr<RLPolicy> active_policy_;
  std::atomic<std::shared_ptr<RLPolicy>> pending_policy_{nullptr};
};
```

**关键设计**：加载和 warmup 在非实时线程中完成（可能需要 100-500 ms），切换在实时线程中通过原子指针交换完成（< 1 $\mu$s）。这保证了策略切换不影响实时性。

### 在线 A/B 测试框架

A/B 测试在腿足部署中有其特殊性——不能像网页 A/B 测试那样随机分配用户，而是按时间片交替使用两个策略:
...
**安全约束**：A/B 测试期间，SafetyGuard（64.7 节）始终开启。如果新策略在任何时间片内触发 Level 2 以上降级，立即终止 A/B 测试并锁定旧策略。

### 跨平台部署的挑战

同一个策略在不同硬件平台上的行为可能不同:

| 问题 | 原因 | 解决方案 |
|------|------|---------|
| 浮点精度差异 | x86 用 80-bit FPU, ARM 用 64-bit | 导出模型时用 `float32` 一致性；验证 CPU vs GPU 输出差异 < 1e-5 |
| SIMD 指令集差异 | AVX-512 (x86) vs NEON (ARM) | LibTorch/ONNX Runtime 自动适配，但性能差异需 benchmark |
| 内存对齐差异 | x86 容忍不对齐，ARM 可能 SIGBUS | 使用 Eigen 的对齐分配器 `Eigen::aligned_allocator` |
| TensorRT engine 不通用 | 编译针对特定 GPU 架构 | 在目标平台上重新编译，缓存 engine 文件 |
...
> 💡 **概念误区：认为同一模型文件在所有硬件上行为相同**
> **新手想法**："我在桌面 PC 上验证通过了，Jetson 上肯定也行"
> **实际上**：(1) 浮点精度差异可能导致策略在 OOD 边界附近做出不同决策；(2) Jetson 的 GPU 调度与桌面 GPU 不同，推理延迟的 P99 分布可能完全不同；(3) 功耗模式（15W vs 25W）影响 GPU 频率，进而影响推理延迟
> **正确做法**：在目标部署平台上做完整的 benchmark 和行为验证，不依赖跨平台一致性假设

### 练习

**练习 64.9.1**：实现 `PolicyManager` 类，支持在线加载新策略和原子切换。编写一个测试：在策略切换瞬间测量 `infer()` 的延迟，验证切换不引入额外延迟。

**练习 64.9.2**：设计一个 A/B 测试协议，用于比较"MLP [256, 128, 12]"策略和"MLP [512, 256, 128, 12]"策略在 Go2 上的性能。定义 (a) 测试指标（至少 5 个）；(b) 时间片长度；(c) 统计显著性标准；(d) 安全终止条件。
...
| 策略类型 | 参数量 | 推理延迟 (CPU) | 推理延迟 (GPU) | 代表工作 |
|---------|--------|--------------|--------------|---------|
| 小 MLP [256,128,12] | ~50K | ~50 $\mu$s | ~30 $\mu$s | legged_gym |
| 大 MLP [512,256,128,12] | ~200K | ~100 $\mu$s | ~40 $\mu$s | Walk These Ways |
| MLP + GRU (Belief Encoder) | ~500K | ~200 $\mu$s | ~60 $\mu$s | Miki 2022 |
| Transformer (小) | ~5M | ~2 ms | ~0.5 ms | DTC |
| Diffusion Policy | ~50M | ~20 ms | ~5 ms | Chi et al. 2023 |
| VLA (7B) | ~7B | 不可行 | ~100 ms | OpenVLA |

**关键转折点**：当推理延迟从微秒级跨入毫秒级，部署架构需要根本性变化——不再能在每个控制周期（1 ms）做一次推理，必须引入**异步推理**或**降频控制**。
...
```
高频控制线程 (1 kHz, RT)          低频推理线程 (50 Hz, 非 RT)
    │                                │
    │  读取最新 action（原子操作）      │  读取最新 observation
    │  PD 控制 → 关节扭矩             │  模型推理（~20 ms）
    │  安全检查                       │  写入新 action（原子操作）
    │                                │
    ▼ 每 1 ms                        ▼ 每 20 ms
```

在这种架构中，控制线程以 1 kHz 运行 PD 控制器（跟踪关节角度参考），推理线程以 50 Hz 更新关节角度参考。两者通过无锁共享内存通信。

**这意味着什么**：大模型策略的输出不再是"每毫秒一个动作"，而是"每 20 毫秒一个动作序列"——策略需要输出未来 20 个时间步的动作（action chunk），PD 控制器依次执行。这正是 Diffusion Policy 和 Action Chunking Transformer 的设计思路。

### TensorRT 优化流程

对于大模型，TensorRT 的优化效果比小 MLP 显著得多（因为大模型有更多可融合的算子）：

```bash
# 从 ONNX 转 TensorRT engine（在目标 Jetson 上执行）
trtexec --onnx=policy.onnx \
        --saveEngine=policy.engine \
        --fp16 \                    # 使用 FP16 精度（速度翻倍，精度损失 < 1%）
        --workspace=1024 \          # 允许使用 1GB 临时内存
        --minShapes=obs:1x48 \      # 动态 batch 的最小 shape
        --optShapes=obs:1x48 \      # 优化的 shape
        --maxShapes=obs:1x48        # 最大 shape
```

| 优化手段 | 加速比 | 精度影响 | 适用模型 |
|---------|--------|---------|---------|
| 图优化（算子融合） | 1.5-2x | 无 | 所有 |
| FP16 量化 | 2-3x | < 0.1% | 大多数 |
| INT8 量化 | 3-5x | 1-5% | 需要校准数据集 |
| 动态 shape 优化 | 1.2-1.5x | 无 | 变长输入 |

**FP16 量化的安全性**：对于腿足 RL 策略，FP16 通常是安全的——策略的输出是关节角度（范围 $[-\pi, \pi]$），FP16 在这个范围内的精度约 $10^{-3}$ rad，远小于 PD 控制器的控制精度。但需要验证：对 1000 个随机输入，FP16 和 FP32 输出的最大绝对误差 < $10^{-2}$ rad。

### 端侧推理 vs 云端推理

对于 VLA 等超大模型（7B+ 参数），端侧推理在当前硬件上不可行。两种工程方案：

| 方案 | 延迟 | 可靠性 | 成本 | 适用场景 |
|------|------|--------|------|---------|
| 端侧推理（Jetson） | < 5 ms | 极高 | 硬件成本 | 小模型（< 50M） |
| 5G/WiFi 云端推理 | 50-200 ms | 受网络影响 | 云计算成本 | 大模型（> 1B） |
| 混合方案 | 小模型端侧 + 大模型云端 | 高（安全回退到端侧） | 中等 | 研究前沿 |

**混合方案的架构**：端侧运行一个小 MLP 作为"安全策略"（始终保证基本行走能力），云端运行 VLA 提供高层指令（语义导航、任务规划）。当网络中断时，自动回退到端侧策略——这正是 64.7 节安全降级的自然延伸。
...
> 🧠 **思维陷阱：认为 VLA 会完全替代小策略**
> **新手想法**："VLA 这么强，以后不需要小 MLP 了"
> **实际上**：安全关键系统永远需要一个**可验证的安全策略**作为 fallback。VLA 的黑箱性质使其无法提供形式化安全保证。在可预见的未来，小 MLP（或 MPC）作为安全策略 + 大模型作为高层决策将是工程标准架构

### 练习

**练习 64.10.1**：将 足式/190_腿足RL训练栈 训好的策略从 FP32 量化到 FP16（使用 ONNX Runtime 的量化工具或 TensorRT 的 --fp16 选项）。测量：(a) 推理延迟的变化；(b) 1000 个随机输入上的最大输出误差；(c) 在仿真中部署量化策略，行走性能是否有可测量的退化？

**练习 64.10.2**：设计一个"小 MLP 端侧 + 大 VLA 云端"的混合推理架构。画出系统图，标注：(a) 每个模型的推理频率和延迟；(b) 端侧-云端通信协议；(c) 网络中断时的降级策略；(d) 安全性保证机制。

---

## 如果跳过本章会怎样

1. **你的 RL 策略永远停留在 Python 仿真中**——无法在真机上运行，因为 Python 的 GIL 和 GC 使延迟不可控
2. **你不知道 sim-to-real 失败的原因**——90% 的 sim-to-real 问题来自观测归一化不一致、关节顺序错误、动作缩放不匹配——都是本章的核心内容
3. **你的真机缺乏安全保护**——没有 SafetyGuard 的策略部署是"裸奔"，一次 NaN 就可能损坏硬件
4. **你无法理解 足式/210_RL与MPC混合范式 的混合架构**——RL+MPC 混合的工程实现依赖本章的 C++ 推理基础
...
| 阅读方式 | 时间 | 适合谁 |
|---------|------|--------|
| 精读（含练习和 benchmark） | 25-35 小时 | 需要在真机上部署 RL 的工程师 |
| 速读（跳过 rl_sar 精读和 Jetson 细节） | 10-15 小时 | 了解部署流程但暂不需要真机 |
| 速查（只看对比表和代码模板） | 2-3 小时 | 遇到具体部署问题时回来查 |
...
| 误解 | 正确理解 |
|------|---------|
| "Python 推理在真机上够用" | GIL + GC 导致延迟不确定，生产级部署必须用 C++ |
| "TorchScript trace 和 script 效果一样" | trace 只捕获一次执行路径，有控制流的网络必须用 script |
| "ONNX Runtime 一定比 LibTorch 快" | 对小 MLP 差异在 20% 以内，需要在目标平台实测 |
| "导出模型后不需要验证" | 必须用 100+ 随机输入验证 Python 和 C++ 输出一致性 < $10^{-5}$ |
| "安全降级只是 NaN 检查" | 需要多层检查（NaN + 限位 + 变化率 + 统计异常）和分级降级 |
| "观测归一化可以在 C++ 端重算" | 归一化参数必须从训练端导出，不能重新统计——分布不同 |
| "新模型训好直接部署" | 需要版本管理、warmup、灰度测试、回滚机制 |
| "FP16 量化会破坏策略" | 对腿足 RL，FP16 精度损失通常 < 0.1%，完全可接受 |
...
| 工具/库 | 版本 | 状态 | 说明 |
|---------|------|------|------|
| LibTorch | PyTorch 2.7+ | 活跃 | C++ 前端，ABI 稳定 |
| ONNX Runtime | 1.25+ | 活跃 | 跨框架推理引擎 |
| TensorRT | 10.x | 活跃 | NVIDIA GPU 极致优化 |
| rl_sar | 社区维护 | 活跃 | 腿足 RL 部署参考 |
| robot_lab | 社区维护 | 活跃 | IsaacLab 训练端配套 |
| PyTorch JIT | PyTorch 2.7+ | 内置 | TorchScript IR 和优化 pass |
| CUDA Toolkit | 12.x | 活跃 | GPU 推理的底层依赖 |
...
| 研究方向 | 与本章的关系 | 代表工作 |
|---------|------------|---------|
| Diffusion Policy 部署 | 64.10 的直接应用——推理延迟 5-20 ms，需要异步架构 | Chi et al. 2023 |
| VLA 端侧推理 | 64.10 的前沿挑战——7B 参数的模型如何在嵌入式平台运行 | OpenVLA, pi0 |
| 安全 RL | 64.7 的理论化——形式化安全保证而非启发式检查 | Constrained Policy Optimization |
| 在线适应 | 64.9 A/B 测试的扩展——策略在真机上持续学习 | RMA, RAPID |
| 多机器人部署 | 64.9 版本管理的扩展——多台机器人同时更新策略 | Fleet Learning |
| INT8 量化 | 64.10 FP16 的进一步——通过校准数据集实现更激进的量化 | TensorRT INT8 |
| 模型蒸馏 | 64.10 的实用方案——将大 Teacher 策略蒸馏为小 Student 用于部署 | Miki 2022 |
| 实时性能分析 | 64.6 的工业化——CI/CD 管线中自动检测推理性能回退 | MLOps for Robotics |
| 硬件感知调度 | 64.6 的扩展——根据 CPU/GPU 负载动态调整推理频率 | Adaptive Control |
| 联邦学习部署 | 64.9 的分布式扩展——多台机器人协同更新策略 | Federated RL |

**未来展望**：RL C++ 部署正在从"把模型搬到 C++"的工程问题，演变为"如何在计算受限的边缘设备上运行越来越大的模型"的系统性挑战。掌握本章的基础知识（TorchScript/ONNX 导出、实时约束、安全降级）是应对这一挑战的必要条件——它们在技术演进中保持不变，只是应用的模型规模在变化。

**部署技术的三个时代**：

| 时代 | 模型规模 | 推理延迟 | 部署方式 | 代表时期 |
|------|---------|---------|---------|---------|
| MLP 时代 | 50K-500K 参数 | 50-200 $\mu$s | CPU 单线程推理 | 2019-2024 |
| Transformer 时代 | 1M-50M 参数 | 0.5-20 ms | GPU 异步推理 | 2024-2026 |
| Foundation Model 时代 | 100M-7B 参数 | 50-500 ms | 边缘-云端混合 | 2026+ |

我们正处于 MLP 时代向 Transformer 时代的过渡期。本章的内容覆盖了 MLP 时代的完整部署技术栈，并在 64.10 节展望了 Transformer/Foundation Model 时代的架构变化。

---

## 知识导航回顾

回到本章开头的核心问题：如何把 Python 训好的 RL 策略搬到 C++ 实时循环中？现在我们有了完整的答案：

```
问题分解:
  1. 为什么要用 C++？      → 64.1（GIL + GC + 延迟不确定性）
  2. 模型怎么导出？        → 64.2（TorchScript trace/script）
  3. C++ 怎么加载推理？    → 64.3-64.4（LibTorch / ONNX Runtime）
  4. 观测怎么对齐？        → 64.5（rl_sar 精读，逐项对齐清单）
  5. 推理延迟达标吗？      → 64.6（预分配 + warmup + 监控）
  6. 出错了怎么办？        → 64.7（4 级安全降级）
  7. 完整流程怎么走？      → 64.8（8 步 pipeline + Jetson 部署）
  8. 版本怎么管理？        → 64.9（版本管理 + A/B 测试）
  9. 未来怎么扩展？        → 64.10（大模型策略部署展望）
```

---

分别掌握了 MPC(足式/110_OCS2完整栈与双线程MPC)和 RL(足式/190_腿足RL训练栈)之后，自然的问题是:能否融合两者的优势?足式/210_RL与MPC混合范式 正是腿足控制最活跃的研究前沿——OCS2 MPC-Net、Cafe-MPC VWBC、RAMBO、残差 RL 四条 RL+MPC 混合路线都要讲透。足式/220_腿足感知数据结构 则衔接到感知的工程侧——讲清楚 Grid Map、Elevation Mapping 等数据结构和工程实现，为 足式/230_Perceptive_MPC 的 Perceptive MPC 算法做好数据层准备。
