...
> **本章定位**：这是腿足机器人动力学的数学核心。从"为什么 3D 向量不够用"出发，构建 6D 空间向量代数的完整框架，最终推导出浮动基座的全身动力学方程。本章是 足式/90_WBC分层优化与TSID（WBC）和 足式/110_OCS2完整栈与双线程MPC（OCS2 MPC）的直接前置——不理解本章内容，后续所有控制算法都无法深入。
>
> **前置依赖**：Eigen 基础（02_C++基础与进阶/40_通用库剖析）、李群与 manif（01_数学/20_微分几何与李群，SE(3)/SO(3)/伴随表示）、足式/30_Pinocchio深度精读（Pinocchio Model-Data 架构）
>
...
1. **[02_C++基础与进阶/40_通用库剖析]** 给定 $3 \times 3$ 反对称矩阵 $[\boldsymbol{\omega}]_\times$，写出它的显式形式。它与叉积 $\boldsymbol{\omega} \times \boldsymbol{v}$ 的关系是什么？

2. **[01_数学/20_微分几何与李群]** $SE(3)$ 群的元素 $T = (R, t)$ 如何作用于一个点 $\boldsymbol{p} \in \mathbb{R}^3$？写出变换公式。

3. **[01_数学/20_微分几何与李群]** 什么是 $SE(3)$ 的伴随表示 $\operatorname{Ad}_T$？它是一个多大的矩阵？它把什么映射到什么？

4. **[足式/30_Pinocchio深度精读]** 在 Pinocchio 中，`model.nq` 和 `model.nv` 分别表示什么？为什么对于含 FreeFlyer 关节的模型，`nq != nv`？

5. **[刚体力学]** 牛顿-欧拉方程写成什么形式？平动部分和转动部分分别是什么？

> 如果第 3 题答不出来——本章的数学核心就是 $\operatorname{Ad}_T$ 在 6D 空间中的具体矩阵形式。请务必先回 01_数学/20_微分几何与李群 复习李群伴随表示。
...
7. **理解** Centroidal Momentum Matrix 如何桥接全身动力学与简化模型（足式/70_腿足简化模型理论）
...
在 01_数学/20_微分几何与李群 中我们学过，$\mathfrak{se}(3)$ 是 $SE(3)$ 的李代数，由 $4 \times 4$ 矩阵组成：
...
在 WBC（足式/90_WBC分层优化与TSID）和 MPC（足式/110_OCS2完整栈与双线程MPC）中，接触 wrench 通过接触 Jacobian $J_c$ 映射到广义力空间：
...
$$\mathbf{p}_i^A = \mathbf{V}_i \times^* (\hat{I}_i \mathbf{V}_i) + \sum_{j:\lambda(j)=i} {}^i\!X_j^* \mathbf{p}_j^A$$
...
在 足式/70_腿足简化模型理论 中，我们将学习 LIPM（线性倒立摆模型）和 SRBM（单刚体模型）等简化模型。这些模型把整个机器人视为一个"质点"或"刚体"，用质心的运动来规划步态。
...
这两个方程是 足式/70_腿足简化模型理论 中所有简化模型的出发点：
...
```python
"""
足式/50_空间向量与浮动基座动力学 完整实操：Go2 空间向量代数与动力学验证
...
## 49.12 质心动力学与 RNEA/CRBA 的关系推导 ⭐⭐⭐

### 为什么需要质心动力学——从全身到简化的严格桥梁 ⭐⭐⭐

49.10 节引入了 Centroidal Momentum Matrix（CMM）$A_G$ 和质心处的 Newton-Euler 方程。但一个关键问题尚未回答：CMM 与我们在 49.6-49.8 学过的 RNEA/CRBA 之间的精确数学关系是什么？理解这个关系对于腿足 MPC 至关重要——因为 MPC 常用的单刚体模型（SRBD）本质上就是从全身动力学通过 CMM 降维得到的。

**从全身动力学方程出发**。浮动基座的全身动力学方程为：

$$M(q)\dot{v} + h(q,v) = S^T\tau + J_c^T\lambda$$

CMM 的定义为 $\mathbf{h}_G = A_G(q) v$，其中 $\mathbf{h}_G = (\boldsymbol{k}_G, \boldsymbol{l}_G)^T$ 是质心处的角动量和线动量。（本教程约定角动量在前、线动量在后，与部分 Pinocchio API 版本的排列可能不同。）

**关键推导**：对 $\mathbf{h}_G = A_G v$ 两边求时间导数：

$$\dot{\mathbf{h}}_G = \dot{A}_G v + A_G \dot{v}$$

利用质心处的 Newton-Euler 方程 $\dot{\mathbf{h}}_G = \sum_c X_c^* \mathbf{f}_c + \mathbf{g}_G$（其中 $\mathbf{g}_G$ 是质心处的重力 wrench），可以得到：

$$A_G \dot{v} = \sum_c X_c^* \mathbf{f}_c + \mathbf{g}_G - \dot{A}_G v$$

这正是全身动力学方程 $M\dot{v} + h = S^T\tau + J_c^T\lambda$ 的**前 6 行**（对应基座的 6 个自由度）在质心坐标系下的等价形式。

> **本质洞察**：CMM $A_G$ 本质上是质量矩阵 $M$ 的前 6 行在质心坐标系下的表达。CRBA 计算完整的 $M \in \mathbb{R}^{n_v \times n_v}$，而 CCRBA 只计算其中的"前 6 行"——这就是为什么 CCRBA 的计算量远小于 CRBA。对于 MPC 来说，如果你只需要质心动力学（如 SRBD 模型），CCRBA 就足够了，不需要计算完整的质量矩阵。

### Composite Rigid Body vs Articulated Body：直觉对比 ⭐⭐⭐

CRBA 和 ABA 是 Featherstone 算法族中最容易混淆的两个算法。它们都是"从树叶到树根的聚合"，但聚合的对象和目的完全不同。

**Composite Rigid Body（复合刚体）思想**：

想象把一棵关节树从某个关节"冻结"——冻结关节 $i$ 以下的所有关节，使得从关节 $i$ 到所有末端的连杆变成一个刚体（就像用胶水把它们粘在一起）。这个"粘合体"的 $6 \times 6$ 空间惯量就是**复合惯量** $I_i^c$。CRBA 通过从叶到根逐级聚合这些复合惯量来构建质量矩阵 $M$。

**直觉类比**：CRBA 就像称一根连锁项链的重量——你可以从最末端开始，逐节把珠子的重量累加到链的根部。每一节的"累积重量"就是该节的复合惯量。最终你得到的是**整条链的惯量分布矩阵**（即 $M$）。

**Articulated Body（铰接体）思想**：

铰接体惯量 $I_i^A$ 更加复杂——它不是简单的"冻结后的刚体惯量"，而是考虑了关节 $i$ 以下的子树在**受力后会如何运动**的等效惯量。关节 $i$ 以下的连杆不是被"冻结"的，而是被视为一个**可以在施加力矩后自由运动的子系统**。$I_i^A$ 编码了"如果我在关节 $i$ 施加一个 wrench，关节 $i$ 以下的整个子树会如何响应"这一信息。

**直觉类比**：ABA 就像计算一串弹簧连接的质量块系统的等效阻抗——你不能简单把质量加起来（那是 CRBA 的做法），而必须考虑弹簧（关节自由度）允许各质量块独立运动的效果。铰接体惯量是一个"降秩"的惯量——它比复合惯量小，因为关节自由度"吸收"了部分运动。

| 维度 | CRBA（复合刚体） | ABA（铰接体） |
|------|----------------|--------------|
| **目的** | 构建质量矩阵 $M(q)$ | 计算正动力学 $\dot{v} = f(q, v, \tau)$ |
| **聚合对象** | 复合惯量 $I_i^c$（冻结子树） | 铰接惯量 $I_i^A$（活跃子树） |
| **聚合方向** | 叶 → 根（一遍） | 根 → 叶 → 根 → 叶（三遍：前向速度传播 → 后向铰接惯量聚合 → 前向加速度计算） |
| **复杂度** | $O(N^2)$ | $O(N)$ |
| **关节自由度的影响** | 不考虑（冻结关节） | 显式考虑（关节允许运动） |
| **典型用途** | 需要显式 $M$ 的场景（QP、CRBA+Cholesky 逆动力学） | 仿真器中的正动力学积分 |

> **反事实推理**：如果不用 ABA 而用 CRBA+Cholesky 来做正动力学，计算 $\dot{v} = M^{-1}(S^T\tau + J_c^T\lambda - h)$ 需要 $O(N^2)$ 构建 $M$ + $O(N^3)$ Cholesky 分解。对于 Go2 这样 $n_v = 18$ 的系统，$N^3 \approx 5800$，尚可接受。但对于人形机器人（$n_v = 30+$），$N^3 \approx 27000$，ABA 的 $O(N)$ 优势就非常明显了。这就是为什么 MuJoCo 的默认正动力学求解器使用 ABA 变体而非 CRBA+Cholesky。
...
空间向量代数 (1987, Featherstone)
    ...
    Centroidal Momentum: hG = AG q̇              [简化投影]
         ↓
    简化模型 (LIPM / SRBM → 足式/70_腿足简化模型理论)              [步态规划]
```
...
| 天 | 内容 | 任务 |
|----|------|------|
| 第 1 天 | 49.0-49.1 | 前置自测 + 空间向量动机，读 Featherstone 第2章前半 |
...
| 第 14 天 | 复习 + 习题 | 总结笔记，完成所有练习，预习 足式/60_QP_NLP建模（QP/NLP） |

> **下一章预告**：足式/60_QP_NLP建模 将介绍 QP 和 NLP 建模——掌握了本章的动力学方程后，足式/60_QP_NLP建模 将把它们作为优化问题的约束条件，为 足式/90_WBC分层优化与TSID（WBC）和 足式/110_OCS2完整栈与双线程MPC（OCS2 MPC）做最后的数学准备。

---

## 累积项目：四足控制器——空间向量计算 + 浮动基座全身动力学模块

> 本模块是"累积项目：四足控制器"的动力学基础层。完成后你将拥有一套可复用的空间向量工具库和完整的全身动力学计算管线，为后续 WBC（足式/90_WBC分层优化与TSID）和 MPC（足式/110_OCS2完整栈与双线程MPC）提供核心接口。
...
- 验证 $\dot{\mathbf{h}}_G$ 与接触力之间的关系（足式/70_腿足简化模型理论 简化模型的数学基础）
