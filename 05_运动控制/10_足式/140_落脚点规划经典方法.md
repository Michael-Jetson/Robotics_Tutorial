...
| MIQP 求解超时或返回 infeasible | 候选接触区域定义不合理、Big-M 值过大导致松弛间隙、或整数变量过多 | 1. 减少候选区域数量（只保留可达范围内的区域） 2. 检查 Big-M 值是否紧致 3. 给 MIQP 设置时间上限并使用 warm-start | 足式/140_落脚点规划经典方法.9 |
...
| 症状 | 可能原因 | 排查步骤 | 相关章节 |
|------|---------|---------|---------|
| Raibert落脚偏移 | k_p标定不准 | 用Poincare映射离线标定k_p；扫参对比收敛速度 | 足式/140 §58.2 |
| 落脚超出可达范围 | 步幅限制未考虑 | 添加kinematic reachability约束；clamp到髋关节可达半径内 | 足式/140 §58.3 |
| 高速时Capture Point不稳 | LIPM近似失效 | 切换到SRB-based或预览控制；检查质心高度假设 | 足式/70 |
| ZMP计算抖动 | F/T传感器噪声/加速度估计差 | 低通滤波ZMP信号；检查传感器采样率与滤波器截止频率 | 足式/130 |
| 地面投影高度错误 | 地形估计延迟 | 融合IMU高度与elevation map；检查TF时间戳同步 | 足式/160 |
...
当地形表示为凸区域的并集时(如通过 IRIS 算法分解),落脚点到区域的分配是整数变量,整个问题成为 MIQP——这就是 Deits & Tedrake 2014 的核心贡献(58.9 节详述)。
