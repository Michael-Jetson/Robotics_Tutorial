...
**问题设定**:考虑一个半隐式(semi-implicit)时间步格式,把动力学离散在速度级。回顾 足式/50_空间向量与浮动基座动力学:浮动基座动力学方程为 $M(q)\dot{v} + h(q,v) = S^T\boldsymbol{\tau} + J_c^T\boldsymbol{\lambda}$,其中 $M$ 是 $n_v\times n_v$ 的对称正定质量矩阵,$h$ 含科氏力与重力,$J_c$ 是接触 Jacobian(法向行记为 $J_n$)。我们想求解一个时间步 $\Delta t$ 后的接触冲量 $\boldsymbol{\lambda}$(这里 $\boldsymbol{\lambda}$ 已吸收 $\Delta t$,量纲是冲量 N·s)。
