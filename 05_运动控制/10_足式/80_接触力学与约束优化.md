...
> **难度**: ⭐⭐~⭐⭐⭐ | **前置**: 足式/50_空间向量与浮动基座动力学 空间向量, 足式/60_QP_NLP建模 QP/NLP, 足式/70_腿足简化模型理论 简化模型 | **学时**: 20-25 小时
...
1. **空间力(Wrench)的 6 维表达** $\boldsymbol{f} = [\boldsymbol{n}^T, \boldsymbol{f}^T]^T$ 中,力矩和力的排列顺序是什么?(→ 足式/50_空间向量与浮动基座动力学)
2. **QP 问题的标准形式** $\min \frac{1}{2}x^T H x + g^T x$ s.t. $Ax \le b$ 中,不等式约束如何编码"单侧接触"?(→ 足式/60_QP_NLP建模)
3. **Centroidal Dynamics** 的 6 维动量 $\boldsymbol{h}_G = [k^T, l^T]^T$ 的物理含义是什么?(→ 足式/70_腿足简化模型理论)
4. **Pinocchio** 中 `getFrameJacobian()` 返回的矩阵维度是什么?`LOCAL_WORLD_ALIGNED` 参考系的含义?(→ 足式/30_Pinocchio深度精读)
5. **LIPM** 假设角动量为零,这在什么情况下会严重失效?(→ 足式/70_腿足简化模型理论)
...
3. **理解多面体锥的 H-rep 与 V-rep 对偶**——并用 GIWC 给出"机器人能否静止站稳"的几何判据
4. **推导刚体动力学到 LCP 的完整链条**——Delassus 算子与互补约束的来龙去脉
5. **理解互补约束的三大处理路线**——预定义模式/软化/Contact-Implicit TO——及各自的数学基础
6. **推导接触 Jacobian 的对偶关系**——从运动空间到力空间的映射
7. **掌握 CCRBA 算法**——O(N) 计算 CMM 的完整步骤
8. **理解角动量的守恒与可控性**——飞行相位的约束与猫翻身悖论
9. **能把接触约束编码为 QP/NLP 约束**——为 足式/90_WBC分层优化与TSID WBC 和 足式/100_DDP家族与Crocoddyl DDP 打基础

---

## 知识导航

本章围绕一条主线展开:**接触力受什么约束（物理）→ 这些约束如何进入优化器（建模）→ 接触力如何在动力学里传递（桥梁）→ 这套机制在控制、仿真与学习中如何落地（应用）**。下表给出全章 10 个大节的定位，括号内是难度与所属层次。

| 节 | 主题 | 在主线中的位置 |
|----|------|--------------|
| **52.1** 三大铁律 ⭐⭐ | 法向非负 / 摩擦锥 / 互补性 | 物理：接触力必须满足什么 |
| **52.2** 摩擦锥线性化 ⭐⭐ | 多面体近似、误差界、Wrench Cone（52.2.4）、H-rep/V-rep 对偶（52.2.5）、静平衡判据（52.2.6） | 建模：把凸的 SOC 约束变成 QP 能吃的线性不等式，并给出"能否站稳"的几何判据 |
| **52.3** 互补约束三路线 ⭐⭐⭐ | 刚体动力学→LCP（52.3.0）、预定义模式 / 软化 / Contact-Implicit TO | 建模：非光滑的互补性怎么交给求解器 |
| **52.4** 接触 Jacobian ⭐⭐ | 对偶映射 $J_c/J_c^T$、漂移项、参考系选择 | 桥梁：接触力与关节空间互相翻译 |
| **52.5** CMM 与 CCRBA ⭐⭐⭐ | O(N) 计算质心动量 | 桥梁：Centroidal 动力学的计算基础 |
| **52.6** 角动量 ⭐⭐ | 守恒、可控性、猫翻身、摆臂 | 应用：理解全身动力学相对 Centroidal 模型的盲区 |
| **52.7** 非线性 vs 线性化 ⭐⭐⭐ | 精度-性能权衡、SOC 求解器 | 应用：工程选型 |
| **52.8** 软接触 vs 刚体接触 ⭐⭐⭐ | LCP vs 弹簧阻尼 / Hydroelastic | 应用：仿真器选择 |
| **52.9** RL 仿真中的接触 ⭐⭐⭐ | 仿真器对比、域随机化 | 应用：连接 RL 训练栈 |

**知识依赖关系**：52.1（物理）是全章地基；52.2 与 52.3 是两条并列的"建模"支线（凸的摩擦锥 → 线性化；非光滑的互补性 → 三路线），二者在 52.3.0 处由 LCP 串起来；52.4、52.5 是把接触力接回动力学的两座"桥"；52.6–52.9 都是建立在前面之上的应用层。其中 52.2.5/52.2.6（锥的对偶与静平衡）与 52.6（角动量可控性）共享同一个几何工具——**接触力锥张成的可达 wrench 集合**。

**推荐阅读路径**：

- **速查（2 h）**：52.1 三大铁律 → 52.2.1 线性化推导 → 52.4.1 接触 Jacobian 对偶 → 52.10 小结表。只求知道"有哪些约束、怎么进 QP"。
- **主线（精读 12–15 h）**：按 52.1 → 52.2（含 52.2.4–52.2.6）→ 52.3（含 52.3.0）→ 52.4 → 52.5 → 52.6 顺序通读，这是 WBC/MPC 工程师必经之路。
- **进阶（研究向，+8 h）**：在主线基础上深入 52.3.3（Contact-Implicit TO）、52.7–52.9（工程权衡与仿真），以及延伸阅读中的可微接触前沿。
...
**反面案例**:假设你在 足式/60_QP_NLP建模 的 QP 中忘记加摩擦锥约束,只加了牛顿-欧拉方程作为等式约束。求解器会怎么做?
...
> **关键区别**: 力和力矩不是独立的。如 Caron 等(2015)所指出:固定合力后,力矩的可行范围(moment cone)的形状会随之改变。因此 6D wrench cone 不是 3D 力锥和 3D 力矩锥的简单笛卡尔积——它们之间有耦合约束。
...
### 52.2.5 两种锥表示——H-rep 与 V-rep 的对偶 ⭐⭐⭐

到目前为止,52.2.1–52.2.4 节都把摩擦锥/wrench cone 写成"一堆线性不等式"——$A\boldsymbol{\lambda}\le0$。这叫**半空间表示(half-space representation, H-rep)**:锥是若干半空间的交。但同一个多面体锥还有**另一种完全等价的表示**,而且在很多场景下更好用——**生成元表示(vertex/ray representation, V-rep)**:锥是若干"射线"(生成元 generator)的非负组合。

**动机:为什么需要两种表示?** 考虑一个具体痛点——你想知道"四足机器人四只脚的接触力,合成到质心后,能产生哪些总的力/力矩?"用 H-rep 很难直接回答,因为每只脚的力是被不等式**约束**的;但用 V-rep 就很自然——把每只脚摩擦锥的生成元(锥的"棱")收集起来,合成到质心,**它们张成的锥就是答案**。这正是 52.6 节角动量可控性、以及"机器人能否站稳"问题的几何核心。

**V-rep 的形式**:把 $k$ 边线性化摩擦锥的 $k$ 条棱写成生成元(单位射线)。第 $i$ 条棱方向(在接触局部系,$z$ 为法向)为:

$$\boldsymbol{g}_i = \begin{bmatrix} \mu\cos\theta_i \\ \mu\sin\theta_i \\ 1 \end{bmatrix}, \quad \theta_i = \frac{2\pi i}{k}, \quad i = 0,\dots,k-1$$

任何锥内的接触力都可以写成这些生成元的**非负组合**:

$$\boldsymbol{\lambda} = \sum_{i=0}^{k-1} \beta_i \boldsymbol{g}_i, \quad \beta_i \ge 0$$

这与 H-rep 描述的是**同一个**多面体锥——这是凸分析里的 **Minkowski-Weyl 定理**:每个多面体锥既可以写成有限多个半空间的交(H-rep),也可以写成有限多条射线的非负包(V-rep),两者可以相互转换。

> **本质洞察**:H-rep 和 V-rep 不是两种不同的锥,而是**同一个锥的"内"与"外"两种视角**。H-rep 从外部"切"——每个不等式砍掉一个半空间,适合**检验**一个给定的力是否合法(逐行代入即可)。V-rep 从内部"撑"——每条生成元从原点向外射,适合**生成/参数化**所有合法的力(给一组 $\beta_i\ge0$ 就得到一个合法力)。优化建模时这个区别是实打实的:WBC 把接触力当决策变量、要施加约束,所以用 H-rep($A\boldsymbol{\lambda}\le0$ 直接进 QP);而可达性分析要枚举所有可能的合力,所以用 V-rep。

**Contact Wrench Cone(CWC)的两种算法**:52.2.4 节给出的矩阵 $U$ 是单个矩形面接触 wrench cone 的 H-rep。当多个接触点协同时(人形双脚 + 双手,或四足四脚),把每个接触的力锥变换到公共参考系(质心)并求 **Minkowski 和**,得到的总锥称为 **Gravito-Inertial Wrench Cone (GIWC)**。计算它有两条路:

| 路线 | 输入表示 | 算法 | 适用 |
|------|---------|------|------|
| 闭式公式 | 单个矩形接触 | Caron et al. 2015 (ICRA) 给出 CWC 的解析 $U$ 矩阵 | 单面接触(人形单脚支撑) |
| 双重描述法 | 多接触力锥的 V-rep | Caron et al. 2015 (RSS) 用 double description 求 Minkowski 和后转回 H-rep | 多接触(双脚/多肢) |

**双重描述法(Double Description, DD)** 是 V-rep ↔ H-rep 相互转换的经典算法(Motzkin et al. 1953)。它的作用:先把所有接触锥的生成元(V-rep)合成,得到总锥的 V-rep;再转成 H-rep,这样就能用一组紧凑的不等式 $A_{\text{GIWC}}\boldsymbol{w}_G\le0$ 判断"质心 wrench 是否可达"。`pycddlib`(C 库 `cdd` 的 Python 绑定)是常用实现。

```python
import numpy as np
import cdd  # pycddlib: double description method

def friction_cone_generators(mu, k=4):
    """Return V-rep generators (rays) of a linearized friction cone."""
    rays = []
    for i in range(k):
        th = 2 * np.pi * i / k
        rays.append([mu * np.cos(th), mu * np.sin(th), 1.0])
    return np.array(rays)  # shape (k, 3), each row is a ray

def cone_v_to_h(rays):
    """Convert a cone from V-rep (rays) to H-rep (A x <= 0) via DD method.

    cdd format: each generator row is [t, x...]; t=0 marks a ray (not vertex).
    Output inequalities are b + A x >= 0; for a cone b=0, so A x <= 0
    corresponds to the negated cdd rows.
    """
    gens = np.hstack([np.zeros((rays.shape[0], 1)), rays])  # t=0 => ray
    mat = cdd.Matrix(gens, number_type='float')
    mat.rep_type = cdd.RepType.GENERATOR
    poly = cdd.Polyhedron(mat)
    ineq = np.array(poly.get_inequalities())  # rows: [b, a...] meaning b + a.x >= 0
    return ineq  # H-rep of the same cone
```

> **⚠️ 编程陷阱:V-rep 与 H-rep 的"组合爆炸"方向相反**
>
> **错误**: 以为"边数 $k$ 越大,两种表示都只是线性增长"。
>
> **现象**: 对**多接触** Minkowski 和,V-rep 的生成元数是各接触生成元数之**和**($n_c\cdot k$,线性);但转成 H-rep 后的不等式行数可能**指数级**膨胀(最坏 $O(k^{n_c})$),DD 算法在高维退化锥上会变得很慢。
>
> **根因**: 多面体锥的"面数"和"棱数"没有简单的线性关系——这是计算几何里 H↔V 转换的固有难度(McMullen 上界定理)。
>
> **正确做法**: 多接触时**离线**预计算 GIWC 的 H-rep(机器人接触几何固定时锥不变),在线只做矩阵乘法判定;或直接在 WBC 里用每接触的 H-rep 而不显式构造 GIWC。

### 52.2.6 静平衡可行性——"机器人能站稳吗?"的几何判据 ⭐⭐⭐

V-rep 不仅是数学换皮,它直接回答一个工程问题:**给定一组接触点和摩擦系数,机器人能否静止站立?** 这是练习 52.1.1(斜坡站立)和 52.6 节(支撑域)反复用到的判据,这里给出完整的判定路径。

**静平衡条件**:机器人静止($\dot{v}=0$,所有 link 速度为零)时,Newton-Euler 方程退化为"接触力的合力/合力矩 = 重力的合力/合力矩"。把所有接触力变换到质心 $G$ 处求和,记总接触 wrench 为 $\boldsymbol{w}_G^{\text{contact}}=\sum_c{}^G\boldsymbol{X}_c^*\boldsymbol{\lambda}_c$,则:

$$\boldsymbol{w}_G^{\text{contact}} = -\boldsymbol{w}_G^{\text{gravity}} = \begin{bmatrix} \boldsymbol{0} \\ -m\boldsymbol{g} \end{bmatrix}_G$$

(重力过质心,绕质心力矩为零,所以右边角动量率部分是 $\boldsymbol{0}$。)

**判据**:机器人能静止站立 $\iff$ 重力 wrench 的相反数落在 **GIWC** 内部,即:

$$\boxed{-\boldsymbol{w}_G^{\text{gravity}} \in \mathcal{W}_{\text{GIWC}} = \Big\{\sum_c{}^G\boldsymbol{X}_c^*\boldsymbol{\lambda}_c : \boldsymbol{\lambda}_c\in\text{friction cone}_c\Big\}}$$

这正是 V-rep 大显身手的地方:GIWC 是各接触摩擦锥(用生成元描述)变换、Minkowski 求和后张成的锥。判定"重力 wrench 是否在锥内"等价于一个**小的线性可行性问题**(LP):

$$\text{找 } \beta_{c,i}\ge0 \text{ 使得 } \sum_{c,i}\beta_{c,i}\,({}^G\boldsymbol{X}_c^*\boldsymbol{g}_{c,i}) = -\boldsymbol{w}_G^{\text{gravity}}$$

若该 LP 可行,则存在一组合法接触力支撑重力,机器人能站稳;若不可行(LP 无解),无论怎么分配接触力都无法平衡重力——必然滑倒或倾覆。

**回到练习 52.1.1 的斜坡**:四足站在 $30°$ 斜坡、$\mu=0.5$。单脚摩擦锥半顶角 $\alpha=\arctan(0.5)\approx26.6°$。重力相对**斜面法向**的倾角恰好是斜坡角 $30°$。由于 $30° > 26.6°$,重力方向**落在摩擦锥之外**——LP 无解,机器人无法仅靠静摩擦站稳,会下滑。临界摩擦系数是 $\mu_{\min}=\tan30°\approx0.577$。这就是为什么练习问"$\mu=0.5$ 能否站立"——答案是不能,需要 $\mu\ge0.577$。

> **本质洞察**:静平衡判据把"力学问题"彻底转化成了"几何包含问题"——能不能站稳,等价于**重力这一个向量,是否落在接触摩擦锥张成的那个锥里**。这就是为什么足式机器人领域如此痴迷于各种"区域"(support polygon、ZMP region、CoM static-equilibrium region):它们都是 GIWC 在不同子空间上的**投影**。ZMP 必须落在支撑多边形内,本质上就是"重力+惯性 wrench 落在 GIWC 内"在平地、点接触假设下的退化特例——这条线索把本章的摩擦锥和 足式/70_腿足简化模型理论 的 ZMP 判据统一了起来。

> **🧠 思维陷阱:静平衡可行 $\ne$ 动态稳定**
>
> **陷阱**: "GIWC 判据说能站稳,机器人就一定不会倒。"
>
> **现实**: GIWC 判据只保证存在一组**静态**接触力平衡重力(必要条件),它不考虑(1)关节力矩能否真的产生这组接触力(受 52.1.5 节力矩限幅约束);(2)受扰动后能否恢复(这是动态稳定性,需要 capturability/MPC 分析)。"能摆出一个平衡姿态"和"能稳定维持"是两件事——前者是静力学,后者是控制。
...
**练习 52.2.3** (⭐⭐): 阅读 OCS2 的 `FrictionConeConstraint.cpp` 源码,回答:它是把摩擦锥**线性化成多面体棱**,还是保留为**非线性二阶锥(SOC)**?它返回的是每个接触点几条约束还是一条标量约束?约束值表达式中的 `gripperForce` 和 `regularization` 各起什么作用?(提示:核心是 `coneConstraint()` 返回 `mu*(Fz + gripperForce) - sqrt(Fx^2 + Fy^2 + regularization)`,这是单条 SOC 标量约束;OCS2 通过 augmented Lagrangian + 一/二阶近似把它喂给求解器,而非预先切成 $k$ 条直线。)
...
### 52.3.0 从刚体动力学到 LCP——互补约束的来龙去脉 ⭐⭐⭐

在 52.1.3 节我们断言"接触问题是 LCP 的标准形式",但没有展示**刚体动力学方程是怎么一步步变成 LCP 的**。这一步推导是理解后续三大路线的前提——只有看清 LCP 的结构,才能明白为什么"预定义模式"能绕开它、为什么"软化"在做什么近似。本节补上这条推导链。

**问题设定**:考虑一个半隐式(semi-implicit)时间步格式,把动力学离散在速度级。回顾 足式/50_空间向量与浮动基座动力学:浮动基座动力学方程为 $M(q)\dot{v} + h(q,v) = S^T\boldsymbol{\tau} + J_c^T\boldsymbol{\lambda}$,其中 $M$ 是 $n_v\times n_v$ 的对称正定质量矩阵,$h$ 含科氏力与重力,$J_c$ 是接触 Jacobian(法向行记为 $J_n$)。我们想求解一个时间步 $\Delta t$ 后的接触冲量 $\boldsymbol{\lambda}$(这里 $\boldsymbol{\lambda}$ 已吸收 $\Delta t$,量纲是冲量 $\mathrm{N\cdot s}$)。

**步骤 1:离散化动力学,解出下一时刻速度**

用前向欧拉离散加速度项 $\dot{v}\approx (v^+ - v)/\Delta t$,其中 $v$ 是当前速度、$v^+$ 是步末速度:

$$M(v^+ - v) = \Delta t\,(S^T\boldsymbol{\tau} - h) + J_c^T\boldsymbol{\lambda}$$

把已知量(当前速度对未来速度的贡献、外力)合并成"自由速度" $v^{\text{free}} = v + \Delta t\,M^{-1}(S^T\boldsymbol{\tau} - h)$——这是**假设没有接触力**时机器人在下一步会达到的速度。于是:

$$v^+ = v^{\text{free}} + M^{-1}J_c^T\boldsymbol{\lambda}$$

> **阶段小结**:到这里我们把"未来速度"写成了"自由速度 + 接触力的修正项"。接下来要做的是:把速度级约束(脚不能继续穿透)套在 $v^+$ 上,从而解出未知的 $\boldsymbol{\lambda}$。

**步骤 2:写出法向接触点的速度级互补条件**

只看法向方向。接触点的法向相对速度为 $\dot{\phi}^+ = J_n v^+$。速度级 Signorini 条件(52.1.3 节给过)要求:接触点要么分离($\dot{\phi}^+ > 0$,此时无力 $\lambda^n = 0$),要么保持($\dot{\phi}^+ = 0$,此时可以有力 $\lambda^n \ge 0$)。两种情形恰好是互补的:

$$0 \le \dot{\phi}^+ \perp \lambda^n \ge 0$$

为了不穿透,还要加一个 Baumgarte 风格的位置修正偏置 $b = \frac{\gamma}{\Delta t}\phi$(把当前的微小穿透 $\phi<0$ 在一步内"推"回零,$\gamma\in(0,1]$ 是修正比例)。于是约束写成 $\dot{\phi}^+ + b \ge 0$。

**步骤 3:Schur 补——消去 $v^+$,得到只含 $\lambda^n$ 的 LCP**

把步骤 1 的 $v^+$ 代入 $\dot{\phi}^+ = J_n v^+$:

$$\dot{\phi}^+ = J_n v^{\text{free}} + \underbrace{J_n M^{-1} J_n^T}_{=\,\boldsymbol{A}}\,\lambda^n$$

定义 **Delassus 算子** $\boldsymbol{A} = J_n M^{-1} J_n^T$(这是接触空间里的"等效逆惯量"——它通过 $M^{-1}$ 这个 Schur 补把全身惯量投影到接触法向方向)和常数项 $\boldsymbol{w} = J_n v^{\text{free}} + b$。代入互补条件,得到标准 LCP:

$$\boxed{0 \le \lambda^n \perp \boldsymbol{A}\lambda^n + \boldsymbol{w} \ge 0}$$

这就是 LCP 的标准形式 $\text{LCP}(\boldsymbol{A}, \boldsymbol{w})$:求 $\lambda^n \ge 0$ 使得 $\boldsymbol{A}\lambda^n + \boldsymbol{w} \ge 0$ 且 $(\lambda^n)^T(\boldsymbol{A}\lambda^n + \boldsymbol{w}) = 0$。

> **本质洞察**:LCP 不是凭空规定的"数学游戏",而是**把"接触力修正速度"(动力学)和"速度决定有无力"(单边约束)这两个互相缠绕的逻辑用 Schur 补解开后的必然结果**。Delassus 算子 $\boldsymbol{A} = J_n M^{-1}J_n^T$ 是这条推导的核心——它把"一个接触点的力如何通过整副身体的惯量影响所有接触点的速度"压缩成一个稠密的 $n_c\times n_c$ 矩阵。正是这个稠密耦合,使得多接触问题不能逐点独立求解。

**为什么 $\boldsymbol{A}$ 半正定、LCP 有解?** 因为 $M\succ 0$ 所以 $M^{-1}\succ 0$,对任意向量 $x$ 有 $x^T\boldsymbol{A}x = (J_n^T x)^T M^{-1}(J_n^T x) \ge 0$,故 $\boldsymbol{A}\succeq 0$。半正定 LCP 一定有解(Cottle-Pang-Stone 定理),且当 $J_n$ 行满秩时 $\boldsymbol{A}\succ 0$ 解唯一。这解释了 52.4.2 节"`JMinvJt_damping` 在 $J_c$ 不满秩时保证可逆"的工程做法——它在 $\boldsymbol{A}$ 对角线上加 $\epsilon I$,把半正定退化情形正则化为严格正定。

**加入摩擦后:LCP 变成什么?** 上面只处理了法向。一旦把线性化摩擦锥(52.2 节的 $k$ 边)也写进来,切向冲量被表示为锥生成元的非负组合,滑动方向引入额外的互补对,整个问题变成一个**更大的混合 LCP**(Stewart-Trinkle 1996 的经典格式)。维度从 $n_c$ 膨胀到约 $n_c(k+2)$。这正是 52.8.1 节 $w = M^{-1}(\cdots)$、$0\le w\perp\lambda\ge0$ 那个公式的展开来源。

> **💡 概念误区:LCP 是凸优化吗?**
>
> **错误理解**:"LCP 可以写成 QP,所以它总是凸的。"
>
> **正确理解**:当 $\boldsymbol{A}\succeq 0$(对称半正定,纯法向、无摩擦或关联摩擦模型)时,LCP 等价于一个凸 QP $\min \frac12\lambda^T\boldsymbol{A}\lambda + \boldsymbol{w}^T\lambda$ s.t. $\lambda\ge0$,可用 QP 求解器。但**加入库仑摩擦后** $\boldsymbol{A}$ 通常**非对称**(Stewart-Trinkle 格式),对应的不再是凸 QP,而是需要 Lemke 算法或 PATH 求解器的一般 LCP。这就是为什么"刚体接触 + 真实库仑摩擦"在数学上比"纯法向接触"难一个量级。

### 52.3.1 路线 1:预定义接触模式(Mode-Scheduled) ⭐⭐

看清了 LCP 的结构后,回到本节开头的问题:既然互补约束这么难,工程上怎么绕开它?最直接的一条路就是**根本不让求解器去判断接触状态**——这就是第一条路线。
...
| H-rep / V-rep 对偶 | 锥 = 半空间交 = 生成元非负包(Minkowski-Weyl) | 检验力 vs 枚举可达力 | ⭐⭐⭐ |
| Wrench Cone | 6D 力/力矩联合约束 | 人形机器人面接触 | ⭐⭐⭐ |
| GIWC + 静平衡判据 | $-\boldsymbol{w}_G^{\text{gravity}}\in$ GIWC(双重描述法) | "能否站稳"的几何 LP | ⭐⭐⭐ |
| LCP + Delassus 算子 | $0\le\lambda^n\perp \boldsymbol{A}\lambda^n+\boldsymbol{w}\ge0$,$\boldsymbol{A}=J_nM^{-1}J_n^T$ | 刚体接触求解/仿真器 | ⭐⭐⭐ |
 ...
 * to the WBC QP formulation from 足式/60_QP_NLP建模.
 */
...
本章涉及的核心 API 签名与一句话说明。摩擦锥/wrench cone 多为手写矩阵(见 52.2.3 代码),此处汇总库函数。

### Pinocchio——接触 Jacobian 与 Centroidal

| API | 签名要点 | 作用 | 关联节 |
|-----|---------|------|-------|
| `computeJointJacobians(model, data, q)` | 输入 $q$,填充 `data` | 计算所有关节 Jacobian(调 `getFrameJacobian` 前必需) | 52.4.1 |
| `getFrameJacobian(model, data, frame_id, ref, J)` | `ref` 取 `LOCAL` / `WORLD` / `LOCAL_WORLD_ALIGNED`;`J` 为 $6\times n_v$ | 取某 frame 的 6D Jacobian | 52.4.1/52.4.3 |
| `getFrameJacobianTimeVariation(model, data, frame_id, ref, dJ)` | 需先 `computeJointJacobiansTimeVariation` | 取 $\dot{J}_c$,用于漂移项 $\dot{J}_c\dot{q}$ | 52.4.2 |
| `getFrameClassicalAcceleration(model, data, frame_id, ref)` | 零关节加速度时返回 $\dot{J}_c\dot{q}$ | 直接拿 bias acceleration | 52.4.2 |
| `ccrba(model, data, q, v)` | 填充 `data.Ag`($6\times n_v$ CMM)、`data.hg`、`data.Ig` | O(N) 计算 CMM + 质心动量 + CCRBI | 52.5.3 |
| `dccrba(model, data, q, v)` | 填充 `data.dAg` | CMM 时间导数,$\dot{h}_G=\dot{A}_G v + A_G a$ | 52.5.3 |
| `computeCentroidalMomentum(model, data, q, v)` | 只返回 `data.hg` | 仅要动量、不要 $A_G$ 时更快 | 52.5.3 陷阱 |
| `forwardDynamics(model, data, q, v, tau, J_c, gamma, damping)` | 解 KKT 系统,`gamma`$=-\dot{J}_c\dot{q}$ | 约束前向动力学(Crocoddyl 内部用) | 52.4.2 |

### OCS2 / ProxSuite——约束与求解

| API | 作用 | 关联节 |
|-----|------|-------|
| `FrictionConeConstraint::coneConstraint(F)` | 返回单条 SOC 标量 $\mu(F_z+\text{gripper}) - \sqrt{F_x^2+F_y^2+\text{reg}}\ge0$ | 52.2.3/52.7 |
| `ModeSchedule{eventTimes, modeSequence}` | 用位掩码编码每段接触模式(预定义步态) | 52.3.1 |
| `proxsuite::proxqp::dense::QP::init(H,g,A_eq,b_eq,A_ineq,...)` | 初始化稠密 QP(含 box/等式/不等式) | 52.7.2 |

### pycddlib——锥 H-rep ↔ V-rep 转换

| API | 作用 | 关联节 |
|-----|------|-------|
| `cdd.Matrix(rows, number_type)` + `.rep_type = RepType.GENERATOR` | 以生成元(V-rep)建锥;首列 $t=0$ 标记射线 | 52.2.5 |
| `cdd.Polyhedron(mat).get_inequalities()` | 双重描述法转出 H-rep($b + Ax\ge0$) | 52.2.5 |
...
9. **Caron S., Pham Q.-C., Nakamura Y. (2015)** "Stability of Surface Contacts for Humanoid Robots: Closed-Form Formulae of the Contact Wrench Cone for Rectangular Support Areas" — ICRA. 单矩形面接触 CWC 的解析 $U$ 矩阵(对应 52.2.4 节)。

10. **Caron S., Pham Q.-C., Nakamura Y. (2015)** "Leveraging Cone Double Description for Multi-contact Stability of Humanoids with Applications to Statics and Dynamics" — RSS. 用双重描述法从各接触力锥算 GIWC(对应 52.2.5/52.2.6 节)。

11. **Cottle R. W., Pang J.-S., Stone R. E. (1992)** *The Linear Complementarity Problem* — Academic Press. LCP 解的存在性与算法(Lemke)的标准参考(对应 52.3.0 节)。

### 软接触与仿真 ⭐⭐⭐

12. **Masterjohn J. et al. (2022)** "Velocity Level Approximation of Pressure Field Contact Patches" — RA-L. Drake Hydroelastic 的理论基础。

13. **Wensing P. M., Orin D. E. (2016)** "Improved Computation of the Humanoid Centroidal Dynamics and Application for Whole-Body Control" — 改进的 CCRBA 在 WBC 中的应用。

### 前沿方向 ⭐⭐⭐⭐

14. **ContactBench (Simple Robotics, 2024)** — 接触模型与求解器的系统性 benchmark,比较了 PGS/Lemke/proximal 等方法。

15. **Proximal methods for contact (RSS 2024)** — 将 proximal point 方法应用于接触逆动力学,扩展 MuJoCo 的方法到刚性和柔性接触。
...
### 可微接触 (Differentiable Contact) 前沿 ⭐⭐⭐⭐

**动机：为什么需要可微接触？**

传统接触模型（LCP / 互补约束）本质上是**不可微的**——接触状态的切换（从接触到离地）是一个离散跳变，梯度在切换点处不存在或为零。这对端到端学习管线（如 RL 策略优化、可微仿真器反向传播）构成了根本性障碍：如果仿真器不可微，就无法通过仿真器反向传播梯度来优化控制策略。

**三大可微化路线**：

| 路线 | 核心思想 | 代表工作 | 优缺点 |
|------|---------|---------|--------|
| **Penalty / 软接触** | 用连续的弹簧-阻尼模型替代刚性接触 | MuJoCo MJX, DiffTaichi | 自然可微，但时间步长受刚度限制 |
| **隐函数微分** | 把 LCP/NCP 的解看作参数的隐函数，用 KKT 条件求导 | Dojo (2022), ContactNets | 精确梯度，但需要求解 KKT 系统的逆 |
| **Randomized Smoothing** | 在接触参数上加噪声后取期望，得到光滑的梯度 | Suh et al. (2022), Pang et al. (2023) | 无需修改仿真器，但梯度有偏差且方差大 |

**Randomized Smoothing 方法详解**：

Randomized Smoothing 的核心洞察：即使单次仿真轨迹不可微（接触开关是阶跃函数），**对仿真参数取期望后的目标函数是光滑的**。具体地，设目标函数为 $J(\theta)$（$\theta$ 是控制参数），定义平滑版本：

$$\tilde{J}(\theta) = \mathbb{E}_{\epsilon \sim \mathcal{N}(0, \sigma^2 I)}[J(\theta + \epsilon)]$$

$\tilde{J}$ 总是可微的（高斯卷积的平滑性），其梯度可以用蒙特卡罗估计：

$$\nabla_\theta \tilde{J}(\theta) \approx \frac{1}{K} \sum_{k=1}^{K} J(\theta + \epsilon_k) \frac{\epsilon_k}{\sigma^2}$$

这就是"零阶优化 + 高斯平滑"的组合。$\sigma$ 控制平滑程度：$\sigma$ 大则梯度更平滑但偏差大，$\sigma$ 小则更精确但方差大。

**工程实践**：MuJoCo MJX (2024) 和 Brax (2023) 都支持 JAX 自动微分 + 软接触的端到端可微仿真。Drake 的 SAP (Semi-Analytic Primal) 求解器通过精心设计的接触模型实现了鲁棒的隐函数微分。

> **本质洞察**：可微接触的核心矛盾是**物理精度 vs 数学光滑性**。刚性接触更符合物理现实（地面确实是"硬"的），但数学上不可微。软接触牺牲了物理精度（引入了人为的穿透），换来了数学上的光滑性。Randomized Smoothing 则试图两全——保持刚性接触的物理模型，通过统计平均获得光滑梯度。

> **本质洞察**：接触力学的三大铁律（法向非负、摩擦锥、互补性）不是数学游戏，而是物理现实的忠实编码——地面不能拉住机器人（非负），摩擦力有上限（摩擦锥），脚在空中时没有地面力（互补性）。任何控制器如果违反这些约束，得到的"最优解"在物理上都是不可执行的。这就是为什么**每一个**足式控制器都必须包含这些约束，无一例外。
...
| # | 现象 | 可能原因 | 排查方法 |
|---|------|---------|---------|
| 1 | **摩擦锥未激活**：QP 求解的接触力 $\lambda$ 全为零，机器人自由落体 | 接触调度与实际步态不一致（所有脚标记为 swing），或接触 Jacobian $J_c$ 因 frame 选择错误导致全零 | 打印接触调度表，确认当前相位有着地脚；检查 `getFrameJacobian()` 的 frame 参数是否为 `LOCAL_WORLD_ALIGNED` |
| 2 | **互补约束 QP 不可行**：OSQP 返回 `PRIMAL_INFEASIBLE` | 互补约束的松弛参数 $\sigma$ 过小导致约束集为空集；或同时强制 $\phi > 0$（离地）和 $\lambda_n > 0$（有力）矛盾 | 逐步放大 $\sigma$ 直到可行；检查接触模式是否自洽（离地脚不应有力约束，着地脚不应有离地约束） |
| 3 | **接触 Jacobian 坐标系错误**：力矩输出方向反转或量级异常 | $J_c$ 使用了 `LOCAL` frame 但力是在世界系下表达的，导致旋转矩阵缺失；或混用了不同 frame 的 Jacobian | 用功率一致性测试验证：$P = f^T \cdot J_c \dot{q}$ 应等于 $\tau^T \dot{q}$，若不等则 frame 不匹配 |
| 4 | **摩擦锥线性化精度不足**：机器人在小摩擦系数地面上侧滑 | $k=4$ 棱的线性化摩擦锥最大误差为 $1 - \cos(45°) \approx 29\%$，对低 $\mu$ 场景过于保守 | 增大棱数至 $k=8$（误差降至 7.6%）或 $k=16$；或使用 SOCP 求解器处理原始二阶锥约束 |
| 5 | **接触力法向分量为负**：QP 输出的 $\lambda_z < 0$ | QP 忘记添加 $\lambda_z \geq 0$ 的不等式约束；或 $z$ 轴方向定义与接触法向不一致（非平坦地形） | 显式添加 $\lambda_z \geq 0$ 约束；打印接触面法向 $\mathbf{n}$ 确认与重力方向一致 |
| 6 | **软接触模型中穿透量过大**（$> 5$ mm） | 接触刚度 $K_n$ 过小或仿真步长过大导致数值积分误差累积 | 增大 $K_n$（同时减小步长以保持数值稳定）；或切换到 Drake Hydroelastic 模型 |
| 7 | **仿真中接触力高频振荡** | 接触模型刚度过高导致数值刚性（stiff system），显式积分器无法处理 | 使用隐式或半隐式积分器；添加接触阻尼 $D_n$；降低接触刚度并增大步长 |

### 跨章综合练习 ⭐⭐⭐

**综合题：接触约束在 MPC 和 WBC 两层中的不同编码**

本题需要综合 足式/70_腿足简化模型理论（SRBD MPC）、本章（摩擦锥与互补约束）和 足式/90_WBC分层优化与TSID（WBC）三章知识。

1. **MPC 层的接触约束**（足式/70_腿足简化模型理论 + 本章）：在 SRBD MPC 中，接触力 $\mathbf{f}_i \in \mathbb{R}^3$ 是决策变量，摩擦锥 $C_\mu \mathbf{f}_i \leq 0$ 是不等式约束。写出 trot 步态下（对角脚同时着地），MPC 的接触约束矩阵维度（多少行、多少列）。说明 swing 脚的接触力如何处理（提示：强制为零）。

2. **WBC 层的接触约束**（足式/90_WBC分层优化与TSID + 本章）：在 WBC 中，接触力 $\lambda_i \in \mathbb{R}^3$ 也是决策变量，但 WBC 还有接触约束 $J_c \ddot{q} + \dot{J}_c \dot{q} = 0$（接触点加速度为零）。解释为什么 MPC 不需要这个约束（提示：MPC 用简化模型，没有关节加速度 $\ddot{q}$），而 WBC 必须有。

3. **一致性问题**：MPC 输出的期望接触力 $\mathbf{f}^*_{\text{MPC}}$ 和 WBC 实际求解的接触力 $\lambda^*_{\text{WBC}}$ 通常不完全相同。为什么？设计一个 WBC 代价项使 $\lambda^*_{\text{WBC}}$ 尽量接近 $\mathbf{f}^*_{\text{MPC}}$，并讨论该项的权重应该如何设置。
...
**向前承接**:
- 足式/50_空间向量与浮动基座动力学 空间向量代数 → 52.4 接触 Jacobian 的空间力变换
- 足式/60_QP_NLP建模 QP/NLP 建模 → 52.2 摩擦锥变成 QP 线性不等式
- 足式/70_腿足简化模型理论 LIPM/Centroidal → 52.5 CMM 是 Centroidal 动力学的计算基础

**向后指向**:
- 52.2 摩擦锥约束 → 足式/90_WBC分层优化与TSID WBC 的关键不等式约束
- 52.3 互补约束 → 足式/100_DDP家族与Crocoddyl DDP 接触模型(Crocoddyl contact-fwddyn)
- 52.4 接触 Jacobian → 足式/110_OCS2完整栈与双线程MPC OCS2 FrictionConeConstraint 的实现基础
- 52.5 CCRBA → 足式/110_OCS2完整栈与双线程MPC OCS2 Centroidal MPC 的在线计算
- 52.6 角动量 → 足式/90_WBC分层优化与TSID WBC 角动量跟踪任务
- 52.9 RL 仿真 → 足式/190_腿足RL训练栈 腿足 RL 训练栈
...
- [ ] 能说清锥的 H-rep 与 V-rep 何时各自更好用,并用 GIWC 判断机器人能否在斜坡上静止站稳
- [ ] 能从刚体动力学一步步推到 LCP,写出 Delassus 算子并解释它为何半正定
- [ ] 能识别三种互补约束处理路线的适用场景
