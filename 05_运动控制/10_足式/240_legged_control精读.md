...
3. `ros_control` 的 `ControllerInterface::update()` 方法由谁调用?调用频率由什么决定?（足式/180_腿足硬件栈）
4. Pinocchio 的 `crba()` 计算的是什么矩阵?它的计算复杂度是多少?（足式/30_Pinocchio深度精读）
5. Unitree A1 的电机接口使用什么通信协议?`LowCmd` 结构体包含哪些字段?（足式/180_腿足硬件栈）
...
我们在 足式/110_OCS2完整栈与双线程MPC 精读了 OCS2 的完整架构——五层抽象、SQP 求解器、双线程 MPC。OCS2 本身提供了一个 `ocs2_legged_robot` 示例,演示了如何用 Centroidal Model 做四足 MPC。
...
| **legged_perceptive** | qiayuanl 本人 | 加入感知的扩展版（足式/230_Perceptive_MPC 相关） |
...
> 2. `ConvexMPCLocomotion.cpp`：MPC 求解核心（对应 足式/110_OCS2完整栈与双线程MPC 理论）
> 3. `BalanceController.cpp`：QP 力分配（对应 足式/70_腿足简化模型理论 SRBD + 足式/90_WBC分层优化与TSID WBC 简化版）
> 4. `GaitScheduler.cpp`：步态管理（对应 足式/120_步态管理与接触序列）
>
...
上节看到 `update()` 如何查询 MPC 策略,本节深入看 MPC 如何被集成。理解这部分需要回顾 足式/110_OCS2完整栈与双线程MPC 的双线程架构。
...
Controller 线程和 MPC 线程之间的数据交互通过 `MPC_MRT_Interface` 完成（足式/110_OCS2完整栈与双线程MPC 详细讲过 Triple Buffer 机制）：
...
> 💡 **对照 足式/80_接触力学与约束优化**：足式/80_接触力学与约束优化 §52.2 详细推导了为什么用线性金字塔而非 SOC 锥。简单回顾：线性化后可以用普通 QP 求解器（qpOASES），不需要 SOCP 求解器。此处 `frictionPyramid(5,3)` 对应 $k=4$ 的外切多面体近似，其面积比圆锥大约 27%（外切 $k=4$ 多面体近似，见 §52.2.2）——对控制来说精度足够，且加密侧面数（$k=6/8$）可进一步降低误差。
...
**零空间投影的直觉理解**（回顾 足式/90_WBC分层优化与TSID）：
...
在 足式/180_腿足硬件栈 中我们学习了 ros_control 的架构——Controller 通过 HardwareInterface 与硬件解耦。legged_control 的硬件层继承这个架构，提供了两种 HardwareInterface 实现。
...
很多学习者在读完 足式/110_OCS2完整栈与双线程MPC（OCS2）后直接跳到 legged_control,会困惑"legged_control 到底改了什么？还是只是 OCS2 的 wrapper？"。本节系统回答这个问题。
...
这个 KF 的简洁性正是它的优势——300 行代码，足够用于教学和原型验证。但精度不如 InEKF（足式/130_腿足状态估计）。
...
**场景**：你想把 `LinearKalmanFilter` 替换为 足式/130_腿足状态估计 学过的 InEKF。
...
```
足式/30_Pinocchio深度精读: 加载 URDF + Pinocchio 接口
足式/50_空间向量与浮动基座动力学: 正/逆运动学 + Centroidal 模型
足式/90_WBC分层优化与TSID: WBC QP 公式化 + qpOASES 求解
足式/110_OCS2完整栈与双线程MPC: OCS2 MPC 集成 + 双线程架构
────────────────────────────────
足式/240_legged_control精读 (本章新增):
...
| 后续章节 | 本章提供的基础 |
|---------|--------------|
| 足式/250_Mini-Legged综合实战 Mini-Legged 综合项目 | legged_control 作为"参考实现",你的项目对标它 |
| 足式/260_研究方向与博士导引 研究方向综述 | legged_control 是大部分研究方向的实验平台 |
| 足式/230_Perceptive_MPC Perceptive MPC(回溯) | legged_perceptive 基于 legged_control 构建 |
