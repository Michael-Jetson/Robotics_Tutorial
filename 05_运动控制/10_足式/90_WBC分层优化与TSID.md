...
> **本质洞察**：WBC 的本质不是"求解一个 QP"，而是**在全身动力学约束下，将高层规划的抽象意图（质心轨迹、足端位置）翻译为每个电机此刻应该施加的力矩**。QP 只是实现这个翻译的数学工具。理解这个定位，就能理解为什么 WBC 层需要动力学约束（翻译必须物理可行）、需要优先级（多个意图可能冲突）、需要实时性（翻译必须在 1ms 内完成）。

> **本质洞察**：HQP（分层优化）与加权 QP 的核心区别不在于数学复杂度，而在于**语义保证**。加权 QP 中 $w_1=100, w_2=1$ 只是"希望" Task 1 优先，但无法保证——当 Task 2 的梯度恰好很大时，它可能压过 Task 1。HQP 通过零空间投影给出**数学保证**：低优先级任务在高优先级任务的零空间中执行，绝不会干扰高优先级任务。这个保证在安全关键场景（如人形机器人平衡）中至关重要。
