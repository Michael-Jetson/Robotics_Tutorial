...
足式/30_Pinocchio深度精读 Pinocchio ──┐
足式/40_CppAD与代码生成 CppAD/CG ──┤
足式/50_空间向量与浮动基座动力学 Centroidal ──┼──> [足式/110_OCS2完整栈与双线程MPC OCS2 完整栈]──> 足式/120_步态管理与接触序列 步态管理
足式/60_QP_NLP建模 HPIPM/QP ──┤                        ├──> 足式/230_Perceptive_MPC Perceptive MPC
...
| **Mobile Manipulator** (OCS2 示例) | 底盘+机械臂(复合/30_多模态MPC) | ocs2_mobile_manipulator |
...
在深入 OCS2 的非线性 SQP 框架之前，有必要理解线性 MPC 的预测方程构造方法——这既是理解 SQP 中 QP 子问题的基础，也是 MIT Cheetah 凸 MPC（足式/70_腿足简化模型理论 SRBD）的直接实现思路。
...
MRT: 按当前时间 t 插值查询
    ...
    ▼
WBC (足式/90_WBC分层优化与TSID) 或 PD 控制器: 跟踪 MPC 输出
    │
    ▼
关节力矩 tau -> 电机驱动器
```
...
**对腿足场景**:
- `flowMap()` 实现 Centroidal 动力学(足式/50_空间向量与浮动基座动力学):
...
OCS2 的自动微分流水线(足式/40_CppAD与代码生成 的直接应用):
...
> **前情回顾**: 足式/100_DDP家族与Crocoddyl 详细讲了 DDP/FDDP/Crocoddyl。结论是——DDP 对无约束或软约束问题很高效,但腿足场景有大量硬约束。本节从 SQP 的角度重新审视。
...
> **足式/60_QP_NLP建模 已详细介绍 HPIPM**。本节聚焦: OCS2 如何把 OCP 映射到 HPIPM 的 QP 格式。
...
> **足式/50_空间向量与浮动基座动力学 的直接应用**。这里给出 OCS2 中的具体维度。
...
│ (OCS2)   │  │ (TSID, 足式/90_WBC分层优化与TSID)     │  │ (ros_control)│
└──────────┘  └──────────────────┘  └──────────────┘
                     │
              ┌──────┴──────┐
              │ 状态估计     │
...
这是 足式/30_Pinocchio深度精读-48 模板化 Scalar 模式在 OCS2 层级的延续——从 Pinocchio 的 `SE3Tpl` 向上传播到整个运动学层。一份运动学代码,三种用途:运行时计算、自动微分、代码生成。
...
#### 阶段 4: 接入 WBC (足式/90_WBC分层优化与TSID 的延续) (6-8 小时)
...
4. **Sleiman J.-P., Farshidian F., et al. (2021)** "A unified MPC framework for whole-body dynamic locomotion and manipulation" — RA-L. OCS2 扩展到 loco-manipulation(与 复合/30_多模态MPC 关联)。
