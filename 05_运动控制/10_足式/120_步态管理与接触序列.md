...
| 症状 | 可能原因 | 排查步骤 | 相关章节 |
|------|---------|---------|---------|
| 步态切换时跌倒 | 接触序列与MPC不同步 | 检查eventTimes精度/用状态机保护切换过程 | 足式/120.5 |
| 摆动腿刮地 | 足端轨迹高度不足 | 增大swing_height参数/检查地形补偿逻辑 | 足式/120.4 |
| 支撑相时间不准 | 时钟漂移/传感器延迟 | 用硬件时间戳/增加phase margin | 足式/130 |
| RL步态不自然 | reward中缺少步态正则项 | 添加AMP style reward或对称惩罚 | 足式/190 |
| trot→gallop切换震荡 | 状态机hysteresis不足 | 增大切换阈值的死区；检查过渡stance相设计 | 足式/120.7 |
