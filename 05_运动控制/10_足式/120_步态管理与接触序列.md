...
```
          Four approaches to contact sequence

  Predefined --- TOWR --- CITO --- MCTS+TO --- RL+MPC
  (OCS2)        (2018)   (2014)    (2024)     (2022+)

  Simple <---------- Flexibility ----------------> Flexible
  Real-time <-------- Computation ---------> Offline  Real-time
  Flat ground <------ Terrain ---> Arbitrary  Arbitrary
...
| 实时自适应（前沿） | RL + MPC | 足式/190_腿足RL训练栈 将展示如何用强化学习训练自适应步态策略:RL 智能体在数千种随机地形上探索步态参数,学会根据感知信息实时切换步态类型和时序,相当于用数据驱动替代人工设计的步态状态机 |
...
| 症状 | 可能原因 | 排查步骤 | 相关章节 |
|------|---------|---------|---------|
| 步态切换时跌倒 | 接触序列与MPC不同步 | 检查eventTimes精度/用状态机保护切换过程 | 足式/120 §56.5 |
| 摆动腿刮地 | 足端轨迹高度不足 | 增大swing_height参数/检查地形补偿逻辑 | 足式/120 §56.4 |
| 支撑相时间不准 | 时钟漂移/传感器延迟 | 用硬件时间戳/增加phase margin | 足式/130 |
| RL步态不自然 | reward中缺少步态正则项 | 添加AMP style reward或对称惩罚 | 足式/190 |
| trot→gallop切换震荡 | 状态机hysteresis不足 | 增大切换阈值的死区；检查过渡stance相设计 | 足式/120 §56.7 |
