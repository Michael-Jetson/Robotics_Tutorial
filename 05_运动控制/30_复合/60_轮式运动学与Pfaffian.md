...
**连接前序知识**：复合/20 中我们建立了浮动基座的全身动力学框架——$SE(3)$ 配置空间中的广义坐标和速度。本节将这个三维框架投影到二维平面，得到轮式底盘的 $SE(2)$ 运动学。这种投影不是"简化"——而是因为轮子与地面的接触约束把运动限制在了平面上。理解这个投影关系，是后续将 Pfaffian 约束嵌入全身动力学的基础。
...
这个问题不只是数学好奇心——它直接决定了运动规划的策略。如果约束是完整的，机器人的可达空间被降维了，规划在低维流形上进行。如果约束是非完整的，可达空间不变，但每一步的运动方向受约束，规划需要"绕路"。搞错了约束类型会导致规划器给出不可执行的路径。

**可积性判定的实际工程流程**

1. **写出 Pfaffian 约束**：$A(q)\dot{q} = 0$
2. **求零空间基向量场**：$g_1, \ldots, g_m$ 使得 $A g_i = 0$
3. **计算所有 Lie bracket**：$[g_i, g_j]$ 对所有 $i, j$
4. **检查 Frobenius 条件**：$[g_i, g_j] \in \text{span}(g_1, \ldots, g_m)$？
5. **如果不可积**，进一步计算 LARC 验证可控性
6. **如果可积**，找到积分约束 $\phi(q) = 0$

这个流程可以完全用 SymPy 自动化，下面的代码框架可以处理任意 Pfaffian 约束。

### 可积性的含义

如果 Pfaffian 约束 $A(q)\dot{q} = 0$ 可以积分为位置约束 $\phi(q) = 0$，则称该约束是**可积的**（integrable）或**完整的**。否则称为**不可积的**（non-integrable）或**非完整的**。

> **跨领域类比**：一个向量场是否是某个标量函数的梯度？在微积分中，$\mathbf{F} = \nabla \phi$ 当且仅当 $\nabla \times \mathbf{F} = 0$（无旋条件）。Frobenius 定理是这个条件的高维推广——将"无旋"推广为"对合性"（involutivity）。相似之处：两者都判断一个微分关系是否可以积分为一个有限关系。不同之处：旋度只适用于三维空间中的单个约束，Frobenius 定理适用于任意维度的分布。

**可积性的物理意义**

从工程角度，可积性决定了机器人的可达性：

| 约束类型 | 可积性 | 对可达位形的影响 | 运动规划复杂度 |
| --- | --- | --- | --- |
| 完整约束 | 可积 | 位形空间被降维——某些位形永远不可达 | 在降维空间中规划 |
| 非完整约束 | 不可积 | 位形空间不变——所有位形仍然可达 | 在完整空间中规划，但路径受约束 |

> **本质洞察**：非完整约束的"非直觉性"正是它有趣的地方。它限制了你在任何时刻可以移动的方向，但不限制你最终能到达的位置——你只需要"绕路"。差速底盘不能横移，但通过"前进-转弯-前进"的组合可以到达任何位姿。这正是平行泊车的数学本质：汽车（阿克曼转向，非完整）不能侧移，但通过多次前进-倒退可以"蹭"进停车位。如果汽车可以侧移（全向轮，完整），平行泊车就不需要"多点转"了。
...
### 几何直觉：Lie bracket 生成新方向的过程

为了建立对 Lie bracket 的几何直觉，考虑差速底盘的以下操作序列：

1. **沿 $g_1$ 前进 $\epsilon$**：机器人向前移动 $\epsilon$
2. **沿 $g_2$ 转动 $\epsilon$**：机器人原地旋转 $\epsilon$
3. **沿 $-g_1$ 后退 $\epsilon$**：机器人向后移动 $\epsilon$
4. **沿 $-g_2$ 转动 $-\epsilon$**：机器人原地反转 $\epsilon$

如果这四步后机器人回到了起始点（$[g_1, g_2] = 0$），约束就是完整的。但差速底盘做完这四步后不会回到起点——它会有一个侧向位移 $O(\epsilon^2)$。这个侧向位移正是 $[g_1, g_2]$ 的方向——"前进-转弯-后退-反转"的组合产生了原来不存在的侧向运动。

$$
\Delta q \approx \epsilon^2 [g_1, g_2] + O(\epsilon^3)
$$

这就是平行泊车的数学本质——通过反复"前进-打方向-后退-回正方向"，汽车逐步侧移进入停车位。每次操作产生 $O(\epsilon^2)$ 的侧向位移，需要多次重复才能完成。

> **本质洞察**：Lie bracket 不是一个抽象的数学操作——它精确描述了"通过已有运动方向的交替使用，系统能产生什么新的运动方向"。差速底盘的 $[g_1, g_2] = g_3$（侧向）告诉我们：前进和旋转的交替可以产生侧移。阿克曼转向的 LARC 也成立，但需要更多步操作——这反映了阿克曼转向在狭小空间中停车比差速底盘更困难的物理现实。
...
### 从 Frobenius 到 Chow：理论路径

Frobenius 定理回答了"约束是否非完整"。紧接着的问题是：如果约束是非完整的，系统是否可控？Chow 定理给出了肯定的条件。

| 定理 | 回答的问题 | 条件 | 结论 |
| --- | --- | --- | --- |
| Frobenius | 约束是否可积？ | Lie bracket 在分布内 | 约束是完整的（可积） |
| Chow | 系统是否可控？ | Lie bracket 生成全空间 | 系统完全可控 |

> **反事实推理**：如果一个系统既有非完整约束又不满足 Chow 条件会怎样？这意味着某些位形不可达——系统被困在位形空间的低维子流形上。实际工程中，纯轮式机器人通常满足 Chow 条件，但加入拖车的铰接约束后可能出现可控性退化。因此在设计轮足系统的约束时，验证 LARC 是必要的工程步骤。
...
LARC 中 Lie bracket 的阶数有直接的运动学含义：

| Lie bracket 阶数 | 运动学含义 | 示例 |
| --- | --- | --- |
| 0 阶（原始向量场） | 直接可控的运动方向 | 差速车的前进和原地转 |
| 1 阶（$[g_i, g_j]$） | 通过交替使用两个输入产生的新方向 | 差速车的侧移（通过前进+转弯组合） |
| 2 阶（$[g_i, [g_j, g_k]]$） | 需要更复杂的输入序列才能产生的方向 | 拖车的某些自由度 |
| $k$ 阶 | 需要 $k+1$ 步交替操作 | 高阶非完整系统 |

可控性等级直接决定了运动规划的"机动次数"——等级为 1 的系统（如差速底盘）只需要 2 步操作（前进+转弯）就能产生侧移效果；等级为 2 的系统可能需要 3 步以上。

> **跨领域类比**：可控性等级类似于魔方的"解法步数"。一步可以直接完成的操作（如旋转一个面）对应 0 阶可控；需要"先做 A 再做 B 才能实现 C"的操作对应 1 阶；需要更复杂的序列才能实现的操作对应更高阶。差速底盘的可控性等级为 1，意味着侧移"只需要前进+转弯两步组合"——像解魔方中的简单组合。阿克曼转向的可控性等级也是 1，但最小转弯半径的约束让"组合"需要更大的空间。
...
前面五节的所有推导都建立在"纯滚动"假设上——轮子与地面的接触点没有滑动。这个假设在以下条件下近似成立：
...
但在实际轮足机器人运行中，以下场景会让纯滚动假设严重失效：

| 场景 | 失效原因 | 纵向滑移率 | 侧向滑移角 |
| --- | --- | --- | --- |
| 湿滑地面 | 摩擦系数下降至 0.2-0.4 | 5-20% | 5-15° |
| 急加速/急刹车 | 轮胎力矩超过摩擦极限 | 10-30% | < 5° |
| 急转弯 | 侧向力超过摩擦极限 | < 5% | 10-30° |
| 松软地面（草地/泥土） | 轮胎沉入地面 | 5-15% | 5-10° |
| 高速运动（> 2 m/s） | 气动效应和离心力 | 3-10% | 3-8° |

> **本质洞察**：Pfaffian 约束 $A(q)\dot{q} = 0$ 描述的是"理想运动学"——它假设轮子和地面是刚性接触且无滑动。真实世界中轮胎是弹性体，接触区域有有限面积，轮胎力通过变形产生而非理想约束。滑移建模就是在 Pfaffian 框架中引入"约束的偏差"——从 $A(q)\dot{q} = 0$ 到 $A(q)\dot{q} = b(q, \sigma)$，其中 $b$ 依赖于滑移参数 $\sigma$。
...
纵向滑移率 $\kappa$ 和侧向滑移角 $\alpha$ 直接量化了 Pfaffian 约束的违反程度。回忆纯滚动 Pfaffian 约束 $A(q)\dot{q} = 0$。当存在滑移时：

$$
A(q)\dot{q} = r(\kappa, \alpha) \neq 0
$$
...
| 残差分量 | 物理含义 | 与滑移参数的关系 |
| --- | --- | --- |
| 纵向残差 $r_{lon}$ | 轮子旋转速度与地面速度的差 | $r_{lon} = R\dot{\phi} \cdot \kappa$（线性近似） |
| 侧向残差 $r_{lat}$ | 轮子侧向滑动速度 | $r_{lat} = v \cdot \sin\alpha$（小角近似） |

当 $\kappa = 0, \alpha = 0$ 时，$r = 0$——回到理想的纯滚动。当滑移增大时，约束残差增大，纯滚动运动学模型的预测越来越不准确。

**滑移对导航定位的影响**

基于纯滚动假设的轮式里程计（wheel odometry）在有滑移时会积累定位误差。误差增长率近似为：

$$
\dot{e}_{pos} \approx v \cdot \sqrt{\kappa^2 + \alpha^2}
$$

对于 $v = 1$ m/s, $\kappa = 0.05, \alpha = 3°$，$\dot{e}_{pos} \approx 0.07$ m/s——即每秒 7 cm 的定位漂移。在 10 秒内就会积累 70 cm 的误差——对于配送机器人在门口精确放置包裹来说完全不可接受。

这就是为什么实际导航系统不能只依赖轮式里程计——必须融合 IMU、视觉和/或 LiDAR 来补偿滑移导致的定位误差。本章 76.6 节的滑移建模为里程计误差模型提供了数学基础，导航系统可以根据估计的滑移参数动态调整里程计的信任权重。

> **反事实推理**：如果在光滑大理石地面上不考虑滑移使用纯轮式里程计会怎样？大理石地面的摩擦系数可能低至 0.3-0.5，急转弯时侧向滑移角可达 10-20 度。10 秒的行驶可能积累 1-2 m 的定位误差。在室内配送场景中，这可能导致机器人走错走廊或撞到墙壁。融合 LiDAR SLAM 后，即使轮式里程计完全不准，定位精度仍能保持在 5-10 cm——但 SLAM 的计算开销和传感器成本都显著增加。
...
实时估计滑移参数对轮足导航和控制至关重要。两种主要方法：

**方法一：基于 IMU-轮速差异的直接估计**

纵向滑移可以通过对比 IMU 速度和轮速推算速度来估计：

$$
\hat{\kappa} = 1 - \frac{v_{IMU}}{R\omega_{wheel}}
$$

侧向滑移可以通过 IMU 的侧向加速度估计：

$$
\hat{\alpha} \approx \arctan\left(\frac{a_{lateral}}{a_{longitudinal}}\right)
$$

这些估计在稳态运动中准确，但在急加速/减速时因为 IMU 的惯性效应而有偏差。

**方法二：基于卡尔曼滤波的融合估计**

将滑移参数作为扩展状态纳入 EKF，通过融合 IMU、编码器和视觉里程计来联合估计位姿和滑移。这种方法在动态条件下更鲁棒，但计算复杂度更高。
...
前面六节分别讨论了纯轮式底盘的 Pfaffian 约束和轮胎滑移。本节将它们统一到轮足混合系统中——同一台机器人可以用轮子滚动也可以用腿走路，甚至可以同时使用。统一 Pfaffian 框架让我们能在一个数学对象中描述所有运动模式。

**为什么需要统一框架**

考虑一台轮足机器人从平路滚动到台阶行走的过程。在模式切换瞬间，运动学约束发生变化——滚动模式有 4 个轮子的纯滚动约束，行走模式有 0-4 个脚底的接触约束（取决于有几条腿在支撑）。如果没有统一框架，每种模式需要单独的运动学模型，切换时需要重新初始化——这会导致瞬时的不连续和控制抖动。

统一 Pfaffian 框架的核心思想是：把所有模式的约束写成一个参数化矩阵 $A(q, m)$，其中 $m$ 是模式参数。模式切换只是 $m$ 的变化，约束矩阵平滑变化（或通过插值平滑），运动学模型始终保持一致。
...
每种底盘类型的运动学不同——差速底盘接受 $(v, \omega)$，阿克曼接受 $(v, \phi)$，全向底盘接受 $(v_x, v_y, \omega)$。如果导航规划器为每种底盘硬编码运动学，换一种底盘就要重写规划器——这违反了软件工程的"开闭原则"（对扩展开放，对修改关闭）。

nav2 的解决方案是定义一个标准化的运动学插件接口：

$$
\text{Plugin}: (q, u) \to \dot{q}
$$

规划器只需要调用 `getForwardKinematics(q, u)` 得到状态变化率，不需要知道底盘的具体类型。添加新底盘只需实现这个接口——规划器代码不变。

这与本章 Pfaffian 约束的理论完美对应：

$$
\dot{q} = G(q) u \quad \Leftrightarrow \quad \text{getForwardKinematics}(q, u) = G(q) \cdot u
$$

其中 $G(q)$ 是 Pfaffian 约束零空间的基矩阵，$u$ 是降维后的控制输入。每种底盘类型的 $G(q)$ 不同，但接口形式统一。

| 底盘类型 | $u$ 维度 | $G(q)$ 矩阵 | 输入含义 |
| --- | --- | --- | --- |
| 差速 | 2 | $[\cos\theta, \sin\theta, 0; 0, 0, 1]^T$ | $(v, \omega)$ |
| 阿克曼 | 2 | $[\cos\theta, \sin\theta, \tan\phi/L; 0, 0, 0]^T$ | $(v, \phi)$ |
| 全向 | 3 | $I_{3\times3}$ | $(v_x, v_y, \omega)$ |
| 麦克纳姆 | 3 | $I_{3\times3}$（近似） | $(v_x, v_y, \omega)$ |
...
部署到新机器人时，运动学参数（轮距 $2L$、轮半径 $R$、阿克曼轴距等）可能与 CAD 标称值不同。在线标定方法：

1. **直线测试**：机器人直行 5 m，比较编码器里程和外部测量距离，校正轮半径 $R$
2. **旋转测试**：机器人原地旋转 360 度，比较编码器角度和外部测量角度，校正轮距 $2L$
3. **持续自校正**：运行中融合 IMU 角速度和编码器差速角速度，在线估计 $L/R$ 比值

```python
import numpy as np


def calibrate_wheel_radius(encoder_distance: float,
                           measured_distance: float,
                           nominal_radius: float) -> float:
    """中文注释：通过直线测试校正轮半径。"""
    scale = measured_distance / encoder_distance
    return nominal_radius * scale


def calibrate_wheel_base(encoder_angle: float,
                         measured_angle: float,
                         nominal_base: float) -> float:
    """中文注释：通过旋转测试校正轮距。"""
    scale = measured_angle / encoder_angle
    return nominal_base / scale
```
...
| 误解 | 正确理解 |
| --- | --- |
| 非完整约束限制了可达位置 | 只限制瞬时速度方向，可达位形空间不变（Chow 定理） |
| 全向轮完全无约束 | 全向轮无非完整约束，但有关节速度限制和轮间协调约束 |
| Pfaffian 矩阵是固定的 | 对于某些系统（如阿克曼转向），$A(q)$ 随位形变化 |
| 差速底盘最简单 | 运动学简单但控制上是非完整的，路径规划比全向底盘复杂 |
| 滑移总是有害的 | 轮足控制中有意的滑移可以提高敏捷性（如快速转弯） |
| Lie bracket 只是数学工具 | Lie bracket 直接决定了通过"交替操控"能产生哪些新运动方向 |
| Frobenius 定理难以工程应用 | SymPy 可以自动完成 Lie bracket 计算和可积性判断 |
| 所有非完整系统都一样难控制 | 可控性等级不同，等级越高运动规划越复杂 |
...
在实际系统中，Pfaffian 约束 $A(q)\dot{q} = 0$ 从不被精确满足。违反来源和量级如下：

| 违反来源 | 典型量级 | 影响 | 处理方法 |
| --- | --- | --- | --- |
| 轮子打滑 | $0.01-0.1$ m/s | 累积定位误差 | 里程计融合 + 滑移检测 |
| 编码器量化 | $10^{-4}$ rad | 速度计算噪声 | 低通滤波 |
| 控制器离散化 | $O(\Delta t)$ | 步间约束不精确满足 | 减小控制周期 |
| 模型参数误差 | 1-5% | $A(q)$ 矩阵不准确 | 在线参数估计 |
| 地面变形 | 变化大 | 纯滚动假设失效 | 滑移建模 + 自适应 |
...
在理想情况下 $r(t) = 0$。在实际系统中，$\|r(t)\|$ 的大小反映了约束违反的严重程度。

| $\|r(t)\|$ 范围 | 含义 | 处理 |
| --- | --- | --- |
| $< 0.01$ | 正常运行 | 无需干预 |
| $0.01 - 0.1$ | 轻微滑移 | 融合 IMU/视觉修正 |
| $0.1 - 0.5$ | 显著滑移 | 切换到滑移模型，降低速度 |
| $> 0.5$ | 严重失控 | 紧急减速，重新定位 |
...
def monitor_constraint_residual(A: np.ndarray, q_dot: np.ndarray,
                                 thresholds: dict) -> str:
    """中文注释：监控 Pfaffian 约束残差，返回状态等级。"""
    residual = A @ q_dot
    norm = np.linalg.norm(residual)
    
    if norm < thresholds.get('normal', 0.01):
        return 'NORMAL'
    elif norm < thresholds.get('mild_slip', 0.1):
        return 'MILD_SLIP'
    elif norm < thresholds.get('severe_slip', 0.5):
        return 'SEVERE_SLIP'
    else:
        return 'EMERGENCY'
```

> **跨领域类比**：约束残差监控类似于工厂中的统计过程控制（SPC）。SPC 通过监控产品质量的统计偏差来检测生产过程是否失控。同样地，监控 $\|r(t)\|$ 的均值和方差可以检测轮式系统是否在正常工况下运行。如果 $\|r(t)\|$ 的均值突然增大，可能意味着轮胎磨损、地面条件变化或载荷超限。

### 轮足系统中 Pfaffian 约束的在线估计

当轮足机器人在未知地面上运动时，Pfaffian 约束矩阵可能因滑移参数变化而需要实时更新。一种实用方法是用扩展卡尔曼滤波器（EKF）在线估计滑移参数：

**状态向量**：$x = [q^T, \dot{q}^T, \sigma_{lon}, \sigma_{lat}]^T$，其中 $\sigma_{lon}$ 和 $\sigma_{lat}$ 是纵向和侧向滑移参数。

**过程模型**：

$$
\begin{aligned}
\dot{q} &= G(q, \sigma_{lon}, \sigma_{lat}) u + w_q \\
\dot{\sigma}_{lon} &= w_{\sigma,lon} \\
\dot{\sigma}_{lat} &= w_{\sigma,lat}
\end{aligned}
$$

其中 $w$ 是过程噪声，$G$ 是考虑了滑移的修正输入矩阵。

**观测模型**：IMU 提供角速度和加速度，编码器提供轮速。通过对比 IMU 测量和轮速推算的速度差异，EKF 可以在线估计滑移参数。

这种在线估计让导航系统能够自适应地补偿滑移——在干燥路面上 $\sigma \approx 0$，在湿滑路面上 $\sigma$ 增大，系统自动调整运动学模型以保持定位精度。
...
## Pfaffian 约束与现代 RL 控制的融合视角

### 约束感知的 RL 训练

当 RL 策略控制轮足机器人时，Pfaffian 约束不再以解析形式出现在控制器中，而是通过奖励函数和训练设计隐式编码。以下是三种约束编码方式的对比：

| 编码方式 | 实现 | 约束满足度 | 训练难度 | 部署风险 |
| --- | --- | --- | --- | --- |
| 奖励惩罚 | $r_{constraint} = -w\|A(q)\dot{q}\|^2$ | 软性（可违反） | 低 | 中（可能学会违反） |
| 动作投影 | $a_{safe} = \text{proj}(a, \text{null}(A))$ | 硬性（不可违反） | 中 | 低 |
| 课程学习 | 先在无约束环境训练，逐步加入约束 | 渐进 | 中 | 中 |

**奖励惩罚的权重选择**

约束惩罚权重 $w$ 的选择至关重要。$w$ 太小，策略忽视约束频繁打滑。$w$ 太大，策略过于保守不敢运动。一个实用的自适应方法是：

$$
w(t) = w_0 \cdot \max\left(1, \frac{\|A(q)\dot{q}\|_{avg}}{\epsilon_{target}}\right)
$$

当平均约束违反度超过目标阈值 $\epsilon_{target}$ 时，自动增大权重。

> **跨领域类比**：约束编码类似于法律体系中的"硬法"和"软法"。动作投影是"硬法"——不可违反，直接在执行前强制满足。奖励惩罚是"软法"——违反有代价但不被阻止。在实际社会中，最有效的治理通常是"少量硬法 + 大量软法"。机器人控制也是如此：关键安全约束（如关节限位）用硬性投影，性能约束（如能效）用奖励惩罚。

### 从单刚体到多刚体的 Pfaffian 推广

本章主要讨论了单底盘（单刚体）的 Pfaffian 约束。当轮足机器人的腿与轮子构成多刚体系统时，约束结构变得更复杂：

**单底盘约束**：

$$
A_{chassis}(q)\dot{q}_{chassis} = 0
$$

**多刚体约束**（轮足）：

$$
\begin{bmatrix} A_{chassis}(q) & 0 \\ A_{contact}(q) & J_{leg}(q) \end{bmatrix} \begin{bmatrix} \dot{q}_{chassis} \\ \dot{q}_{leg} \end{bmatrix} = 0
$$

其中 $A_{contact}(q)$ 描述脚轮与地面的接触约束，$J_{leg}(q)$ 是腿部雅可比。这个扩展矩阵的零空间定义了整个轮足系统的允许运动。

**零空间维度的物理意义**

$$
\dim(\text{null}(A_{extended})) = n_{total} - \text{rank}(A_{extended})
$$

对于四轮足机器人（$n_{total} = 3 + 4 \times 3 = 15$ 维），如果所有轮子都在纯滚动状态（$\text{rank} = 4 + 4 = 8$），零空间维度为 7——系统有 7 个独立可控输入。当某条腿抬起时，对应的接触约束消失，rank 减少，零空间维度增加——系统获得了更多自由度，但也失去了该腿的支撑力。
...
本章的数学理论（Pfaffian 约束、Frobenius 定理、Chow 定理）直接影响以下工程决策：

| 工程决策 | 依赖的数学理论 | 决策影响 |
| --- | --- | --- |
| 底盘类型选择 | Pfaffian 约束维度 | 非完整底盘需要更复杂的规划器 |
| 运动规划算法 | 可控性等级（LARC） | 等级高的系统需要更多规划步骤 |
| 导航精度要求 | 约束违反对定位的影响 | 滑移大时需要更多传感器融合 |
| 控制器设计 | 零空间维度 | 控制输入维度 = 零空间维度 |
| 模式切换策略 | 约束矩阵的模式依赖性 | 切换时约束结构突变需平滑过渡 |
| 安全约束设计 | 约束违反的严重程度 | 大违反意味着打滑，需要紧急处理 |

**从数学到代码的映射**

| 数学对象 | 代码实现 | 用途 |
| --- | --- | --- |
| $A(q)$ 约束矩阵 | `def pfaffian_matrix(q):` | 运动学约束检查 |
| $\text{null}(A)$ 零空间 | `scipy.linalg.null_space(A)` | 允许运动方向 |
| $[g_i, g_j]$ Lie bracket | `sympy` 符号计算 | 可控性验证 |
| $\text{rank}(\mathcal{L})$ LARC | `sympy` 秩计算 | 可控性等级确定 |
| $A(q)\dot{q}$ 约束残差 | `residual = A @ q_dot` | 滑移检测 |
| $\sigma_{lon}, \sigma_{lat}$ 滑移参数 | `pacejka(sigma, B, C, D, E)` | 高级运动模型 |

> **本质洞察**：本章的价值不在于记住每个公式，而在于建立从"运动约束分析"到"工程决策"的思维链条。当你面对一个新的轮式或轮足机器人时，第一步应该是"写出 Pfaffian 约束矩阵"——它告诉你系统有多少自由度、能往哪个方向动、需要几步才能侧移。这些信息直接决定了规划算法、控制器和安全系统的设计。
...
| 术语 | 英文 | 含义 |
| --- | --- | --- |
| Pfaffian 约束 | Pfaffian Constraint | 形如 $A(q)\dot{q} = 0$ 的速度约束 |
| 完整约束 | Holonomic Constraint | 可积分为位形约束的速度约束 |
| 非完整约束 | Nonholonomic Constraint | 不可积分的速度约束，限制瞬时速度但不限制可达位形 |
| 刚体平面运动群 | SE(2) | 二维平面上的刚体运动：平移 + 旋转 |
| Frobenius 定理 | Frobenius Theorem | 判断 Pfaffian 约束是否可积（完整 vs 非完整）的充要条件 |
| Lie 括号 | Lie Bracket | 两个向量场的交换子 $[f, g]$，用于 Frobenius 和 Chow 定理 |
| Chow 定理 | Chow's Theorem | 如果 Lie 括号生成的分布的秩等于位形空间维度，则系统完全可控 |
| LARC | Lie Algebra Rank Condition | Chow 定理的秩条件 |
| 差速底盘 | Differential Drive | 两个独立驱动轮的底盘，通过差速实现转向 |
| 阿克曼转向 | Ackermann Steering | 汽车式转向几何，内外轮转角不同 |
| 全向轮 | Omnidirectional Wheel | 可以在任意方向滚动的轮子（无非完整约束） |
| 麦克纳姆轮 | Mecanum Wheel | 带 45 度斜辊的全向轮 |
| 滑移参数 | Slip Ratio / Slip Angle | 描述轮胎纵向滑移率和侧向滑移角 |
| Pacejka 魔术公式 | Pacejka Magic Formula | 经验性的轮胎力-滑移关系模型 |
| 瞬时旋转中心 | Instantaneous Center of Rotation (ICR) | 刚体瞬时纯旋转的中心点 |
| 可控性 | Controllability | 系统从任意初始状态到达任意目标状态的能力 |
| 可控性等级 | Controllability Degree | LARC 条件中需要的 Lie 括号阶数 |
| 零空间 | Null Space | $A(q)$ 的零空间定义了允许的瞬时速度方向 |
| body frame | Body Frame | 固定在机器人质心的坐标系 |
| world frame | World Frame | 固定在环境中的惯性坐标系 |
...
| 误解 | 正确理解 |
| --- | --- |
| 非完整约束限制了机器人能去的地方 | 非完整约束只限制瞬时速度方向，不限制可达位形（Chow 定理保证） |
| 全向轮没有约束 | 全向轮没有非完整约束，但有关节速度限制和轮子间的运动学协调约束 |
| Pfaffian 约束只对轮式机器人有用 | Pfaffian 约束也描述腿足机器人的接触滚动约束 |
| 差速底盘是最简单的底盘 | 从运动学角度差速底盘最简单，但从控制角度它是非完整的，运动规划比全向底盘困难 |
| Frobenius 定理只是数学游戏 | Frobenius 定理直接决定了运动规划器需要几步才能实现侧移 |
| 滑移可以忽略 | 高速或粗糙地面上滑移显著改变运动学，忽略会导致导航误差累积 |
| 可控性等级越低越好控制 | 可控性等级高意味着需要更多次"倒车入库"式操作，运动规划更复杂 |
| Pacejka 公式在所有场景通用 | Pacejka 参数高度依赖轮胎材料、地面类型和速度范围 |

---

## Pfaffian 约束在轮足RL控制中的角色

### 从解析约束到学习约束

传统运动学控制直接使用 Pfaffian 约束矩阵 $A(q)$ 来约束运动规划。在 RL 控制中，策略不显式使用 $A(q)$，而是通过训练中的约束违反惩罚隐式学习遵守约束。

| 方法 | 约束处理 | 优点 | 缺点 |
| --- | --- | --- | --- |
| 解析约束（MPC/WBC） | 显式 $A(q)\dot{q} = 0$ | 精确、可证明 | 需要精确模型 |
| RL 惩罚 | $r_{slip} = -w \|A(q)\dot{q}\|^2$ | 不需要精确模型 | 不保证严格满足 |
| RL + 投影 | 策略输出投影到 $\text{null}(A(q))$ | 精确 + 灵活 | 需要实时计算 $A(q)$ |

> **本质洞察**：Pfaffian 约束在 RL 控制中的角色从"硬约束"变成了"软正则"。RL 策略通过惩罚信号学会了大致遵守纯滚动约束，但不保证精确满足。对于轮足机器人，适度的"约束违反"（即滑移）有时反而是有用的——快速转弯时有意让轮子打滑可以提高敏捷性。关键是让策略学会"什么时候该遵守约束，什么时候可以违反"。

### 模式切换与约束矩阵的动态变化

轮足机器人在行走模式和滚动模式之间切换时，Pfaffian 约束矩阵发生离散变化。设行走模式的约束为 $A_{walk}(q)$，滚动模式为 $A_{roll}(q)$：

$$
A(q, m) = \begin{cases} A_{walk}(q) & m = \text{walk} \\ A_{roll}(q) & m = \text{roll} \end{cases}
$$

模式切换瞬间，约束维度和方向可能都不同——滚动模式有纯滚动的非完整约束，行走模式则没有（脚掌可以自由放置）。
...
| --- | --- | --- |
| 地形坡度 | $< 15°$（平坦） | $> 25°$（陡峭） |
| 障碍物高度 | $< 5$ cm | $> 10$ cm（台阶/路缘） |
| 速度需求 | $> 1.5$ m/s | $< 0.5$ m/s（精确定位） |
| 能效权衡 | 滚动 COT 更低 | 行走可以原地转向 |

> **反事实推理**：如果不设置切换缓冲区会怎样？假设切换条件是"坡度 > 15 度则行走，< 15 度则滚动"。在 15 度附近的斜坡上，由于坡度测量噪声，系统会频繁在两种模式之间切换——每次切换都伴随约束矩阵的突变和短暂的动态过渡。这种"抖动切换"不仅浪费能量，还可能导致在过渡期间失稳。正确做法是引入滞后（hysteresis）：上升阈值 > 下降阈值，例如"坡度 > 20 度切换到行走，坡度 < 10 度切换回滚动"。

### nav2 中 Pfaffian 约束的实时使用

在 ROS2 nav2 框架中，运动学约束通过 `CostmapModel` 和 `ControllerPlugin` 接口传递给规划器。Pfaffian 约束矩阵 $A(q)$ 的零空间定义了允许的瞬时速度方向：

$$
\dot{q} \in \text{null}(A(q)) \Rightarrow \dot{q} = G(q) u
$$

其中 $G(q)$ 是零空间基矩阵，$u$ 是降维后的控制输入。规划器在 $u$ 空间中搜索路径，而不是在完整位形空间中搜索。

对于差速底盘，$u = [v, \omega]^T$（线速度和角速度），规划器只需搜索二维空间。对于阿克曼转向，$u = [v, \phi]^T$（速度和转向角），规划器同样在二维空间中搜索，但需要考虑最小转弯半径约束。

```python
import numpy as np


def pfaffian_to_input_basis(A: np.ndarray, q: np.ndarray) -> np.ndarray:
    """中文注释：从 Pfaffian 约束矩阵计算允许的输入基矩阵 G(q)。
    G(q) 的列向量张成 A(q) 的零空间。"""
    # 中文注释：SVD 分解求零空间
    U, S, Vt = np.linalg.svd(A)
    # 中文注释：奇异值接近零的右奇异向量是零空间基
    null_mask = S < 1e-10
    # 中文注释：如果 A 是 k × n，零空间维度是 n - rank(A)
    rank_A = np.sum(~null_mask)
    G = Vt[rank_A:].T  # 中文注释：零空间基矩阵
    return G


def check_constraint_violation(A: np.ndarray, q_dot: np.ndarray,
                                threshold: float = 0.01) -> tuple:
    """中文注释：检查当前速度是否违反 Pfaffian 约束。"""
    violation = A @ q_dot
    violation_norm = np.linalg.norm(violation)
    is_violated = violation_norm > threshold
    return is_violated, violation_norm
```

> **跨领域类比**：Pfaffian 约束在运动规划中的作用类似于高速公路的车道线。车道线不限制你最终能到达哪里（可达性），但限制了你在任何时刻可以移动的方向（瞬时速度约束）。遵守车道线的驾驶（遵守 Pfaffian 约束的运动）是安全高效的；偶尔变道（有控制地违反约束）是允许的；完全无视车道线（大幅违反约束，即严重打滑）则是危险的。RL 策略学习"何时遵守何时违反"，就像经验丰富的司机知道何时该严格保持车道、何时可以灵活变道。
