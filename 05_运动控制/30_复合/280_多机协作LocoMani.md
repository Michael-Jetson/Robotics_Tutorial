...
## 98.16 CollaBot 与大语言模型驱动的协作规划 ⭐⭐⭐

### 从手工任务图到 LLM 生成的协作方案

传统多机协作的任务图（如 98.2 节所述）需要人工定义阶段和切换条件。2024-2025 年的研究开始探索用大语言模型（LLM）自动生成协作任务方案。

CollaBot 等框架使用 Vision-Language Model（VLM）从场景图像中识别需要搬运的物体，用 LLM 推理协作策略（哪些机器人负责什么角色、物体如何搬运），然后生成可执行的运动约束。

这种方法的优势是灵活性——面对新场景（如一个从未见过的大型家具），LLM 可以基于常识推理出合理的协作方案。劣势是缺乏物理保证——LLM 不知道机器人的力矩极限、摩擦约束和通信延迟。

### LLM 与物理约束的接口

LLM 生成的协作方案必须经过物理可行性验证：

| LLM 输出 | 物理验证 | 拒绝条件 |
|---------|---------|---------|
| 角色分配 | 检查每台机器人的力矩裕度 | 某台裕度不足 |
| 抓取点 | 检查可达性和抓取矩阵秩 | 抓取矩阵秩亏 |
| 搬运路径 | 检查过道宽度和转弯半径 | 路径不可行 |
| 搬运速度 | 检查支撑裕度和内力 | 超出安全阈值 |

$$\text{valid}(plan) = \bigwedge_{i=1}^{N} \text{feasible}_i(plan) \land \text{safe}(plan)$$

如果 LLM 生成的方案被物理层拒绝，应向 LLM 反馈具体的违反原因，让 LLM 修正方案。这种"LLM 提案 → 物理验证 → 反馈修正"的循环通常在 2-5 轮内收敛。

### 与本章框架的关系

LLM 驱动的协作规划可以嵌入 98.2 节的分层架构中：

| 层级 | 传统方案 | LLM 增强方案 |
|------|---------|------------|
| L0 任务语义 | 人工脚本 | LLM 生成 |
| L1 物体规划 | 规划器 | LLM + 物理验证 |
| L2 协调分配 | 规则或优化 | LLM 建议 + 能力加权 |
| L3 单机控制 | MPC/WBC | 不变 |
| L4 硬件保护 | 限幅/急停 | 不变 |

关键设计原则：LLM 只影响上两层（语义和规划），不触及控制和硬件层。物理安全仍由传统方法保证。

> **本质洞察**：LLM 在多机协作中的角色是"常识推理引擎"——它知道桌子应该从底部抬起、门应该从铰链侧推开、长杆应该两端各一人。但它不知道摩擦锥、力矩饱和和通信延迟。因此 LLM 应该做**高层建议**，物理层做**最终裁决**。

**练习**

1. 设计一个"LLM 提案 → 物理验证 → 反馈修正"的循环流程。给出 LLM 的 prompt 模板和物理层的验证函数签名。
2. 讨论 LLM 生成的协作方案在安全性方面的风险。列出至少 3 种 LLM 可能犯的物理常识错误。
...
真实世界的多机协作往往涉及异构机器人——如一台四足+臂和一台轮式移动操作平台。异构系统引入了同构系统没有的新问题。
...
异构协作的关键是定义一个**能力抽象层**——把每台机器人的具体硬件特性抽象为标准化的能力描述：

$$\text{Capability}_i = \{W_i^{\max}, \mathcal{R}_i, v_i^{\max}, h_i^{\max}, \text{sensors}_i, \text{contact}_i\}$$

| 字段 | 含义 | 四足+臂 | 轮式操作平台 |
|------|------|---------|------------|
| $W_i^{\max}$ | 末端最大 wrench | 中（臂小） | 大（臂长） |
| $\mathcal{R}_i$ | 工作空间 | 高度灵活 | 水平大但高度限 |
| $v_i^{\max}$ | 最大速度 | 0.5 m/s | 1.5 m/s |
| $h_i^{\max}$ | 可越障高度 | 15 cm | 2 cm |
| sensors | 传感器组合 | IMU+相机+力 | 里程计+相机+力 |
| contact | 接触类型 | 足+夹爪 | 轮+夹爪 |

协调层根据能力描述分配角色，而不是根据机器人类型。这使得新型机器人可以通过注册能力描述直接加入协作团队。

### 物体交接协议

异构机器人之间的物体交接是一个关键操作——它涉及"一台机器人释放、另一台接管"的精确时序。

交接协议应包含以下阶段：

1. **对齐**：两台机器人同时接触物体，建立共同支撑。
2. **负载转移**：逐步把负载从交出方转移到接收方。
3. **确认**：接收方确认支撑稳定、内力在安全范围内。
4. **释放**：交出方缓慢卸载并松开。
5. **撤离**：交出方远离物体。

每个阶段的切换条件应基于力和位姿，而非时间：

$$\text{handover\_complete} = (f_{\text{receiver}} > f_{\min}) \land (f_{\text{giver}} < f_{\max}) \land (\|e_{\text{object}}\| < \epsilon)$$

> **陷阱框**
>
> 物体交接时最危险的瞬间是"双方都认为对方在支撑"——交出方以为接收方已经接管，开始卸力；接收方以为交出方还在支撑，没有加力。结果物体无人支撑而掉落。
>
> 防止这种情况的方法是**双方确认协议**：接收方主动报告"我已建立支撑"，交出方收到确认后才开始卸力。网络延迟可能导致确认消息滞后——因此确认后还应等待一个安全余量（如 200 ms）才开始真正卸力。

**练习**

1. 定义一个四足+臂和轮式操作平台的能力表。说明在搬运长杆任务中如何根据能力分配角色。
2. 设计物体交接的五阶段协议，写出每个阶段的进入条件和安全不变量。
3. 讨论异构系统中"最弱环节"决定系统能力上限的现象。如何在任务分配中缓解这个问题？
...
本章大部分内容聚焦于双机协作。当机器人数量增加到 3 台、5 台甚至更多时，系统面临新的可扩展性挑战。

| 瓶颈 | 双机 | 多机（$N > 3$） | 缓解策略 |
|------|------|----------------|---------|
| 通信量 | $O(N)$ | $O(N^2)$（全连接）或 $O(N)$（层级） | 层级通信拓扑 |
| 一致性延迟 | 1 轮 ADMM | 多轮收敛 | 限制迭代次数 |
| 抓取矩阵维度 | $6 \times 6$ | $6 \times 3N$ | 稀疏 QP 求解 |
| 角色分配 | 二选一 | 组合优化 | 贪心或拍卖算法 |
| 故障影响 | 直接影响队友 | 级联故障风险 | 局部隔离和冗余 |

### 通信拓扑选择

全连接拓扑让每台机器人都能直接与其他所有机器人通信——信息最新但通信量随 $N^2$ 增长。层级拓扑设置一个或几个协调节点——降低通信量但引入单点故障。环形拓扑让每台机器人只与邻居通信——鲁棒但信息传播慢。

| 拓扑 | 通信量 | 信息延迟 | 鲁棒性 | 适用场景 |
|------|--------|---------|--------|---------|
| 全连接 | $O(N^2)$ | 1 跳 | 高 | $N \leq 4$ |
| 星形（层级） | $O(N)$ | 2 跳 | 中（中心故障） | $N \leq 10$ |
| 环形 | $O(N)$ | $O(N)$ 跳 | 高 | 松耦合 |
| 混合 | 可调 | 可调 | 可调 | 通用 |

### 任务分配的拍卖机制

当机器人数量较多时，集中式角色分配变得不实际。分布式拍卖机制让每台机器人根据自身能力对子任务"出价"，价高者得：

$$\text{bid}_i(task_k) = \alpha \cdot \text{capability}_i(task_k) + \beta \cdot \text{proximity}_i(task_k) - \gamma \cdot \text{load}_i$$

能力强、距离近、当前负载轻的机器人出价更高。这种机制天然是分布式的——不需要全局协调节点，只需要广播出价和任务分配结果。

> **本质洞察**：多机协作的可扩展性不是纯算法问题，而是**通信、计算和安全的联合约束问题**。算法层面可以用分布式优化解决，但通信延迟和单点故障需要系统架构层面的设计。对于 loco-manipulation 这种强物理耦合的任务，$N > 4$ 的系统目前仍处于研究阶段——工业部署以双机为主。

### 多机系统的安全难题

当 $N$ 增加时，安全保证变得更难。单机的安全可以通过局部 Lyapunov 分析保证；多机的安全需要考虑**级联故障**——一台机器人失稳可能通过物体拖拽导致相邻机器人也失稳。

安全不变量需要从单机扩展到系统级：

$$\mathcal{S}_{\text{system}} = \bigcap_{i=1}^{N} \mathcal{S}_i \cap \mathcal{S}_{\text{inter}}$$

其中 $\mathcal{S}_i$ 是第 $i$ 台机器人的单机安全集合，$\mathcal{S}_{\text{inter}}$ 是机器人间的交互安全集合（如内力上界、最小距离、物体加速度上界）。

**练习**

1. 分析从双机到四机搬运桌子时，通信量、ADMM 迭代次数和抓取矩阵维度如何变化。
2. 设计一个三机协作的拍卖机制。定义出价函数、分配规则和冲突解决方案。
3. 讨论级联故障：如果三机搬运中一台突然断电，其他两台应该如何反应？
