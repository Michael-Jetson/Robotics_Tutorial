...
$$
p(a | o) = \sum_k w_k \cdot \mathcal{N}(a | \mu_k(o), \Sigma_k(o))
$$
...
$$
\Delta_{neg}^{(e)} = \text{Loss}_{mixed}^{(e)} - \text{Loss}_{single}^{(e)}
$$
...
$$
\text{Loss}_{total} = \lambda_1 \mathcal{L}_{internet} + \lambda_2 \mathcal{L}_{sim} + \lambda_3 \mathcal{L}_{teleop} + \lambda_4 \mathcal{L}_{explore}
$$
...
1. **数据集版本号**：每次数据更新（新增、删除、修正）都应递增版本号
2. **数据血缘**：记录每条数据的来源（哪台机器人、哪个操作者、什么时间）
3. **模型-数据对应**：每个模型检查点都应记录使用的数据集版本
4. **数据审计**：定期检查数据中是否有标注错误、时间不对齐或异常值
...
$$
\hat{o}_{t+1:t+H} = f_{world}(o_{t}, a_{t:t+H}^{candidate})
$$

$$
a^* = \arg\max_{a} \; \text{VLA\_score}(\hat{o}_{t+1:t+H})
$$
...
1. **基础模型替代的是感知和规划层，不是控制层**。WBC/MPC/PD 等底层控制在 VLA 时代仍然必要且不可替代。

2. **动作 tokenization 是模型能力的瓶颈**。离散 token 有量化误差和延迟问题，连续生成方法（Flow Matching/Diffusion）在接触密集任务中更有优势，但计算成本更高。

3. **频率分层是架构设计的必然选择**。语义理解（秒级）和力矩控制（毫秒级）的时间尺度差异太大，不可能用同一个模型同一个频率处理。

4. **安全过滤的投资回报率最高**。一次碰撞事故的成本可能超过整个安全系统的开发成本。安全层应该是独立于模型的、不可绕过的系统组件。

5. **数据基础设施比模型架构更重要**。统一的标注规范、版本管理和质量检查流程，对最终性能的贡献大于模型架构的改进。
