...
前面各节以 Go2+Z1（单臂四足）为主要例子。当系统扩展到人形双臂（如 G1 的左臂和右臂）时，统一动力学框架需要做两处扩展：

1. **多末端雅可比**：每条臂都有独立的末端雅可比 $\mathbf{J}_{ee,L}$ 和 $\mathbf{J}_{ee,R}$，各自 $6 \times n_v$ 维。末端力项从单项扩展为求和：

$$\mathbf{J}_{ee,L}^T \mathbf{f}_{ee,L} + \mathbf{J}_{ee,R}^T \mathbf{f}_{ee,R}$$

2. **CMM 臂列块分裂**：$\mathbf{A}_{G,a}$ 需要进一步分为左臂和右臂两个子块 $\mathbf{A}_{G,aL}$ 和 $\mathbf{A}_{G,aR}$，分别表示左右臂对质心动量的贡献。

### 双臂零空间的层级结构

双臂系统的零空间层级比单臂更深：

| 优先级 | 任务 | 维度 | Go2+Z1 | G1 双臂 |
|--------|------|------|--------|---------|
| P0 | 接触不滑 | $3 \times n_c$ | 12 | 6-12 |
| P1 | 基座稳定 | 3-6 | 3-6 | 3-6 |
| P2a | 左臂末端 | 6 | — | 6 |
| P2b | 右臂末端 | 6 | 6 | 6 |
| P3 | 腰部姿态 | 1-3 | — | 1-3 |
...
左右臂末端任务可以设为同优先级（P2a = P2b），也可以设一主一从（P2a > P2b）。同优先级意味着两条臂平等竞争零空间资源——如果零空间不够两条臂同时精确跟踪，两条臂会各退让一些。一主一从意味着主臂优先跟踪，从臂在主臂的零空间内尽力跟踪。

### Pinocchio 中的双末端雅可比提取

```python
import pinocchio as pin
import numpy as np

model = pin.buildModelFromUrdf("g1.urdf", pin.JointModelFreeFlyer())
data = model.createData()
q = pin.neutral(model)
...
# 中文注释：提取左右臂末端雅可比
left_ee_id = model.getFrameId("left_gripper_link")
right_ee_id = model.getFrameId("right_gripper_link")

J_left = pin.getFrameJacobian(model, data, left_ee_id, pin.LOCAL_WORLD_ALIGNED)
J_right = pin.getFrameJacobian(model, data, right_ee_id, pin.LOCAL_WORLD_ALIGNED)

print(f"左臂雅可比: {J_left.shape}")   # (6, nv)
print(f"右臂雅可比: {J_right.shape}")  # (6, nv)

# 中文注释：双臂联合雅可比（用于双臂协作任务）
J_bimanual = np.vstack([J_left, J_right])  # (12, nv)
print(f"双臂联合雅可比: {J_bimanual.shape}")

# 中文注释：零空间——在双臂任务满足后的自由空间
J_bimanual_pinv = np.linalg.pinv(J_bimanual)
N_bimanual = np.eye(model.nv) - J_bimanual_pinv @ J_bimanual
print(f"双臂后零空间秩: {np.linalg.matrix_rank(N_bimanual, tol=1e-6)}")
# 中文注释：期望为 nv - 12（如果双臂雅可比满秩）
```

### 双臂协作与多机协作的统一视角

双臂协作操作（如搬箱子）和多机协作搬运（280_多机协作LocoMani）在数学上是高度同构的：

| 概念 | 双臂 | 多机 |
|------|------|------|
| 末端数量 | 2（左右手） | $N$（$N$ 台机器人） |
| 抓取矩阵 | $G \in \mathbb{R}^{6 \times 12}$ | $G \in \mathbb{R}^{6 \times 3N}$ |
| 内力 | 双手之间的挤压力 | 多机之间的拉扯力 |
| 通信延迟 | 0（同一机器人） | 非零（网络） |
| 约束耦合 | 强（共享动力学） | 弱（各自独立动力学） |

双臂可以看成"零延迟、强耦合的二机协作"。因此 280 章的抓取矩阵、内力分解和虚拟耦合理论可以直接应用于双臂操作——只是物理实现从网络通信变成了肌肉骨骼的内部耦合。

### 练习 72.15

**[A] ⭐⭐** 计算 G1 人形在双臂抬起箱子时的联合雅可比秩。如果箱子是刚体（12 个约束：左右手各 6D），联合雅可比是否满秩？冗余度是多少？

**[B] ⭐⭐⭐** 推导双臂搬运箱子时的内力分解。使用 $G$ 矩阵将双手接触力分解为物体运动力和内力。解释为什么适度内力对夹持稳定性是必要的。

**[C] ⭐⭐⭐** 在 Pinocchio 中实现一个三级层级 IK：P0 = 双脚不滑，P1 = 左手跟踪圆形轨迹，P2 = 右手跟踪直线轨迹。验证 P1 优先级高于 P2。

---

## 72.16 Pinocchio 3.x 与 CasADi 后端的符号计算 ⭐⭐⭐

### 为什么需要符号计算

前面各节的代码都是数值计算——给定具体的 $\mathbf{q}$，计算具体的 $\mathbf{M}(\mathbf{q})$。但 MPC 求解器（如 OCS2 使用的 SLQ/SQP）需要动力学方程对状态和输入的**雅可比**（灵敏度矩阵）。

数值微分（有限差分）可以近似雅可比，但精度低且计算量大（每个变量需要额外一次前向评估）。Pinocchio 3.x 支持 CasADi 后端，可以直接生成动力学方程的符号表达式，进而自动求导。
...
# 中文注释：定义符号变量
q_sym = casadi.SX.sym("q", model.nq)  # 25 维配置
v_sym = casadi.SX.sym("v", model.nv)  # 24 维速度
a_sym = casadi.SX.sym("a", model.nv)  # 24 维加速度

# 中文注释：符号化 RNEA——得到广义力的符号表达式
cpin.rnea(cmodel, cdata, q_sym, v_sym, a_sym)
tau_sym = cdata.tau  # CasADi 符号表达式

# 中文注释：自动求导——对状态求雅可比
dtau_dq = casadi.jacobian(tau_sym, q_sym)  # (nv, nq)
dtau_dv = casadi.jacobian(tau_sym, v_sym)  # (nv, nv)

# 中文注释：编译为快速可调用函数
rnea_func = casadi.Function("rnea", [q_sym, v_sym, a_sym], [tau_sym])
jac_func = casadi.Function("jac_rnea", [q_sym, v_sym, a_sym], [dtau_dq, dtau_dv])

# 中文注释：数值验证
q_num = pin.neutral(model)
v_num = np.zeros(model.nv)
a_num = np.zeros(model.nv)

tau_val = rnea_func(q_num, v_num, a_num)
jac_q_val, jac_v_val = jac_func(q_num, v_num, a_num)
print(f"RNEA 符号计算结果: tau shape = {tau_val.shape}")
print(f"雅可比 d(tau)/d(q) shape = {jac_q_val.shape}")
```

### 符号计算在 MPC 中的应用

OCS2 的 SLQ 求解器需要动力学方程的一阶和二阶导数。传统做法是手动推导或数值差分。CasADi 后端让这些导数自动生成：

| 量 | 用途 | 手动推导 | CasADi 自动 |
|----|------|---------|------------|
| $\frac{\partial f}{\partial x}$ | 状态转移雅可比 | 困难（$n_v > 20$） | 自动 |
| $\frac{\partial f}{\partial u}$ | 输入雅可比 | 困难 | 自动 |
| $\frac{\partial^2 \ell}{\partial x^2}$ | 代价 Hessian | 视代价函数而定 | 自动 |
| $\frac{\partial g}{\partial x}$ | 约束雅可比 | 中等 | 自动 |

> **实践建议**：CasADi 的代码生成（`casadi.Function.generate`）可以把符号表达式编译为纯 C 代码，运行速度接近手写代码。这是将 Pinocchio 符号计算部署到嵌入式系统的推荐方式。

### 练习 72.16

**[A] ⭐⭐** 使用 Pinocchio CasADi 后端计算 Go2+Z1 在 neutral 构型下的 RNEA 值，与数值 `pin.rnea` 对比。验证二者差异 < $10^{-10}$。

**[B] ⭐⭐⭐** 使用 CasADi 自动微分计算质心位置对广义坐标的雅可比 $\frac{\partial \mathbf{p}_{com}}{\partial \mathbf{q}}$。与 Pinocchio 的 `data.Jcom`（通过 `jacobianCenterOfMass`）对比验证。

---

## 72.17 从教学到实战：统一动力学调试的黄金流程 ⭐⭐

### 调试不是猜测，而是物理验证

统一动力学相关的 bug 通常不会导致立即崩溃——它们表现为缓慢的漂移、不自然的力矩分配、或在特定构型下的突然失稳。以下是经过实践验证的调试流程。

### 黄金调试流程

**Step 1：静态验证**。让机器人在 neutral 构型下完全静止。计算 $\mathbf{h}(\mathbf{q}, \mathbf{0})$——这应该等于纯重力项。如果不为零（排除数值误差），说明重力方向或质量参数有错。

**Step 2：单力测试**。在末端施加单位力（如 $[1, 0, 0, 0, 0, 0]^T$），计算 $\mathbf{J}_{ee}^T \mathbf{f}$。检查每个关节的力矩方向是否符合直觉——例如沿 $x$ 方向的末端力应该主要影响臂的肩关节和肘关节。

**Step 3：质量矩阵对称性**。验证 $\|\mathbf{M} - \mathbf{M}^T\|_F < 10^{-10}$。如果不满足，说明 CRBA 的上三角化处理有误。

**Step 4：能量守恒**。在无摩擦、无外力仿真中让系统自由运动。计算动能 $T = \frac{1}{2}\mathbf{v}^T\mathbf{M}\mathbf{v}$ 和势能 $V$，验证 $T + V$ 恒定。如果能量漂移，说明积分器或动力学方程有数值问题。

**Step 5：ZMP 一致性**。站立时计算 ZMP 位置，验证它在支撑多边形内部。如果 ZMP 在边界外但机器人没有摔倒，说明接触模型或 ZMP 计算有误。

### 常见 Bug 分类

| Bug 类型 | 典型症状 | 根因 | 检测方法 |
|---------|---------|------|---------|
| 坐标系错误 | 末端力方向反 | WORLD/LOCAL 混用 | 单力测试 |
| 索引错误 | 某条腿力矩异常 | 关节索引偏移 | 逐关节禁用测试 |
| 质量参数错 | 静态力矩偏大/偏小 | URDF 质量不匹配 | 称重验证 |
| CRBA 未对称化 | QP 求解器报错 | 只用上三角 | 对称性检查 |
| 雅可比参考系错 | WBC 跟踪方向反 | 参考系不一致 | 可视化力向量 |

> **调试原则**：永远从最简单的验证开始。不要在复杂的 MPC+WBC+RL 混合系统中调试动力学——先用 Pinocchio 的纯数学接口，在最简单的构型和输入下验证每一项。确认基础物理量正确后，再逐步加入控制层。90% 的统一动力学 bug 可以在 Step 1-3 中发现。

### 练习 72.17

**[A] ⭐⭐** 对 Go2+Z1 执行黄金调试流程的 Step 1-3。记录每一步的数值结果。如果某一步发现异常，分析可能的原因。

**[B] ⭐⭐⭐** 人为引入一个 bug（如将某个关节的质量设为 0），重新执行调试流程。记录 bug 在哪一步被检测到，以及检测到的具体异常。

---

> **全章最终回顾**：本章从 URDF 加载开始，经过统一方程推导、ZMP 分析、CMM 计算、零空间投影、状态估计和开源项目精读，最终到达 GR00T-WBC 前沿和多末端扩展。这条线索的核心是一个事实：**浮动基座系统的控制不能分开设计**。臂运动影响基座，基座运动影响末端，接触力平衡所有。只有在统一动力学方程中同时考虑三类力，才能得到物理一致的全身控制。
>
> 如果你只记住一个结论，请记住这个：**臂看似只在操作物体，实际上也在操作自己的基座。** 这个事实通过 $\mathbf{M}_{ba}$ 耦合项、ZMP 偏移公式和 CMM 臂列块在数学上表达得非常清楚。它不是一个可以靠调参消除的"二阶效应"，而是浮动基座系统的基本物理特征。

### 从本章到后续章节的核心接口

| 后续章节 | 需要的本章输出 | 接口形式 | 频率 |
|---------|--------------|---------|------|
| 复合/30 多模态 MPC | $\mathbf{A}_G, \dot{\mathbf{h}}_G$ | CentroidalModelInfo | 50-200 Hz |
| 复合/40 RL 全身控制 | $\mathbf{M}, \mathbf{h}, \mathbf{J}_{ee}$ | 仿真器内部 | 仿真频率 |
| 复合/50 操作技能接口 | $\mathbf{J}_{ee}$, 零空间 | task-frame 转换 | 50-100 Hz |
| 复合/120 底盘臂联合规划 | 正运动学, $\mathbf{J}_{ee}$ | IK/WBC 接口 | 10-50 Hz |
| 复合/160 四足臂动力学概览 | 全部（72.2-72.7） | qm_control 框架 | MPC+WBC |
| 复合/220 经典人形全身控制 | CMM 双臂扩展 | TSID/HQP | 500-1000 Hz |
| 复合/230 人形全身 RL | 仿真动力学 + 奖励 shaping | IsaacLab 环境 | 训练时 |
| 复合/280 多机协作 | 抓取矩阵, 内力分解 | 物体层协调 | 10-50 Hz |

这张表说明了本章作为"动力学基石"的角色：后续几乎每个复合章节都直接或间接使用本章建立的统一方程、CMM 分块和零空间投影。理解本章是高效学习后续内容的前提。

> **最终建议**：如果你正在开发一个复合机器人控制系统，请确保团队中至少有一个人完整理解了本章的 72.2（统一方程）、72.4（CMM）和 72.5（零空间）。这三节的知识构成了全身控制的数学语言——团队成员之间用这套语言交流，可以避免 80% 的接口误解和集成问题。

> **术语对照表**：
> - 浮动基座 (Floating Base)：没有固定连接到地面的机器人基座
> - 广义坐标 (Generalized Coordinates)：描述系统配置的最小坐标集
> - 广义速度 (Generalized Velocity)：配置流形切空间中的速度向量
> - 质心动量矩阵 (Centroidal Momentum Matrix, CMM)：将广义速度映射到质心动量的线性映射
> - 零空间投影 (Null-Space Projection)：将次要任务投影到主任务零空间的操作
> - 阻尼伪逆 (Damped Pseudo-Inverse)：带正则化的伪逆，避免奇异点附近的数值发散
> - 选择矩阵 (Selection Matrix)：区分驱动关节和欠驱动基座的矩阵
> - 非线性力项 (Nonlinear Force Term)：包含科氏力、离心力和重力的合项
> - 接触雅可比 (Contact Jacobian)：将广义速度映射到接触点速度的矩阵
> - 力变换 (Force Transform)：空间力在不同坐标系之间的变换
> - 复合刚体算法 (Composite Rigid Body Algorithm, CRBA)：递归计算质量矩阵的 $O(n)$ 算法
> - 递归牛顿欧拉算法 (Recursive Newton-Euler Algorithm, RNEA)：递归计算逆动力学的 $O(n)$ 算法
> - 铰接体算法 (Articulated Body Algorithm, ABA)：递归计算正向动力学的 $O(n)$ 算法
> - 层级二次规划 (Hierarchical QP, HQP)：按优先级逐层求解的 QP
> - 加权二次规划 (Weighted QP)：用权重近似优先级的单一 QP
> - 惯性反力矩 (Inertial Reaction Torque)：子系统加速度通过耦合项对其他子系统产生的力矩
> - 锁关节惯量 (Locked Inertia)：所有关节固定时基座的等效惯量
> - ZMP 偏移 (ZMP Shift)：末端力或臂运动导致的零力矩点位置变化
> - 臂可操作度 (Arm Manipulability)：衡量臂在当前构型下运动灵活性的标量指标
