...
Unitree Go2 是目前学术界和工业界使用最广泛的四足平台之一。它的硬件架构代表了当前主流消费级四足机器人的典型设计：

| 层次 | Go2 具体实现 | 说明 |
|------|-------------|------|
| 计算平台 | NVIDIA Jetson Orin NX (EDU) / 定制主板 (Air/Pro) | EDU 版提供 40 TOPS 算力，可部署 RL 策略 |
...
| 关节电机 | 36 槽 42 极 BLDC 外转子电机 | 准直驱设计，行星齿轮减速比 ~6.22:1 |
| 关节编码器 | 绝对式磁编码器 | 集成在电机内部，无需上电归零 |
| IMU | MEMS IMU（集成在主控板） | 6 轴（加速度计 + 陀螺仪） |
| 足端传感器 | 足底力传感器 | 测量法向接触力（Go2 有，A1 没有） |
...
Unitree Go2 的关节电机（根据拆解分析）采用以下设计：

| 参数 | Go2 电机规格 |
|------|-------------|
| 电机类型 | 36 槽 42 极 BLDC 外转子 |
| 绕线方式 | Delta 绕组 |
| 减速器类型 | 单级行星齿轮 |
| 减速比 | ~6.22:1 |
| 峰值关节扭矩 | 45 Nm |
...
|------|-------|---------|-------------|------|
| MIT Cheetah 自研 | 6:1 | 180 Nm | ~212 Nm/kg | MIT Mini Cheetah |
...
- 2-3 ms 延迟等效于约 160-250 Hz 的截止频率

你的 200 Hz 力控指令在 250 Hz 截止频率附近会产生显著的相位延迟和幅值衰减，导致力控不稳定。**解决方案**：将力控带宽限制在 100 Hz 以下，或使用前馈补偿延迟。
...
public:
    ...
    bool sendMotorCommand(uint32_t motor_id,
        ...
        // Pack 5 parameters into 8 bytes using fixed-point encoding
        // Each value: 16-bit signed, scaled by a known factor
        int16_t q_raw   = static_cast<int16_t>(q_des * 1000.0f);
        int16_t dq_raw  = static_cast<int16_t>(dq_des * 100.0f);
        int16_t kp_raw  = static_cast<int16_t>(kp * 10.0f);
        // ... (pack remaining values into frame.data)
...
SDO 用于**非实时的参数配置**（如设置 PID 增益、读取电机序列号、修改运行模式）。SDO 利用 PDO 帧中的空闲带宽传输，不占用额外的通信周期。
...
Go2 使用集成在电机内部的磁编码器，通过检测永磁体的磁场方向来确定转子角度。编码器安装在减速器输出端（关节侧），直接测量关节角度，避免了减速器背隙导致的角度误差。
...
public:
    Go2LowLevelController() {
        ...
        cmd_pub_.reset(new ChannelPublisher<unitree_go2::msg::dds_::LowCmd_>(
            "rt/lowcmd"));
        state_sub_.reset(new ChannelSubscriber<unitree_go2::msg::dds_::LowState_>(
            "rt/lowstate"));
    ...
    void stateCallback(const void* msg) {
        const auto& state =
            *static_cast<const unitree_go2::msg::dds_::LowState_*>(msg);

        // Store latest joint state (lock-free via atomic/copy)
        for (int i = 0; i < 12; ++i) {
            joint_pos_[i] = state.motor_state()[i].q();
            joint_vel_[i] = state.motor_state()[i].dq();
            joint_tau_[i] = state.motor_state()[i].tau_est();
            joint_temp_[i] = state.motor_state()[i].temperature();
        }
...
private:
    uint32_t computeCRC(const unitree_go2::msg::dds_::LowCmd_& cmd) {
        // CRC32 of the command data for integrity check
        // Implementation depends on SDK version
        return 0;  // Placeholder: use SDK's CRC utility
    }

    std::unique_ptr<ChannelPublisher<unitree_go2::msg::dds_::LowCmd_>> cmd_pub_;
    std::unique_ptr<ChannelSubscriber<unitree_go2::msg::dds_::LowState_>> state_sub_;

    double joint_pos_[12]{}, joint_vel_[12]{}, joint_tau_[12]{};
    double joint_temp_[12]{};
    ...
    hardware_interface::CallbackReturn on_configure(
        ...
        cmd_pub_ = std::make_unique<
            unitree::robot::ChannelPublisher<
                unitree_go2::msg::dds_::LowCmd_>>("rt/lowcmd");
        state_sub_ = std::make_unique<
            unitree::robot::ChannelSubscriber<
                unitree_go2::msg::dds_::LowState_>>("rt/lowstate");
...
private:
    ...
    std::unique_ptr<unitree::robot::ChannelPublisher<
        unitree_go2::msg::dds_::LowCmd_>> cmd_pub_;
    std::unique_ptr<unitree::robot::ChannelSubscriber<
        unitree_go2::msg::dds_::LowState_>> state_sub_;
};
...
public:
    ...
    Status checkCommand(int joint_idx,
