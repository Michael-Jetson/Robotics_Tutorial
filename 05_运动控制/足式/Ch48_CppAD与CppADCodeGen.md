...
**对于 H1 人形机器人（19-DOF，广义坐标 $q \in \mathbb{R}^{25}$），情况更加严峻**。数值差分的耗时随输入维度**线性增长**，因为中心差分需要 $2n$ 次函数求值来计算 $n$ 维 Jacobian 的每一列：

| 机器人 | 关节 DOF | 广义坐标维度 | 差分输入维度 $n$ | 数值差分（中心） | CppADCodeGen |
|:---|:---|:---|:---|:---|:---|
| Go2 四足 | 12 | 18 | 48 | ~20 $\mu$s | ~1.5 $\mu$s |
| H1 人形 | 19 | 25 | 63 | ~55 $\mu$s | ~3.0 $\mu$s |
| Atlas 人形 | 28 | 34 | 73 | ~90 $\mu$s | ~4.5 $\mu$s |
...
1. **[估算题]** H1 人形机器人 19-DOF（含浮动基 $q \in \mathbb{R}^{25}$，$v \in \mathbb{R}^{24}$），MPC horizon $N=30$，SQP 迭代 4 次。假设 Jacobian 输入维度为 $n = 2 \times 24 + 19 = 67$（状态维度 $n_x = 48$ 加控制维度 $n_u = 19$），单次动力学求值耗时 0.3 $\mu$s。分别估算：(a) 数值差分（中心）求一次 Jacobian 的耗时；(b) 单次 MPC 求解中 Jacobian 计算的总耗时；(c) 该总耗时占 10 ms 预算的百分比。对比 CppADCodeGen 方案（假设单次 3 $\mu$s），讨论哪种方案可行。
...
int main() {
    ...
    // std::sin(AD<double>) 不会被 tape 录制——它会调用隐式转换为 double，
    // 导致 tape 上该运算被视为常量
...
- **必须**使用 `CppAD::sin`、`CppAD::exp` 等函数，绝不能用 `std::` 版本。后者会触发 `AD<double>` 到 `double` 的隐式转换，在 tape 上留下一个常量而非运算节点。
...
以下是一个完整的、可编译运行的 CppADCodeGen 示例。函数是简化的 2-DOF 动力学 $\tau = M(q)\dot{q}$：
...
int main() {
    ...
    ay[0] = M00 * ax[2] + M01 * ax[3];  // tau1
    ay[1] = M10 * ax[2] + M11 * ax[3];  // tau2
    ...
    // Step 2: 配置代码生成
    // ============================================================
    CppAD::cg::ModelCSourceGen<double> cgen(fun, "dynamics_2dof");
    ...
    compiler.addCompileFlag("-ffast-math");    // 放宽浮点精度（机器人安全）
...
CppADCodeGen 将符号 tape 翻译为什么样的 C 代码？以上面 2-DOF 模型的 Jacobian 为例，生成代码的结构大致如下（简化后便于阅读）：

```c
/* 由 CppADCodeGen 自动生成 —— 请勿手动编辑 */
/* 模型: dynamics_2dof, Jacobian: 2 输出 x 4 输入 */

void dynamics_2dof_sparse_jacobian(
    ...
    unsigned long       *nnz)
{
    ...
    // dtau1/dq1 = 0  → 稀疏格式中直接跳过，不输出
    // dtau1/dq2 = -sin(q2)*dq1 + cos(q2)*dq2
    jac[0] = v2 * x[2] + v0 * x[3];
    // dtau1/ddq1 = 2.0 + cos(q2)
    jac[1] = 2.0 + v0;
    // dtau1/ddq2 = sin(q2)
    jac[2] = v1;
    // dtau2/dq2 = cos(q2)*dq1
    jac[3] = v0 * x[2];
    // dtau2/ddq1 = sin(q2)
    jac[4] = v1;
    // dtau2/ddq2 = 1.0（常数）
...
| `-Ofast` | `-O3` + `-ffast-math` | 比 `-O3` 快 5--15% | 放宽 IEEE 浮点规范 |
| `-march=native` | 启用本机 CPU 指令集（AVX2/AVX-512） | 10--30% 提升 | `.so` 不可跨 CPU 架构移植 |
| `-ffast-math` | 允许浮点运算重排序 | 5--15% 提升 | 允许 `a+b+c` 重排为 `(a+c)+b` 等 |
...
>
> 3. **URDF 变化需要重新生成**：机器人模型的任何变化（关节质量、连杆惯量、运动链拓扑）都使得已生成的 `.so` 失效。OCS2 的解决方案优雅：对 URDF 文件内容计算 SHA-256 hash，将 `.so` 缓存在 `~/.ocs2/` 目录下，hash 匹配则直接加载缓存。这使得开发阶段频繁修改 URDF 时只在首次编译慢，后续如果改回之前的配置可以直接命中缓存。
...
OCS2 的 hash 缓存是其工程上的亮点：

```
createModels() 内部流程：
  |
  +-- 计算 modelHash_
  |     hash 来源：modelName + inputDim + outputDim + compilerFlags
  |
  +-- 检查 libraryFolder_/{modelName}_{hash}.so 是否存在
  |
  +-- [存在且 hash 匹配] --> dlopen 加载 --> 返回
  |
  +-- [不存在或 hash 不匹配]
...
- 后续运行：如果模型未变，毫秒级加载
- 修改了 URDF 或成本函数 -> hash 变化 -> 自动触发重编译
...
2. **[思考题]** OCS2 的 hash 缓存只考虑了 `modelName + inputDim + outputDim + compilerFlags`，但没有包含 URDF 文件内容本身的 hash。这会导致什么问题？如何改进？
