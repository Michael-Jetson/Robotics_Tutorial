...
Hartley et al. (2020, IJRR) 将腿足状态估计的状态嵌入 $SE_k(3)$（扩展特殊欧氏群）。对于四足机器人（$k=5$，包含速度 + 4 只脚的位置），状态矩阵为：
...
\end{bmatrix} \in SE_5(3) \subset \mathbb{R}^{10 \times 10}$$
...
| 一致性 | 最优（精确线性） | 近似，可能不一致 | 理论保证 |
| 大初始误差收敛 | N/A（不估计姿态） | 可能发散（>45$^\circ$） | **保证收敛**（>180$^\circ$） |
