...
# 输出形如: (0, 0, 0, 1, 0, 0)  即角速度为零，线速度沿 x 方向
# Pinocchio 的 Motion 类型内部用 (angular, linear) 顺序
...
# 也可以当作 numpy 数组使用
twist_np = twist.np  # 返回 6D numpy 数组 [angular; linear]
```
...
**练习 49.10.2**：验证线动量——从 CMM 的最后 3 行提取线动量部分，验证 $\boldsymbol{l}_G = m\boldsymbol{v}_G$。其中 $\boldsymbol{v}_G$ 由 `pin.centerOfMass(model, data, q, v)` 后从 `data.vcom[0]` 获取。
