...
class GaitScheduler {
  ...
  // Key difference: does NOT generate ModeSchedule
  // MPC only knows current contact state, not future
};
```
...
| MPC 对步态的感知 | 知道整个 horizon 的接触序列 | 只知道当前接触状态 |
| 步态切换 | 替换模板 + 过渡 stance | 修改 phase offset/duty cycle |
| MPC 类型 | NMPC（非线性，处理切换） | 凸 MPC（线性化，假设不切换） |
| 适用场景 | 复杂步态、非平坦地形 | 简单步态、平地高速 |

**为什么 MIT Cheetah 只需要知道当前接触状态？** 因为它使用**凸 MPC**（Di Carlo 2018）——在整个 MPC horizon 内，假设当前的接触状态保持不变。MPC 的 horizon 步长是 30ms，步态半周期是 200ms，所以这个假设在大部分时间是合理的。只有在模式切换时刻附近，这个近似会引入一定误差——但对于凸 MPC 来说，求解速度极快（<3ms），高频重规划弥补了预测误差。
