...
注意这里有两层 BV 测试在协同：**节点级 BV**（AABB/OBB）做粗剪枝，只有当两个叶子的 BV 都相交，才落到最底层做**三角形-三角形**精确相交。一个 1000×1000 三角形的 mesh 对，朴素全量是 $10^6$ 次三角形测试；BVH 在典型场景下把它降到几十到几百次。
...
> **理论-工程桥接**：正因为胶囊距离是**纯解析**（解一个 2×2 线性系统 + 几次 clamp，无迭代），它比凸包的 GJK（迭代 3-10 次）快一个量级。这就是 §4.3 选型表里"1 kHz 控制循环 / 腿足 WBC 自碰撞"清一色用球/胶囊的根本原因——不是它们更"准"，而是它们的距离有 $O(1)$ 闭式解，能塞进微秒级预算。几何选择的背后，永远是"这个几何的距离查询有没有解析解"这个工程问题。
...
| 两个明明分开的细长物体被判相交 | SAT/OBB 测试漏了边-边叉积轴 | 1. 确认 OBB 测试覆盖全 15 轴（含 3×3 边叉积） 2. 检查平行边叉积为零时是否正确跳过 3. 优先用 Coal 的成熟实现 | §2.2.1 |
