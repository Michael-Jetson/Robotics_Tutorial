...
**ALOHA 2.0 (Google DeepMind, 2024)**：硬件升级为定制的 ARX5 leader 和 Trossen ViperX 300 follower，关键改进是 leader 端增加了扭矩反馈（通过电流环估计），使操作者能感知约 $\pm$2 Nm 的粗略接触力。虽然不是 D05-D07 意义上的"力反馈遥操作"（精度差两个数量级），但实验显示在需要接触力感知的任务（如"从平面上抠起薄片"）上，数据采集质量显著提升。
