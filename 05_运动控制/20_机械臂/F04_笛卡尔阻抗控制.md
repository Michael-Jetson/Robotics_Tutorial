...
> 回顾 F03 第 3/5 节：我们在 F03 中推导了笛卡尔阻抗控制律 $\tau = J^T(-K_d\tilde{x} - D_d\dot{\tilde{x}}) + C\dot{q} + g$，并用 Lyapunov 方法证明了其渐近稳定性。该推导在数学上是完备的，但留下了六个工程问题——姿态误差如何计算、惯性整形是否需要、零空间如何处理、重力补偿由谁负责、安全限幅如何设计、以及速度信号如何获取。本章的任务是逐一解决这些工程问题，将 F03 的理论公式落地为可部署的真实控制器代码。
>
> 回顾 M01（Pinocchio 刚体动力学）：$M(q), C(q,\dot{q}), g(q)$ 的计算和 Jacobian $J(q)$ 的获取是本章代码精读的基础。libfranka 通过 `model.mass()`, `model.coriolis()`, `model.zeroJacobian()` 等 API 提供这些量。
...
| 模式 | 时间 | 说明 |
|------|------|------|
| **精读** | 6-8 小时 | 逐行推导 + 代码精读 + 练习，适合首次学习 |
| **速读** | 2-3 小时 | 跳过代码逐行注释和练习，重点看各节开头的动机、对比表和本质洞察 |
| **速查** | 15-20 分钟 | 直接查阅本章小结表、符号表、故障排查手册和定理速查表 |

---

## 如果跳过本章会怎样

如果你跳过本章直接进入后续章节，会遇到以下具体障碍：

- **F05（导纳控制工程）**：导纳控制和阻抗控制是力控的两大范式，F05 会频繁对比两者的工程差异。没有本章的 libfranka 代码精读和 CI vs. OSC 分析，你无法理解"为什么位置接口型机器人用导纳而不用阻抗"的根本原因。
- **F06（变阻抗与无源性）**：本章第 11 节建立的无源性判据和采样稳定性分析是 F06 的直接前置。跳过本章意味着 F06 的能量注入分析和时域无源性控制（TDPC）将完全看不懂。
- **F09（学习型力控——SERL/CRISP + RL）**：CRISP 的七项控制律叠加和 error clipping 机制在本章第 6 节详细解析。F09 中 RL 策略输出阻抗参数时，你需要理解这些参数如何被 CRISP 底层消费。
- **实际开发**：如果你使用 Franka 机器人做力控实验，不理解 FCI 的实时约束（第 2 节）和四元数双覆盖（第 3 节），几乎必然会遇到"莫名急停"和"姿态翻转"两大经典故障，且无法自行诊断。
...
│
├── 硬件与平台基础
│   ├── libfranka FCI 架构（§2）——1 kHz 实时约束、RobotState
│   └── 实时编程约束——零堆分配、零 I/O
│
├── 核心算法实现
│   ├── 6D 姿态阻抗（§3）——四元数误差 + 双覆盖
│   ├── libfranka 代码精读（§4）——逐行对应 F03 公式
│   └── 惯性整形与 OSC（§5）——Λ 矩阵与 CI vs. OSC 对比
│
├── 工业级控制器
│   ├── CRISP 架构（§6）——七项控制律 + error clipping
│   ├── matthias-mayr CIC（§7）——完整 6×6 K + rqt 调参
│   └── franka_ros2 精读（§9）+ ros2_control 导纳关系（§10）
│
├── 参数调节与变阻抗（§8）
│   └── 刚度切换 / 连续变阻抗 / 学习驱动
│
├── 稳定性与无源性理论（§11）
│   ├── 耦合稳定性定理（Colgate-Hogan）
│   ├── 采样无源判据（Colgate-Brown）
│   ├── Z-width 概念
│   └── 时域无源性控制（TDPC）
│
└── 工程数值陷阱（§12）
...
Step 2: 计算位置误差（3D 欧几里得）
  e_pos = p - p_d ∈ ℝ³   ← $e_{pos} = p - p_d \in \mathbb{R}^3$
         ↓
Step 3: 计算姿态误差（四元数 → 3D 角度误差）
  e_rot = -R · (Q⁻¹ ⊗ Q_d).vec() ∈ ℝ³    ← $e_{rot} = -R \cdot (q^{-1} \otimes q_d)_{vec} \in \mathbb{R}^3$（本章重点！）
         ↓
Step 4: 合成 6D 误差
  e = [e_pos; e_rot] ∈ ℝ⁶   ← $e \in \mathbb{R}^6$
         ↓
Step 5: 计算笛卡尔力/力矩（阻抗律）
  F_imp = -K_d · e - D_d · (J · dq) ∈ ℝ⁶   ← $F_{imp} \in \mathbb{R}^6$
         ↓
Step 6: 映射到关节力矩
  τ_task = J^T · F_imp ∈ ℝ⁷   ← $\tau_{task} = J^T F_{imp} \in \mathbb{R}^7$
         ↓
Step 7: 叠加补偿项 + 零空间
  τ_cmd = τ_task + τ_coriolis + τ_nullspace   ← $\tau_{cmd} = \tau_{task} + \tau_{coriolis} + \tau_{nullspace}$
...
```cpp
// Step 1: 从 $4 \times 4$ 齐次矩阵提取旋转矩阵和四元数
...
注意量纲差异：
  ...
  合理比较需要引入特征长度 l：
    等效刚度比 = K_rx / (K_x · l²)
    对于 Franka（$l \approx 0.1$ m），10 / (150 × 0.01) = 6.67
    → 旋转方向实际上比平移方向"更硬"
```
...
**这就是完整的操作空间阻抗控制律**。它需要：
1. $\Lambda(q)$ ——操作空间惯量（每周期计算一次 6$\times$6 矩阵求逆）
...
**证明思路（Nyquist + 正实性）**：耦合系统的特征方程为 $1 + Z_r(s) Y_e(s) = 0$。环路传递函数 $L(s) = Z_r(s) Y_e(s)$。关键引理：两个正实函数的乘积，其 Nyquist 轨迹 $L(j\omega)$ 的相位被限制在 $[-\pi, \pi]$ 内——因为正实函数的相位本身在 $[-\pi/2, +\pi/2]$ 内（$\text{Re} \geq 0$ 等价于相位不超过 $\pm 90°$）。两个相位各在 $\pm 90°$ 内的函数相乘，总相位在 $\pm 180°$ 内，因此 $L(j\omega)$ **永远不会包围 $-1$ 点**。由 Nyquist 判据，闭环稳定。$\blacksquare$

> **本质洞察**：耦合稳定性定理的威力在于它**解耦了机器人和环境的设计**。你不需要知道环境的具体参数（$K_e$ 是 $10^3$ 还是 $10^6$），只要环境是无源的（绝大多数被动物理环境——弹簧、阻尼、质量——都是无源的），并且你保证机器人侧阻抗无源，稳定性就自动成立。这是一个**"一次证明，对所有无源环境成立"**的结论——正是 11.1 节所求的"全类别稳定"。
...
其中：
- $b$ 是系统中的**物理阻尼**（机械臂关节摩擦、传动阻尼等真实存在的、连续的耗散）
- $K$ 是虚拟墙（控制器）的刚度
- $B$ 是虚拟墙的阻尼（控制器命令的阻尼，注意它和物理阻尼 $b$ 不同——$B$ 由 ZOH 离散化，不一定耗能甚至可能注入能量）
- $T$ 是采样周期
...
1. **为什么提高采样率（减小 $T$）能让接触更稳**：$b > KT/2$ 中 $T$ 减半，允许的 $K$ 翻倍。这就是 Franka 坚持 1 kHz、CRISP 强调高频柔顺层的根本原因。从 250 Hz 提到 1 kHz，可稳定渲染的刚度上界提高 4 倍。
...
用 $b > KT/2$ 估算 Franka Panda 能稳定渲染的笛卡尔刚度上界。取末端等效物理阻尼 $b \approx 5$ Ns/m（Franka 末端方向的固有阻尼，量级估计），采样周期 $T = 1$ ms：
...
| 影响因素 | 对 $Z_{max}$（硬度上界）的影响 | 对 $Z_{min}$（软度下界）的影响 |
|---------|--------------------------|--------------------------|
...
> **本质洞察**：$Z_{max}$ 和 $Z_{min}$ 之间存在一个根本张力。物理阻尼 $b$ 是把双刃剑——增大 $b$ 让你能渲染更硬的墙（$Z_{max}$ 提高），但同时让机器人在"应该自由"时变得笨重（$Z_{min}$ 变差）。Franka 用关节力矩传感器 + 高带宽力矩环把固有摩擦补偿掉，本质上是在**人为降低有效物理阻尼以改善 $Z_{min}$（背驱动性）**，再用 1 kHz 高采样率把 $Z_{max}$ 拉回来——这是两个指标的精妙权衡。Weir 与 Colgate（2008）进一步用"主动电气阻尼"在不牺牲 $Z_{min}$ 的前提下提高 $Z_{max}$，是这个张力的高级解法。
...
>
> 新手想法："我的控制器在自由空间跑了一小时都没问题，它看起来是稳定的。"
>
...
笛卡尔阻抗控制律 $\tau = J^T F$ 看似简单，但 $J$ 和 $F$ **必须表达在同一个坐标系**，否则映射完全错误。这是真机调试中最隐蔽的 bug 来源之一。
...
**陷阱 1：$\sqrt{AB} \neq \sqrt{A}\sqrt{B}$（非交换性）**。当 $K$ 和 $\Lambda$ 不可交换（特征向量不对齐）时，$K\Lambda$ 甚至可能**不对称**——而不对称矩阵的"平方根"定义本身就有歧义。
...
>
> 根本原因：$\tau = J^T F$ 要求 $J$ 和 $F$ 同坐标系。world 系的力配 body 系的 Jacobian，相当于漏了一个旋转 $R$
>
...
| 编号 | 误解 | 正确理解 | 相关章节 |
|:----:|------|---------|---------|
| 1 | 阻抗参数（$K_d$, $D_d$）越大越好，刚度高则精度高 | 过高的 $K_d$ 会导致接触振荡甚至失稳（采样无源判据 $b > K_d T/2$ 给出了刚度上界）；过高的 $D_d$ 在高频反而注入能量。参数选取是性能与稳定性的权衡 | §11.4, §11.5 |
| 2 | 笛卡尔阻抗控制不需要动力学模型 | 即使最简单的实现也需要 Jacobian $J(q)$ 和 Coriolis 补偿 $C(q,\dot{q})\dot{q}$；完整的 OSC 还需要操作空间惯量 $\Lambda = (JM^{-1}J^T)^{-1}$。所谓"不需要模型"指的是不需要精确的环境模型，而非不需要机器人模型 | §4.3, §5.2 |
| 3 | CI（笛卡尔阻抗）和 OSC（操作空间控制）本质相同 | CI 不做惯性整形，闭环惯量是位形相关的 $\Lambda(q)$；OSC 通过 $\Lambda$ 矩阵整形为期望惯量 $M_d$，代价是对模型精度更敏感。CRISP 实测 CI-clipped 比 OSC-clipped 更鲁棒 | §5.3 |
| 4 | 四元数就是一种"4D 欧拉角"，姿态误差直接做减法 | 四元数空间 $S^3$ 是非欧几里得的，直接减法无物理意义。必须用 $q^{-1} \otimes q_d$ 计算误差四元数，并处理双覆盖（$q$ 和 $-q$ 代表同一旋转） | §3.2 |
| 5 | 控制器在自由空间稳定 = 接触时也稳定 | 自由空间稳定性和耦合稳定性是两个不同问题。接触引入环境动力学改变了闭环对象，必须用 Colgate-Hogan 耦合稳定性定理或无源性判据单独验证 | §11.2, §11.3 |
| 6 | $D = 2\sqrt{K}$ 就是精确的临界阻尼公式 | 这是假设操作空间惯量 $M_d = 1$ kg 的启发式简化。真实阻尼比 $\zeta = D/(2\sqrt{K\Lambda})$，$\Lambda$ 是位形相关的（典型 1-10 kg），所以实际系统通常欠阻尼 | §4.2 |
| 7 | 无源性是力控稳定的必要条件 | 无源性是耦合稳定的**充分条件**，非必要条件。非无源控制器对特定已知环境仍可能稳定——但失去了"对所有无源环境一次证明"的保证 | §11.2 |
...
> **注**：与 F02/F03 共享的符号（$\Lambda, M, C, g, J, \tilde{x}, K_d, D_d, M_d$ 等）见 F02 符号约定表，本章沿用其定义不重复列出。
...
| 连续阻抗无源性 | $M_d\ddot{x}+D_d\dot{x}+K_d x$ 在 $D_d>0$ 时无源，接触任意刚度环境稳定 | §11.3 |
...
| Jacobian-误差坐标系自洽 | $\tau=J^T F$ 要求 $J$ 与 $F$ 同坐标系，否则恢复力方向随姿态错位 | §12.2 |
...
|---------|-----------|-------------------|
| F05 导纳控制与 ROS2 | 本章讲力矩接口阻抗，F05 讲位置接口导纳——两者是力控的对偶实现 | §1（理论 vs 工程差异）、§10（阻抗→导纳决策） |
| F06 变阻抗与无源性 | 本章的固定参数阻抗是 F06 时变参数的起点 | §8（参数调节策略）、§11（无源性判据——时变参数破坏无源性的机制） |
| F07 WBC 力控 | 本章的操作空间控制是 WBC 中任务优先级的基础 | §5（OSC 与惯性整形）、§4（零空间投影） |
| F09 学习型力控 | CRISP 是 RL 策略的底层执行器 | §6（CRISP 七项控制律）、§8（学习驱动的变阻抗） |

## 研究实践建议

**给初学者的建议**：

1. 先在 libfranka 仿真模式（`franka::Robot::Control` + MuJoCo）中运行 `cartesian_impedance_control` 示例，调节 $K_d$ 和 $D_d$ 观察行为
2. 逐行对照本章第 4 节的代码注释和 F03 的公式，确保理解每行代码的物理含义
3. 尝试让末端接触刚性桌面，体验振荡和稳定之间的参数边界

**给有经验者的建议**：

1. 精读 CRISP 源码和 matthias-mayr CIC——理解研究级控制器如何处理本章第 12 节的工程陷阱
2. 实验 CI vs OSC 在不同构型下的行为差异——这是理解"模型鲁棒性 vs 解耦精度"权衡的最直观方式
3. 关注 TDPC（时域无源性控制）的工程实现——这是保证变阻抗安全的前沿方法
...
| 工具/库 | 本章参考版本 | 说明 |
|---------|-----------|------|
| libfranka | 0.13+ | Franka FCI 力矩接口 |
| franka_ros2 | Humble+ | ROS2 Franka 驱动 |
| Pinocchio | 3.x | 动力学计算 |
| MuJoCo | 3.x | 力控仿真 |
| CRISP | 2025 (arXiv 2509.06819) | TUM 双频力控框架 |
| Eigen | 3.4+ | 线性代数 |
