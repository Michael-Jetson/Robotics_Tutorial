...
4. **Self-collision kernel 的稀疏性优化**。机器人自碰撞检测需要检查每一对 link 之间的距离，N 个 link 有 O($N^2$) 对，但很多对（相邻 link）不需要检查。cuRobo 的 self-collision kernel 针对"稀疏检查对 > 512$\times$1024"的情况做了特殊处理。这类针对**稀疏性** + **GPU 线程块** + **原子操作**的性能优化代码是学习 CUDA 优化的进阶素材。
