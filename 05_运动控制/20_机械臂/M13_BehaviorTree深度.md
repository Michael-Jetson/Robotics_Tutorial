...
📋 **前置自测**（答不出 ≥ 2 题 → 先回 `02_C++基础与进阶/10_C++语言核心` / `02_C++基础与进阶/30_软件工程` / `02_C++基础与进阶/50_ROS2工程化` 复习）
...
3. ROS2 Action 的三个组成部分（Goal/Feedback/Result）分别表达什么语义？（`02_C++基础与进阶/50_ROS2工程化/40_硬件集成与RL部署`）
4. `std::any` 是什么？它如何在运行时实现类型安全的多态存储？（`02_C++基础与进阶/10_C++语言核心/10_类型系统与值类别推导`）
5. 有限状态机（FSM）的状态数与转移数在复杂系统中如何增长？（`02_C++基础与进阶/30_软件工程/10_设计模式与高级惯用法`）
