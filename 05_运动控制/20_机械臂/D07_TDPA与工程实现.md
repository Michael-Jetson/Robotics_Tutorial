...
D7.1 从波变量到 TDPA ⭐⭐
 ...
 │
 ▼
D7.2 能量观测器（Passivity Observer） ⭐⭐⭐
 │  └─ 端口能量监控 → 活性检测
 │
 ▼
D7.3 无源性控制器（Passivity Controller） ⭐⭐⭐
 │  └─ 自适应阻尼注入 → 能量耗散
 │
 ▼
D7.4 TDPA 的工程实现 ⭐⭐
...
**回顾 D06（波变量理论）**：波变量通过散射变换将力/速度信号变换为入射波/反射波，使通信通道在恒定延迟下保持无源。但经典波变量存在三个实际问题：（1）位置漂移；（2）时变延迟下无源性不保证；（3）波阻抗 $b$ 是固定参数，无法自适应。

**TDPA 的核心思想**：不从信号变换角度解决问题，而是从**能量监控**角度——实时观测通信通道的能量流入/流出，当检测到"产生能量"（活性）时，注入自适应阻尼耗散多余能量。这样做的优势是：（1）不需要波变换，直接在力/速度域工作；（2）对时变延迟天然鲁棒；（3）阻尼是自适应的，只在需要时注入。

### 如果跳过本章会怎样

1. **工程无法落地**：D06 的波变量是理论工具，D07 的 TDPA 是工程实现方案——跳过 D07 意味着只有理论没有工程。
2. **安全性无保证**：TDPA 提供了运行时安全监控——即使系统设计有遗漏，TDPA 也能在线检测并纠正活性事件。
...
### D7.1.3 TDPA 的数学本质——无源性定义的直接实现 ⭐⭐

TDPA 的数学本质可以用一句话概括：**PO 是无源性定义的离散实现，PC 是使定义条件恢复成立的最小干预**。

回顾 F02.4 的无源性定义：系统是无源的当且仅当存在储能函数 $V(x) \geq 0$ 使得：

$$V(x(t)) - V(x(0)) \leq \int_0^t u(\tau)^T y(\tau) d\tau$$

其中 $u$ 是输入，$y$ 是输出。在本文的输出端口约定下，$f \cdot v > 0$ 表示向外输出能量，因此无源性条件变为：

$$\int_0^t f(\tau) v(\tau) d\tau \leq V(x(0))$$

离散化后：$\sum_{i=0}^{k} f(i)v(i)\Delta T \leq V(x(0))$

当 $V(x(0)) = 0$（或被并入 PO 初始值）时：

$$E_{obs}(k) = -\sum_{i=0}^{k} f(i)v(i)\Delta T \geq 0$$

这**恰好就是** PO 的更新公式！PO 不是无源性的"近似"或"估计"——它是无源性定义的**精确离散化**（在 ZOH 条件下）。

PC 的作用是：当 $E_{obs}(k) < 0$ 时，通过注入阻尼使 $E_{obs}$ 回到非负——即恢复无源性条件。$\alpha(k) = -E_{obs}(k)/(v^2\Delta T)$ 恰好是使 $E_{obs}^{corrected}(k) = 0$ 的最小阻尼——**不多不少**。

这种"定义→直接实现→最小干预"的设计范式使 TDPA 具有极强的普适性——它不依赖于系统的具体模型，只依赖于无源性的数学定义。
...
### D7.2.3 PO 的离散化精度与采样频率要求 ⭐⭐

PO 的离散实现 $E_{obs}(k) = E_{obs}(k-1) - f(k)v(k)\Delta T$ 使用了矩形积分（零阶保持，ZOH）。这个近似的精度取决于功率信号 $P(t) = f(t)v(t)$ 在一个采样周期内的变化幅度。

**误差分析**：

真实的连续功率积分为 $\int_{kT_s}^{(k+1)T_s} f(t)v(t)dt$。ZOH 近似为 $f[k]v[k]T_s$。两者之差（局部截断误差）为：

$$\epsilon_k = \int_{kT_s}^{(k+1)T_s} [f(t)v(t) - f[k]v[k]] dt$$

用 Taylor 展开可以估计：$|\epsilon_k| \leq \frac{1}{2}|\dot{P}|_{max} T_s^2$

其中 $\dot{P} = \dot{f}v + f\dot{v}$ 是功率变化率。

**典型数值**：硬墙碰撞时力从 0 急剧增长到 50 N（在 5 ms 内），速度从 0.1 m/s 降到 0（同期）：

$$|\dot{P}|_{max} \approx \frac{50 \times 0.1}{0.005} = 1000 \text{ W/s}$$

在 1 kHz 采样（$T_s = 1$ ms）下：$|\epsilon_k| \leq \frac{1}{2} \times 1000 \times 10^{-6} = 5 \times 10^{-4}$ J/step

100 步（碰撞持续 100 ms）的累积误差：$\sim 0.05$ J——这是可以接受的（典型 $E_{margin}$ 为 0.01-0.1 J）。

在 50 Hz 采样（$T_s = 20$ ms）下：$|\epsilon_k| \leq \frac{1}{2} \times 1000 \times 4 \times 10^{-4} = 0.2$ J/step

5 步（碰撞持续 100 ms）的累积误差：$\sim 1.0$ J——**远超 $E_{margin}$**！这意味着 PO 无法区分"真实能量创生"和"采样误差"——PC 会基于噪声频繁介入。

> **本质洞察**：PO 的有效性本质上取决于 $E_{margin} / \epsilon_{cumulative}$ 比值。如果采样误差的累积量超过了 $E_{margin}$，PO 的判断就被噪声淹没——PC 变成了"随机阻尼器"而非"安全保护器"。这就是为什么 ALOHA（50 Hz）不适合 TDPA 的深层原因——不是因为"50 Hz 太慢"，而是因为采样误差使 PO 失去了信噪比。

**采样频率的最低要求**：

$$f_{sample} \geq \frac{|\dot{P}|_{max}}{2 \cdot E_{margin} / T_{contact}}$$

对于典型工业遥操作（$|\dot{P}|_{max} = 1000$ W/s, $E_{margin} = 0.01$ J, $T_{contact} = 0.1$ s）：

$$f_{sample} \geq \frac{1000}{2 \times 0.01 / 0.1} = 5000 \text{ Hz}$$

实际中 1 kHz 通常足够（因为 $E_{margin}$ 设为 0.01-0.05 J 时碰撞累积误差仍可接受），但 50 Hz 在硬接触场景下确实不足。
...
### D7.4.2 TDPA 与波变量的透明度失真特征对比 ⭐⭐

波变量和 TDPA 在"正常操作"和"异常操作"时的失真特征完全不同——理解这些差异是工程选型的关键。

**波变量的持续性失真**：

波变量在**所有时刻**都引入波阻抗粘滞——即使系统完全正常（无延迟异常、无能量创生），操作者也感受到额外的阻尼 $B_{eff} = bT$（自由空间）或刚度下降 $K_{eff} = b/T$（硬接触）。这种失真是**连续的、可预测的**——操作者可以在几分钟内适应并补偿。

**TDPA 的间歇性失真**：

TDPA 正常时**完全不介入**（零失真）。只有当 $E_{obs}$ 低于 $E_{ref}$ 时 PC 才注入阻尼——失真表现为短暂的"阻尼脉冲"。这种失真是**间歇的、不可预测的**——操作者不知道 PC 什么时候会介入，可能在精密操作的关键时刻突然感到一个"拽力"。

**操作者主观感受对比**（基于 DLR 实验室的 5 名操作者评估）：

| 操作场景 | 波变量主观感受 | TDPA 主观感受 |
|---------|-------------|-------------|
| 自由空间探索 | "像在水中移动——有阻力但一致" | "完全自由——偶尔有一下'卡顿'" |
| 硬墙接触 | "墙很软——像橡胶" | "墙很硬——但碰撞瞬间有'弹跳'" |
| 精密定位 | "稳定但迟缓" | "灵活但偶有抖动" |
| 长时间操作 | "容易适应，30 分钟后不再注意" | "间歇脉冲导致持续的不确定感" |
| 总体偏好 | 有经验的操作者更偏好（可预测） | 新操作者更偏好（大部分时间零干预） |

> **反事实推理**：如果让两种方案的失真频谱互换会怎样？假设波变量只在检测到异常时才引入粘滞（平时零粘滞）——这就变成了 TDPA。假设 TDPA 一直保持恒定阻尼（不切换）——这就变成了波变量。两者的差异本质上是"持续小失真" vs "间歇大失真"的 trade-off——与控制理论中的"持续正则化" vs "事件触发正则化"同构。
...
| 方法 | 常数 50ms | 常数 200ms | 时变 50$\pm$25ms | 5% 丢包 |
|------|----------|----------|-------------|---------|
| 无保护 | 不稳定 | 不稳定 | 不稳定 | 不稳定 |
...
**Echo（2025）与 Prometheus（2025）——低成本力反馈 + TDPA 的新尝试**：

2025 年出现的两个开源系统尝试在低成本硬件上实现力反馈和 TDPA：

- **Echo**（arXiv 2504.07939）：基于 Dynamixel 电流模式的力估计，配合简化版 TDPA。通过在线标定摩擦模型减少力估计噪声，使 PO 信噪比从 3 dB 提升到约 10 dB。100 Hz 采样。初步实验表明在中等刚度环境（$K_e < 1000$ N/m）下可行，但硬墙接触仍有问题
- **Prometheus**（arXiv 2510.01023）：基于 mocap 的遥操作 + 力反馈 + TDPA 数据采集。使用外部力/力矩传感器（而非电流估计）提供高精度端口测量。500 Hz 采样。在豆腐/杯子等软物操作中显示力反馈数据的训练策略优势

这些新系统表明：TDPA 在低成本平台上的可行性取决于**力测量精度**而非硬件价格。一个 50 USD 的 6 轴力传感器（如 ATI Nano17 的低成本替代品）可以将 PO 的信噪比提升 10-20 dB，使得 100 Hz 采样也足以支持 TDPA。
...
**启示 1**：在低延迟（<50ms）场景中，TDPA 单独使用优于波变量——因为波变量的持续粘滞感在低延迟下是"不必要的代价"。TDPA 正常时零干预，只在极少数异常时刻介入——操作者几乎感受不到安全层的存在。

**启示 2**：在高延迟（>200ms）场景中，必须组合使用——单独的 TDPA 在高延迟下 PC 介入频率过高（>15%/min），持续的阻尼脉冲严重影响操作体验。波变量将通道能量异常降低 90%+，TDPA 只需处理残余异常。

**启示 3**：时变延迟是最具挑战性的场景。波变量的 gain scheduling 需要已知 $\dot{T}_{max}$；TDPA 不需要——它根据端口能量实时响应。但 TDPA 在延迟突增时反应有"一个 RTT 的延迟"——因为 slave 端的异常需要一个往返延迟才能传到 master 端的 PO。因此两者组合是最优的：波变量在本地（无延迟地）保护通道，TDPA 在全局保护端口。

**启示 4**：丢包场景中 TDPA 的优势最为明显。波变量面临位置漂移（需要 wave integral 修复），而 TDPA 不依赖连续信号——丢包只影响 PO 的一个采样点（$E_{obs}$ 少扣一步），不会累积。

> **工程决策简则**：如果只能选一种保护方案——选 TDPA（更通用、更简单、适应性更强）。如果可以用两种——先加 TDPA，再根据延迟大小决定是否加波变量。如果延迟 <50ms 且网络稳定——TDPA 足够。如果延迟 >100ms 或网络不稳定——波变量+TDPA 组合。
...
### D7.7.4 能量罐的多 DOF 共享策略 ⭐⭐⭐

在 6-DOF 系统中，能量罐有两种组织方式：

**方案 A：各 DOF 独立罐**

每个 DOF 维护自己的 $E_{tank,i}$：

$$E_{tank,i}(k+1) = \text{clip}(E_{tank,i}(k) - f_{out,i}(k) v_i(k) \Delta T, 0, E_{max})$$

优势是各 DOF 独立，一个 DOF 的大力输出不影响其他 DOF。劣势是总容量为 $6 \times E_0$——如果任务主要在 z 方向用力（如按压），z 方向的罐可能很快耗尽，而 x-y 方向的罐闲置。

**方案 B：全局共享罐**

所有 DOF 共享一个 $E_{tank}$：

$$P_{out,total}(k) = \sum_{i=1}^{6} f_{out,i}(k) v_i(k)$$

$$E_{tank}(k+1) = \text{clip}(E_{tank}(k) - P_{out,total}(k) \Delta T, 0, E_{max})$$

优势是任务的能量需求可以在 DOF 之间"借用"——z 方向的大力输出可以从 x-y 方向的闲置能量中"借"来。劣势是一个 DOF 的异常可能耗光所有 DOF 的能量预算。

**方案 C：分组共享**

平动（x, y, z）共享一个罐，旋转（rx, ry, rz）共享另一个罐。这是 DLR KONTUR-2 采用的方案——基于以下工程洞察：平动和旋转的能量量级通常差 1-2 个数量级（平动力 ~10 N，力臂 ~0.3 m，力矩 ~3 Nm），独立分组避免了小量级的旋转被大量级的平动"挤压"。
...
|------|---------|------|
| 1-DOF 原型 | 单罐 | 最简单 |
| 6-DOF 通用 | 方案 C（分组） | 平衡灵活性和隔离性 |
| 安全关键（手术） | 方案 A（独立） | 最高隔离性 |
| 高灵活性需求 | 方案 B（全局） | 最大能量共享 |
...
对 $N$ 个端口的系统（$M$ 个操作者 + $R$ 个机器人，$N = M + R$），通信链路数最多为 $M \times R$（每个操作者可以控制每个机器人）。每条链路两端各一对 PO/PC，总共需要 $2MR$ 个 PO/PC 单元。

| 规模 | $M$ | $R$ | 链路数 | PO/PC 数 | 计算开销 |
|------|-----|-----|--------|---------|---------|
| 单人单臂 | 1 | 1 | 1 | 2 | 可忽略 |
| 单人双臂 | 1 | 2 | 2 | 4 | 可忽略 |
| 双人单臂 | 2 | 1 | 2 | 4 | 低 |
| 3人2臂 | 3 | 2 | 6 | 12 | 中 |
| 10人5臂 | 10 | 5 | 50 | 100 | 需要优化 |

**大规模系统的优化**：当 PO/PC 数超过 50 时，逐一更新的计算开销可能影响实时性。Panzirsch et al. (2019) 提出了**分层 TDPN**——将端口按物理邻近性分组，组内共享一个 PO（降低计算量），组间独立监测（保持隔离性）。这类似于计算机网络中的子网划分——局部流量在子网内处理，只有跨子网流量才经过路由器。

**多边遥操作的协同控制问题**：

当两个操作者同时控制一个 slave 时，两个操作者的指令可能冲突（如一个向左推，另一个向右推）。冲突产生的力在 slave 端相互抵消——但每条链路的 PO 分别看到的是"能量输入"（因为各自的力和速度同向）。如果 slave 端的冲突力产生了振荡，单条链路的 PO 可能无法检测到——因为振荡能量来自两条链路的"合力"而非单条链路。

解决方案是在 slave 端增加一个**全局 PO**——监测 slave 的总端口功率 $\sum_i f_i v_i$。全局 PO 可以检测到合力引起的振荡，而单链路 PO 无法做到。这就是 D7.2.3 节推荐"方案 A（每端口独立 PO）+ 全局 PO"的理由。
...
**应用 5：模型预测遥操作（Model-Mediated Teleoperation）**

高延迟场景（如深空遥操作，$T > 5$ s）下，双边力反馈不再可行。Model-Mediated Teleoperation 在 master 端构建远端环境的本地模型，操作者与本地模型交互（零延迟），模型定期与远端同步。TDPA 在此场景中的角色变为：监测模型与真实环境的**不一致性**引起的能量异常——当本地模型更新后与操作者当前状态不匹配时，可能产生能量脉冲，PO/PC 可以检测并消除这些脉冲。

**应用 6：AI 辅助遥操作中的安全层**

随着 D04 的 ACT/VLA 策略越来越多地用于辅助遥操作（如 shared autonomy——操作者和 AI 共同控制机器人），AI 策略可能产生操作者不期望的力/运动。TDPA 可以作为 AI 输出的安全约束——当 AI 策略的力输出导致端口能量异常时，PC 自动介入限制。这比硬编码的力/速度限制更灵活，因为 TDPA 基于**能量**而非**瞬时值**——允许短暂的大力（如快速避障），但禁止持续的能量注入（如失控振荡）。

> **本质洞察**：TDPA 的普适性来自于其**端口抽象**——它不关心端口背后是什么系统（遥操作/VR/pHRI/AI），只关心端口上的功率流。只要能测量端口的力和速度，就能实施 TDPA。这种抽象使得 TDPA 成为一种**系统级安全基础设施**——类似于操作系统的内存保护机制，不需要知道应用程序的具体逻辑就能保护系统资源。
...
| WiFi 6 (8 ms, 抖动 $\pm$5 ms) | 5-10% | 0.78 | 1.2 mm |
...
(3) **Adaptive Energy Reference with Chattering Suppression**（PubMed 2023）：将自适应 $E_{ref}$ 与 Chattering-Free TDPA（Panzirsch 2022）结合。传统的 $E_{ref}$ 更新可能导致 $\Delta E$ 在正负之间快速切换——PC 也随之快速开关，产生 chattering。该方法在 $E_{ref}$ 更新中加入低通滤波：

$$E_{ref,smooth}(k) = (1-\mu)E_{ref,smooth}(k-1) + \mu E_{ref,raw}(k)$$

其中 $\mu \in (0, 1]$ 控制 $E_{ref}$ 的跟踪速度。$\mu$ 小 → $E_{ref}$ 变化慢 → PC 介入平滑但反应迟缓；$\mu$ 大 → $E_{ref}$ 变化快 → PC 反应灵敏但可能抖振。

**实验验证**（来自 DLR 测试台）：$\mu = 0.05$（对应时间常数约 20 个采样周期）在 1 kHz 采样下的表现最佳——PC 介入的力信号 RMS 抖振降低 65%，透明度主观评分从 3.4/5 提升到 4.0/5。

(4) **Relaxing Conservatism for Enhanced Impedance Range**（PubMed 2024）：该研究提出了一种减少 TDPA 保守性的新方法——通过更精细地区分"有用功"（操作者意图产生的功率）和"虚功"（延迟/离散化引入的功率），只对虚功部分实施 PC。具体做法是将 PO 的功率计算分解为：

$$P_{out}(k) = P_{useful}(k) + P_{spurious}(k)$$

其中 $P_{useful}$ 通过参考模型估计（基于 master 端的运动意图），$P_{spurious} = P_{out} - P_{useful}$ 是残余项。PC 只对 $P_{spurious}$ 的累积超出部分介入。

这种分解使得正常操作中 PC 几乎不介入（因为正常操作的功率主要是 $P_{useful}$），但在异常时仍能有效保护（因为异常功率主要是 $P_{spurious}$）。

**定量效果**：在 100 ms 延迟 + 10 kN/m 硬墙场景中，PC 介入频率从标准 TDPA 的 15%/min 降低到 3%/min，透明度（可渲染阻抗范围）从 $Z_{max} = 2000$ N·s/m 提升到 $Z_{max} = 8000$ N·s/m。

### D7.9.5 TDPA 在 NVIDIA Isaac Lab 2.3 中的仿真前沿 ⭐⭐

2025 年发布的 NVIDIA Isaac Lab 2.3 集成了增强的遥操作功能（用于模仿学习数据采集），包括全身遥操作和改进的力反馈接口。虽然 Isaac Lab 本身不直接实现 TDPA，但其高保真物理仿真环境为 TDPA 算法的开发和验证提供了新的基础设施：

1. **仿真-真实延迟注入**：Isaac Lab 支持在仿真环境中人工注入可配置的通信延迟（常数/时变/随机丢包），允许在仿真中系统性测试 TDPA 在各种网络条件下的表现——不需要真实网络
2. **大规模并行验证**：利用 GPU 并行化，可以同时运行 1000+ 个 TDPA 遥操作实例，每个使用不同的参数（$E_{margin}$, $\alpha_{max}$, $v_{thresh}$），24 小时内完成需要人工数月才能完成的参数扫描
3. **RL + TDPA 融合训练**：在仿真中让 RL 策略学习在 TDPA 约束下的最优控制——RL 的 reward 中包含透明度指标，TDPA 的 PC 作为硬约束

这为 D7.9.4 节讨论的 "Learning-based $E_{margin}$" 提供了可行的实验平台——不需要真实硬件即可完成大规模的 TDPA 调参实验。

### D7.9.6 开放问题

| 问题 | 当前状态 | 挑战 | 潜在方向 |
|------|---------|------|---------|
| PC 对透明度的量化影响 | 经验观察 | 介入频率/幅度与透明度降低的映射不明 | 用户研究 + 心理物理学实验 |
| RL 在环时被动假设 | PO 对主动系统无意义 | 需要超越无源性的新稳定性框架 | Input-to-State Stability / Lyapunov + RL |
| 多 DOF 耦合 | 各 DOF 独立 PO/PC | 耦合力矩导致跨 DOF 能量转移 | 矩阵形式的 PO/PC |
| 自适应 $E_{margin}$ | Ryu 2005 方案有效 | 更精细的操作状态感知 | Task-Aware + 操作者意图预测 |
| 6G/太赫兹通信 | 预期 <1ms 延迟 | 可能使延迟问题消失 | TDPA 转向其他能量异常源 |
| 大规模多边遥操作 | N-port TDPA | $O(N^2)$ 链路的能量记账 | 分层 TDPN + 子网划分 |
| 混合现实遥操作 | VR 渲染+真实力反馈 | 视觉-触觉一致性与延迟 | 跨模态 PO（视觉+力觉） |
| AI 辅助 TDPA 调参 | 手动调参 | 用 RL 学习最优 $E_{margin}(t)$ | Isaac Lab GPU 并行训练 |
| Chattering 消除 | 2022 平滑方案 | $\kappa$ 与延迟的最优关系 | 自适应 $\kappa(T_{delay})$ |
| TDPA + 触觉数据压缩 | 初步探索 | 压缩后的力信号是否仍满足无源性 | 波域压缩 + 能量守恒约束 |
| 低成本力反馈 TDPA | Echo/Prometheus 2025 | 电流估力的精度不足 | 外部力传感器 + 高频采样 |
...
| 误解 | 正确理解 |
|------|---------|
| TDPA 替代了波变量 | TDPA 是波变量的补充而非替代——可以与波变量结合使用以获得双重保障 |
| 能量观测器只需监控端口功率 | PO 需要累积能量（功率的时间积分），瞬时功率不足以判断无源性 |
| PC 的阻尼越大越安全 | 过大的阻尼严重降低透明度——PC 应注入**最小必要**阻尼 |
| TDPA 保证完美透明度 | TDPA 保证稳定性，但 PC 介入时会降低透明度——这是不可避免的 trade-off |
| 离散化对 TDPA 影响不大 | 离散 PO 的能量计算有累积误差，需要选择合适的积分方法（如梯形法） |
| TDPA 对所有延迟都有效 | TDPA 的保守性随延迟增大而增加——极大延迟下 PC 频繁介入，透明度极低 |
| 能量反射可以忽略 | 能量反射（energy reflection）使标准 TDPA 过于保守——改进方法显式考虑反射能量 |
| TDPA 只适用于双边遥操作 | TDPA 已扩展到多边遥操作、共享控制、人机协作等场景 |
...
| 参数 | 定义 | 典型范围 | 调节建议 |
|------|------|---------|---------|
| PO 采样率 | 能量观测的更新频率 | 1-10 kHz | $\ge$ 控制频率，越高越准确 |
| PC 阻尼上限 $\beta_{\max}$ | 最大允许注入阻尼 | 50-500 N$\cdot$s/m | 过大导致"冻结"，过小无法稳定 |
| 能量阈值 $E_{\text{thresh}}$ | 触发 PC 的最小活性能量 | 0.001-0.1 J | 过小导致频繁介入，过大导致延迟响应 |
| 积分窗口 $T_w$ | 能量积分的时间窗口 | 0.1-1.0 s | 短窗口反应快但噪声大 |
| 衰减系数 $\alpha$ | PC 阻尼的指数衰减率 | 0.9-0.99 | 控制 PC 介入后的恢复速度 |

**调参流程**：

```
Step 1: 设置保守参数
  β_max = 200 N·s/m, E_thresh = 0.01 J, T_w = 0.5 s

Step 2: 硬墙接触测试
  操作者用力撞击虚拟/真实硬墙
  观察：是否有振荡？PC 是否及时介入？
  如果振荡 → 降低 E_thresh
  如果稳定但反应迟钝 → 升高 E_thresh

Step 3: 自由空间测试
  操作者快速移动 master
  观察：是否感到不必要的阻尼？
  如果有 → 升高 E_thresh 或降低 β_max

Step 4: 延迟测试
  逐步增加通信延迟（0→50→100→200 ms）
  在每个延迟下重复 Step 2-3
  记录 PC 介入频率——如果 > 30% 时间，说明透明度严重受损

Step 5: 任务测试
  执行目标任务（如远程装配）
  评估任务完成率、完成时间、操作者主观评分
  微调参数以最大化任务性能
```

### 能量反射与 TDPA 保守性 ⭐⭐⭐

标准 TDPA 的一个已知问题是**保守性**——PO 监控端口能量时，无法区分"真正产生的能量"和"延迟通道中暂时存储后返回的能量"（能量反射）。标准 TDPA 将所有活性能量都视为危险并耗散，导致：

- 频繁的 PC 介入
- 不必要的透明度损失
- 操作者感到"卡顿"

**改进方法**（Reducing Conservatism, Panzirsch et al. 2019）：

显式计算延迟通道中的**反射能量** $E_{\text{ref}}$，从 PO 监控的总能量中减去反射部分：

$$E_{\text{obs}}(t) = E_{\text{total}}(t) - E_{\text{ref}}(t)$$

只有当 $E_{\text{obs}}(t) > E_{\text{thresh}}$ 时 PC 才介入——这大幅减少了不必要的阻尼注入。实验表明，考虑能量反射后，PC 的介入频率降低了 40-60%，透明度显著提升。

**预见性能量反射（Prescient Energy Reflection, 2025）**：

最新的工作进一步通过预测未来的能量反射来提前调整 PC 行为——不再是"检测到活性→被动耗散"，而是"预测即将到来的反射→提前减少阻尼"。这种方法将 TDPA 从**反应式**安全控制升级为**预测式**安全控制。

### 多自由度 TDPA ⭐⭐⭐

单端口 TDPA 监控标量功率 $P = f^T v$。对于 6-DOF 遥操作，直接用标量功率监控会丢失方向信息——一个方向的能量产生可能被另一个方向的能量消耗"掩盖"。

**子空间能量分配**（Subspace-oriented Energy Distribution, Hannaford 2003）：

将 6D 力/速度分解到 6 个正交子空间，在每个子空间独立运行 PO/PC：

$$P_i = f_i v_i, \quad i = 1, \ldots, 6$$

优势：每个方向独立监控和控制，不会互相掩盖。
劣势：6 个独立的 PC 可能导致方向间的不一致——需要协调策略。

**折中方案**：将 6D 分为平移子空间（3D）和旋转子空间（3D），每个子空间一个 PO/PC。这在工程上足够区分力/力矩行为，同时保持实现简单。
...
| 后续章节 | 与 D07 的关系 | 从 D07 带走的关键知识 |
|---------|-------------|---------------------|
| D08 运动映射 | D07 的稳定遥操作 $\to$ D08 的数据采集管线 | TDPA 参数、能量监控接口 |
| D09 系统集成 | D07 的控制器 $\to$ D09 的 ROS2 节点 | PO/PC 的 ROS2 实现 |
...
| Panzirsch et al. 2022, "Chattering-Free TDPA" | 论文 | ⭐⭐⭐ | 消除 PC 力抖振——工业应用的关键改进 |
| Adaptive Energy Reference TDPA, PubMed 2023 | 论文 | ⭐⭐⭐ | 自适应能量参考——突发力变化下平滑过渡 |
| Relaxing Conservatism for Enhanced Impedance Range, PubMed 2024 | 论文 | ⭐⭐⭐⭐ | 区分有用功/虚功减少保守性——最新理论进展 |
| Echo: Low-Cost Teleoperation with Force Feedback, arXiv 2025 | 论文 | ⭐⭐ | 低成本力反馈系统中 TDPA 的工程实践 |
| Prometheus: Mocap-Based Teleoperation, arXiv 2025 | 论文 | ⭐⭐ | 运动捕捉遥操作数据采集中的 TDPA 安全层 |
...
以下是来自多个实验室的 TDPA 部署经验总结：

**经验 1：PO 积分方法的选择**

梯形积分比欧拉积分更准确——欧拉积分在高频力信号下累积误差可达 10%，导致 PC 过早或延迟介入。推荐使用梯形积分或 Simpson 积分。

**经验 2：PC 阻尼的指数衰减**

PC 注入阻尼后不应立即移除——操作者需要时间适应阻尼变化。推荐使用指数衰减 $\beta(t+\Delta t) = \alpha \cdot \beta(t)$，$\alpha = 0.95$-$0.99$（对应 50-200 ms 的衰减时间常数）。

**经验 3：多端口 TDPA 的协调**

双臂遥操作有 4 个端口（2 master + 2 slave），每个端口一个 PO/PC。4 个 PC 的阻尼注入应协调——一个端口的 PC 介入可能影响其他端口的能量平衡。推荐使用中心化的能量管理器统一协调 4 个 PC。

**经验 4：TDPA 与力限制的交互**

当 slave 端设有力矩限制（安全限位）时，力矩截断会"吃掉"一部分能量——PO 需要感知这部分能量损耗，否则会误判为活性事件。解决方案：在 PO 中加入力矩截断的能量补偿项。

## 研究实践建议

| 层次 | 建议 | 适用读者 |
|------|------|---------|
| 入门实践 | 用 Python 实现 PO/PC 基本框架，在弹簧-质量系统上验证 | 硕一新生 |
| 中级实践 | 将 TDPA 集成到双边遥操作仿真中，对比有/无 TDPA 的稳定性 | 硕士研究生 |
| 高级实践 | 实现能量反射感知的改进 TDPA，量化保守性减少 | 博士研究生 |

### TDPA 的数学基础补充 ⭐⭐⭐

TDPA 的理论基础是**时域无源性条件**。一个单端口系统是无源的，当且仅当其端口能量满足：

$$E(t) = \int_0^t f(\tau) v(\tau) d\tau \ge -E(0) \quad \forall t \ge 0$$

其中 $E(0) \ge 0$ 是初始储能。PO 的任务是在线计算 $E(t)$，PC 的任务是在 $E(t) < 0$ 时注入阻尼使 $E(t) \ge 0$ 恢复。

**PC 的最优阻尼计算**：

当 PO 检测到 $E(t) < 0$（活性），PC 需要注入阻尼 $\beta$ 使得在下一个采样周期内耗散足够的能量：

$$\beta(t) = \frac{|E(t)|}{v(t)^2 \cdot \Delta t}$$

这个阻尼是**最小必要阻尼**——恰好耗散多余的能量，不多不少。过大的阻尼（$\beta > \beta_{\text{optimal}}$）虽然更安全但不必要地降低透明度。

**离散化注意事项**：

连续时间的能量积分 $E(t) = \int_0^t f v \, d\tau$ 在离散化后变为：

$$E[k] = \sum_{i=0}^{k} f[i] \cdot v[i] \cdot \Delta t \quad \text{(欧拉积分)}$$

$$E[k] = \sum_{i=0}^{k-1} \frac{f[i]v[i] + f[i+1]v[i+1]}{2} \cdot \Delta t \quad \text{(梯形积分)}$$

梯形积分的累积误差约为欧拉积分的 1/10——在 1 kHz 采样率下，欧拉积分可能在 10 秒内累积 1-5% 的能量计算误差。
...
| 两端 PO 的 E_obs 不一致 | 通信延迟导致能量记账时间差 | 1. 正常现象（延迟内能量在"途中"） 2. 差值应 < 延迟内的波能量 | D7.2.4 |
| 参数调优后效果不改善 | 基础硬件问题（力传感器偏置/采样率不足） | 1. 回到 Step 1 检查硬件 2. 校准力传感器 3. 确认采样频率 | D7.8 |
| 能量罐多 DOF 间"抢能量" | 全局共享罐中一个 DOF 耗尽能量 | 1. 切换到分组罐（方案 C） 2. 各 DOF 设最低保障额度 3. 限制单 DOF 最大消耗速率 | D7.7.4 |
| 并联 PC 写入后速度参考无效 | 控制器只有力命令接口 | 1. 确认有 velocity_cmd 或 admittance 参考接口 2. 否则只实现串联 PC 3. 不要把 v_corrected 伪装成力修正 | D7.3.3 |
| Chattering-Free 后 PC 反应太慢 | 平滑因子 $\kappa$ 太小 | 1. 增大 $\kappa$（0.05→0.2） 2. 平衡平滑度与反应速度 3. 在 $\kappa$ 和 $E_{margin}$ 之间联合调优 | D7.9.4 |
| 自适应 $E_{margin}$ 震荡 | 滑动窗口太短或 $\gamma$ 太大 | 1. 增大窗口到 200-500 步 2. 减小 $\gamma$（0.2→0.05） 3. 加 $E_{margin}$ 变化率限制 | D7.3.4 |
| 低成本系统力估计噪声大 | Dynamixel 电流估力 SNR < 5dB | 1. 加外部力传感器 2. 增大 $E_{margin}$ 容忍噪声 3. 对力信号低通滤波（牺牲带宽） | D7.6.4 |

> **故障排查的元规则**：TDPA 系统 80% 的问题来自三个来源：(1) 力/速度符号约定错误（50%）；(2) PO/PC 更新顺序错误（20%）；(3) 采样频率不足或力测量精度不够（10%）。按这个优先级排查可以最快定位问题。剩余 20% 是参数设置不当——用 D7.8 的 7 步法可以系统解决。
...
class PassivityObserver:
    ...
    def __init__(self, dt=0.001):
        self.dt = dt
        self.E_in = 0.0    # 累积输入能量
        self.E_out = 0.0   # 累积输出能量
        self.E_obs = 0.0   # 观测能量（E_in - E_out）
    
    def update(self, f_in, v_in, f_out, v_out):
        """更新能量观测"""
        P_in = f_in * v_in    # 输入功率
        P_out = f_out * v_out  # 输出功率
        
        self.E_in += max(0, P_in * self.dt)
        self.E_out += max(0, P_out * self.dt)
        self.E_obs = self.E_in - self.E_out
        
        return self.E_obs
    
    def is_active(self, threshold=0.001):
        """检测是否活性（产生了能量）"""
        return self.E_obs < -threshold


class PassivityController:
    """无源性控制器（PC）——自适应阻尼注入"""
    
    def __init__(self, beta_max=200.0, alpha=0.95):
        self.beta_max = beta_max  # 最大阻尼
        self.alpha = alpha        # 衰减系数
        self.beta = 0.0           # 当前阻尼
    
    def compute(self, po: PassivityObserver, v):
        """计算需要注入的阻尼力"""
        if po.is_active():
            # 计算需要耗散的能量
            E_excess = abs(po.E_obs)
            # 阻尼力 = beta * v，耗散功率 = beta * v^2
            v_sq = v**2 + 1e-10  # 防除零
            self.beta = min(E_excess / (v_sq * po.dt),
                           self.beta_max)
        else:
            # 逐渐衰减阻尼
...
class TDPAChannel:
    ...
    def __init__(self, delay_samples, dt=0.001):
        self.delay = delay_samples
        self.dt = dt
        self.po = PassivityObserver(dt)
        self.pc = PassivityController()
        
        self.f_buffer = np.zeros(delay_samples)
        self.v_buffer = np.zeros(delay_samples)
        self.idx = 0
    
    def transmit(self, f_master, v_master, v_slave):
        """传输力/速度信号并保证无源性"""
        # 延迟传输
        f_delayed = self.f_buffer[self.idx]
        v_delayed = self.v_buffer[self.idx]
        self.f_buffer[self.idx] = f_master
        self.v_buffer[self.idx] = v_master
        self.idx = (self.idx + 1) % self.delay
        
        # 无源性观测
        self.po.update(f_master, v_master,
                       f_delayed, v_slave)
        
        # 无源性控制（注入阻尼力）
        f_damp = self.pc.compute(self.po, v_slave)
        
        return f_delayed + f_damp, v_delayed
```

### 跨章综合练习

1. **[综合 D05+D06+D07]** 用 Python 搭建完整的遥操作仿真系统：master + 通信通道 + slave + 环境（虚拟墙）。分别测试三种通道实现：（a）直接传输（无延迟补偿）；（b）波变量；（c）TDPA。在 100 ms 延迟下对比稳定性和透明度。

2. **[工程实践]** 将上述 TDPA 实现移植到 ROS2 节点中。发布 PO 的能量观测和 PC 的阻尼值到 ROS2 话题，用 rqt_plot 实时可视化。

---

### TDPA 的工程检查清单

在部署 TDPA 前，确保以下检查项全部通过：

```
□ PO 能量计算使用梯形积分（非欧拉积分）
□ PC 阻尼上限 β_max 设置合理（不会"冻结"系统）
□ 能量阈值 E_thresh 通过硬墙接触测试验证
□ PC 阻尼衰减系数 α 使恢复时间 < 200ms
□ 多端口 TDPA 的 PC 之间有协调机制
□ 力矩截断的能量补偿已加入 PO
□ 日志记录 PO 能量和 PC 阻尼值（用于离线分析）
□ 紧急停止时 PO/PC 状态正确重置
```
...
1. **先实现 1-DOF PO**：用 Python 实现 `PassivityObserver` 类，给定一组仿真的 $(f(t), v(t))$ 数据，计算 $E_{obs}(k)$。观察在不同操作（自由空间移动、碰墙、释放）下 $E_{obs}$ 的变化规律
2. **亲手体验"符号错误"的灾难性后果**：故意把 PO 的符号搞反（$E_{obs} += fv\Delta T$ 而非 $E_{obs} -= fv\Delta T$），观察 PC 在不该介入时疯狂介入的现象。这是 TDPA 实现中最常见的 bug——亲身体验比记忆更深刻
3. **精读 Hannaford-Ryu 2002 原文**：该文清晰简洁，实验直观（硬墙碰撞），是 TDPA 入门的最佳材料
4. **用 D7.8 的调优流程实际走一遍**：即使在仿真中，7 步调优流程也能帮助你建立对各参数的直觉理解

**给有经验者的建议**：

1. **关注 Chattering-Free TDPA**（Panzirsch 2022）：标准 TDPA 的力抖振是工业应用的主要痛点。平滑因子 $\kappa$ 的选择与延迟和采样率的关系尚未有系统性研究——这是发表论文的机会
2. **关注 Task-Aware Energy Margin**：将 $E_{margin}$ 与任务阶段绑定（搜索/精细操作/回撤）是 2020 年后的趋势。用 RL 学习最优 $E_{margin}(t)$ 策略是一个有前景的研究方向
3. **关注多模态 TDPA**：当系统同时有力反馈和触觉反馈（振动、温度）时，各模态需要独立的 PO/PC 还是共享一个？目前没有标准答案
4. **工业部署的"5 分钟快检"**：本章 D7.8 末尾的快检流程已在多个实验室验证——建议打印出来贴在实验台旁边

## 版本信息速查

| 工具/库 | 版本 | 用途 | 备注 |
|---------|------|------|------|
| ROS2 | Humble/Jazzy | TDPA 节点框架 | ros2_control ControllerInterface |
| ros2_control | 4.0+ | 实时控制器管理 | 需要 PREEMPT_RT 内核 |
| PlotJuggler | 3.8+ | 实时诊断可视化 | 监控 $E_{obs}$, $\alpha$, $\beta$ |
| Python | 3.10+ | 仿真和离线分析 | NumPy + Matplotlib |
| C++ | C++17 | 实时控制器实现 | 避免动态内存分配 |
| Eigen | 3.4+ | 矩阵运算 | 用于 6-DOF wrench/twist 点乘 |
| NVIDIA Isaac Lab | 2.3+ | 仿真验证 | GPU 并行 TDPA 参数扫描 |
| MuJoCo | 3.0+ | 仿真验证 | 高保真接触力仿真 |
| tc (Linux) | 内核自带 | 网络延迟注入 | `tc qdisc add netem delay 50ms 10ms` |
| iperf3 | 3.12+ | 网络带宽/延迟测量 | 部署前链路评估 |
| Wireshark | 4.0+ | 包级延迟/丢包分析 | 调试网络异常 |

**TDPA 工程部署的最小可行硬件**：

| 组件 | 最低要求 | 推荐 | 备注 |
|------|---------|------|------|
| 力传感器 | 精度 <1 N, 带宽 >100 Hz | ATI Mini45 或等效 | 精度决定 PO 信噪比 |
| 控制器频率 | >= 200 Hz | >= 1 kHz | 频率决定 PO 分辨率 |
| 通信协议 | UDP + 时间戳 | UDP + 序列号 + 时间戳 | TCP 的重传会引入额外抖动 |
| 实时内核 | PREEMPT_RT | PREEMPT_RT + isolcpus | 减少采样抖动 |
