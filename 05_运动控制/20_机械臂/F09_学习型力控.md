...
1. **直接跑 SERL**：你会看到 25 分钟学会 PCB 插入的惊人结果，但不理解底层阻抗控制器的角色——SERL 的学习策略输出的是什么、阻抗底层保证了什么
2. **直接用 Diffusion Policy 做力控**：你会发现策略生成的轨迹在接触时不稳定——因为没有底层柔顺控制器来处理接触瞬态
...
学习型力控中"安全"不是一个单一的开关，而是一个多层级的保障体系。以下从内到外列出五个安全层级：

| 层级 | 机制 | 作用频率 | 失效后果 |
|------|------|---------|---------|
| L1 动作空间约束 | $K_d \in [K_{min}, K_{max}]$，$\Delta x \in [-\delta, \delta]$ | 每步 (10-30 Hz) | 策略输出超范围被裁剪 |
| L2 底层阻抗限幅 | $e_x$ 误差裁剪，$\tau$ 力矩饱和 | 每步 (1 kHz) | 即使策略跳变，力矩受限 |
| L3 无源性/能量监控 | 能量罐、KB 条件、$\dot{K}_d$ 限幅 | 每步 (1 kHz) | 变阻抗跳变时触发能量约束 |
| L4 碰撞检测 | 动量观测器残差阈值 | 每步 (1 kHz) | 检测到意外碰撞 → 急停 |
| L5 物理急停 | 硬件急停按钮 | 人工触发 | 所有电机断电 |

> **不是 X 而是 Y**：学习型力控的安全不是因为"用了阻抗控制器就天然安全"，而是因为五层独立的安全机制叠加。任何单层失效（如 RL 策略输出了极端 $K_d$），下层机制（力矩饱和、能量监控）仍能阻止损坏。这种"纵深防御"与航空安全的冗余设计哲学一致。

### 从经典到学习的渐进路线 ⭐

对于初学者，推荐以下渐进路线进入学习型力控：

```
Phase 1 (经典基础, 2 周):
  掌握 F01-F06 的阻抗/导纳控制
  在 MuJoCo 中实现固定参数的笛卡尔阻抗控制
  完成至少一个接触任务 (如恒力打磨)

Phase 2 (残差 RL, 1 周):
  在固定阻抗基础上加残差策略 (F9.2 变体 B)
  残差限幅 = 5% 额定力矩, 风险极低
  学习: 如何设计观测空间和奖励函数

Phase 3 (变阻抗 RL, 2 周):
  实现 VICES 完整框架 (F9.2)
  动作空间: 阻抗参数 [K_d, D_d, x_d]
  学习: 如何设计安全层 (L1-L3)

Phase 4 (真机部署, 2 周):
  选择 SERL (真机 RL) 或 Diffusion Policy (模仿学习)
  学习: sim-to-real 策略和数据采集
```
...
5. ⭐⭐ **安全层分析**：对 VICES 框架，分析如果只保留 L1（动作空间约束）而去掉 L2-L4，系统在什么场景下会产生危险行为？
...
**为什么需要力估计而不直接使用力传感器**：人形机器人的手部通常没有六维力传感器（成本、重量、空间限制），因此 FALCON 通过关节力矩传感器间接估计末端力。力估计网络本质上是一个学习版的广义动量观测器（参考 F06），但用神经网络替代了解析模型——因为人形机器人的动力学模型精度远低于 Franka 这样的工业臂。

**力估计与动量观测器的对比**：

| 特性 | 解析动量观测器（F06） | 神经网络力估计 |
|------|---------------------|-------------|
| 模型依赖 | 需要精确的 $M(q), C(q,\dot{q}), g(q)$ | 不需要（数据驱动） |
| 训练数据 | 不需要 | 需要仿真中带标签的力数据 |
| 精度（好模型时） | 高（$< 0.5$ N，Franka 级别） | 中（1-2 N） |
| 精度（差模型时） | 低（建模误差传导） | 中-高（从数据中学补偿） |
| 适用平台 | 高精度工业臂 | 人形机器人、低成本机器人 |
| 泛化能力 | 自动适应新构型 | 需要训练数据覆盖的构型范围 |

> **类比**：解析动量观测器就像用精确的物理公式预测天气——模型越好预测越准，但模型误差会直接传导。神经网络力估计就像用历史数据训练的天气预报 AI——不需要完美的物理模型，但需要足够多样的训练数据覆盖各种天气模式。

### FALCON 的奖励函数设计 ⭐⭐⭐

FALCON 的奖励函数设计体现了分层控制的哲学——locomotion 和 manipulation 各有独立的奖励，但通过力反馈耦合：

**Agent L（Locomotion）奖励**：

$$r_L = w_{vel} r_{vel} + w_{alive} r_{alive} + w_{smooth} r_{smooth} + w_{energy} r_{energy}$$

| 分项 | 公式 | 物理含义 |
|------|------|---------|
| $r_{vel}$ | $\exp(-\|v_{cmd} - v_{actual}\|^2 / \sigma_v^2)$ | 速度跟踪精度 |
| $r_{alive}$ | $+1$ per step | 存活奖励（不摔倒） |
| $r_{smooth}$ | $-\|\Delta a_t\|^2$ | 动作平滑性 |
| $r_{energy}$ | $-\|\tau\|^2$ | 能耗最小化 |

**Agent M（Manipulation）奖励**：

$$r_M = w_{track} r_{track} + w_{force} r_{force} + w_{posture} r_{posture}$$

| 分项 | 公式 | 物理含义 |
|------|------|---------|
| $r_{track}$ | $\exp(-\|x_{ee,d} - x_{ee}\|^2 / \sigma_x^2)$ | 末端位姿跟踪 |
| $r_{force}$ | $-\max(0, \|f_{ext}\| - f_{safe})$ | 力安全约束 |
| $r_{posture}$ | $-\|q_{arm} - q_{arm,default}\|^2$ | 臂默认姿态正则化 |

**力课程在奖励中的体现**：在每个课程阶段，$r_{force}$ 的安全阈值 $f_{safe}$ 随阶段递增（20 N → 50 N → 100 N），使策略逐步适应更大的外力而不触发力安全惩罚。
...
| 误解 | 正确理解 |
|------|---------|
| "学习型力控可以替代经典力控" | 当前所有成功的学习型力控方案都依赖经典力控作为底层——学习策略输出的是阻抗参数而非力矩 |
| "SERL 的 25 分钟训练不需要先验知识" | SERL 需要人类演示作为初始化、预定义的奖励函数、以及预配置的阻抗控制底层 |
| "Diffusion Policy 直接输出力矩" | Diffusion Policy 输出的是目标位姿轨迹（10 Hz），由底层阻抗控制器（1 kHz）柔顺跟踪 |
| "sim-to-real 在力控中和视觉任务一样" | 力控的 sim-to-real 更难——接触动力学的微小差异（摩擦/刚度/几何）会导致完全不同的力行为 |
| "端到端力矩输出是最终目标" | 端到端力矩输出在仿真中可行，但真机上缺乏安全保证——阻抗底层是工程安全的关键 |
...
| 神经网络阻抗进展 | 在线自适应/FACET/滑模增强/GMM | ⭐⭐⭐ | F06 |

### 符号表

| 符号 | 含义 | 首次出现 |
|------|------|---------|
| $K_d$ | 笛卡尔刚度矩阵 (N/m 或 Nm/rad) | F9.1 |
| $D_d$ | 笛卡尔阻尼矩阵 (Ns/m 或 Nms/rad) | F9.1 |
| $x_d$ | 期望末端位姿 | F9.1 |
| $e_x = x_d - x$ | 笛卡尔位姿误差（期望减当前） | F9.2 |
| $\tau$ | 关节力矩向量 | F9.1 |
| $J$ | 几何雅可比矩阵 | F9.1 |
| $g(q)$ | 重力补偿力矩 | F9.1 |
| $f_{pos}(s)$ | DMP 位置强迫项 | F9.1 |
| $f_{force}(s)$ | DMP 力强迫项 | F9.1 |
| $s$ | DMP 相位变量 | F9.1 |
| $K_{coupling}$ | DMP-force 力-位置耦合增益 (m/N) | F9.1 |
| $\Delta\tau$ | 残差力矩 | F9.2 |
| $\pi_\theta(o)$ | 参数化策略网络 | F9.2 |
| $f_{ext}$ | 外力估计 | F9.5 |
| $K_e$ | 环境等效刚度 (N/m) | F9.6 |
| $\mu$ | 摩擦系数 | F9.6 |
| $\eta_K$ | 在线阻抗学习率 | 神经网络阻抗进展 |

### 定理速查表

| 公式/方法 | 一句话说明 | 对应节 |
|----------|----------|--------|
| DMP-force | 用两个耦合 DMP 分别编码位置轨迹和期望力曲线，通过力-位置耦合机制实现力跟踪 | F9.1 |
| ProMP + 阻抗联合分布 | 编码位置-刚度-阻尼的联合高斯分布，条件化到新目标时阻抗参数自动调整 | F9.1 |
| VICES 阻抗动作空间 | RL 策略输出 $(K_d, D_d, x_d)$ 而非力矩，底层阻抗控制器提供安全执行 | F9.2 |
| SAC + 变阻抗 | 最大熵 RL 在连续阻抗空间中自适应探索，自动调节探索-利用平衡 | F9.2 |
| 残差策略 $\tau = \tau_{base} + \text{clip}(\Delta\tau)$ | 在手工基础控制器上叠加有界残差修正，基础越好残差越小训练越快 | F9.2 |
| SERL RLPD | 对称采样 + LayerNorm + UTD=20 + demo warm-start，25 分钟真机学会 PCB 插入 | F9.3 |
| Diffusion Policy + 底层阻抗 | 扩散模型生成末端位姿序列，底层 1 kHz 阻抗提供安全力矩执行 | F9.4 |
| FALCON 力课程 | 从 0 N 渐增到 $\sim$100 N 的渐进式训练策略，避免初始策略因大外力频繁摔倒 | F9.5 |
...
|---------|-------------|---------------------|
| F10 综合实战 | F09 的学习型方法可集成到 F10 的综合系统中，替代手工 FSM 参数 | F9.2 VICES, F9.3 SERL |
| D04 双臂学习 | 双臂行为克隆/RL 使用与 F09 相同的底层阻抗架构 | F9.4 Diffusion + 底层阻抗 |
| 足式 RL | FALCON 的力课程思想直接应用于四足 loco-manipulation | F9.5 FALCON |

### 术语速查表

| 术语 | 英文 | 一句话定义 |
|------|------|----------|
| VICES | Variable Impedance Control in End-effector Space | RL 输出阻抗参数 $K_d, D_d$ 的方法 |
| SERL | Sample Efficient Reinforcement Learning | Berkeley 的真机高效 RL 框架 |
| Diffusion Policy | Diffusion Policy | 用扩散模型生成动作序列的策略 |
| CRISP | Compliant Robot Interaction Stack | TUM 的高频 C++ 底层 + 低频 Python 策略架构 |
| FALCON | Force-Aware Learning with Curriculum ON | 双智能体 RL 力课程方法 |
| 残差力矩 | Residual Torque | 学习策略输出的力矩修正量，叠加在基层控制器上 |
| HIL-SERL | Human-in-the-Loop SERL | 带人类反馈的 SERL 变体 |
...
对于想在自己的机器人平台上实施学习型力控的工程师和研究者，推荐以下分阶段路线图：

**Phase 1: 底层力控基础（2-4 周）**

| 任务 | 目标 | 验收标准 |
|------|------|---------|
| 实现阻抗控制 | 末端在接触中稳定 | 恒力精度 $\pm 3$ N |
| 传感器标定 | 力测量准确 | 零偏 $< 0.5$ N |
| 安全限幅 | 力/速度/力矩限幅 | 碰撞时 $< 100$ N |

**Phase 2: 策略训练环境（2-4 周）**

| 任务 | 目标 | 验收标准 |
|------|------|---------|
| MuJoCo 仿真环境 | 复现目标任务 | 接触行为定性合理 |
| 奖励函数设计 | 力+位混合奖励 | 策略能在仿真中收敛 |
| 动作空间选择 | $(x_d, K_d, D_d)$ 而非 $\tau$ | 策略输出在物理合理范围内 |

**Phase 3: Sim-to-Real（2-4 周）**

| 任务 | 目标 | 验收标准 |
|------|------|---------|
| 域随机化 | 接触参数随机化 | 策略对 $K_e \pm 50\%$ 鲁棒 |
| 真机微调 | SERL 或 RLPD | 真机 $< 50$ 分钟收敛 |
| 性能评估 | 对比经典力控 | 成功率 $\geq$ 经典方法 |

> **本质洞察**：学习型力控的成功 90% 取决于 Phase 1 的底层质量。一个糟糕的阻抗控制底层无法被学习策略"修复"——策略只能在底层提供的安全范围内优化行为。这就是 CRISP 架构的核心设计原则："安全由底层保证，智能由上层提供"。

### 学习型力控的开源工具链

| 工具 | 用途 | 推荐版本 | 备注 |
|------|------|---------|------|
| MuJoCo | 仿真环境 | $\geq$ 3.x | 免费开源，接触仿真质量最佳 |
| Gymnasium | RL 环境接口 | $\geq$ 1.0 | 标准 RL 环境 API |
| SAC (Stable-Baselines3) | RL 算法 | $\geq$ 2.0 | 连续动作空间 RL 的默认选择 |
| Diffusion Policy (diffusion_policy) | IL 算法 | main | 扩散模型动作生成 |
| robomimic | IL 数据管理 | $\geq$ 0.3 | 演示数据管理和训练 |
| SERL (serl_robot_infra) | 真机 RL | main | Berkeley 真机高效 RL |
| libfranka / franka_ros2 | 真机底层 | $\geq$ 0.13 | Franka 力矩接口 |

### 练习

1. ⭐⭐ **动作空间对比**：在 MuJoCo 的 peg-in-hole 任务中，分别用三种动作空间训练 SAC：(a) 关节力矩 $\tau$，(b) 目标位姿 $x_d$，(c) 目标位姿+阻抗参数 $(x_d, K_d)$。对比三种方案的训练效率和成功率。
2. ⭐⭐⭐ **CRISP 架构实现**：在 MuJoCo + Python 中实现 CRISP 的双频架构——C++ 底层 1 kHz 阻抗控制 + Python 上层 10 Hz 策略（可以用简单的 PD 策略代替 RL/IL）。验证底层在策略"崩溃"时仍能保证安全。
...
| 后续章节 | 关系 | 本章铺垫的关键知识 |
|---------|------|------------------|
| F10 综合项目 | 系统集成 | 将 F01-F09 所有知识整合到完整的力控系统 |
...
| 症状 | 可能原因 | 诊断方法 | 解决方案 |
|------|---------|---------|---------|
| Reward 完全不涨 | 奖励函数设计错误 | 打印各奖励分项 | 检查力/位奖励的方向和缩放 |
| Reward 快速上升后崩溃 | 策略找到了 reward hacking | 观察策略行为 | 加入约束或修改奖励 |
| 力精度差 | 力奖励权重太小 | 对比力/位奖励量级 | 增大力奖励权重至可比量级 |
| 接触不稳定 | 底层阻抗参数不当 | 固定策略测试底层 | 先调好底层阻抗再训练策略 |
| Sim-to-real 失败 | 接触参数域随机化不足 | 对比仿真和真机力曲线 | 增大域随机化范围 |

**奖励函数设计的"力控专用"原则**：

1. **力奖励必须在接触时才计算**——自由空间中的力误差无意义
2. **力奖励用指数衰减而非二次惩罚**——$r_f = \exp(-\alpha \|f - f_d\|)$ 比 $r_f = -\|f - f_d\|^2$ 更稳定
3. **加入接触奖励**——鼓励策略维持接触而非反复脱离
4. **力变化率惩罚**——$r_{\dot{f}} = -\beta \|f_k - f_{k-1}\|$ 防止力跳变

> **反事实推理**：如果力奖励在自由空间和接触中都计算会怎样？自由空间中 $f = 0$，力误差 $= f_d$（目标力），策略为了减小这个误差会尽快接触——可能导致高速碰撞。正确做法：自由空间中不计算力奖励（或给零权重），只在检测到接触后才启用力跟踪奖励。
...
当前学习型力控领域缺乏统一的评估标准。以下是推荐的评估维度和基准任务：

**评估维度**：

| 维度 | 指标 | 为什么重要 |
|------|------|-----------|
| 训练效率 | 真机训练时间 / 仿真样本数 | 真机时间 = 设备磨损 + 人力成本 |
| 任务成功率 | 100 次试验的成功率 | 力控任务对失败零容忍 |
| 力精度 | 接触力的 RMSE (N) | 力控的核心指标 |
| 泛化能力 | 对新物体/新位姿的成功率 | 工业场景物体多样 |
| 安全性 | 最大接触力 / 碰撞次数 | 不能损坏工件或人 |
| 计算延迟 | 策略推理延迟 (ms) | 影响力控带宽 |

**推荐基准任务**：

| 任务 | 描述 | 难度 | 关键挑战 |
|------|------|:----:|---------|
| Peg-in-hole | 销轴插入孔（间隙 0.2 mm） | ⭐⭐ | 精密力控 + 搜索 |
| 恒力打磨 | 沿曲面保持恒定法向力 | ⭐⭐ | 曲面自适应 |
| 柔性物体操作 | 弯曲电缆/插入 USB | ⭐⭐⭐ | 非线性接触 |
| 人机协作 | 双人协作搬运 | ⭐⭐⭐ | 人类意图理解 |
...
| robomimic 文档 | 文档 | ⭐⭐ | 模仿学习数据管线 |
| DROID 数据集 | 数据 | ⭐⭐⭐ | 76K+ 遥操作轨迹（含力数据） |
| Impedance Learning-Based Adaptive Force Tracking, IEEE TRO 2025 | 论文 | ⭐⭐⭐ | 在线自适应阻抗学习 |
| FACET: Force-Adaptive Control via Impedance Reference Tracking, 2025 | 论文 | ⭐⭐⭐ | 足式机器人阻抗参数学习 |
...
1. **先确保经典方法可用**：在同一环境中用固定阻抗参数完成任务。如果固定参数就能解决（如恒力打磨已知材质），不需要学习型方法。
2. **从残差 RL 起步**：残差幅度限制在 5-10% 额定力矩，安全风险极低，但能体验完整的 RL 训练流程（观测设计、奖励工程、超参调优）。
3. **重视数据质量**：无论是 RL 的 replay buffer 还是 IL 的 demo 数据集，力传感器标定和偏置管理都直接影响策略质量。

**给有经验者的建议**（已有 RL/IL 经验，向力控扩展）：

1. **理解底层控制器**：学习型力控的成功 80% 取决于底层阻抗控制器的设计——动作空间范围、安全层配置、控制频率匹配。
2. **关注 sim-to-real 的力控特殊性**：接触模型误差是力控 sim-to-real 的主要瓶颈，DR 的参数范围和分布设计需要力控领域知识。
3. **基准对比要公平**：将 RL 方法与手工调参的经典方法对比时，两者都应在相同的评测条件下（试验次数、指标定义、统计显著性）进行。
...
| API / 库 | 功能 | 本章相关 |
|---------|------|---------|
| `stable_baselines3.SAC` | 连续动作 RL | VICES/SERL 基础算法 |
| `diffusion_policy` (GitHub) | 扩散模型策略 | Diffusion Policy 实现 |
| `robomimic` (GitHub) | IL 数据管理和训练 | 演示数据管理 |
| `serl_robot_infra` (GitHub) | SERL 真机 RL 基础设施 | 真机高效训练 |
| `gymnasium.make()` | RL 环境创建 | 仿真环境接口 |
| `mujoco.MjModel` | MuJoCo 模型加载 | 力控仿真环境 |
| `torch.jit.script()` | PyTorch 模型编译 | 策略实时推理加速 |
| `franka::Torques` | libfranka 力矩接口 | 底层阻抗控制输出 |
...
| 库/平台 | 推荐版本 | 备注 |
|---------|---------|------|
| PyTorch | $\geq$ 2.0 | GPU 策略推理 |
| Stable-Baselines3 | $\geq$ 2.0 | SAC/TD3 实现 |
| MuJoCo | $\geq$ 3.x | 力控仿真环境 |
| Gymnasium | $\geq$ 1.0 | RL 环境接口 |
| diffusion_policy | main | 扩散模型策略 |
| SERL | main | 真机高效 RL |
| libfranka | $\geq$ 0.13.x | Franka 底层 |
...
在学习型力控系统中，安全约束必须在多个层级实施：

| 层级 | 约束内容 | 实施方式 | 失效后果 |
|:----:|---------|---------|---------|
| 策略输出限幅 | $x_d \in [x_{min}, x_{max}]$, $K_d \in [K_{min}, K_{max}]$ | `np.clip()` | 策略输出超出物理合理范围 |
| 底层力矩限幅 | $\tau \in [\tau_{min}, \tau_{max}]$ | 硬件级限幅 | 关节力矩过载 |
| 速度限制 | $\|\dot{x}\| < v_{max}$ | 底层速度限幅 | 碰撞能量过大 |
| 力限制 | $\|f\| < f_{max}$ | 力传感器监控+停止 | 工件/环境损坏 |
| 工作空间限制 | $x \in \mathcal{W}_{safe}$ | 底层位置限幅 | 碰撞固定结构 |
| 能量罐 | $E_{tank} > E_{min}$ | 变阻抗能量监控 | 无源性破坏→不稳定 |
...
| 症状 | 可能原因 | 排查步骤 | 相关章节 |
|------|---------|---------|---------|
| RL 训练 reward 不涨 | 奖励函数设计/缩放问题 | 1. 打印各奖励分项 2. 检查力/位奖励量级平衡 3. 简化任务测试 | F9.1, F9.2 |
| 策略在仿真中好但真机差 | Sim-to-real gap | 1. 增大域随机化范围 2. 对比仿真/真机力曲线 3. 在真机上做 RLPD | F9.6 |
| 底层阻抗控制不稳定 | 策略输出的 $K_d$ 超出安全范围 | 1. 在底层加 $K_d$ 限幅 2. 缩小策略动作空间 3. 检查能量罐状态 | F9.2, F06 |
| Diffusion Policy 生成的轨迹不平滑 | 采样步数不够/噪声调度不当 | 1. 增加 DDPM 采样步数 2. 调整噪声调度 3. 检查训练数据质量 | F9.4 |
| SERL 真机训练碰撞频繁 | 安全约束不完善 | 1. 降低探索噪声 2. 增加安全限幅 3. 用人类演示初始化策略 | F9.3 |
| 策略推理延迟 > 50 ms | 模型太大/GPU 利用率低 | 1. 简化网络结构 2. 使用 TorchScript 编译 3. 检查 GPU 利用率 | F9.7 |
