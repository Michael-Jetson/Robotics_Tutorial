...
力控系统的调试特别困难，因为问题可能出在多个层的交互上——控制器参数没错，但力传感器有偏置；力传感器没偏置，但 FSM 的切换条件不合理；FSM 逻辑正确，但安全层误触发。以下"分层隔离"方法论系统化了调试过程。

**核心原则**：一次只验证一层，固定其他层为已知正确的状态。

```
Layer 0: 硬件验证（最底层）
  测试: 关节读写是否正常？力传感器方向是否正确？
  方法: 发送已知力矩，检查关节运动方向和速度
  判据: 各关节响应符合预期，力传感器轴向和符号与约定一致
  隔离: 不启动上层控制器，直接用 mj_step 或 ros2_control 测试节点

Layer 1: 控制器验证
  测试: 阻抗控制律是否产生正确的力矩？
  方法: 固定 FSM 为 IDLE 状态，手动设置 x_d，观察末端是否移到目标
  判据: 无接触时位置跟踪误差 < 1mm，阶跃响应无超过 10% 过冲
  隔离: 不启动 FSM，不启动安全层（或设极高阈值）

Layer 2: 单任务验证
  测试: 单个力控任务（如打磨）是否工作？
  方法: 固定 FSM 为 TASK 状态，手动确保稳定接触后启动任务
  判据: 力稳态误差 < 1N，位置跟踪 RMSE < 2mm
  隔离: 跳过 APPROACH / CONTACT 阶段，直接从接触状态开始

Layer 3: FSM 过渡验证
  测试: 状态切换是否平滑？条件是否合理？
  方法: 从 IDLE 开始完整运行，记录每次状态切换的时刻和力/位置曲线
  判据: 切换时力冲击 < 目标值，无状态抖振（chattering）
  隔离: 安全层设极高阈值（防止干扰调试）

Layer 4: 安全层验证
  测试: 碰撞检测和急停是否按预期工作？
  方法: 在 Layer 3 验证通过的系统上施加已知外力，验证检测和响应
  判据: 外力超阈值 -> 1ms 内检测 -> 100ms 内停止
  隔离: 完整系统

Layer 5: 鲁棒性验证
  测试: 系统在参数变化下是否仍然稳定？
  方法: 变化环境刚度 +-50%，添加 2N 随机扰动，增加 1mm 位置偏差
  判据: 所有任务仍能完成，成功率无显著下降
```

> **本质洞察**：分层隔离的关键价值不是"发现问题"——用全系统运行也能发现问题。它的价值是**定位问题到具体层**——当你知道 Layer 1 通过但 Layer 2 失败时，问题一定出在任务级逻辑或力传感器的接触行为上，而不是阻抗控制律本身。这种定位将调试时间从"小时级"降到"分钟级"。

### 变阻抗导纳混合控制——力控精度的进一步提升 ⭐⭐⭐

F10.4 的打磨任务中，z 轴使用固定增益的导纳控制。当环境刚度变化时（如从铝板移到木板），固定增益的导纳控制器会出现两个问题：（1）力稳态误差增大；（2）振荡/过冲恶化。2024-2025 年的最新研究表明，变阻抗导纳（Variable Admittance）结合深度强化学习可以自动适应不同刚度的工件表面。

**变阻抗导纳控制的核心思想**：导纳增益 $A(t)$ 不再固定，而是由 RL 策略或在线自适应算法动态调整：

$$\Delta z(t) = -A(t) \cdot (f_{target} - f_{contact}) \cdot \Delta t$$

$$A(t) = A_{base} + \Delta A_{RL}(o_t)$$

其中 $o_t = [f_{contact}, \dot{f}_{contact}, e_z, \dot{e}_z, K_e^{est}]$ 是观测向量，$K_e^{est}$ 是在线估计的环境刚度。

**TD3 优化变阻抗参数**（Variable Admittance + TD3, Applied Sciences 2025）的训练设置：

| 配置项 | 值 | 说明 |
|--------|------|------|
| 状态空间 | 10D | $[f_z, \dot{f}_z, e_z, \dot{e}_z, A_{prev}, K_e^{est}, \text{phase}, \text{v}_{xy}, f_{xy,norm}, t/T]$ |
| 动作空间 | 1D | $\Delta A \in [-0.0005, 0.0005]$ m/s/N |
| 奖励 | $r = -w_f\|f_e\|^2 - w_A\|\Delta A\|^2$ | 力误差 + 增益平滑 |
| 环境随机化 | $K_e \in [10^3, 10^6]$ N/m | 覆盖海绵到钢铁 |
| 训练步数 | 200K | 在 MuJoCo 中训练 |

> **反事实推理**：如果不做变阻抗而使用固定导纳增益 $A = 0.001$ m/s/N 会怎样？
> - 对 $K_e = 10^3$ N/m（海绵）：$A$ 偏小，力调整太慢，稳态误差 $\sim$3 N
> - 对 $K_e = 10^6$ N/m（钢铁）：$A$ 偏大，系统在接触边界振荡
> - 变阻抗导纳让 $A$ 在海绵上自动增大（加快响应）、在钢铁上自动减小（避免振荡），力稳态误差在两种材质上都保持 $<1$ N
...
1. 默认使用 MoveIt2 位置轨迹跟踪（关节空间 PD 控制）
2. 当动量观测器检测到外力 $\|\hat{f}_{ext}\| > f_{threshold}$（如 3N），切换到阻抗控制模式
3. 切换时，阻抗控制器的平衡位置 $x_d$ 初始化为当前末端位姿
4. 当任务完成（如 peg 插入到位）或外力消失，切换回位置控制
...
| API / 函数 | 库 | 签名 | 用途 |
|-----------|-----|------|------|
| `computeGeneralizedGravity` | Pinocchio | `pin::computeGeneralizedGravity(model, data, q)` | 计算重力补偿力矩 $g(q)$ |
| `computeJointJacobians` | Pinocchio | `pin::computeJointJacobians(model, data, q)` | 计算所有关节的雅可比矩阵 |
| `getFrameJacobian` | Pinocchio | `pin::getFrameJacobian(model, data, frame_id, LOCAL_WORLD_ALIGNED)` | 获取指定 frame 的 6D 雅可比 |
| `forwardKinematics` | Pinocchio | `pin::forwardKinematics(model, data, q, v)` | 计算正运动学（位置+速度） |
| `computeCoriolisMatrix` | Pinocchio | `pin::computeCoriolisMatrix(model, data, q, dq)` | 计算科氏力矩阵 $C(q, \dot{q})$ |
| `mj_step` | MuJoCo | `mujoco.mj_step(model, data)` | 执行一步物理仿真 |
| `mj_forward` | MuJoCo | `mujoco.mj_forward(model, data)` | 前向动力学（不积分，只计算力） |
| `mj_contactForce` | MuJoCo | `mujoco.mj_contactForce(model, data, contact_id, force)` | 获取指定接触点的力 |
| `mj_id2name` | MuJoCo | `mujoco.mj_id2name(model, obj_type, obj_id)` | 将 MuJoCo 内部 ID 转为名称字符串 |
| `set_value` | ros2_control | `get_command_interface(name, type).set_value(val)` | 向硬件接口写入命令值 |
| `get_value` | ros2_control | `get_state_interface(name, type).get_value()` | 从硬件接口读取状态值 |
