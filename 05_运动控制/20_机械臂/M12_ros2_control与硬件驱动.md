...
3. ROS2 Lifecycle Node 的四个主要状态是什么？状态转换回调函数的命名规则是什么？（`02_C++基础与进阶/50_ROS2工程化/40_硬件集成与RL部署`）
4. 什么是 RAII？在实时控制循环中为什么禁止使用 `new/delete`？（`02_C++基础与进阶/10_C++语言核心/40_RAII与智能指针`、M11）
