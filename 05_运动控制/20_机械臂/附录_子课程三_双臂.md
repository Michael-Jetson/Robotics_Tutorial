...
| C F02 操作空间 | D01, D03 | $\Lambda$, J̄ 的双臂扩展;Augmented Object Model |
| C F02.4 无源性 | D05, D06, D07 | 端口无源 → Llewellyn 准则 → 散射/波变量 → TDPA |
| C F04 libfranka 阻抗 | D08, D09 | slave 侧阻抗底层;双臂 impedance_controller |
| C F05 导纳控制 | D09 | 双臂 admittance_controller $\times$ 2 配置 |
| C F09 学习型力控 | D04 | VICES→双臂阻抗 action space;SERL→双臂 RL |
| 续篇 复合/140_VLA移动操作 VLA | D04 | ACT/Diffusion/$\pi$0 的概念已知;本大纲补底层控制分析 |
...
| 组件 | 推荐 | 价格 |
|------|------|------|
| 双臂 | 2$\times$ Koch v1.1 / SO-100(LeRobot 支持) | ~$1k-2k |
| Leader | 2$\times$ GELLO(3D 打印 + Dynamixel) | ~$600 |
| 相机 | 3$\times$ RealSense D435(top + wrist $\times$2) | ~$900 |
...
| 组件 | 推荐 | 价格 |
|------|------|------|
| 双臂 | 2$\times$ Franka Research 3 | ~$50-70k |
| F/T 传感器 | 内置(Franka)或 2$\times$ ATI Mini40 | ~$5k 外置 |
| 遥操作 | ALOHA 2 leader $\times$ 2 或 GELLO $\times$ 2 | ~$1-5k |
| 力反馈(可选) | 2$\times$ Haply Inverse3 或 1$\times$ Omega.7 | ~$5-25k |
...
**错误 9:TDPA 的 PC 在低速区 $\alpha$ → $\infty$**
```
症状:slave 突然"卡住"(极大阻尼)
