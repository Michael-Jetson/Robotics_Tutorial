...
| 核心数学 | Grasp matrix G、Relative Jacobian、散射/波变量、TDPA | 阻抗方程、$\Lambda$/J̄/N、无源性 | 空间向量代数、RNEA/ABA、QP/NLP | 微分平坦、Min-Snap/MINCO |
...
| 控制范式 | 协调阻抗 + 波变量通信 + TDPA | $\tau$ = J^T F / QP-WBC | JointTrajectoryController | SE(3) 几何控制 / NMPC |
...
**跨越二：从"单端口力控"到"双端口网络"。** 单臂阻抗控制处理"机器人-环境"一个交互端口;遥操作处理"操作者-通信-环境"**两个端口加一条有损通道**。无源性从 ḣ $\le$ y^T u(单端口)推广到 h-参数矩阵的 Llewellyn 条件(双端口)。散射变量把力/速度变换为入射波/反射波,使时延通道"免费变成"无损传输线——这个变换的物理直觉(传输线类比)比公式本身更重要。
...
|-------------------|--------------|---------|
| Pinocchio Jacobian(单臂 6$\times$7) | Augmented Jacobian J_aug(12$\times$14)——两臂拼接 | 零迁移 |
...
| 无源性证明(ḣ $\le$ y^T u) | Llewellyn 稳定准则 + 散射/波变量无源性 | 中——概念一致但数学扩展 |
