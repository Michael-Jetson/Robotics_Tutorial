...
本章的知识结构可以画成一棵"从需求到实现"的树：根是"机械臂控制为什么需要动力学引擎"，树干是 Pinocchio 的设计哲学与架构，树枝是各动力学算法的原理与 API。下面这张地图标注了每个小节在树上的位置——读之前先扫一遍，建立全局方位感。

```
                   机械臂实时控制 → 需要高性能可微分动力学引擎
                                    │
        ┌───────────────────────────┼───────────────────────────┐
        ▼                           ▼                           ▼
   【为什么需要】              【引擎怎么设计】              【算法怎么用】
   §1 动力学引擎动机          §2 设计哲学                  §5 URDF 加载
   ─ MPC 的 1ms 约束          ─ Model/Data 分离            §6 正向运动学 FK
   ─ 手写动力学的困境          ─ 算法即自由函数              §7 雅可比矩阵
   ─ 四大库对比                §3 CRTP 访问者模式           §8 逆动力学 RNEA
   ─ INRIA 学派定位            ─ 静态多态消除虚函数          §9 正动力学 ABA
        │                      §4 标量参数化                §10 惯量矩阵 CRBA
        │  (动机基础)           ─ double/CppAD/CasADi       §11 碰撞检测
        └──────────────►       ─ 自动微分 + CodeGen         §12 解析导数
                                                            §13 约束动力学
```
...
| 主线 | 含义 | 体现在哪些小节 |
|------|------|--------------|
| **性能（O(N) + 零开销抽象）** | 动力学引擎必须在 1ms 内完成数十次调用——这驱动了 CRTP、Eigen 对齐、递归算法等所有设计 | §1（需求）、§3（CRTP）、§4（标量参数化）、§8-§10（递归算法） |
| **可微分（解析梯度）** | MPC / 轨迹优化需要 $\partial \tau/\partial q$ 等梯度——这驱动了标量参数化和解析导数 | §4（CppAD/CasADi）、§12（解析导数）、§13（约束动力学） |
...
- **快速上手路径**（只需会调 API）：§1 → §2 → §5 → §6 → §7 → §8（跳过 §3/§4 的源码分析）
- **深入理解路径**（理解设计决策）：§1 → §2 → §3 → §4 → §5 → 顺序通读
- **MPC/优化开发者路径**（需要梯度）：§1 → §2 → §4 → §8 → §9 → §12 → §13

### 预计阅读时间

| 档位 | 时长 | 路径 |
|------|------|------|
| **精读**（推荐） | 15-20 学时（约 1.5 周） | 全章顺序读 + 完成每节编程练习 + 用 Franka Panda 跑完整 pipeline |
| **速读** | 4-6 小时 | §1 动机 + §2 设计哲学 + §5 加载示例 + §7 雅可比 + §8 RNEA，跳过 CRTP 源码和约束动力学 |
| **速查** | 30 分钟 | 直接看 §1.4 四大库对比表、§7.2 三种坐标系表、§8-§10 的 API 调用示例、章末故障排查手册 |
...
$$\text{虚函数开销} \approx 5\text{ ns/call} \times 7 \text{ joints} \times 13{,}000 \text{ calls/s} = 0.455 \text{ ms/s}$$
...
| 编号 | 误解 | 正确理解 |
|:----:|------|---------|
| 1 | "Pinocchio 只能用于串联链机械臂" | Pinocchio 3.x 支持闭链、浮动基座（`JointModelFreeFlyer`）、树形拓扑和 mimic joint。腿足机器人、并联机构、双臂系统均可建模 |
| 2 | "Model/Data 分离只是代码风格偏好" | 分离反映了物理规律的数学性质——拓扑结构（Model）是常量，数值状态（Data）是变量。多线程并行（一个 Model + N 个 Data）成为零同步开销的自然推论 |
| 3 | "CRTP 总是优于虚函数" | 当关节数增大（$>16$ DOF）或函数体较重时，vtable 查找开销可忽略。Crocoddyl 在上层优化器中选择虚函数，正因为 CRTP 编译膨胀在该层大于性能收益 |
| 4 | "RNEA 和 ABA 只是速度不同，功能相同" | RNEA 求解**逆动力学**（输入 $q, \dot{q}, \ddot{q}$，输出 $\tau$），ABA 求解**正动力学**（输入 $q, \dot{q}, \tau$，输出 $\ddot{q}$）。二者解决完全不同的问题 |
| 5 | "Pinocchio 的碰撞检测是内置的" | Pinocchio 通过 Coal（原 hpp-fcl）集成提供碰撞查询。Pinocchio 计算几何体世界位姿，Coal 做实际碰撞/距离计算 |
| 6 | "解析导数和自动微分效果相同" | 解析导数精度达 $10^{-16}$ 且耗时低于正向计算。CppAD 虽同精度但需录制 tape，循环中重复录制慢 10-100 倍。CasADi CodeGen 可恢复原生性能但编译更复杂 |
...
| $\dot{q}$ | 广义速度向量，维度 $n_v$ | $\S$1 |
| $\ddot{q}$ | 广义加速度向量 | $\S$1 |
| $\tau$ | 关节力矩向量 | $\S$1 |
| $M(q)$ | 广义惯量矩阵（$n_v \times n_v$ 对称正定） | $\S$1.1 |
| $C(q,\dot{q})$ | 科里奥利/向心力矩阵 | $\S$1.1 |
| $g(q)$ | 重力补偿力矩向量 | $\S$1.1 |
| $h(q,\dot{q})$ | 非线性力项：$h = C\dot{q} + g$ | $\S$1.1 |
| $S_i$ | 第 $i$ 个关节的运动子空间矩阵 | $\S$3 |
| ${}^iX_{p(i)}$ | 从父连杆 $p(i)$ 到子连杆 $i$ 的空间变换 | $\S$3 |
| $J(q)$ | 几何雅可比矩阵（$6 \times n_v$） | $\S$7 |
| $f_i^B$ | RNEA 反向传递中第 $i$ 个连杆受到的合力 | $\S$8 |
| $T \in SE(3)$ | 齐次变换矩阵 | 前置自测 |
| $R \in SO(3)$ | 旋转矩阵 | 前置自测 |
