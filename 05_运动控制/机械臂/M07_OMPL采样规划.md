...
> **前置**: M03(IK求解器), M04(碰撞检测 FCL/Coal); v8 Ch6(继承多态), Ch29(设计模式)
...
开始本章之前，请独立回答以下 5 题。若答不出 3 题以上，建议先回顾 M03/M04 和相关 v8 章节。
...
这三层对应设计模式中的**策略模式(Strategy)**和**门面模式(Facade)**（v8 Ch29），让算法与碰撞检测完全解耦。
