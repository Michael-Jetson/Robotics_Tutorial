...
📋 **前置自测**（答不出 ≥ 2 题 → 先回 v8 Ch6/Ch29/Ch31 复习）

1. 什么是 Composite Pattern？它如何用统一接口处理叶子节点和容器节点？（v8 Ch29）
2. C++ 共享库如何通过 `dlopen`/符号导出在运行时加载？ROS `pluginlib` 在此基础上又增加了什么？（v8 Ch31）
3. ROS2 Action 的三个组成部分（Goal/Feedback/Result）分别表达什么语义？（v8 Ch31）
4. `std::any` 是什么？它如何在运行时实现类型安全的多态存储？（v8 Ch6）
5. 有限状态机（FSM）的状态数与转移数在复杂系统中如何增长？（v8 Ch29）
...
> **跨领域类比**：Decorator 模式在 BT 中的应用与 v8 Ch29 中学过的 Decorator 设计模式完全一致——不修改原始节点，通过包装增强行为。`RetryUntilSuccessful` 就像网络请求的重试中间件，`Timeout` 就像 gRPC 的 deadline。
