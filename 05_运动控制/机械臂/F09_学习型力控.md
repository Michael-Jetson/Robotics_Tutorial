...
5. **理解** FALCON 的力课程：从 0 kg 外力渐增到 10 kg 的渐进式训练策略
...
FALCON（Zhang et al., CMU LeCAR Lab, L4DC 2026）让人形机器人在行走同时承受外力。
...
```
阶段 1: f_ext = 0 kg  -> 基本运动协调
阶段 2: f_ext = 2 kg  -> 轻微力补偿
阶段 3: f_ext = 5 kg  -> 步态调整抵抗外力
阶段 4: f_ext = 10 kg -> 大幅调整体姿/步态/臂配置

每阶段训练到收敛后才进入下一阶段
```

> **反事实推理**：如果直接用 10 kg 训练？初始策略不知道应对大外力 -> 频繁摔倒 -> 有效探索时间极短 -> RL 学不到有用策略。力课程的本质：**把困难问题分解为渐进的简单问题**。
...
| 配置 | 最大外力 | 最大速度 |
|------|---------|---------|
| Unitree G1 | 10 kg | 0.5 m/s |
| Unitree H1 | 10 kg | 0.8 m/s |
| Booster T1 | 10 kg | 0.6 m/s |
