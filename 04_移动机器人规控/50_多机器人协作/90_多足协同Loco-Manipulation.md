...
> 📋 **答不出 ≥ 2 题** → 标 ① 的回第 4 章（分布式 MPC），标 ② 的回第 5/7 章（grasp matrix 与内力），标 ③ 的回 `05_运动控制/10_足式` 单体足式，标 ④ 的回第 1/2 章。本章会大量复用这些前置，但不重新教它们。
...
| 13 | 用单点位置误差做姿态对齐 | 单点只约束 3 个平移自由度；须用 constellation（≥3 非共线点）同时编码位置+姿态 | §8.6 |
...
| 3 constellation | 纯旋转 $5°$ 时 $d_{\text{con}}$ 明显增大、单点位置误差≈0；说明为何用 3 非共线点 | §8.6 机制1、练习 8.6.1 |
...
| `constellation_distance` | 点集 ≥3 非共线 | 用单点→不约束姿态（陷阱 8.6.2） |
