...
> 答不出 **≥ 2 题**，先回第 1 章（权重公式推导、receding horizon）与 CUDA / Eigen 基础，再读本章。
...
| §2.2 | CUDA 并行化策略（线程映射 / 内存层级） | ⭐⭐⭐ | 把"能用"变成"实时"（~50× 加速） | §2.1、CUDA 基础 |
...
- **场景二**：你想给一个四旋翼写 MPPI 穿越障碍林，代价里用了"碰到障碍就 +∞"的 indicator。你的同事建议用 iLQR，结果它在碰撞点梯度为零处直接失效。你不理解为什么——因为你没读 §2.4，不知道这正是 MPPI 不可替代的核心场景。
...
所以代价各项的权重和温度 $\lambda$ 不是独立的——它们一起决定权重的尖锐程度（ESS）。实践上：先把代价设计在一个合理的量级（各项可比、不爆炸），再用 ESS 调 $\lambda$（§2.1 λ/Σ）；别一边乱放大代价、一边乱调 $\lambda$，那是在和自己较劲。
...
读这张表的关键是"正交"二字：四列彼此独立——换 Dynamics 不影响 Cost 怎么写，换 Sampler 不影响主循环怎么跑。正因正交，组合数是乘法（4 个动力学 × 5 个代价 × 3 个采样器……），而你只需各写一份、主循环写一遍。
...
本章把第 1 章那段 20 行的理论核，沿"算法 → 实时 → 抽象 → 选型"四步，长成了一套可部署、可复用、可择优的工程系统。§2.1 补上了 $\rho$ 减法、warm-start、SGF 三个决定成败的工程细节，让玩具变算法；§2.2 用 CUDA 把 $K$ 条独立 rollout 并行到 GPU、讲清线程映射与三级内存层级，让算法变实时（~50× 加速）；§2.3 以 MPPI-Generic 为范本，用 C++ 模板把动力学/代价/采样器/反馈控制器四维正交解耦，让一次性脚本变可复用的库；§2.4 把 MPPI 放回最优控制版图，与 iLQR/DDP 逐维对比，讲清了 MPPI 那个不可替代的核心理由——能解梯度法解不了的非光滑/黑箱问题。
...
|--------|---------|----------------|--------|
| **ESS / K** | 落在约 $5\%\sim50\%$ | 接近 $1/K$ → 权重坍缩（$\lambda$ 太小或提议太差）；接近 $100\%$ → 近均匀（$\lambda$ 太大） | §2.1 λ/Σ、样本效率 |
...
| **权重数值** | 全部有限、和为 1 | 出现 NaN → $\rho$ 减法缺失或代价溢出（GPU fp32 下尤甚） | §2.1 ρ 减法、§2.2 fp32 |
...
| 温度 $\lambda$（§2.1 λ/Σ） | 权重多挑剔（利用强度） | 权重趋均匀、方向模糊 | 权重过尖、退化成贪婪、方差大 | 用 **ESS** 当探针，维持在 $K$ 的 $5\%\sim50\%$ |
| 协方差 $\Sigma$（§2.1 λ/Σ） | 采样撒多开（探索广度） | 探索强但浪费样本、更抖；过大还会被钳位吃掉 | 探索不足、遇大动作找不到 | 按控制维度分设，标尺≈执行器范围的一个比例 |
