...
| L2 | 任务注册 | `uv run list-envs` | `python scripts/list_envs.py` | 列出可用任务 |
| L3 | 场景构建 | `uv run export-scene <TASK>` | — | 导出 MJCF/USD 无错 |
| L4 | Zero agent 可视化 | `uv run play <TASK> --agent zero` | `python scripts/rsl_rl/play.py --task <TASK> --load_run null` | 机器人站在场景中不动 |
| L5 | Random agent | `uv run play <TASK> --agent random` | 需自行编写 random wrapper | 机器人有动作但不爆飞 |
| L6 | 最小训练 | `uv run train <TASK> --env.scene.num-envs 16` | `python scripts/rsl_rl/train.py --task <TASK> --num_envs 16` | reward 曲线有变化 |
...
3. **`pip install` 成功 ≠ 可以训练。** `pip install mjlab` 只安装了 Python 包，但 MuJoCo Warp 的 GPU kernel 编译发生在首次运行时——如果 CUDA 版本不匹配，安装不会报错但运行会失败。
...
```bash
# 拉取官方镜像
docker pull mujocolab/mjlab:latest

# 运行（挂载 GPU + 端口映射用于 Viser + 挂载工作目录）
docker run --gpus all \
    -p 7860:7860 \
    -v $(pwd)/logs:/workspace/logs \
    -it mujocolab/mjlab:latest
...
| 版本约束 | 说明 | 如何检查 |
|---------|------|---------|
| CUDA Driver → CUDA Toolkit | Driver 版本 ≥ Toolkit 版本 | `nvidia-smi` vs `nvcc --version` |
| CUDA Toolkit → PyTorch | PyTorch 编译时的 CUDA 版本 ≤ Toolkit | `python -c "import torch; print(torch.version.cuda)"` |
...
| Python | ≥ 3.10 | **3.12** | 升级 Python |
| PyTorch | ≥ 2.0 | **2.10.0+cu128** | 升级 PyTorch |
...
# Isaac-Velocity-Flat-Unitree-H1-v0
...
4. 用 `python scripts/list_envs.py` 列出 Isaac Lab 的所有内置任务。数一数每个类别（locomotion/manipulation/dexterous/classic）各有多少个任务。与 §2.3 的任务速览表对照。
...
安装完成后，你可以用 `python scripts/list_envs.py` 查看所有可用任务。以下是按类别整理的内置任务概览——后续章节会逐一深入：
...
| 操作 | mjlab | Isaac Lab |
|------|-------|-----------|
| **训练** | `uv run train <TASK> --env.scene.num-envs 4096` | `python scripts/rsl_rl/train.py --task <TASK> --num_envs 4096 --headless` |
| **评估** | `uv run play <TASK> --wandb-run-path org/proj/run` | `python scripts/rsl_rl/play.py --task <TASK> --checkpoint /path/model.pt` |
...
| **任务列表** | `uv run list-envs` | `python scripts/list_envs.py` |
...
```bash
python scripts/list_envs.py                              # 列出所有任务
python scripts/rsl_rl/train.py --task <TASK> --headless   # 训练
python scripts/rsl_rl/play.py --task <TASK> --checkpoint <PATH>  # 评估
...
```bash
# 方案 A：headless + 录制视频
python scripts/rsl_rl/play.py --task <TASK> --checkpoint <PATH> \
...
以下是 Go2 flat terrain 任务（4096 envs × 1500 iter）在不同 GPU 上的预期训练时间：
...
1. **训练时间预期不合理。** 4096 环境 × 1500 iteration 在 A100 上约 20 分钟，在 RTX 3090 上约 40 分钟。如果训练超过 2 小时还没收敛，很可能是配置问题（而非需要更多迭代）。
...
| **任务列表** | `uv run list-envs` | `python scripts/list_envs.py` | 入口方式不同 |
| **训练命令** | `uv run train <TASK>` | `python scripts/rsl_rl/train.py --task <TASK>` | 项目脚本 vs 直接执行 |
...
| **RL 后端** | RSL-RL（仅 PPO） | RSL-RL / RL Games / SKRL / SB3 | 单一 vs 多选 |
...
# 这是发论文时保证可复现性的最佳方案
docker pull mujocolab/mjlab:latest
docker run --gpus all -it mujocolab/mjlab:latest
```
...
| 3 | 任务列表 | `list-envs` 输出 ≥5 个任务 | `list_envs.py` 输出 ≥10 个任务 |
| 4 | Zero agent | Go2 站在地上不动 | ANYmal 站在地上不动 |
| 5 | Random agent | Go2 有动作但不爆飞 | ANYmal 有动作但不爆飞 |
| 6 | 最小训练 | 16 envs × 50 iter 完成 | 同左 |
| 7 | 完整训练 | 4096 envs × 1500 iter 收敛 | 同左 |
...
| 7 | GPU 显存估算 | max_envs ≈ (VRAM - 2GB) × 1000 / per_env_MB | 2.2 | ⭐⭐ |
...
| uv sync 失败 | 依赖解析冲突 | pyproject.toml 约束过严或网络问题 | 1. 检查 Python 版本 ≥ 3.10 → 2. `uv sync --no-cache` |
...
| 任务名找不到 | `KeyError` 或 `ValueError` | 名称拼写错误或 registry 未加载 | `uv run list-envs` 或 `python scripts/list_envs.py` 查看正确名称 |
...
> **本教材版本锚定**：本章所有安装命令和验证步骤以 mjlab 1.2.0 + Isaac Lab 2.3.0 + rsl_rl ≥ 4.0.0 为准。Isaac Lab 3.0 Beta 的安装命令以注释框标注。版本更新后如有 API 变化，请查阅各框架的 CHANGELOG。
