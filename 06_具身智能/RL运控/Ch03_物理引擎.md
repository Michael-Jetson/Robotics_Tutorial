...
**Integrator 选型**：MuJoCo 提供两种主要 integrator，对训练稳定性有直接影响：

| Integrator | 核心思想 | 对 actuator 的处理 | mjlab 默认 |
|------------|---------|-------------------|-----------|
| `euler` | 显式 Euler 积分 | 视所有力矩为"外部常量" | ❌ |
| `implicitfast` | 半隐式积分 | 对 builtin actuator 隐式处理速度依赖 | ✅ 默认 |
...
| 参数 | 含义 | 默认值 | 调参方向 |
|------|------|--------|---------|
| `num_position_iterations` | TGS 位置求解迭代数 | 4 | 增加→更精确但更慢 |
| `num_velocity_iterations` | TGS 速度求解迭代数 | 1 | 通常不需要改 |
| `contact_offset` | 两个形状距离小于此值时生成接触 | 0.02 m | 过小→穿透，过大→虚假接触 |
| `rest_offset` | 两个形状静止时的间距 | 0.01 m | 通常与 contact_offset 配合 |
| `bounce_threshold_velocity` | 触发弹跳的最小相对速度 | 0.2 m/s | 降低→更自然的弹跳 |
...
> **双重解读**：`num_position_iterations` 在 PhysX TGS 中有两重含义：
>
...
回顾 Ch01 §1.3：Newton 1.0 GA（GTC 2026 发布，NVIDIA × Google DeepMind × Disney Research，Apache 2.0，Linux Foundation 托管）不是一个新的物理引擎——它是一个**求解器聚合平台**，用统一 API 封装了 7 个求解器。
...
| 方向 | 工具 | 说明 | 信息丢失 |
|------|------|------|---------|
| MJCF → URDF | `mujoco.export_model()` | MuJoCo 原生导出 | tendon/equality/site/class defaults |
| URDF → MJCF | `mujoco.MjSpec.from_file("robot.urdf")` | MuJoCo 3.x MjSpec | 无（URDF 是 MJCF 的子集） |
...
2. 尝试将 Go2 的 MJCF 用 `mujoco.export_model()` 转换为 URDF。比较文件大小和内容差异。列出丢失的信息（至少 3 项）。
...
| "Newton = 新引擎" | Newton 是求解器聚合平台，不是新求解器 |
...
读完本章后，打开 mjlab 的 `velocity_env_cfg.py`，你应该能回答以下问题：
