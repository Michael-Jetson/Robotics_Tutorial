...
`condim="4"` 表示轮子使用完整的 3D 摩擦模型（法向 + 切向 + 扭转 + 滚动），这对差速驱动至关重要——如果只用 `condim="1"`（只有法向力），轮子在地面上没有切向摩擦，底盘无法移动。`friction="1.0 0.005 0.001"` 的三个值分别是滑动摩擦、扭转摩擦和滚动摩擦。滑动摩擦决定了底盘的抓地力，扭转摩擦影响原地旋转的能耗，滚动摩擦影响直线行驶的阻力。
...
- 正确做法：驱动轮使用 `condim="4"`（完整 3D 摩擦），被动轮可以用 `condim="3"`
...
轮式 + YAM 的动作空间有三种量纲：轮速（rad/s，范围 ±10），臂关节增量（rad，范围 ±0.1），夹爪开合（m，范围 0-0.04）。如果直接拼接，网络输出一个 9 维向量（2 轮 + 6 臂 + 1 夹爪），但这些维度的 scale 差了两个数量级。PPO 的 action std 是全局共享的（或按维度独立的），如果用一个全局 std，对轮速合理的 std 对臂来说太大（臂会剧烈抖动），反之对轮速太小（底盘几乎不动）。

**反事实推理：如果不做量纲归一化会怎样？** 考虑一个极端情况：策略输出 $[-1, 1]$ 范围的 action，轮速的 scale 是 10（mapping 到 ±10 rad/s），臂关节的 scale 是 0.05（mapping 到 ±0.05 rad 增量）。在训练初期，策略输出接近标准正态分布。轮速维度的 action 在 ±10 rad/s 之间随机跳动，底盘疯狂来回冲刺。同时臂关节的 action 只在 ±0.05 rad 范围内微调，基本不动。策略很快发现"不动臂但大幅移动底盘"能偶尔让 base-to-object 距离减小（纯靠运气），于是梯度集中在轮速维度——臂的学习被饿死。
...
`scale` 的选择原则：每一维的 action 从 0 变到 1 时，对任务进度的贡献应该大致相等。轮速 scale 2.0 意味着满 action 时底盘速度约 0.16 m/s（轮半径 0.08 m × 2 rad/s），一步（0.01 s）移动约 1.6 mm。臂关节 scale 0.05 意味着满 action 时末端移动大约 1-3 mm（取决于 Jacobian）。两者在同一数量级，策略不会偏好某一组。

### Action Scale 的系统化选择方法 ⭐⭐

选择 action scale 不应该靠猜测。以下是一个系统化的方法：

**Step 1：估算每维的物理效果。** 对每个 action 维度，计算 action=1.0 时产生的物理位移/速度。对轮子：位移 = scale × $r$ × $dt$ = 2.0 × 0.08 × 0.01 = 1.6 mm/step。对臂末端：通过 Jacobian 矩阵估算，典型值 1-5 mm/step。对夹爪：scale × $dt$ = 0.02 × 1 = 0.02 m/step（夹爪不需要乘 $dt$，因为是位置增量不是速度）。
...
在教学实验中推荐单 MLP（最简单），在研究项目中可以探索双头或 RNN 架构。RSL-RL 默认使用 MLP，切换到 LSTM 只需要在 `RslRlModelCfg` 中设置 `rnn_type="lstm"`。
...
- 错误做法：假设 action 向量的前两维是轮速，但实际上 Isaac Lab 按 field 名字母序排列
- 后果：底盘收到臂的 action，臂收到底盘的 action——底盘不动，臂剧烈旋转
- 正确做法：打印 `env.action_manager.action_term_dim` 确认每个 term 占据的维度范围
...
在 Isaac Lab 中，可以使用 `DelayedPDActuatorCfg` 直接在 actuator 层面引入延迟，无需手动管理 buffer——这是 Isaac Lab 对 sim2real 的内置支持。
...
⚠️ **编程陷阱：底盘到物体向量忘记减 `env_origins`**
- 错误做法：直接用 `object_pos_w - base_pos_w`，在多环境并行时包含了 env origin 偏移
- 后果：不同 env 中相同的物理配置产生不同的 observation，策略无法泛化
- 正确做法：先把两个位置都减去 `env_origins`，再计算相对向量
...
2. **[设计题]** 如果要加入深度相机（64×64 分辨率）作为视觉输入，actor observation 的维度如何变化？哪些状态项应该从视觉版的 actor obs 中移除？为什么？（提示：参考 Ch18 的 teacher-student 信息退化阶梯）
...
| 原则 | 说明 | 例子 |
|------|------|------|
| 后续阶段权重 ≥ 前序 | 防止策略"卡在接近不抓取" | $w_{\text{carry}} > w_{\text{reach}} > w_{\text{nav}}$ |
| 正则项远小于任务项 | 正则是约束，不是目标 | $w_{\text{smooth}} \ll w_{\text{nav}}$ |
| stop reward 适中 | 太大底盘不敢动，太小不起作用 | 先设 0.5，看漂移再调 |
| $\sigma$ 和空间尺度匹配 | $\sigma$ 太小信号太尖锐 | $\sigma_{\text{nav}}$ ≈ 底盘移动范围的 1/3 |
...
3. **[跨章综合题]** 结合 Ch08 的 EventManager 和本节的 reward 设计，如果在训练的阶段 4 加入物体质量 DR（±30%），应该同时调整哪些 reward 参数？为什么？（提示：物体更重时 lift reward 的门槛需要降低）
...
| 电机增益 | ×0.7-1.3 | 轮速跟踪精度 | Phase 4+ |
| 观测噪声 | ±2 cm 位置 | 感知精度 | Phase 4+ |
...
|------|-----------------|-------------|------|
| 轮地摩擦 μ | $U(0.2, 0.8)$ | $U(0.4, 1.2)$ | 全尺寸底盘更重，摩擦上限更高 |
...
| `scene_cfg.py` | 固定 YAM + cube + table | 增加 `ground_plane`，去掉 `table`，加大 `env_spacing=5.0` | 修改 |
...
最关键的一项是底盘执行延迟。真实底盘从接收速度指令到轮子达到目标转速，通常有 20-80 ms 的延迟（取决于电机控制器和通信协议）。如果仿真中不模拟这个延迟，策略会学到"发出指令后底盘立即响应"的假设。部署到真机时，这个假设破裂——底盘总是比策略预期慢一拍，导致过冲和振荡。在 Ch08 的 EventManager 框架中，可以用 step mode 的 action delay 来模拟：

```python
# 在 EventTermCfg 中添加执行延迟
action_delay = EventTermCfg(
...
- 正确做法：`env_spacing` ≥ 底盘最大移动范围的直径 × 1.5（推荐 5.0+）
...
| G5 | 五阶段 Curriculum | `envs/.../curriculum_cfg.py` | Phase 0 grasp success > 50% 才推进到 Phase 1 |
...
| 维度 | 单臂（本章实现） | 双臂（扩展） |
|------|---------------|------------|
| Action 维度 | 2(base) + 6(arm) + 1(gripper) = 9 | 2 + 6×2 + 1×2 = 16 |
...
| 症状 | 可能原因 | 排查步骤 | 相关节 |
|------|---------|---------|--------|
| 底盘不动 | 轮子 condim 设错 / velocity actuator 配置错 | 1. 检查轮子 `condim` ≥ 3 2. 检查 actuator 类型是 `velocity` 3. 用 `mj_step` + 非零 ctrl 手动测试 | 21.2 |
...
| 多环境训练时 obs 异常 | 未减 `env_origins` | 1. 打印相对向量 2. 比较不同 env_id 的值 3. 检查 base_to_object 函数 | 21.4 |
| attach 后 joint 名字冲突 | 底盘和臂有同名 joint | 1. 列出所有 joint name 2. 重命名冲突项 3. 重新 compile | 21.8 |
| Curriculum 卡在 Phase 0 不推进 | 抓取 success 阈值太高 / 物体超出臂可达范围 | 1. 打印 success rate 曲线 2. 降低推进阈值到 30% 3. 检查物体 spawn 范围 vs 臂 workspace | 21.5 |
