...
| **HITTER** | 外部动捕（OptiTrack × 9） | 360 Hz | ~3 ms | 毫米级精度 | 9 台相机，成本极高 |
...
**关键观察**：除了 ETH 羽毛球外，其他系统都依赖**外部动捕系统**——这在实验室可行但不可移动。ETH 的 perception-aware training 是目前唯一实现了机载感知部署的球类运动系统，但其对象是羽毛球（速度 ≤ 12 m/s，远慢于网球）。网球的高速（20-45 m/s）对机载感知提出了更高要求——更高帧率、更低延迟、更精确的检测。
...
1. **[估算题]** 球速 35 m/s，相机帧率 60 fps。相邻两帧之间球移动了多少米？如果 3D 检测精度为 ± 2 cm，差分速度估计 $v = \Delta p / \Delta t$ 的绝对误差是多少？相对于 35 m/s 的球速是百分之多少？
...
2. **[估算题]** 球距相机 15 m，fovy=45 度，图像宽 160 px。球在图像中占多少像素？提示：视场宽度 = 2 × 15 × tan(22.5°)。
...
**Step 3：按球速分档调 $Q$。** 高速球的 drag 效应更强（Ch26：drag ∝ $v^2$），模型误差更大，因此 $Q$ 应该更大。
...
| 8 | Kalman 增益物理含义 | R→∞ 信模型，P→∞ 信观测 | 27.4 | ⭐⭐⭐ |
