...
>
> 本章不是一个独立的技术模块，而是一张**覆盖全书 24 章的调参索引地图**。你在 Ch06 学会了设计 reward，在 Ch07 学会了配置 PPO 超参，在 Ch08 学会了 DR，在 Ch09 学会了 Teacher-Student——但面对具体的训练问题时，你可能不知道"这个症状应该从哪一章的工具箱里找解法"。Ch25 就是这张从症状到解法的路由表。
>
...
Step 2: Reward 是否在涨？
  ├── 不涨 → 看 KL 和 Entropy
  │     ├── KL 和 Entropy 都低 → 模式三（学习停滞）
  │     ├── KL 高 + 振荡 → 模式四（训练震荡，但方向错误）
  │     └── Entropy 不降 → 模式三变体（obs/reward 信号不足）
  │
  └── 在涨 → 看 Entropy
...
> **这一节解决什么问题**：提供一张覆盖全书 24 章的调参速查表，让你从具体症状出发，快速找到应该调什么、怎么调、参考哪一章。
...
def plot_reward_decomposition(step_rewards, episode_id):
    fig, axes = plt.subplots(len(step_rewards[0]), 1,
                              figsize=(12, 3*len(step_rewards[0])),
                              sharex=True)
...
如果 actor 网络直接输出 std（而非 log_std），数值不稳定可能导致某些时刻 std < 0，此时 $\log(\sigma)$ 产生 NaN。RSL-RL 默认使用固定的 `noise_std`（不是网络输出），所以这个问题在 RSL-RL 中较少见。但如果你自定义了网络结构让 std 可学习，必须确保 std 永远为正。
...
修复：padding contacts 到固定大小（在 MJCF 中设置 `nconmax`），或关闭 CUDA Graph capture。在 Isaac Lab 中，`SimulationCfg.disable_contact_processing = False` 时需要特别注意 nconmax 的设置。
...
ETH 的羽毛球机器人系统（Science Robotics, 2025）在训练中使用了 **constrained PPO**（Lagrangian 方法）直接约束关节速度和力矩到安全范围以下。对于网球等高速击打任务（Ch28），这个约束至关重要——击球瞬间的角速度可以达到 12 m/s，不加约束的策略可能输出超过电机额定值 50% 的指令。
...
RSL-RL 的 P3O 扩展（Zhang et al. 2022）实现了这个机制。在 AGILE 和 ETH 的羽毛球系统中，约束项包括关节速度限制、力矩限制和自碰撞约束。

```python
# Constrained PPO 配置示例（基于 RSL-RL P3O 扩展）
...
- 二阶 Butterworth 在 4 Hz 处有约 90° 的相位延迟。对于 50 Hz 策略，这相当于约 5 ms 的额外延迟
