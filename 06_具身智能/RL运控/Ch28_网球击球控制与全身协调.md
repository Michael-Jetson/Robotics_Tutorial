...
| **KungfuBot** | 端到端：motion tracking + adaptive curriculum | 全关节 PD targets | tracking error + AMP | bi-level adaptive tolerance |
...
**为什么不直接用 $J^{-1}$（伪逆）？** 当机器人接近 workspace 边界或奇异构型（Jacobian 秩下降）时，$J^T J$ 的特征值接近零，伪逆 $J^+$ 给出极大的关节角变化——策略输出一个微小的 task-space 动作，关节却跳变几十度。DLS 通过添加 $\lambda^2 I$ 来正则化——代价是精度稍有下降，但换来了数值稳定性。
...
**为什么用指数衰减而非线性？** $\exp(-d^2/\sigma^2)$ 在距离大时梯度也不为零（远处有学习信号），距离小时梯度最大（近处引导最强）。线性衰减 $\max(0, 1 - d/d_{\max})$ 在 $d > d_{\max}$ 时梯度为零——策略远处没有方向信号。$\sigma$ 应该和 arm workspace 尺度匹配（0.3-0.5 m），让球拍在整个工作空间内都有非零信号。

**Timing Reward**：在正确时间接近击球点

```python
def compute_timing_reward(env, tau=0.05):
    """时间协调 reward：球拍应该在 time_to_impact ≈ 0 时到达击球点。
    ...
    # 定义"准备度"：距离小且时间短
    # 理想状态：tti ≈ 0.1s 时距离 ≈ 0（提前 0.1s 到位开始挥拍）
    readiness = torch.exp(-distance / 0.2) * torch.exp(-tti / tau)
...
def compute_outcome_reward(env):
    ...
    # 检查落点是否在目标区域（near-side service box）
    landing_pos = env.event_buffer.first_landing_after_contact  # [N, 3]
    in_target = (
        (landing_pos[:, 0] > -6.40) & (landing_pos[:, 0] < 0.0) &
...
class StrikeEventBuffer:
    ...
    def update_post_contact(self, ball_pos_local, ball_vel):
        ...
        # 检查过网
        crossed = active & (ball_pos_local[:, 0] < 0)  # 球到了 near side
...
def compute_approach_reward_with_physics_predictor(env, sigma=0.3):
    ...
    # 用 physics predictor（ground truth + 物理模型）计算精确击球点
    # 这个信息只在训练时可用（privileged）
    physics_impact_pos = env.physics_predictor.compute_impact_point(
        env.scene["ball"].data.root_pos_w[:, :3] - env.scene.env_origins,
        env.scene["ball"].data.root_lin_vel_w[:, :3],
    )
...
```python
# Isaac Lab 中的 DR 配置
from omni.isaac.lab.envs.mdp import events as mdp_events
...
| **任务** | ball landed after contact | 中 | **True** | 正常落地 → value bootstrap |
| **仿真** | timeout (5 秒) | 低 | **True** | 防止无限等待 |

**关键设计**：`ball landed after contact` 的 `time_out=True`——contact 后球落地是正常的 episode 结束（not failure），PPO 应该对最终状态做 value bootstrapping（$V(s_T) \neq 0$）。如果错误地设为 `False`，PPO 会把每次成功击球后的 episode 结束当作负面信号——策略可能学到"不击球"来避免 episode 结束（Ch25 中模式九"自杀策略"的变体）。
...
# Lambda/joint_velocity_limit 振荡 → 策略在"保守"和"冒险"之间反复 → 通常是最优状态
```
...
**教训 1：Dynamics DR 是非negotiable 的。** 去掉后成功率从 91% → 14-29%。这说明仿真中的物理参数和真实世界之间存在显著的 gap——即使你认为自己标定得很好。DR 的本质不是让仿真更"真实"，而是让策略对参数不确定性**鲁棒**。
...
# LATENT 消融实验的定量结果
latent_ablation = {
    "full_system": {"forehand": 90.9, "backhand": 77.8},
    "no_dynamics_dr": {"forehand": 29.0, "backhand": 14.0},
    "no_obs_noise": {"forehand": 50.0, "backhand": 0.0},
    "no_any_dr": {"forehand": 10.0, "backhand": 5.0},
}
```
...
- LATENT: sim 96.5% → real 91%（网球，落点精度放宽）
...
class TennisStrikeEnvCfg(ManagerBasedRlEnvCfg):
    ...
    # === Simulation 配置 ===
    sim = SimCfg(
        timestep=0.001,    # 1000 Hz（击球需要高频仿真）
        render_interval=2,
        gravity=(0.0, 0.0, -9.81),
    )
    decimation = 20  # 控制频率 = 1000/20 = 50 Hz
