...
└── 代表工作
    ...
    ├── Agarwal et al. 2023（Egocentric Vision Locomotion）
...
> **本质洞察**：对 locomotion 视觉控制，depth 是性价比最高的模态——它提供了 locomotion 最需要的几何信息（地形高度、障碍物距离），同时 sim-to-real gap 最小（深度不受光照和材质影响）。这就是为什么 extreme-parkour、ANYmal perceptive locomotion、RMA 系列工作都以 depth camera 为主要传感器。RGB 只在需要语义理解时才引入。
...
**Isaac Lab 中的 height scan 实现**：Isaac Lab 使用 PhysX 的 raycast 或 USD-based `RaycastSensorCfg`。配置语法类似但命名略有不同。Isaac Lab 还支持 mesh-based raycast（对非平面地形更精确）。
...
如果任务配置了 `TiledCamera` 但启动时没有加 `--enable_cameras`，渲染管线不会初始化——相机的 output 全为零（或上一帧的残留值）。策略训练时看到的全是黑色图像，但不会报错——只是收敛极慢或完全不收敛。自检方法：训练初期保存一帧 depth 图到磁盘，用 matplotlib 可视化确认内容正确。
...
MTS 的解决方案：student 的 heading command 是 teacher 预测和自己预测的线性组合：

$$\theta_{\text{student}} = \alpha \cdot \theta_{\text{teacher}} + (1 - \alpha) \cdot \theta_{\text{predicted}}$$

$\alpha$ 在训练过程中从 1.0 逐步退火到 0.0——一开始完全信任 teacher 的 heading，逐步过渡到完全信任自己的预测。这给了 student 的 heading prediction 网络时间来"追上" teacher 的准确度。

**MTS 的退火策略选择：**

| 退火方式 | 公式 | 特点 | 适用场景 |
|---------|------|------|---------|
| 线性 | $\alpha = 1 - t/T$ | 均匀过渡 | 大多数场景的默认 |
| 余弦 | $\alpha = 0.5(1 + \cos(\pi t/T))$ | 初期慢、中期快、末期慢 | heading 预测较难学的场景 |
| 阶梯式 | $\alpha = 1$ if $t < T/2$ else $0$ | 前半段全 teacher，后半段全 student | 快速切换但有突变风险 |

extreme-parkour 使用线性退火，$T \approx 5000$ iterations（与 Phase 2 总训练量相当）。如果你发现 student 在退火中期出现严重的步态退化（表现为 episode length 突降），说明退火太快——student 的 heading 预测还没收敛就被迫承担全部 heading 决策。解决方案：增大 $T$ 到 8000-10000。
...
**路径二：使用 RSL-RL 的 Student-Teacher Runner 扩展。** RSL-RL 3.x 提供了实验性的 `StudentTeacherDistillationRunner`，支持 DAgger-style 训练。使用方式：

```python
# RSL-RL 3.x Student-Teacher Runner 配置（示意）
from rsl_rl.runners import StudentTeacherDistillationRunner

runner = StudentTeacherDistillationRunner(
    env=env,
    teacher_policy=teacher_policy,     # Stage 1 frozen teacher
    student_policy=student_policy,     # CNN + GRU + MLP
    distill_cfg=DistillCfg(
        loss_type="mse",               # MSE or L1
        dagger_mode=True,              # student 执行 rollout
        teacher_action_weight=1.0,     # teacher 标注的权重
        student_lr=1e-4,               # student 学习率
        cnn_lr_scale=0.1,             # CNN lr = student_lr × 0.1
    ),
)
runner.learn(num_iterations=5000)
```
...
**关键配置参数（从 `go1_config.py` 提取的工程要点）：**

```python
# 地形配置——parkour 需要高度多样的障碍物
class terrain:
    mesh_type = "trimesh"
    curriculum = True           # 地形难度课程
    num_rows = 20              # 地形网格行数
    num_cols = 20
    terrain_proportions = [
        0.2,  # flat
...
# Reward 配置——与纯 locomotion 的关键区别
class rewards:
    tracking_sigma = 0.25      # velocity tracking 的 sigma
    # 注意：parkour 不完全跟踪速度命令
    # 而是用 waypoint-based 方向命令
    class scales:
        tracking_lin_vel = 1.0
        tracking_ang_vel = 0.5
        lin_vel_z = -2.0       # 惩罚垂直速度
        ang_vel_xy = -0.05     # 惩罚 roll/pitch 角速度
        orientation = -5.0     # 惩罚偏离水平
        base_height = -5.0     # 惩罚 base 高度偏离
        collision = -1.0       # 碰撞惩罚
...
| max_iterations | 15000 | 未收敛 → 20000 |
| learning_rate | 1e-3 | 不稳定 → 3e-4 |
| batch_size | 4096×24/4 | 内存不足 → 减小 |
| entropy_coef | 0.01 | 探索不足 → 0.02 |
| gamma | 0.99 | 短视 → 0.995 |
| terrain_levels | 10 | 最难地形未涉及 → 12 |

**Phase 1 收敛标准**：episode length 在最难地形（level 10）上 > 500 步（10 秒），且 50% 以上的机器人能通过。如果最难地形始终无法通过，检查地形参数是否超出了机器人的物理能力（如 gap 宽度 > 2× 体长，物理上不可能跳过）。
...
```python
# extreme-parkour 的 depth CNN
class DepthEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(1, 32, kernel_size=5, stride=2, padding=2)
        self.conv2 = nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=1)
        # 输入 58×87 → conv1 → 29×44 → conv2 → 29×44
        self.flatten_dim = 32 * 29 * 44  # = 40832
        self.linear = nn.Linear(self.flatten_dim, 128)
        self.gru = nn.GRU(128, 64, batch_first=True)
    
    def forward(self, depth, hidden_state):
        x = F.relu(self.conv1(depth))
        x = F.relu(self.conv2(x))
        x = x.flatten(start_dim=1)    # [B, 40832]
        x = F.relu(self.linear(x))    # [B, 128]
        x, hidden_state = self.gru(x.unsqueeze(1), hidden_state)
        return x.squeeze(1), hidden_state  # [B, 64], hidden
```
...
def get_heading_command(teacher_heading, student_heading, alpha):
    """
    alpha: 从 1.0 线性衰减到 0.0（约 5k iterations）
    训练初期：完全信任 teacher heading
    训练后期：完全信任 student 自己的预测
    """
    return alpha * teacher_heading + (1 - alpha) * student_heading
```

**Phase 2 训练时间**：README 推荐 5-10k iterations，8-10 hours on RTX 3090。

### Phase 1 + Phase 2 的完整数据流

```text
Phase 1 (blind oracle):
  obs = [proprio(48) + scandots(~200)]
    → MLP(256, 256, 128) → action(12)
  Training: PPO, 4096 envs, 10-15k iters

Phase 2 (depth student):
  depth(58×87×1) → CNN(2 conv + linear) → [128]
    → GRU(128→64) → visual_feature[64]
  proprio(48) → concat → [112]
    → MLP(256, 256, 128) → action(12)
  Training: DAgger + MTS, 512-1024 envs, 5-10k iters
...
class VisualPolicyDeployer:
    ...
    def infer(self, depth_frame, proprio):
        ...
        # TensorRT 推理
        outputs = self.context.execute(inputs)
...
extreme-parkour 的真机部署使用了类似的异步架构——CNN 在 Jetson Orin 的 GPU 上以 ~15 Hz 运行，MLP 在 CPU 上以 100 Hz 运行（A1 的控制频率）。
...
import torch
from torchvision.models import vit_small_patch14_dinov2

class DINOv2LocomotionPolicy(nn.Module):
    def __init__(self, proprio_dim=48, action_dim=12):
        super().__init__()
        # 加载预训练 DINOv2（冻结权重）
        self.visual_encoder = vit_small_patch14_dinov2(pretrained=True)
...
| depth 图全黑 | --enable_cameras 缺失 | 启动命令参数 |
