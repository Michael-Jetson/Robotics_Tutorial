...
> **本质洞察**：DR 和 delta action model 解决 sim-to-real gap 的方式完全不同。DR 说"我不知道真实参数是什么，所以我在一大堆可能的参数上训练"。delta model 说"我知道仿真和真实有差距，让我用真机数据学一个修正项"。DR 不需要真机数据但可能过于保守，delta model 需要真机数据但更精确。两者不互斥——ASAP 的第一阶段就是用 DR 训练 base policy。
...
def _check_interval_timer(self, term_name, env_ids):
    ...
    if len(env_ids_to_apply) > 0:
        new_intervals = torch.rand(len(env_ids_to_apply)) * (hi - lo) + lo
...
def randomize_pseudo_inertia(
    ...
    for b in range(num_bodies):
        ...
        I_recovered = (0.5 * torch.diagonal(sigma_new[:, :3, :3], dim1=1, dim2=2).sum(1, keepdim=True).unsqueeze(-1)
                       * torch.eye(3, device=m.device).unsqueeze(0) - sigma_new[:, :3, :3]
                       + m_new.unsqueeze(-1).unsqueeze(-1) * torch.bmm(c_new.unsqueeze(2), c_new.unsqueeze(1)))
...
def push_by_setting_velocity(env, env_ids, velocity_range):
    ...
    for i, key in enumerate(["x", "y", "z", "roll", "pitch", "yaw"]):
        if key in vel_range:
            lo, hi = vel_range[key]
            random_vel[:, i] = torch.rand(len(env_ids)) * (hi - lo) + lo
...
实现 push curriculum 需要在 CurriculumManager 中根据 iteration 切换 push 配置。或者更简单地：Phase 0-2 不配 push，Phase 3 开始 resume 训练时加入 push。
    ...
    def _run_episodes(self, num_episodes, max_steps):
        ...
        while episode_count < num_episodes and step_count < max_steps * num_episodes:
            ...
            if done.any():
                ...
                fall_count += terminated.any(dim=0).sum().item() if terminated.dim() > 0 else terminated.sum().item()
...
```python
# ASAP delta action model 的核心实现（简化自 LeCAR-Lab/ASAP）
import torch
...
**ASAP 代码仓库结构**（`LeCAR-Lab/ASAP`）：

```text
LeCAR-Lab/ASAP/
├── humanoidverse/         # sim-agnostic env wrapper
│   ├── envs/              # 环境定义
│   └── agents/            # RL agent（PPO wrapper）
├── asap/                  # delta model 核心
│   ├── delta_model.py     # DeltaActionModel 定义
│   └── train_delta.py     # Stage C 训练脚本
├── sim2real/
│   └── rl_policy/
│       └── listener_deltaa.py  # 真机 MoCap 数据采集
├── configs/               # 各阶段配置
└── scripts/               # 训练启动脚本
```
...
🧠 **思维陷阱：认为"有了 delta model 就不需要 DR 了"。** ASAP 的 Stage A 仍然使用 DR——delta model 修正的是 DR 无法覆盖的系统性偏差。没有 DR 的 base policy 可能在真机上表现太差，连采集有意义的 real rollout 数据都做不到。
...
1. `LeCAR-Lab/ASAP/asap/` → delta model 定义和训练
2. `LeCAR-Lab/ASAP/sim2real/rl_policy/listener_deltaa.py` → 真机数据采集
3. `LeCAR-Lab/ASAP/humanoidverse/` → sim-agnostic env wrapper

ASAP 的代码结构值得阅读——它展示了如何把 delta model 无缝插入现有的 RL 训练管线。关键文件是 `delta_model.py`（网络定义，只有 ~50 行）和 `train_delta.py`（训练循环，~200 行）。
