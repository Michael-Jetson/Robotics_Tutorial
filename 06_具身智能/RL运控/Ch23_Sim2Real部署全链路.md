...
> **反事实推理：如果只用 DR 不做 SysID 会怎样？** 假设真实摩擦系数是 0.6，但你不做 SysID，所以你不知道这个值。你猜测摩擦可能在 0.2-1.5 之间，用这个范围做 DR。训练出的策略确实对摩擦鲁棒——但代价是什么？(1) 策略在 μ=0.6 附近的性能比 SysID + 窄 DR 的策略差 15-30%（因为它要"兼顾"太宽的范围）；(2) 训练时间增加 2-3 倍（因为策略需要在更大的参数空间上收敛）。如果先做 SysID 测得 μ≈0.6，DR 范围设为 [0.4, 0.8]——训练更快，策略在真实值附近更优化。
...
| 需要真机数据 | 是 | 否 | 是（少量） |
| 能处理模型结构偏差 | 否 | 部分 | 是 |
| 部署时的修改 | 无 | 无 | 需要携带 δ 网络 |
| 计算成本 | 低 | 训练成本增加 | 额外 δ 网络推理 |

ASAP 在 Unitree G1 上的实验表明，它在高动态动作（如跳跃、旋转）上的 sim2real 效果显著优于纯 DR——tracking error 降低 30-50%。但它需要在真机上采集 rollout 数据（Phase 2a），这意味着策略在 Phase 1 结束后必须"勉强能用"——如果 Phase 1 的策略在真机上完全不能走，Phase 2a 采集到的都是失败轨迹，δ 网络学不到有意义的残差。

> **反事实推理：如果不做 Phase 1 直接采集真机数据会怎样？** 没有预训练的策略在真机上输出随机动作——机器人立即摔倒。采集到的数据只反映了"从站立到摔倒"这个极短的过渡过程，δ 网络只能学到"在即将摔倒时如何修正动作"这个狭窄的场景。Phase 1 的预训练确保了策略在真机上能至少行走几秒钟，产生覆盖正常运动范围的数据——这是 δ 网络有效学习的前提。
...
第一步，逐关节静态测试。给每个关节发送一个小幅正弦波命令（±0.05 rad, 0.5 Hz），目视确认：关节确实在运动？运动方向与命令方向一致？运动幅度与命令幅度成比例？没有其他关节联动？这一步可以在 5 分钟内发现 joint order 映射错误——这是最常见也最危险的部署 bug。
...
3. **单位验证测试**：发送 1.0 的 action，确认电机位移与 action_scale * 1.0 一致。如果 action_scale 是 0.25 rad，电机应该移动约 0.25 rad（±PD 追踪误差）。
...
2. **[实验题]** 在 mjlab 中，分别训练两个 depth 策略：一个不做外参 DR，一个做 ±2 度的外参 DR。在 play 时手动偏移相机外参，比较两个策略的鲁棒性。
...
具体做法：如果摩擦系数的辨识结果是 $\mu = 0.65 \pm 0.1$（通过多次实验的标准差估计），那么 DR 范围设为 $[0.45, 0.85]$（均值 ±2 倍标准差）。这比盲猜的 $[0.3, 1.5]$ 窄得多——训练更高效，策略在辨识值附近更优化，同时仍覆盖合理的不确定性范围。
...
2. **[计算题]** 摩擦系数辨识结果为 $\mu = 0.7 \pm 0.15$。如果 DR 范围设为均值 ±2 倍标准差，范围是多少？与"凭经验猜测的 $[0.3, 1.5]$"相比，范围缩小了多少倍？
...
| Day 2 下午 | D3 悬空低增益测试 | 12 关节方向全正确 | SDK joint order ≠ URDF |
...
第二步，固定 arm 姿态（安全位置），只执行底盘低速移动。验证行走稳定性、制动效果和姿态稳定（pitch/roll 不超过 ±5 度）。
...
**通过标准**：30 分钟内电机温度不超过安全阈值的 80%，标准测试片段的成功率无系统性下降（允许 ±10% 波动）。如果温度持续上升，必须降低 duty cycle 或添加温度相关的动作降额逻辑。
...
| 6 | 训练 decimation × sim_dt = 真机 control_dt | 精确匹配 | 配置文件对比 |
| 7 | 训练中注入的 action delay ≥ 真机 p95 延迟 | delay_range 覆盖 | 训练配置检查 |
...
| # | 检查项 | 通过标准 | 验证方法 |
|---|--------|---------|---------|
| 11 | IMU 静止时加速度读数 ≈ (0, 0, 9.81) m/s² | 误差 < 0.1 m/s² | 静态读数 |
...
**第 1 分钟：检查 ONNX 输出**。在部署端记录策略的原始输出（未限幅前）。如果输出全是 NaN 或固定值——大概率是 obs normalizer 问题。如果输出全在 ±1 范围内但行为错误——大概率是 joint mapping 或 action scale 问题。
