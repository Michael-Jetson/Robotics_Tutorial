...
这个 term 的设计有三个精妙之处：(1) 只在落地瞬间给 reward（不是连续给）；(2) 减去 0.5 秒的 threshold——空中时间太短不给奖励，太长也没额外奖励；(3) 命令接近零时关闭——站立时不应该鼓励抬脚。
...
| 曲线升但视频差 | reward hacking | 1.查分项 reward 2.检查 metrics 3.做 ablation 4.录视频 | 6.9 |
