...
| HITTER（乒乓球，arXiv:2508.21043） | MuJoCo | 2000 Hz | 50 Hz | 5-15 m/s | ~1 ms |
| ETH 羽毛球（Science Robotics'25） | Isaac Gym | 1000 Hz | 50 Hz | ≤12 m/s | ~5 ms |
...
| 自由体 | freejoint | RigidBodyCfg + enable_rigid_body_api=True |
| 外力施加 | `set_external_force_and_torque()` | `apply_force()`（PhysX API） |
...
<!-- 网碰撞体的参数 -->
<geom name="net_collision" type="box"
      pos="0 0 0.457"           <!-- 网中心高度 = 0.914/2 -->
      size="0.01 5.5 0.457"     <!-- 半厚度 1cm, 半宽 5.5m, 半高 0.457m -->
      contype="1" conaffinity="1"
      condim="1"                <!-- 仅法向力（球撞网不需要摩擦） -->
      solref="0.001 1.0"        <!-- 硬接触 -->
      rgba="1 1 1 0.3"/>
```
...
def _get_ball_spec(self):
    ...
    body.add_geom(
        ...
        condim=3,              # 3-DOF 接触：法向 + 2 切向
        friction=[0.6, 0.005, 0.001],  # sliding, torsional, rolling
...
**`condim=3`**：三维接触模型——1 维法向力 + 2 维切向摩擦力。这对球-地面接触是合适的（球在地面上可以滚动和滑动）。如果改为 `condim=1`（仅法向），球落地后会像冰上一样无限滑动，永远不会停下来。如果改为 `condim=6`（加上旋转摩擦），计算量增加但对球体来说改善不大——因为球是高度对称的，旋转摩擦可以忽略。
...
**`friction=[0.6, 0.005, 0.001]`**：三个分量对应 MuJoCo 接触模型的三种摩擦。

| 分量 | 类型 | 值 | 物理含义 |
|------|------|---|---------|
| 第一个 | 滑动摩擦（sliding） | 0.6 | 球与地面的滑动阻力系数。硬地约 0.5-0.7，草地更高 |
| 第二个 | 扭转摩擦（torsional） | 0.005 | 球绕法向旋转时的阻力。对球体影响小 |
| 第三个 | 滚动摩擦（rolling） | 0.001 | 球在地面上滚动时的阻力。太大球会很快停住 |
...
class LaunchCommand(CommandTerm):
    ...
    def _resample_command(self, env_ids: torch.Tensor):
        ...
        vel[:, 1] = vy
        vel[:, 2] = vz
        vel[:, 3] = 0.0        # wx（topspin 绕 y 轴）
        vel[:, 4] = -topspin    # wy（负号因为 topspin 让球向下弯曲）
...
| 初速 (m/s) | 衰减时间常数 τ (s) | 到达 x=0（11m）时的速度 (m/s) | 速度衰减比 |
|-----------|-----------------|--------------------------|----------|
| 20 | 2.48 | 14.2 | 29% |
| 30 | 1.65 | 18.1 | 40% |
| 40 | 1.24 | 20.6 | 49% |
...
速度越快，衰减比例越大——这是因为二次阻力与 $v^2$ 成正比。50 m/s 的发球到达对面时只剩 22 m/s，衰减了 56%！这解释了为什么真空模型下球会飞出底线很远，而有阻力后落点合理——阻力不仅让球飞得近，还改变了弹道的形状（从"对称抛物线"变为"压缩的不对称弧线"）。
...
对标准网球参数（$r = 0.033$ m，$\rho = 1.225$ kg/m³），在 $v = 20$ m/s 和 $\omega = 200$ rad/s 时，$|\mathbf{F}_{\text{Magnus}}| \approx 0.37$ N——约为重力的 65%。这意味着 topspin 网球的有效"重力"几乎翻倍（重力 + 向下的 Magnus 力），弧线会显著更弯。
...
**通过标准**：球落地后明显弹起。弹起高度约为落下高度的 53-58%（对应 COR 0.73-0.76 的硬地场）。如果弹起太高（COR > 0.85），减小 `solref` 的 `dampratio`；如果弹起太低（COR < 0.5），增大 `dampratio` 或减小 `timeconst`。如果球不弹起而是"粘"在地面上——检查 `solref` 的 `timeconst` 是否太大（太"软"的接触会吸收所有动能）。
...
当前 observation 中 `ball_position` 是 world frame，而 `ball_velocity` 和 `ball_angular_velocity` 是 body frame。对 sphere 来说 body frame 的姿态视觉上不明显，但从 API 语义看 frame 不同。
...
<body name="racket" pos="0 0 0.2" euler="0 -15 0">
  ...
  <geom name="racket_face" type="box"
        size="0.12 0.15 0.005"    <!-- 宽24cm × 长30cm × 厚1cm -->
        mass="0.15"                <!-- 线床约 150g -->
        condim="3"
        friction="0.4 0.01 0.001"  <!-- 球拍面摩擦 -->
        solref="0.001 1.0"         <!-- 硬接触 -->
...
**关键参数**：球拍面厚度 `size[2] = 0.005` m（5 mm）。在 2000 Hz 仿真下，30 m/s 球单步移动 0.015 m——仍然是面厚度的 3 倍，理论上还可能穿透。但实际上 MuJoCo 的接触检测有 `margin` 和 `gap` 参数来处理高速穿透。如果仍有穿透问题，可以把面厚度加到 0.01 m（1 cm），或使用 MuJoCo 的 `CCD`（Continuous Collision Detection）扩展。
...
**通过标准**：topspin 球弹跳后水平速度应**增大**（旋转摩擦耦合效应），无 topspin 球弹跳后速度应减小。如果无差异——检查 `condim` 是否为 3（切向摩擦是传递旋转效应的必要条件）。
