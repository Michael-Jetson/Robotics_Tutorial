...
>
> **参考**：✅ ProtoMotions（CALM/MaskedMimic）· ✅ TextOp（RSS'26）· ✅ HumanPlus（CoRL'24）· ✅ HDMI（CMU）· ✅ LeVERB/WholebodyVLA（ICLR'26）
>
> **机器人**：G1/H1 · **累积项目**：**C（续）**
...
其中 $x_0$ 是一段动作序列（$T$ 帧 × $J$ 关节），$c$ 是 CLIP text embedding。
...
**反事实推理：如果不用 CLIP 而是训练一个专用的 text encoder 会怎样？** CLIP 的优势是在数十亿图文对上预训练，对各种自然语言描述有泛化能力。专用 text encoder 只在动作-文本配对数据上训练——数据量有限（BABEL 数据集约 6K 文本标注），泛化能力差。对于"a person moonwalks"这种训练集中没有的描述，CLIP 可能还有一定理解能力，专用 encoder 几乎肯定失败。
...
| 工具 | 输入 | 输出 | 精度 | 速度 | 特点 |
|------|------|------|------|------|------|
| **4DHumans** | 单目 RGB 视频 | SMPL body + 手 | ~5cm | ~15 FPS | 全身 + 手部 |
| **WHAM** | 单目 RGB 视频 | SMPL body + global translation | ~4cm | ~20 FPS | 全局平移更准 |
| **HybrIK** | 单帧 RGB 图像 | 关节角度（非 SMPL） | ~5cm | ~30 FPS | 直接输出关节角度 |
...
**工程含义**：token 化后可以用 NLP 的自回归模型（GPT 风格）来生成动作——输入是 text token 序列，输出是 motion token 序列，然后通过 VQ-VAE decoder 恢复为连续动作。T2M-GPT 和 TextOp 的 RobotMDAR 都使用了这种范式。
...
| `body_pose` | (69,) | 23 关节 × 3 axis-angle | 核心运动参数，需要 IK 转为机器人关节角 |
| `transl` | (3,) | root 位置 | 全局平移，需要身高比缩放 |

**关键工程概念**：SMPL 的每个关节是 **3-DoF ball joint**（用 axis-angle 表示），但 G1 的每个关节是 **1-DoF hinge joint**（用单个角度表示）。这意味着 SMPL 的 23×3=69 维关节空间不能直接映射到 G1 的 29 维——需要通过 **IK（逆运动学）** 求解。TextOp 的一个关键创新就是绕过了 SMPL，直接在 G1 的 29 维空间中训练（Robot-Skeleton 表示），避免了 retarget 误差。
...
2. **[计算题]** CLIP ViT-B/32 的 text embedding 是 512 维。如果你要把它映射到一个 150 帧 × 29 关节的动作序列，中间需要什么结构？估算参数量。
...
**gradient_penalty_weight = 5.0 的含义**：判别器的总 loss = GAN loss + 5.0 × gradient penalty。较大的 penalty weight 让判别器更"保守"——它的输出更平滑，策略获得的 reward 信号更连续。如果 penalty weight 太小（如 0.1），判别器容易过拟合到训练数据，对策略新生成的动作给出极端的判断。
...
def calm_training_step(agent, batch):
    ...
    # 2c. GAN loss
    disc_loss = -torch.mean(torch.log(torch.sigmoid(d_real) + 1e-6)
                           + torch.log(1 - torch.sigmoid(d_fake) + 1e-6))
    ...
    # === 3. AMP Reward（给 PPO 使用的额外 reward） ===
    with torch.no_grad():
        amp_reward = -torch.log(1 - torch.sigmoid(d_fake) + 1e-6)
...
**Phase 2: Masked 蒸馏（Behavioral Cloning）**

Phase 2 把 expert 的全信息策略蒸馏到一个能处理部分信息的 transformer：
...
- BC loss 而不是 PPO——从 expert 直接学习比与环境交互探索更稳定
...
**注意**：MaskedMimic 的 Phase 2 使用 **BC（Behavioral Cloning）** 而不是 PPO。这是因为 BC 更适合从 expert 蒸馏——PPO 需要与环境交互并探索，而 BC 直接从 expert 的输出学习。

### ⚠️ 常见陷阱

⚠️ **编程陷阱：MaskedMimic 的 Phase 2 使用 BC 而非 RL**。错误做法：用 PPO 训练 Phase 2。现象：训练不稳定，因为 mask 后的输入信息不足以让 PPO 有效探索。正确做法：使用 BC 从 Phase 1 expert 蒸馏。
...
| 蒸馏方法 | DAgger（在线，rollout 中收集数据） | BC（离线，从 expert 预录数据学习） |
| Student 的 obs | 部分身体的参考 + proprio | 部分信息（masked description）+ proprio |
| 推理时输入 | 特定身体部位的控制信号 | 文本/关键帧/目标位置等组合 |

**为什么 HOVER 用 DAgger 而 MaskedMimic 用 BC？** HOVER 的 student 需要在真实的物理环境中与 expert 交互——DAgger 让 student 在自己的 rollout 中获得 expert 的标签。MaskedMimic 的 student 是纯 BC——从 expert 预录的 (obs, action) 数据集中学习。BC 更简单但可能受 distribution shift 影响；DAgger 更复杂但能缓解 distribution shift。
...
TextOp（Xie, Zheng, Han et al., to appear at RSS 2026, arXiv 2602.07439, `TeleHuman/TextOp`）是目前最完整的**实时交互式文生动作**系统。它允许用户在 G1 运行过程中随时修改文本指令——机器人平滑地从当前动作过渡到新指令描述的动作。
...
| Tracker 训练 | 1 × 4090 | ~8h | PPO, 4096 envs, 30K iter |
| RobotMDAR 训练 | 1 × 4090 | ~12h | Diffusion, 500 epochs |
| Tracker 重训（含生成数据） | 1 × 4090 | ~6h | 加入 MDAR 生成数据 |
| **总计** | | **~28h** | 单 4090 |
```
...
# 输出: results/wham_output.pkl
# 包含: body_pose (T, 69), global_orient (T, 3), transl (T, 3), betas (10,)
```
...
def retarget_smpl_to_g1(smpl_pkl_path, output_csv_path):
    """SMPL → G1 29-DoF retarget"""
    import pickle
    with open(smpl_pkl_path, 'rb') as f:
        data = pickle.load(f)
    
    body_pose = data['body_pose']  # (T, 69) = 23 joints × 3 axis-angle
...
cd WHAM
python demo.py --video_path "$OUTPUT_DIR/input.mp4" \
    --output_dir "$OUTPUT_DIR/wham/" --save_pkl
cd ..

echo "Step 3: SMPL → G1 retarget"
python retarget_smpl_to_g1.py \
    --input "$OUTPUT_DIR/wham/wham_output.pkl" \
...
```python
# HumanPlus HST reward（基于论文 Table 1）
rewards = {
    # Tracking rewards（exponential kernel）
    "joint_pos_tracking": "exp(-||q - q_tg||^2 / 0.1)",
    "joint_vel_tracking": "exp(-||dq - dq_tg||^2 / 0.5)",
    "root_pos_tracking":  "exp(-||p - p_tg||^2 / 0.2)",
    "root_rot_tracking":  "exp(-||R ⊖ R_tg||^2 / 0.2)",
    
    # Contact matching
    "feet_contact": "bonus when foot contacts match reference",
...
这个 reward 结构和 Ch15 的 motion tracking reward 高度相似——因为 HST 本质上就是一个 motion tracking policy。关键差异在于 HST 在**整个 AMASS**（40 小时）上训练，覆盖面远大于 Ch15 的单条/少量动作。
...
| 策略输出 | 19 维 | 身体关节 |
| PPO envs | 4096 | IsaacGym |
| GPU | 1 × RTX 4090 | 约 24h 训练 |
| 控制频率 | 50 Hz | 策略推理频率 |
| 相机频率 | 25 Hz | WHAM + HaMeR |
...
外部 RGB 相机 (25 Hz)
    ↓
WHAM: 身体姿态估计 → SMPL body_pose + global_orient + transl
    +
HaMeR: 手部姿态估计 → MANO hand_pose
    ↓ 合并
SMPL-X 全身参数 (body + hands)
    ↓ retarget
H1 target_joint_pos (33-DoF)
    ↓ 频率提升 (25 Hz → 50 Hz, 线性插值)
HST 输入: robot proprio + target pose
    ↓ HST 推理 (~1ms on 4090)
H1 joint position commands (50 Hz)
    ↓ PD 控制器
H1 关节力矩 → 执行
```

**频率对齐是工程关键**：相机 25 Hz，策略 50 Hz。每两步策略推理中间需要插值一次 target pose。简单线性插值就足够——因为 25 Hz 下相邻帧的姿态变化很小。
...
数据收集流程:
  ...
  每个任务: ~40 个 demos，每个 demo ~30 秒
  总数据量: ~40 × 30s × 50Hz = ~60K 帧/任务
...
dr_config = {
    # 物理参数随机化
    "friction": [0.5, 1.5],         # 脚底摩擦系数
    "restitution": [0.0, 0.5],      # 弹性系数
    "body_mass": [0.9, 1.1],        # 质量缩放（±10%）
    "com_offset": [-0.02, 0.02],    # 质心偏移 (m)
    
    # 电机参数随机化
    "motor_strength": [0.85, 1.15], # 电机力矩缩放
    "motor_offset": [-0.03, 0.03],  # 电机零位偏移 (rad)
    
    # 观测噪声
    "joint_pos_noise": 0.01,        # rad
    "joint_vel_noise": 1.5,         # rad/s
    "gravity_noise": 0.05,          # 重力方向噪声
    
    # 外力扰动
    "push_force": [0, 50],          # N, 随机推力
    "push_interval": [4, 8],        # s, 推力间隔
}
```

**与 Ch14 DR 的对比**：HumanPlus 的 DR 范围比 Ch14 更保守（如 `push_force` 最大 50N vs Ch14 可能 100N+）。这是因为 HST 需要在**极其多样的动作**（40 小时 AMASS）上工作——如果 DR 太强，策略在困难动作（如单脚站立）上就无法收敛。
...
**为什么使用统一表示而不是针对每种物体设计不同的 obs？** 因为统一表示使策略能够**泛化到新物体**——训练时见过门和箱子，部署时可能遇到没见过的抽屉。如果每种物体用不同的 obs 结构，策略无法处理新物体。
...
```
视频 → 分别提取:
  人体: WHAM → SMPL → retarget → G1 关节角度
  物体: 
    1. 物体检测 (Detic/DINO) → bounding box
    2. 物体 6-DoF 估计 (BundleSDF / FoundationPose)
    3. 物体轨迹: (T, 3+4) position + orientation
  
  对齐: 确保人体和物体在同一个时间和坐标系下
...
⚠️ **编程陷阱：物体位姿估计的累积漂移**。视频中物体位姿的估计会随时间累积误差——特别是旋转估计。HDMI 论文使用了 BundleSDF 等方法来减少漂移，但不能完全消除。
...
| 9 | HDMI 物体跟踪漂移 | 累积位姿误差 | 使用 BundleSDF 或增加关键帧校正 | 16.8 |
...
bash fetch_demo_data.sh
python demo.py --video_path input.mp4 --output_dir results/ --save_pkl
```

**4DHumans**（推荐用于手部精度）：

```bash
git clone https://github.com/shubham-goel/4DHumans.git
cd 4DHumans && pip install -e .
python demo.py --video_path input.mp4 --output_dir results/
```
...
|------|---------|---------|---------|-----|
| WHAM | ~4cm | 无 | ✅ 好 | ~20 |
| 4DHumans | ~5cm | ~3cm | ⚠️ 弱 | ~15 |
| HybrIK | ~5cm | 无 | ⚠️ 弱 | ~30 |
| HaMeR | 无 | ~2cm | N/A | ~25 |

**组合推荐**：
- 全身运动（走、跑）：WHAM alone
- 全身+手部（遥操作）：WHAM + HaMeR
- 快速原型：4DHumans（全身+手部一体化，但精度略低）
