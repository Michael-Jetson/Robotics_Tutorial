...
**机械臂的运动改变了整体的重心（CoM）和角动量。** 一个 2-3 kg 的机械臂（如 ARX L5 Pro 含夹爪约 2.5 kg）安装在 12 kg 的 Go2 底盘上。当臂完全伸展时，CoM 从底盘中心向臂伸出的方向偏移约 5-8 cm。
...
**定量分析 CoM 偏移的影响**：以 Go2 + ARX L5 Pro 为例。Go2 底盘质量 ~12 kg，CoM 在几何中心。ARX L5 Pro 含夹爪 ~2.5 kg，臂长 45 cm。当臂完全水平伸出时：

$$\Delta x_{\text{CoM}} = \frac{m_{\text{arm}} \cdot l_{\text{arm}} / 2}{m_{\text{base}} + m_{\text{arm}}} = \frac{2.5 \times 0.225}{14.5} \approx 3.9 \text{ cm}$$

加上臂安装在底盘前方 15 cm（mount offset），CoM 总偏移约 $15 + 3.9 \approx 19$ cm 在前方。Go2 的前脚支撑线到底盘中心约 20 cm——CoM 偏移 19 cm 意味着几乎到了前支撑线边缘！如果此时机器人在行走（swing phase 只有 3 脚着地），支撑多边形缩小，CoM 可能超出——导致向前翻倒。

如果臂携带了额外负载（如抓起 0.5 kg 的物体），CoM 偏移进一步增大到 ~22 cm——已经超出前支撑线。策略必须学会用前腿更大的步幅和后腿的蹬力来补偿，或者让底盘微微后仰来平衡 CoM。
...
<mujoco model="go2_arx">
  ...
  <!-- 在底盘的 trunk body 上挂载臂 -->
  <body name="arm_mount" pos="0.15 0.0 0.12" parent="trunk">
    <include file="arx_l5_pro.xml"/>
  </body>
  ...
  <actuator>
    ...
    <position name="arm_joint_1" joint="arx_joint_1" kp="10" kd="0.5"/>
    <!-- ... 省略其他 5 个臂关节 ... -->
    
    <!-- 夹爪 -->
    <position name="gripper" joint="arx_gripper" kp="5" kd="0.2"/>
  </actuator>
</mujoco>
```

**`<include>` 的陷阱**：被包含文件中的所有命名（body、joint、geom）进入同一个命名空间。如果底盘和臂都有 `base_link` 这个 body 名称，会报命名冲突。解决方案：在 `<include>` 外层用 `<default class="arm">` 设置命名前缀，或在臂的 MJCF 中手动为所有名称加前缀。

**方式二：`<attach>` 框架挂载（MuJoCo 3.x）。** MuJoCo 3.x 引入了 `<attach>` 指令，允许将一个模型的 frame 挂载到另一个模型的 body 上。这比 `<include>` 更模块化——两个模型可以独立维护，只在运行时组合。
...
<mujoco>
  ...
  <!-- 在 trunk body 上创建一个 attachment site -->
  <body name="trunk">
    <site name="arm_mount_site" pos="0.15 0.0 0.12"/>
  </body>
  
  <!-- attach 臂模型到 mount site -->
  <attach model="arx_l5_pro" site="arm_mount_site"
          prefix="arm_"/>  <!-- 自动为所有臂的名称加 arm_ 前缀 -->
</mujoco>
```
...
这些 URDF 可以用 `mujoco.mjcf.load()` 转换为 MJCF，或用 Isaac Lab 的 URDF-to-USD 转换器转换为 USD。使用预组装模型的优势是安装位置、惯性参数和碰撞几何都已经验证过——从零自己组装时很容易忘记更新惯性参数。

**从 URDF 到 MJCF 的完整转换流程**：

```python
# Step 1: 加载 URDF 并转换为 MJCF
import mujoco

# 方法一：使用 mujoco 内置转换器
model = mujoco.MjModel.from_xml_path("go2_arx.urdf")
# 注意：URDF 的 continuous joint 会被转为 unbounded joint
# 需要手动加回 joint range

# 方法二：使用 mujoco 的 compile 工具
# 命令行：
# python -m mujoco.tool convert go2_arx.urdf go2_arx.xml
```
...
臂关节不需要对抗重力（臂的自重只有 2-3 kg，且大部分时间不在极端伸展姿态），但需要精确的位置控制——这需要"恰好够"的力矩。如果臂 kp 太大（如 40），微小的位置偏差会产生大力矩，导致臂运动过于"硬"和"抖动"。
...
actuators={
    "legs": ImplicitActuatorCfg(
        ...
        stiffness=40.0,
        damping=1.0,
        effort_limit=33.5,     # Go2 电机最大力矩 (Nm)
    ),
    "arm": ImplicitActuatorCfg(
        joint_names_expr=["arm_joint_.*"],
        stiffness={
...
复合模型验证 Checklist（10 分钟）：
  ...
  □ 自由度：打印 model.nq 和 model.nu，确认 = 底盘 DOF + 臂 DOF + 夹爪 DOF
...
1. **[工程题]** 从 awesome-loco-manipulation 下载 Go2-Arx URDF，用 `mujoco.mjcf.load()` 转换为 MJCF。打印关节名称列表，确认底盘和臂的关节都正确导入。
...
class LocoManipActionsCfg:
    ...
    gripper = BinaryGripperActionCfg(
        ...
        open_value=0.04,       # 完全打开时的关节角度
        close_value=0.0,       # 完全闭合时的关节角度
        threshold=0.0,         # 策略输出 > 0 → 打开，< 0 → 闭合
    )
```
...
**Running normalization（RSL-RL 内置）**：RSL-RL 的 `VecNormalize` wrapper 自动计算 obs 的 running mean 和 std，在线归一化。优势：不需要手动设定 scale。劣势：训练初期 mean/std 估计不准，可能引入不稳定。对 loco-manipulation 推荐使用 per-term scaling（更可控），不使用 running normalization。
...
这个"stop during grasp"reward 编码了一个关键的工程直觉：导航和操作不能同时发生——先导航到位，然后停下操作。如果不加这个 reward，策略可能学到"边走边抓"——虽然效率看起来更高，但抓取成功率很低。
...
```text
Stage 1: Low-level goal-reaching policy (RL)
  Input:  proprio(18 joints) + goal(base_vel + EE_target_pose)
  Output: 18 joint positions
  Training: PPO, 4096 envs, ~10k iters
  Key trick: hot-start from locomotion policy

Stage 2: High-level privileged teacher (RL)
  Input:  proprio + object_state(privileged)
  Output: base_vel_cmd(3D) + EE_target_pos(3D) + gripper(1D)
  Training: PPO with low-level frozen, ~5k iters

Stage 3: Visuomotor student (DAgger)
  Input:  proprio + depth_image(320×240→64×64)
  Output: same as Stage 2
  Training: DAgger from Stage 2 teacher, ~5k iters
```
...
标准 RL 通过 reward 惩罚"不良行为"（如关节速度过大）。但 reward 惩罚是"软约束"——策略可能发现在某些情况下"承受惩罚但获取更大 reward"的捷径。Constrained RL 将关节速度/力矩限制作为"硬约束"——策略在优化 reward 的同时必须满足约束。

$$\max_\pi \mathbb{E}[R] \quad \text{s.t.} \quad \dot{q}_i \le \dot{q}_{\max}, \quad \tau_i \le \tau_{\max}$$

**为什么操作比 locomotion 更需要约束？** 挥拍时的关节速度（可达 12 m/s 的末端速度 → 对应极高的关节角速度）可能超出电机的物理极限。如果仿真中不加约束，策略会学到"超出电机能力"的动作——在真机上执行时电机会过载保护或损坏。Constrained RL 确保策略在训练时就在电机的安全范围内操作。

**实现方式**：ETH 使用 Lagrangian relaxation——将约束转化为拉格朗日乘子项添加到 reward 中，乘子在训练过程中自适应调整。
...
| 约束 | 对象 | 限制值 | 来源 |
|------|------|--------|------|
| 关节角速度 | ANYmal 腿关节 | ±15 rad/s | 电机参数手册 |
| 关节角速度 | DynaArm 关节 | ±20 rad/s | 电机参数手册 |
| 关节力矩 | ANYmal 腿关节 | ±80 Nm | 电机参数手册 |
| 关节力矩 | DynaArm 关节 | ±30 Nm | 电机参数手册 |
| 末端速度 | 球拍 tip | ≤15 m/s | 安全考虑 |

### 核心工程贡献三：Shuttlecock Prediction Model

策略不直接使用相机检测到的球的当前位置——而是通过一个学习的预测模型估计球的飞行轨迹和拦截时间/位置。

```text
Prediction Model:
  Input: 最近 N 帧的球位置检测（含噪声）
...
预测模型是一个小 MLP，在仿真中用带噪声的球轨迹数据离线训练（不在 RL 循环内）。
...
| **底盘 obs** | `BaseAngularVelocityObsCfg` 等 | 同名（API 几乎一致） |
| **臂 obs** | `JointPositionObsCfg(joint_names=["arm_.*"])` | 同左 |
| **物体 obs** | `RigidObjectStateCfg` | `RigidObjectStateCfg` |
| **Height scan** | `RaycastSensorCfg` (BVH) | `RayCasterCfg` (PhysX) |
| **Depth camera** | `CameraObsCfg` (MuJoCo Warp Batch Renderer) | `TiledCameraCfg` (RTX) |
...
两个框架的 observation 和 reward API 高度一致（都基于 Manager-based 架构），主要差异在渲染管线（mjlab 的 MuJoCo Warp vs Isaac Lab 的 RTX）和模型格式（MJCF vs USD）。如果你在 mjlab 中开发了一个 loco-manipulation 策略，迁移到 Isaac Lab 主要是修改资产加载和渲染配置——observation、reward 和训练逻辑几乎不变。

### Isaac Lab 内置的 Loco-Manipulation 环境

Isaac Lab 2.3 提供了实验性的 loco-manipulation 环境（如 G1 humanoid 的 PickPlace-Locomanipulation），可以作为参考：

```python
# Isaac Lab 2.3 内置环境的使用方式
# 注意：这些是 humanoid (G1) 的环境，不是四足的
# 但其 env 配置模式可以直接复用
from isaaclab_tasks.envs import PickPlaceLocoManipG1EnvCfg

# 查看其配置结构：
# - observations: base_proprio + arm_proprio + object_relative_pose
# - actions: whole_body joint position (19D for G1 arms)
# - rewards: navigation + reaching + grasping + stability
# - curriculum: adaptive terrain difficulty

# 对于四足 + 臂，需要自定义环境——
# 但可以复用上面的 reward 和 obs 设计模式
```
...
VBC 使用共享编码器（更简单），VIRAL（NVIDIA 2025）使用分支编码器（更精细但需要更多训练计算）。对初学者推荐共享编码器——先跑通管线再优化架构。
...
| CoM 偏移量化 | Go2+ARX 臂伸出时 CoM 偏移 ~19cm，接近支撑多边形边缘 | ⭐⭐ |
...
| Constrained RL | 关节速度/力矩硬约束，Lagrangian relaxation 自适应乘子 | ⭐⭐ |
| Shuttlecock Predictor | 离线训练的轨迹预测 MLP，简化物理 + DR 覆盖 | ⭐⭐ |
...
| 验收项 | 预期结果 | 验证方法 |
|--------|---------|---------|
| 模型加载 | 19 DOF 正确，zero play 不碰撞 | 可视化 + 打印 model.nq |
...
| Gu et al. 2023, "M3: Mobile Manipulation with a Multi-Modal Model" (ICLR Oral) | ⭐⭐⭐ | 移动操作的 reward 设计和 region-goal 导航 |
| Sferrazza et al. 2024, "HumanoidBench" (RSS) | ⭐⭐ | 27-task benchmark，层级 RL vs 端到端 RL 的对比 |
| SLIM 2025, "Synergistic Learning for Integrating Mobile Manipulation" | ⭐⭐⭐ | 轮式移动操作的 DR 策略，特别是 arm mount perturbation |
...
**ETH 羽毛球论文精读建议**：重点关注 Section III-B（perception noise model 的标定方法——如何从真实相机数据中提取噪声参数并注入仿真）和 Section IV-A（constrained RL 的 Lagrangian 实现——拉格朗日乘子如何自适应更新）。Supplementary Material（如果可获取）包含关节约束值和 DR 参数范围——这些数值对你自己的 constrained RL 实现有直接参考价值。注意论文中提到的"whole-body asymmetric actor-critic with no architectural split"——这是端到端训练成功的关键，不要误读为"需要分离底盘和臂的网络"。

**M3 论文精读建议**：M3（ICLR 2023 Oral）的核心贡献是 region-goal navigation reward——不要求机器人精确到达一个点，而是到达一个"区域"就给正 reward。这个思想对 loco-manipulation 很重要——底盘不需要精确到位，只需要让物体进入臂的工作空间即可。重点关注 Section 3.2 的 reward 设计和 Table 2 的消融实验——论文用消融证明了 region-goal 比 point-goal 在移动操作中收敛更快。M3 报告的 cross-configuration 成功率为 71.2%，cross-layout 为 55.0%——这些数值可以作为你自己实验的参考基准。

**SLIM 论文精读建议**：SLIM（2025）的独特贡献是 arm mount perturbation DR——在 reset 时随机扰动臂的安装位置和朝向。这是一个在其他论文中很少提到但对 sim-to-real 极其重要的 DR 维度。论文还提到"PID arm control rather than PD"——因为"PD will always have gravity-induced tracking errors"。如果你的真机臂使用 PD 控制，SLIM 的这个观察值得特别注意。
...
| 症状 | 可能原因 | 排查步骤 | 相关章节 |
|------|---------|---------|---------|
| 模型加载时关节数不对 | URDF→MJCF 转换丢失 fixed joint | 1. 打印 model.nq 2. 对比 URDF 的 joint 数量 3. 检查转换日志 | 19.2 |
