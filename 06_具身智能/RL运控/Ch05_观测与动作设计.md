...
| 如果把 critic 的 obs_groups 也指向 actor group，训练会崩溃吗？ | 检查是否理解 obs_groups routing 的隐蔽 bug |
| MuJoCo 使用 wxyz 四元数约定，这意味着 `quat[0]` 是什么分量？ | 检查是否知道跨框架坐标系约定差异 |
| `base_lin_vel` 在真实四足机器人上从哪个传感器获取？ | 检查是否理解部署可得性的核心困难 |
| `base_lin_vel` 在真实机器人上如何获取？这对 obs 设计有什么影响？ | 检查是否理解可部署性约束 |
| 如果把 critic 的 `obs_groups` 也指向 actor group 会怎样？ | 检查是否理解 obs_groups routing |
...
3. **运用** observation 处理管线（compute → noise → clip → scale → delay → history）诊断训练失败和部署偏移
...
考虑一个四足机器人行走的例子。完整的物理状态 $s$ 包括：机器人 37 维配置和速度（18 + 19，含浮动基座）、所有接触点的力和位置、地形的精确几何形状、空气阻力、关节摩擦系数等。但 actor observation $o$ 通常只有约 48 维——IMU 角速度（3）、重力投影（3）、关节位置（12）、关节速度（12）、上一拍动作（12）、速度命令（3），再加可选的地形扫描。大量状态信息被"遮挡"了：接触力不直接可观、地形精确形状未知、关节摩擦是隐藏参数。
...
observation 维度不是越高越好——这个直觉来自信息论中的 rate-distortion 理论。对于 MLP 策略，第一层参数 = input_dim × hidden_dim。假设 hidden_dim = 512：

| obs 维度 | 第一层参数量 | rollout 存储（4096 envs × 24 steps × float32） | 学习效率影响 |
|---------|-----------|----------------------------------------------|------------|
| 48 | 24,576 | 18.4 MB | 基线 |
| 96 (2× history) | 49,152 | 36.9 MB | 略有下降 |
| 192 (4× history) | 98,304 | 73.7 MB | 明显下降 |
| 480 (10× history) | 245,760 | 184.3 MB | 显著下降 |
...
**从 legged_gym 到 manager-based 的演进**。legged_gym（Rudin et al., CoRL 2021, `github.com/leggedrobotics/legged_gym`，~2.8k Stars）是四足 RL 训练的里程碑，确立了 velocity tracking 的标准 obs 集合（base velocity, angular velocity, gravity projection, joint state, last action, command）和 actor/critic 分组。但 legged_gym 的工程痛点在于：所有逻辑混在一个 `legged_robot.py` 中——observation 计算、reward 计算、reset 逻辑、domain randomization 交织在一起，导致修改一个 reward 可能影响 observation，添加一个 sensor 需要改动 5 个文件。legged_gym 现已官方废弃（README 明确声明"this repository will receive limited updates and support"），其精神继承者是 Isaac Lab 的 manager-based 架构和 mjlab 的等价设计。
...
Pinto et al. 2017 (Asymmetric Actor Critic for Image-Based Robot Learning, RSS) 为"critic 可以看更多"提供了数学基础——actor 只看 image，critic 看 full state。sim-to-real 的工程约束（传感器延迟、执行器饱和、通信带宽）则要求仿真中的 observation/action 接口必须尊重真实硬件的信息边界。
...
**案例五：盲目添加 history。** 训练不稳定时直接把 `history_length` 从 1 增大到 10。history 确实可以缓解部分可观测性问题，但它按倍数放大 observation 维度（48 维 × 10 = 480 维），急剧增加网络参数和显存。如果训练不稳定的根因是 action scale 错误或 reward 设计缺陷，history 只会掩盖问题而非解决问题。
...
observation 维度直接影响网络参数量和样本效率。对于 MLP 策略，第一层参数 = input_dim × hidden_dim。从 48 维增加到 480 维（10 帧 history），仅第一层就增加 10 倍参数。除非信息增益显著超过维度成本，否则应优先选择低维表示。
...
> **这一节解决什么问题**：完整理解从物理状态到策略输入的六步处理链 compute → noise → clip → scale → delay → history，在 mjlab 和 Isaac Lab 中进行源码级对比。

### 处理链总览 ⭐⭐

`ObservationTermCfg` 的 docstring 写明了处理顺序，`compute_group()` 严格按以下顺序执行：

```text
compute → noise → clip → scale → delay → history
```

**为什么是这个顺序？** 想象一个信号从物理世界流向策略网络的管线。compute 阶段从仿真状态中提取原始信号（"世界是什么样的"）。noise 阶段模拟传感器的测量误差（"传感器看到了什么"）。clip 阶段防止极端值击穿后续处理（"合法的测量范围是什么"）。scale 阶段把有物理量纲的值映射到网络友好的数值范围（"数值怎样表达更利于学习"）。delay 阶段模拟传感器和通信延迟（"策略拿到的信息有多旧"）。history 阶段把当前帧和历史帧拼接在一起（"策略能看到多长的时间窗口"）。
...
| delay | `DelayBuffer` (per-term) | 通过自定义 wrapper | Isaac Lab 默认无内置 delay |
| history | `CircularBuffer` | 通过自定义 wrapper | mjlab 内置 history 支持 |

这个对比揭示了一个重要的工程差异：mjlab 在 `ObservationTermCfg` 层面就内置了 delay 和 history 支持（`DelayBuffer` 和 `CircularBuffer` 直接配置在 term cfg 中），而 Isaac Lab 的默认 `ObsTerm` 不包含这些功能——需要通过自定义 observation function 或 wrapper 实现。这不是功能缺失，而是设计哲学差异：Isaac Lab 倾向于把复杂处理留给用户在 observation function 内部实现，mjlab 倾向于把常用处理标准化为 cfg 选项。
...
**CLI 差异。** mjlab 使用 `tyro`（一个从 dataclass 自动生成 CLI 的库）：`uv run train Mjlab-Velocity-Flat-Unitree-Go1 --env.scene.num-envs 4096`。Isaac Lab 使用传统的 argparse + hydra 风格：`python scripts/rsl_rl/train.py --task Isaac-Velocity-Flat-Anymal-C --num_envs 4096`。两者的参数覆盖逻辑不同——mjlab 的 `tyro` 可以覆盖任意嵌套 dataclass 字段（如 `--agent.algorithm.clip-param 0.3`），Isaac Lab 需要通过 `--overrides` 或修改 yaml 文件。在 smoke test 时，确认 CLI 覆盖实际生效——打印最终 cfg 验证。
...
Isaac Lab 中如果需要 delay 和 history，常见做法是在自定义 observation function 中维护 buffer，或使用社区扩展包。这种差异意味着：如果你的任务需要 delay/history 作为核心功能，mjlab 的配置化方式更方便；如果你的任务不需要这些，两个框架的 observation 管线几乎等价。
...
actions = {
    "joint_position": JointPositionActionCfg(
        asset_name="robot",
        joint_names=[".*"],           # 匹配所有关节（正则表达式）
        scale=0.25,                    # raw_action * 0.25 + default_pos
        use_default_offset=True,       # offset = default joint position
        # clip 配置（processed action 级别）
        clip={"lower": None, "upper": None},  # 使用关节限位自动 clip
    ),
}
```
...
def verify_action_config(env):
    ...
    for name, term in am.active_terms.items():
        print(f"\n  Term: {name}")
    ...
    processed = am.action
    print(f"\nZero action → processed action range: "
...
**Step 1（物理范围估计）**：从 default pose 出发，计算每个关节到限位的距离。取最小值的一半到三分之一作为 scale 的起点。例如，髋关节 default=0.1，限位=[-0.8, 0.8]，到限位距离 min(0.9, 0.7)=0.7，初始 scale ≈ 0.7/3 ≈ 0.23。
...
4. **[跨章综合题]** `init_std`（RSL-RL 5.x 的 `GaussianDistributionCfg` 参数）和 `action_scale` 共同决定了初始探索的物理幅度。如果 Go1 的 `action_scale=0.25` 和 `init_std=1.0`，计算训练初期 95% 的 processed action 变化范围。如果你想让 95% 范围控制在 ±0.1 rad 以内（如精细操作任务），应该怎样调整 `action_scale` 和 `init_std`？给出至少两种方案并分析各自的利弊。
...
| 训练 throughput | 基线 | ~2× sim cost | 视任务需要决定 |
...
4. **[跨章综合题]** reward × step_dt 归一化意味着 reward 的绝对值与 decimation 无关。但 GAE 中的 $\gamma^l$ 衰减仍然以 env step 为单位。如果把 decimation 从 4 改成 2（env step 数量翻倍），同一段物理时间对应的 GAE 窗口中会有更多的 step，有效 discount 变化了吗？讨论这个变化对 PPO 训练的影响（提示：考虑 effective horizon 的变化）。
...
当前 mjlab 的 ONNX 导出路径在 `MjlabOnPolicyRunner.export_policy_to_onnx()`。Isaac Lab 使用 RSL-RL 的相同导出机制（`OnPolicyRunner.export_policy()`）。ONNX 通常包含 actor 网络推理（MLP 权重 + activation），可能包含 normalizer。ONNX 不包含环境、manager、传感器驱动和安全逻辑。
...
```bash
# 使用 play 脚本导出
python scripts/rsl_rl/play.py --task Isaac-Velocity-Flat-Anymal-C \
...
# Isaac Lab: 训练环境数量
python scripts/rsl_rl/train.py --task Isaac-Velocity-Rough-Anymal-C --num_envs 4096
# Isaac Lab: 播放
python scripts/rsl_rl/play.py --task Isaac-Velocity-Rough-Anymal-C --num_envs 4
```
...
# 实验 3: zero agent
python scripts/rsl_rl/play.py --task Isaac-Velocity-Flat-Anymal-C \
  --num_envs 4 --load_run zero_agent

# 实验 5: CPU smoke train
python scripts/rsl_rl/train.py --task Isaac-Velocity-Flat-Anymal-C \
...
def print_obs_action_stats(env, num_steps=10):
    ...
    for step in range(num_steps):
        ...
        # 打印 action 统计
        processed = env.action_manager.action  # processed action
        print(f"processed action range: [{processed.min():.3f}, {processed.max():.3f}]")
...
**D. History and delay：**
- delay lag 换算成毫秒（lag × env_step_dt）
...
- 控制频率与训练一致（decimation × physics_dt）
...
这个结构遵循一个简单规则：actor terms ⊂ critic terms。critic 永远是 actor 的超集——它看到 actor 看到的一切，再加上额外的 privileged 信号。在 mjlab 中，这个结构自然落入 `actor` 和 `critic` 两个 group；在 Isaac Lab 中，落入 `policy` 和 `critic` 两个 group。
...
| obs dim × hidden dim | < 500K（第一层参数） | > 1M → 考虑降维或 term-level scale |
...
| **Actor obs 典型组成** | ang_vel(3) + gravity(3) + joint_pos(12) + joint_vel(12) + last_action(12) + cmd(3) = 45D | joint_pos(7) + joint_vel(7) + ee_pos(3) + ee_quat(4) + obj_pos(3) + obj_quat(4) + goal_pos(3) + last_action(8) = 39D |
| **Critic 额外 privileged** | contact forces, terrain height, true base vel | true object velocity, contact state, gripper force |
| **Action type** | JointPosition (12D) | JointPosition(7D) + Gripper(1D) |
...
1. `HOVER/humanoid/envs/hover_env_cfg.py` — 多模态 obs group 和 mask 定义
2. `HOVER/humanoid/envs/observations.py` — 自定义 obs term 实现（mask 生成、目标编码）
3. `HOVER/train/distillation.py` — mask-conditioned distillation 的 teacher-student 训练循环
4. 重点阅读：mask 维度与目标维度的对应关系、蒸馏 loss 的实现细节
...
| 处理链顺序 | compute → noise → clip → scale → delay → history | ⭐⭐ |
...
| obs_groups routing | tuple 可拼接多 group；映射错误不崩溃但效果差 | ⭐⭐ |
...
- 任务：Mjlab-Velocity-Flat-Unitree-Go1 / Isaac-Velocity-Flat-Anymal-C
- GPU：_____________
- 日期：_____________
...
| 资料 | 难度 | 本章关联 |
|------|:---:|---------|
| Pinto et al. 2017, "Asymmetric Actor Critic for Image-Based Robot Learning" (RSS) | ⭐⭐⭐ | asymmetric actor-critic 原始论文 |
...
| Schwarke et al. 2025, "RSL-RL: Fast and Simple RL for Robotics" (arXiv 2509.10771) | ⭐⭐ | RSL-RL 5.x 架构、obs_groups routing、PPO 实现细节 |
