...
📋 **答不出 ≥ 2 题 → 建议先浏览 MuJoCo 官方文档和一篇 PPO 教程**
...
> 这个类比有明确的**边界**：编译器通常给出确定性的错误信息，而仿真器给出的是对物理世界的**近似**。编译通过 ≈ 代码语法正确；仿真成功 ≠ 真机成功。仿真能降低成本，但不会消除 sim-to-real gap。理解这个边界是后续所有章节的前提。
...
| 单环境实时运行时间 | $10^8 / 50 = 2 \times 10^6$ 秒 ≈ **23 天** | 不停机、不维修 |
| 4096 并行环境 | $2 \times 10^6 / 4096 \approx 500$ 秒 ≈ **8 分钟** | GPU 并行的数量级优势 |
...
|------|------|-----------|--------|-----|---------|
| legged_gym (CoRL'21) | ANYmal 速度跟踪 | ~$2 \times 10^9$ | 4096 | 1×RTX 3090 | ~20 分钟 |
| walk-these-ways (RSS'23) | Go1 多步态 | ~$5 \times 10^9$ | 4096 | 1×A100 | ~1 小时 |
| ProtoMotions | G1 全身 motion tracking | ~$10^{10}$ | 4096×4 GPU | 4×A100 | ~12 小时 |
| KungfuBot (NeurIPS'25) | G1 功夫动作 | ~$10^{10}$ | 4096 | 1×A100 | ~8 小时 |
...
MuJoCo（Multi-Joint dynamics with Contact）由 Emo Todorov 在 2012 年发布，最初是闭源商业软件（年许可费约 $500）。2021 年 Google DeepMind 收购 Roboti LLC 后将 MuJoCo 完全开源（Apache 2.0 许可），这对整个社区产生了深远影响——之前很多学生和小团队因为许可费而转向其他引擎（如 PyBullet），开源后 MuJoCo 迅速成为机器人 RL 最广泛使用的物理引擎。
...
Isaac Gym 对社区的影响是巨大的。ETH RSL 的 `legged_gym`（CoRL 2021，*Learning to Walk in Minutes*）在 Isaac Gym 上实现了四足 locomotion 的标准训练范式，后续几乎所有足式机器人 RL 论文都以它为基础。直到 2026 年，`legged_gym` 仍有 2.8k GitHub Stars，是引用最多的开源项目之一。
...
NVIDIA 在 2024 年发布 Isaac Lab（论文：*Isaac Lab: A GPU-Accelerated Simulation Framework for Multi-Modal Robot Learning*），作为 Isaac Gym 的继任者。它的核心创新是 **Manager-Based 架构**——把 MDP 的每个组件（obs、action、reward、termination、event、command、curriculum）拆分到独立的 Manager 中，实现了关注点分离。
...
| 对比 | 任务类型 | 硬件 | 加速比 |
|------|---------|------|--------|
| MuJoCo Warp vs MJX | Locomotion | RTX PRO 6000 Blackwell | **252×** |
| MuJoCo Warp vs MJX | Manipulation | RTX PRO 6000 Blackwell | **475×** |
| MuJoCo Warp vs MJX | Locomotion | RTX 4090 | 152× |
| MuJoCo Warp vs MJX | Manipulation | RTX 4090 | 313× |
...
> **2021** Isaac Gym 发布（NVIDIA，NeurIPS'21）— └ legged_gym（ETH RSL，CoRL'21，2.8k Stars）— 痛点：单体架构，代码不可组合  
...
**rsl_rl ≥ 4.0 的核心变化**：从 4.0 版本开始，actor 和 critic 的配置被**拆分**为独立的 model config——这允许 actor 用 MLP 而 critic 用 CNN（非对称 actor-critic），是视觉策略和 teacher-student 蒸馏的工程基础：
...
| "框架 = 物理引擎" | 框架（mjlab/Isaac Lab）≠ 物理引擎（MuJoCo Warp/PhysX）≠ RL 算法库（RSL-RL） |
...
| "RSL-RL 只有 PPO" | rsl_rl ≥ 4.0 支持 PPO + Distillation + RND + Symmetry Augmentation |
...
8. RSL-RL 除了 PPO 还支持什么功能？rsl_rl ≥ 4.0 的核心变化是什么？
...
| rsl_rl | **≥ 4.0.0** | 稳定 | 新 config 格式（分离 actor/critic） |
| Newton | **1.0 GA** | 稳定（引擎） | Isaac Lab 集成仍 Beta |
| MuJoCo Warp | 随 mjlab 1.2.0 | 稳定 | Google DeepMind + NVIDIA 共同维护 |
| PyTorch | ≥ 2.0 | — | mjlab 需 CUDA；Isaac Lab 3.0 需 2.10.0+cu128 |
| Python | ≥ 3.10 | — | Isaac Lab 3.0 需 3.12 |
...
| 9 | GPU 显存估算 | max_envs ≈ (VRAM - 2GB) / per_env_mem | 1.2 | ⭐⭐ |
...
| 21 | 框架-引擎-工具分层 | 物理引擎 ≠ 框架 ≠ RL算法库 ≠ 可视化 | 1.5 | ⭐ |
| 22 | RSL-RL ≥ 4.0 | PPO + Distillation + RND + Symmetry，拆分 actor/critic config | 1.5 | ⭐⭐ |
...
| 26 | 全书知识树 | 7 Part × 28 章的递进和平行关系 | 1.8 | ⭐ |
...
| Ch07 | 完成一次 4096 env × 3000 iter 的正式训练 |
...
| Borse et al., *ComFree-Sim*, arXiv 2603.12185 | ⭐⭐⭐ | 密集接触场景的替代接触模型，比 MuJoCo Warp 快 2-3× |
...
| "Newton/ComFree-Sim 是什么" | 看到新名词不知道是否需要学 | 生态更新快 | Newton = 多求解器统一引擎（Isaac Lab 3.0 Beta）；ComFree-Sim = 新的密集接触求解器（2026，比 MuJoCo Warp 在密集接触场景快 2-3×）。当前不需要学，了解定位即可 |
