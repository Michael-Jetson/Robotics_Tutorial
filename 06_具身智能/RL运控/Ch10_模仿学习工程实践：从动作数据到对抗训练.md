...
| 维度 | 显式跟踪 | 对抗风格 | BC 蒸馏 |
|------|---------|---------|--------|
| 核心 reward | 帧级 MSE | 判别器 logit | MSE(student, teacher) |
...
- $r_{\text{style}} = -\log(1 - D(s_t, s_{t+1}))$：来自判别器的风格 reward
...
class AMPDiscriminator(nn.Module):
    ...
    def forward(self, s_t: torch.Tensor, s_tp1: torch.Tensor) -> torch.Tensor:
        """
        输入两帧状态，输出 logit（未经 sigmoid）。
        logit > 0 → 判别器认为是"真"的（来自参考数据）。
        """
...
def amp_style_reward(
    ...
    # 判别器前向
    with torch.no_grad():
        logits = discriminator(obs_prev_norm, obs_t_norm)
        prob = torch.sigmoid(logits)
        reward = -torch.log(1.0 - prob + 1e-7)
...
class AMPDiscriminatorTrainer:
    ...
    def compute_disc_reward(self, s_t: torch.Tensor,
        ...
        with torch.no_grad():
            ...
            logits = self.disc(s_t, s_tp1)
            # AMP reward = -log(1 - sigmoid(logit))
            prob = torch.sigmoid(logits)
            reward = -torch.log(1.0 - prob + 1e-7)
        return reward.squeeze(-1)
    ...
    def update(self, reference_data: torch.Tensor) -> dict:
        ...
        # ---- 判别器损失 ----
        fake_logits = self.disc(fake_s, fake_sp)
        real_logits = self.disc(real_s, real_sp)

        # Binary cross-entropy (logit form)
        fake_loss = nn.functional.binary_cross_entropy_with_logits(
            fake_logits, torch.zeros_like(fake_logits)  # fake → label 0
        )
        real_loss = nn.functional.binary_cross_entropy_with_logits(
            real_logits, torch.ones_like(real_logits)    # real → label 1
        )
        ...
        real_logits_gp = self.disc(real_s, real_sp)
        grad = torch.autograd.grad(
            real_logits_gp.sum(), [real_s, real_sp],
            create_graph=True, retain_graph=True,
        )
