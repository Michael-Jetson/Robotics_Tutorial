...
**复杂度**。$H_{ll}$ 块对角，每块 3×3 求逆 $O(n)$；形成 $S$ 的代价 $O(n\cdot m_j^2)$（$m_j$ 为第 $j$ 点被观测的相机数）；最终只需解 $6m$ 维稠密（或稀疏）方程。**总复杂度** $O(m^3 + nm^2)$，相比不 Schur 的 $O((6m+3n)^3)$ 可降低数个数量级。
...
- **Block-Jacobi**：按变量分块对角，6×6 for pose + 3×3 for landmark。
...
|---|---|---|---|---|
| **Dense QR / Cholesky** | 直接 | 稠密小问题 | ≤ 数百变量 | Eigen, LAPACK |
...
7. **Pose3 切向量顺序不一致**。GTSAM 用 $[\omega;\rho]$（旋转在前），Ceres/Eigen 约定常是 $[\rho;\omega]$。跨库桥接协方差/Jacobian 时必须做 6×6 permutation。
