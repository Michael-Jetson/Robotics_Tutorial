...
├── Q1: 有约束吗？
│   ├── 是 → Q2（规模 × 精度）
│   │   ├── n,m ≤ 10³ 且 ε ≤ 10⁻⁸ → IPM (ECOS/Mosek)
│   │   ├── MPC 结构（block-tridiagonal）→ HPIPM (Riccati IPM)
│   │   ├── n,m ∈ [10³,10⁵] QP 且 ε ≈ 10⁻⁴ → OSQP (ADMM)
│   │   ├── n,m ≥ 10⁵ → SCS (ADMM on cones)
...
```
输入：self-concordant barrier φ，初始可行内点 x⁰，参数 μ > 1
1. 初始化 t = t₀ > 0
2. while m/t > ε:
    a. Centering：用 Newton 法求解 x*(t) = argmin[t·f₀(x) + φ(x)]
    b. 更新：t ← μ·t
3. 返回 x*(t)
```
...
问题：min f(x)，f L-光滑凸
├── n ≤ 10³, f 二次 → CG（n 步精确）
├── n ≤ 10³, f 非二次 → L-BFGS（超线性收敛）
├── n > 10⁴, 已知 μ → Nesterov AGD（O(√κ log(1/ε))）
├── n > 10⁴, μ 未知 → AGD + 自适应重启
└── 有限和 f = Σfᵢ → SVRG/SAGA（if n 大）or GD（if n 小）
```
...
├── g = δ_C (约束) → 投影梯度 or 加速投影
│   ├── 投影便宜（Box, Ball）→ 加速 PGD
│   └── 投影贵（核范数球）→ Frank-Wolfe
├── g = λ||·||_* (核范数) → FISTA-SVT or FW
...
```
问题：min f₀(x) s.t. fᵢ(x) ≤ 0
├── LP → HiGHS/Gurobi（单纯形 or IPM）
├── QP, n ≤ 10³ → qpOASES（active-set, 精确）
├── QP, MPC 结构 → HPIPM（Riccati IPM）
├── QP, n > 10³ → OSQP（ADMM）
├── SOCP → ECOS（IPM）or Clarabel
├── SDP, n ≤ 10² → Mosek（IPM, 高精度）
├── SDP, n > 10² → SCS（ADMM, 低精度）
