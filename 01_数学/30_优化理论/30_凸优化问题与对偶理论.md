...
**定理**（Nesterov, Nemirovsky 1994）：对 $n$ 维凸优化问题，任何一阶方法至少需要 $\Omega(\sqrt{n/\epsilon})$ 次梯度计算才能达到 $\epsilon$ 精度。
...
```
初始化: 选择可行点 x_0 和初始工作集 W_0 ⊆ {1,...,m}
for k = 0, 1, 2, ...:
    1. 解等式约束子问题:
       min ½d'Pd + (Px_k+q)'d
       s.t. a_i'd = 0, i ∈ W_k
       得到搜索方向 d_k 和对偶变量 λ_k

    2. if d_k = 0:  # 当前工作集下已最优
       if all λ_i ≥ 0 for i ∈ W_k:
           STOP: x_k 是最优解
       else:
           找 j = argmin_{i∈W_k} λ_i
           W_{k+1} = W_k \ {j}  # 移除最负的乘子对应的约束

    3. else:  # d_k ≠ 0, 沿 d_k 方向走
       计算最大步长 α_max = min_{i∉W_k, a_i'd_k>0} (b_i - a_i'x_k)/(a_i'd_k)
       x_{k+1} = x_k + α_k * d_k
       if 某个约束变 active:
           W_{k+1} = W_k ∪ {该约束}
```
