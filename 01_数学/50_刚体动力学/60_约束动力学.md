...
2. **灵敏度计算**：$\partial x_{k+1}/\partial u_k$，$\partial x_{k+1}/\partial x_k$——就是 §12 的 IFT on KKT
...
### 19.2 实验 2：验证 Drift-off O(t^2) 增长
...
### 19.3 实验 3：Schur vs Dense KKT 性能对比
...
benchmark_solvers(30, 12)  # humanoid ���型
...
**符号计算的价值**：在推导 KKT 系统变体（如含 Baumgarte 项、含摩擦锥线性化）时，手推容易出错。用 SymPy ���证每一步的符号正确性，特别是 $\dot J\dot q$ 的展开（容易漏项）。
...
### 9.5 Gauss 原理与 Udwadia-Kalaba 方程
...
### 9.7.3 迭代求解器（CG/MINRES）在大规模场景中的应用
...
## §10 Vakonomic vs Nonholonomic 力学 ⭐⭐⭐⭐
...
## §11 时步法深入：Stewart-Trinkle 与 Moreau 格式 ⭐⭐⭐⭐
...
## §13 多体接触图与 Island Decomposition ⭐⭐⭐
...
| 50_动力学微分 → 解析导数 | §12 IFT on KKT | KKT 分解复用于梯度计算 |
| 10_变分法 → Euler-Lagrange | §3.2 Lagrange 乘子引入 | 受约束变分问题 |
| 70_辛结构 → Lie-Poisson 约化 | §10 Chaplygin 约化 | 非完整约化的几何根源 |
