...
1. 写出 SE(3) 的矩阵表示形式，并解释为什么 SE(3) 不是 SO(3) 和 R^3 的直积而是半直积？
2. 给定 T = (R, p) in SE(3)，写出 Ad_T 的 6x6 矩阵显式形式（区分 angular/linear 排列）。
3. 解释 Featherstone 的 spatial velocity (motion vector) 对应李代数 se(3) 的哪个元素？body frame 还是 spatial frame？
...
学完本章后，你将能够：
1. **从 Hamilton 原理出发完整推导 Euler-Poincare 方程**，理解约束变分 delta xi = eta_dot + [xi, eta] 的几何本质
2. **写出浮基机器人在 SE(3) x T^n 上的完整动力学方程**，理解 nq != nv 的根本原因
...
如果你选四元数：4 个分量 + 3 个平移 = 7 维表示，但切空间只有 6 维（因为四元数有 |q|=1 的约束）。这意味着**位形维度 7+n 不等于速度维度 6+n**——标准的 Euler-Lagrange 方程 d/dt(dL/dq_dot) - dL/dq = tau 直接失效，因为 q_dot 和广义力 tau 的维度不匹配。

> **本质洞察**：浮基机器人的配置空间天然是 SE(3) x T^n，不是 R^{6+n}。这不是表示的选择问题，而是拓扑结构的硬约束——SO(3) 不可能无奇异地被 R^3 全覆盖（"毛球定理"的推论）。
...
当 theta = pi/2 时，这个矩阵奇异！物理上角速度有限，但欧拉角速率趋于无穷。

**问题 2：伪张量**。用欧拉角写出的质量矩阵 M(q) 包含大量 sin/cos 项，Christoffel 符号 Gamma^k_{ij} 在不同参数化下形式完全不同。你花三天手推出来的公式，换一种欧拉角约定就全部作废。

**问题 3：维度不匹配**。即使勉强用四元数避免奇异，q in R^{7+n} 但 v in R^{6+n}，你必须引入约束 |q_quaternion| = 1，把问题变成 DAE（微分代数方程）。Pinocchio 的解决方案是定义 `integrate(model, q, v*dt)` 使用 Lie 群指数映射，彻底绕过这个问题。

### 几何力学的"一劳永逸"方案

Euler-Poincare 方程直接在 body frame 中工作，用李代数元素 xi = g^{-1} g_dot in se(3) 代替 q_dot，用余伴随作用 ad*_xi 代替 Christoffel 项。它**天然避免表示奇异**——因为根本不使用任何参数化。

用一个类比来说明：

> **跨领域类比**：想象你要描述地球表面一个人的运动。方案 A：用经纬度——在北极（经度未定义）失效。方案 B：直接用人"脚下"的局部切平面——永远有效，因为切空间在每一点都是 R^2。Euler-Poincare 方程就是方案 B 的力学版本：它始终在 body frame 的切空间（李代数）中工作。
...
刚体动能只依赖于角速度和质心线速度——不依赖于刚体在空间中的具体位置和朝向。用数学语言说：动能 T(g, g_dot) 在 G 的左平移作用下不变。这个对称性是 Euler-Poincare 约化（reduction）的起点。
...
这里 Ad_g: g -> g 是 G 的**伴随表示**（adjoint representation）。

> **本质洞察**：body velocity 描述的是"站在刚体上观察自己的运动"——你感受到的角速度和线速度；spatial velocity 描述的是"站在固定参考系中观察刚体运动"。两者通过 Ad_g 变换，**不相等**！区分它们是整个几何力学的第一条生命线。
...
设 R in SO(3)，则 R_dot = R hat{omega_b}（body frame）= hat{omega_s} R（spatial frame），即 omega_s = R omega_b。

刚体动能 T = 1/2 omega_b^T I omega_b 只用 body 角速度写出——这正是 left-invariant Lagrangian 的原型：T(R, R_dot) = T(e, R^{-1} R_dot) = T(e, hat{omega_b}) = l(omega_b)。

### 4.2.4 Left-invariant Lagrangian 的约化

对一个配置在 Lie 群 G 上的系统，设完整 Lagrangian L: TG -> R。若 L 是**左不变的**，即对所有 h in G：

$$L(h \cdot g,\ h \cdot \dot{g}) = L(g,\ \dot{g})$$

则 L 只依赖于 body velocity xi = g^{-1} g_dot，因此**descend**到李代数 g 上：

$$\ell: \mathfrak{g} \to \mathbb{R}, \quad \ell(\xi) := L(e, \xi) = L(g, g\xi)$$

这个 l 称为**约化 Lagrangian**（reduced Lagrangian）。

**为什么这很重要**？原始 Lagrangian L 定义在 TG（切丛）上——对 SE(3) 这是 12 维空间。约化后 l 定义在 g（李代数）上——只有 6 维。这就是"约化"的威力：**利用对称性降维**。

### 4.2.5 与 Pinocchio 的对应

Pinocchio 中：
- `pinocchio::MotionTpl<Scalar>` 对应 se(3) 元素（xi = (omega, v)）
- `data.v[i]` 存储的是关节 i 的 body frame velocity
- `SE3::act(Motion)` 执行 Ad_T 变换
- `SE3::actInv(Motion)` 执行 Ad_{T^{-1}} 变换
...
>
> **实际上**：v_b = R^T dot{p} 确实是质心线速度在 body frame 中的表达。但 spatial velocity 中的 v_s = dot{p} - omega_s x p 并**不是**质心线速度在 world frame 中的表达！它是"空间参考点（原点固定）处的速度"。这是 Featherstone 6D spatial velocity 最容易混淆的地方。
>
> **正确理解**：spatial velocity 是 se(3)* 对偶配对意义下的对象，其物理意义是"对偶于 wrench 在计算功率时的对应量"。
...
在标准 Lagrange 力学中，Hamilton 原理要求 delta S = delta integral L dt = 0，其中变分 delta q 是**自由的**（端点为零即可）。但在 Lie 群上，情况发生了根本变化：约化后的变分 delta xi **不是自由的**——它被一个约束方程锁死。理解这个约束，是推导 Euler-Poincare 方程最精妙也最关键的一步。

### 4.3.1 两参数族与 Maurer-Cartan 形式

考虑 G 中的一个两参数族 g(t, epsilon)：
- t 是时间参数
- epsilon 是变分参数（epsilon = 0 时为真实运动，delta/delta epsilon|_{epsilon=0} 给出变分方向）
...
在 epsilon = 0 处：xi(t, 0) 就是真实运动的 body velocity，eta(t, 0) 就是变分方向（在 body frame 中表达）。
...
其中 delta xi := d xi/d epsilon|_{epsilon=0}，dot 表示 d/dt。

**完整证明**（对矩阵 Lie 群直接验证）：

**Step 1**：计算 d/d epsilon (g^{-1})。

由 g g^{-1} = I 两侧对 epsilon 求导：
...
**Step 2**：对 xi = g^{-1} partial_t g 关于 epsilon 求导：
...
这个引理告诉我们一个深刻的事实：在 Lie 群上，变分 eta 可以任意选取（只要端点为零），但由此引起的速度变分 delta xi **不是自由的**——它被 eta 通过 delta xi = dot{eta} + ad_xi eta 完全确定。

> **跨领域类比**：想象一根不可伸长的绳子约束了两个质点。你可以自由移动一个质点（类比于自由选取 eta），但另一个质点的运动（类比于 delta xi）被绳子约束锁死。在 Lie 群上，"绳子"就是群结构本身——它把位形变分和速度变分绑在一起。

**反事实推理**：如果 delta xi 是自由的（像 R^n 中那样），那么 Hamilton 原理直接给出标准 Euler-Lagrange 方程 d/dt(dl/d xi) - dl/d... 但等等，约化后的 l(xi) 不依赖于"位形"——所以如果 delta xi 自由，我们只会得到 d/dt(dl/d xi) = 0，即动量守恒。这对自由刚体的**空间**角动量确实成立，但对 **body** 动量则不成立（body 动量 pi = I omega 的方向在变化！）。约束变分恰好补回了这个差异——它产生额外的 ad*_xi 项，描述的正是 body frame 本身在旋转这个事实。

### 4.3.4 SO(3) 上的具体验证

在 SO(3) 上：g = R，xi = omega（body angular velocity），eta 也是 R^3 向量，Lie bracket [xi, eta] = omega x eta。

因此约束变分变为：
$$\delta\omega = \dot\eta + \omega \times \eta$$

这正是 Marsden-Ratiu Ch. 15 中刚体问题的标准结果。物理直觉：即使 eta 是固定的空间方向，body frame 中观察到的变分方向也会因旋转而变化——额外的 omega x eta 正是这个"随体旋转"效应。
...
我们现在有了两个关键要素：(1) 约化 Lagrangian l(xi)；(2) 约束变分 delta xi = dot{eta} + ad_xi eta。把它们代入 Hamilton 原理，就能推出几何力学的"主方程"——Euler-Poincare 方程。
...
其中 angle 表示 g* 与 g 之间的对偶配对（即把 dl/d xi in g* 作用在 delta xi in g 上）。

### 4.4.2 代入约束变分

将 delta xi = dot{eta} + ad_xi eta 代入：
...
对 I_2，利用 **coadjoint action** ad*_xi 的定义。对任意 mu in g*，eta in g：
...
**这就是 Euler-Poincare 方程**（加入外广义力 tau in g* 后的形式）。

### 4.4.6 方程各项的物理意义

| 项 | 含义 | R^n 类比 |
|---|---|---|
| dl/d xi in g* | body frame 广义动量 mu | p = dL/d q_dot |
| d/dt(dl/d xi) | body 动量的时间导数 | dp/dt |
| ad*_xi(dl/d xi) | **科里奥利/离心力在 body frame 中的表现** | Christoffel 项 Gamma^k_{ij} q_dot^i q_dot^j |
| tau in g* | 外广义力/力矩 | 外力 Q |

> **本质洞察**：ad*_xi (dl/d xi) 这一项不是"新的物理"，而是 body frame 本身在旋转的几何效应。在 spatial frame 中，自由刚体的动量守恒（dp/dt = 0）；但在 body frame 中，由于坐标系随体旋转，必须补偿这个旋转效应——这正是 ad* 项的角色。它和 R^n 中的 Christoffel 符号扮演完全相同的角色，只是在 Lie 群语言中表达得更加优雅和坐标无关。
...
**李代数**：so(3) 同构于 (R^3, x)，其中 x 是标准叉积。Lie bracket [xi, eta] = xi x eta。

**对偶配对**：angle{mu, eta} = mu . eta（标准内积）。

**ad 算子**：ad_xi eta = [xi, eta] = xi x eta。用矩阵表示：ad_xi = hat{xi}（反对称矩阵）。

**ad* 算子**：由定义 angle{ad*_xi mu, eta} = angle{mu, ad_xi eta} = mu . (xi x eta) = (mu x xi) . eta，因此：

$$\mathrm{ad}^*_\xi\,\mu = \mu \times \xi = -\xi \times \mu$$

**约化 Lagrangian**：l(omega) = 1/2 omega^T I omega，其中 I 是 body frame 转动惯量张量。

**广义动量**：mu = dl/d omega = I omega := pi（body angular momentum）。
...
对于自由旋转（tau = 0），Euler 方程给出 d/dt(I omega) = pi x omega。用 pi = I omega 改写：
...
取 g = se(3) 同构于 R^6，body spatial velocity xi = (omega; v) in R^6。se(3) 的 Lie bracket（即 Featherstone 的 **motion cross**）：
...
约化 Lagrangian：l(xi) = 1/2 xi^T G_b xi。

广义动量：mu = dl/d xi = G_b xi in se(3)*。
...
- 关节 q_j in T^n：n 个关节角度（T 是圆周群 S^1 或 R）

**广义速度**：v = (xi_b; q_dot_j) in R^{6+n}

- xi_b = g^{-1} g_dot 是基座的 body velocity（6 维）
- q_dot_j 是关节速度（n 维）

**关键不匹配**：位形维度 = 7 + n（四元数 4 维 + 平移 3 维 + n 关节），但速度维度 = 6 + n。这是因为 SO(3) 的参数化（4 维四元数）比其切空间（3 维 so(3)）多一维。

> **反事实推理**：如果你忽视 nq != nv，直接写 q_{k+1} = q_k + v_k * dt，会发生什么？四元数部分的模长会偏离 1（打破 SO(3) 约束），旋转矩阵不再正交，之后所有依赖 R 的计算全部错误。正确做法是 Lie 群积分：q_{k+1} = integrate(model, q_k, v_k * dt)，内部对基座部分使用 exp 映射。
...
- M_{bb} in R^{6x6}：基座的"锁定惯性"（locked inertia）——当所有关节锁死时，整机作为单刚体的 6D 惯性
...
其中：
- k in R^3：系统关于质心的总角动量
- l = m v_CoM in R^3：系统的总线动量
- v in R^{6+n}：广义速度
...
其中 F_ext,i 是第 i 个外力（以 wrench 形式，作用点的力矩关于 CoM）。
...
其中 delta F/delta mu in g 是函数 F: g* -> R 的"功能导数"（functional derivative），定义为 dF(mu) . delta_mu = angle{delta_mu, delta F/delta mu}。

"+" 和 "-" 分别对应右不变约化和左不变约化。大多数机器人学文献用左约化（body frame），对应"-"号。

### 4.10.2 Lie-Poisson 方程

对任意 Hamiltonian H: g* -> R，动力学由 Lie-Poisson 方程给出：

$$\dot\mu = \mathrm{ad}^*_{\delta H/\delta\mu}\,\mu$$

（使用"-"号 Poisson 括号时）。

**与 Euler-Poincare 的关系**：通过 Legendre 变换 H(mu) = angle{mu, xi} - l(xi)（其中 mu = dl/d xi），Lie-Poisson 方程等价于 Euler-Poincare 方程。两者是同一个动力学的 Lagrangian 和 Hamiltonian 两面。

### 4.10.3 SO(3) 上的 Lie-Poisson

取 g* = so(3)* ≅ R^3（通过 Killing form 认同），mu = pi（body angular momentum）。

Lie-Poisson 括号：
$$\{F, G\}(\pi) = -\pi \cdot (\nabla F \times \nabla G)$$

Hamiltonian：H(pi) = 1/2 pi^T I^{-1} pi（动能用动量表达）。
...
**SO(3) 具体化**：Ad* 就是 SO(3) 对 R^3 的标准旋转作用，其轨道是**同心球面** S^2_r = {pi in R^3 : |pi| = r}。

- Casimir C = |pi|^2 在每个轨道上为常数（因为 r 是轨道的半径）
- Hamiltonian H = 1/2 pi^T I^{-1} pi 的等值面（能量椭球）与球面 S^2_r 的交线就是 Euler 刚体的**轨迹曲线**
...
SE(3) 的余伴随轨道结构更复杂。设 mu = (m, f) in se(3)*（angular momentum, linear momentum），两个独立的 Casimir：

$$C_1(\mu) = m \cdot f = \|m\| \|f\| \cos\theta, \quad C_2(\mu) = |f|^2$$

其中 C_1 是"螺旋参数"（angular 和 linear momentum 的投影），C_2 是线动量模长平方。余伴随轨道因此是 4 维的（6 维 se(3)* 减去 2 个 Casimir 方向）。
...
其中 R in SO(3) 是旋转，v in R^3 是速度，p in R^3 是位移。
...
其中 U 和 W 只依赖于测量和已知量（重力），不依赖于 X 本身。这就是 Barrau-Bonnabel 的 **group-affine** 结构。

### 4.11.3 与 InEKF 的连接

当动力学是 group-affine 时，**左不变误差** e = X_true^{-1} X_est（或右不变误差 e = X_est X_true^{-1}）的动力学方程**不依赖于真实状态** X_true。这意味着：
...
> ⚠️ **概念误区**：SE_2(3) 和 SE(3) x R^3 是同一个东西
>
> **新手想法**：SE_2(3) 就是把 SE(3) 加一个 R^3 速度分量，和直积 SE(3) x R^3 没区别。
>
...
1. **（手推）**验证 SE_2(3) 的群乘法满足结合律 (T1 T2) T3 = T1 (T2 T3)。
2. **（概念）**解释为什么标准 EKF（在 R^9 上做 ESKF）对 IMU 积分不一致，但 InEKF（在 SE_2(3) 上）一致。关键差异在哪一步？
3. **（进阶）**写出 SE_2(3) 的 9x9 Adjoint matrix Ad_T 的显式形式。
