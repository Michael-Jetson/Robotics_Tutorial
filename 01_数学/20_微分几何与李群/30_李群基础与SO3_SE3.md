...
#### 25.1 SO(3) Rodrigues 公式的完整证明 ⭐⭐

从矩阵指数的 Taylor 级数出发：

$$
e^{\theta n^\wedge} = I + \theta n^\wedge + \frac{\theta^2}{2!}(n^\wedge)^2 + \frac{\theta^3}{3!}(n^\wedge)^3 + \frac{\theta^4}{4!}(n^\wedge)^4 + \cdots
$$
...
第二个等式的证明：$(n^\wedge)^3 = (n^\wedge)(nn^\top - I) = n^\wedge nn^\top - n^\wedge$。由于 $n^\wedge n = n \times n = 0$，所以 $n^\wedge nn^\top = 0$，从而 $(n^\wedge)^3 = -n^\wedge$。

由此得到循环性：

$$
(n^\wedge)^{2k+1} = (-1)^k (n^\wedge), \qquad (n^\wedge)^{2k} = (-1)^{k-1} (n^\wedge)^2 \quad (k \ge 1)
$$
...
**奇数项**：
$$
\theta n^\wedge - \frac{\theta^3}{3!}n^\wedge + \frac{\theta^5}{5!}n^\wedge - \cdots = \sin\theta \cdot n^\wedge
$$

**偶数项**（从 $k=1$ 开始）：
$$
\frac{\theta^2}{2!}(n^\wedge)^2 - \frac{\theta^4}{4!}(n^\wedge)^2 + \cdots = (1-\cos\theta)\cdot (n^\wedge)^2
$$
...
**为什么这个公式如此重要？** 它把无穷级数（矩阵指数）变成了三项有限和，使得 SO(3) 上的指数映射可以用封闭公式计算。没有 Rodrigues 公式，每次计算旋转的指数映射都需要矩阵的 Taylor 级数截断或 Pade 近似，计算量大幅增加。
