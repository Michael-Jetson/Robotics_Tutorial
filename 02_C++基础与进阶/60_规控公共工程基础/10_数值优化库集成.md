...
利用阶段结构后，Riccati 递推的复杂度是 $O(N \cdot n^3)$（每个阶段做一次 $n \times n$ 量级的分解），其中 $N$ 是阶段数，$n$ 是状态维度；而把同一问题当成通用稠密 QP 直接分解约为 $O((Nn)^3)$。
...
HPIPM 用 Riccati 递推代替通用稠密分解：递推复杂度是 $O(N \cdot n^3)$（$N$ 是阶段数，$n$ 是状态维度），而把同一问题当成通用稠密 QP 直接分解约为 $O((Nn)^3)$。
