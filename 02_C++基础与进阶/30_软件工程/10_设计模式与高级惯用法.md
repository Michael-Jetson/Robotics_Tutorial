...
> **难度**：⭐⭐～⭐⭐⭐⭐ | **建议用时**：1.5 周 | **前置要求**：C++语言进阶/类型转换 类型转换、C++语言进阶/变参模板折叠表达式与CRTP 模板基础、C++语言进阶/预处理器与宏 继承与多态

---

## 前置自测

> 📋 答不出 >= 2 题时，先回顾 C++语言进阶/类型转换、C++语言进阶/变参模板折叠表达式与CRTP、C++语言进阶/预处理器与宏 的核心概念。
...
3. **[跨章综合题]** 回顾 C++语言进阶/类型转换 中 `static_cast` 和 `dynamic_cast` 的区别。在 CRTP 中为什么使用 `static_cast<Derived&>(*this)` 而不是 `dynamic_cast`？如果写错了派生类类型参数（比如 `class SO3 : public LieGroupBase<SE3>`），会发生什么？
...
工厂模式的返回类型也值得注意。这里返回 `std::unique_ptr<Registration>` 而不是裸指针，明确表达了"调用方拥有对象"的所有权语义。回顾 C++语言进阶/预处理器与宏 中智能指针的所有权规则：`unique_ptr` 表示独占所有权，对象生命周期由持有者控制。如果工厂返回裸指针，调用方就不知道自己应不应该 `delete` 它——这正是 C++ 内存错误的经典来源。
...
1. **[C++语言进阶/类型转换 + 设计模式与高级惯用法]** 回顾 C++语言进阶/类型转换 中 `static_cast` 的安全边界。CRTP 中的 `static_cast<Derived&>(*this)` 如果 `Derived` 类型写错会触发未定义行为。设计一个编译期检查机制（使用 `static_assert` 或 C++20 Concepts），使这类错误在编译时被发现。
2. **[C++语言进阶/变参模板折叠表达式与CRTP + 设计模式与高级惯用法]** 回顾 C++语言进阶/变参模板折叠表达式与CRTP 中模板特化的机制。为 small_gicp 风格的 Traits 系统增加一个 `static_assert` 检查：如果某个类型没有提供 Traits 特化，在使用时给出清晰的错误信息而不是几百行模板展开错误。
