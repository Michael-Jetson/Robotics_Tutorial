...
到目前为止，本章覆盖了多线程 SLAM 系统的完整知识体系。这些知识点不是孤立的，而是按"为什么需要 → 怎么设计 → 怎么实现 → 怎么调试"的递进关系组织的。
...
C++/Python 混合开发
  └── 边界应在批量数据层面（§4, §11）
        ├── pybind11/nanobind 选型（§19）
        ├── GIL 管理（§19.4）
        └── 零拷贝 Eigen/NumPy（§19.3）

ROS2 交互
  └── Executor 是回调调度器，核心算法自管线程（§18）

实时隔离
  └── SCHED_FIFO + CPU 亲和性（§20）
```
...
1. 回顾 C++语言进阶/Eigen基础与SLAM数学预备 中 Eigen 固定大小矩阵的内存特点，解释为什么实时 Tracking 中应优先使用 `Eigen::Matrix3d` 和 `Eigen::Isometry3d`，而不是频繁创建动态矩阵。
...
## 18. ROS2 多线程 Executor 与 SLAM 线程交互 ⭐⭐⭐
...
### 18.1 ROS2 Executor 不是 SLAM 的线程调度器
...
### 18.2 数据通道设计：ROS2 回调 → 核心算法队列 → ROS2 发布
...
## 21. nanobind 的最小绑定示例与 pybind11 对比 ⭐⭐
