...
> **难度**：⭐⭐⭐～⭐⭐⭐⭐ | **建议用时**：2 周 | **前置要求**：C++语言进阶/类型系统与值类别推导-C++语言进阶/Lambda与STL算法 的 C++ 对象模型、RAII、异常、STL 容器，C++语言进阶/Eigen基础与SLAM数学预备-C++语言进阶/Concepts与Policy 的机器人数据结构和泛型接口基础
...
回顾 C++语言进阶/RAII与智能指针：RAII 保证"获取即初始化，析构即释放"——这在单线程中管理内存、文件和锁非常自然。本章的挑战是：当多个线程同时触碰同一批对象时，RAII 的"析构即释放"必须配合停止协议和队列关闭才能安全工作。析构函数不能在其他线程还在访问对象时就释放资源。回顾 C++语言进阶/移动语义与完美转发：移动语义让 `std::thread` 只能移动不能复制，这与 `std::unique_ptr` 的设计一脉相承——线程句柄是系统资源，复制没有清晰语义。
...
3. **[跨章综合题]**：C++语言进阶/RAII与智能指针 讲了 RAII 管理单线程资源生命周期。在多线程环境下，RAII 的”析构即释放”原则需要配合什么额外机制（停止协议、队列关闭）才能安全工作？
...
3. **[跨章综合题]**：C++语言进阶/移动语义与完美转发 讲了移动语义。`std::thread` 不可复制只能移动——解释这个设计与"线程句柄是系统资源"之间的关系。对比 `std::unique_ptr` 的设计（同样不可复制只能移动）。
...
3. **[跨章综合题]**：C++语言进阶/RAII与智能指针 的 RAII 保证"获取即初始化，析构即释放"。`std::jthread` 是 RAII 在线程管理上的应用。对比 `std::jthread` 和 `std::unique_ptr`：两者都是 RAII 包装器，分别管理什么资源？析构时分别做什么？
...
int main() {
    ...
    const int sum = std::accumulate(ids.begin(), ids.end(), 0);
...
3. **[跨章综合题]**：C++语言进阶/类型系统与值类别推导 讲了 C++ 的值类别和对象模型。在单线程中，对象的"未初始化"是常见陷阱；在多线程中，对象的"并发访问"是常见陷阱。对比两种陷阱的检测工具（单线程用 sanitizer，多线程用 ThreadSanitizer）。
...
3. **[跨章综合题]**：C++语言进阶/错误处理与异常安全 讲了异常安全。在持有锁时如果函数抛异常，`lock_guard` 会确保锁被释放（RAII）。讨论：如果在双锁临界区中，第一把锁获取成功但第二把锁获取前抛异常，系统状态是否一致？`scoped_lock` 如何处理这种情况？
...
3. **[跨章综合题]**：C++语言进阶/错误处理与异常安全 讲了异常安全和 `std::exception_ptr`。`std::future::get()` 会重新抛出任务中的异常——解释这个机制如何与 C++语言进阶/错误处理与异常安全 的异常传播模型配合，以及为什么线程入口函数不能直接抛异常给主线程。

---

## 17.11 `volatile` 不是线程同步 ⭐⭐⭐⭐⭐

### 工程问题：跨语言经验会误导 C++ 并发代码

很多开发者从 Java、C# 或嵌入式 C 经验出发，会把退出标志写成：

```cpp
volatile bool running = true;
```

然后一个线程写 `running = false`，另一个线程循环读 `running`。
在 C++ 中，这不是线程同步。
`volatile` 不提供原子性，不建立 happens-before，不保证跨线程可见性。
如果一个线程写普通 `volatile bool`，另一个线程读它，仍然可能 data race。

C++ 的 `volatile` 主要表达“每次访问都应作为可观察副作用”，典型场景是内存映射 I/O 和某些信号处理边界。
它不是 mutex，也不是 atomic。

### 反面失败：`volatile` 退出标志仍然 data race

下面是反例片段。
它可能在某些机器上看似工作，但 C++ 语义并不保证。
...
`volatile` 只限制编译器对该对象访问的某些优化。
它不让 `stop = true` 与另一个线程的读取建立同步关系。
对线程通信，应使用 `std::atomic<bool>`、mutex 保护状态、条件变量或 `std::stop_token`。

### `volatile` 的真实语义：从 C 语言历史到现代 C++ 的误解根源

要理解为什么 `volatile` 在 C++ 中不能用于线程同步，需要追溯到 `volatile` 被设计的原始场景和 C++ 内存模型的演进。

**`volatile` 的原始设计意图（1978-1989）。** `volatile` 最早出现在 C89 标准中，它的设计目标是告诉编译器："这个变量的值可能被当前程序控制流以外的机制改变——不要对它做基于'值没变'的优化。" 典型场景包括：(1) 内存映射 I/O 寄存器——硬件设备可能随时改变寄存器的值，编译器不能把上一次读取的值缓存在寄存器里重用；(2) 信号处理程序——异步信号处理函数可能在主程序的任意两条指令之间执行，修改全局变量。在这些场景中，`volatile` 确保编译器不会"优化掉"对该变量的读写——每次在源代码中出现的读操作都会真正执行一次内存访问，每次写操作都会真正写入内存。

**为什么 `volatile` 对线程同步无效？** `volatile` 只限制了编译器的优化行为，它没有影响两件对线程同步至关重要的事：(1) **CPU 指令重排**——现代 CPU 可以乱序执行指令，`volatile` 不插入任何内存屏障指令来阻止这种重排；(2) **缓存一致性协议的可见性延迟**——一个核心写入的数据可能停留在 store buffer 中，`volatile` 不强制刷新 store buffer 使数据对其他核心可见。换句话说，`volatile` 保证了"编译器生成的指令确实包含读/写操作"，但不保证"这些读/写操作在多核环境下以正确的顺序和时机被观察到"。

**Java/C# `volatile` 为什么不同？** Java 5（2004）和 C# 的 `volatile` 关键字具有完全不同的语义——它们隐含了内存屏障和线程可见性保证。Java 的 `volatile` 读具有 acquire 语义，写具有 release 语义。这是因为 Java 和 C# 的内存模型在设计时就考虑了多线程场景，把 `volatile` 定义为线程同步工具的一部分。而 C/C++ 的 `volatile` 早于多线程编程模型的标准化——C++ 直到 2011 年（C++11）才引入正式的内存模型和 `std::atomic`。因此，C++ 选择让 `volatile` 保持原始语义（硬件 I/O），用全新的 `std::atomic` 承担线程同步的职责。

**为什么在 x86 上"似乎能工作"？** 这是一个特别危险的陷阱。x86 架构有一个强顺序一致性的内存模型（Total Store Order, TSO）：store 操作按程序顺序对其他核心可见，load 操作不会被重排到 store 之前。因此在 x86 上，`volatile bool stop` 的读写"碰巧"会按预期工作——因为 x86 的硬件保证了可见性和顺序。但这是硬件行为，不是 C++ 语言保证。当同样的代码运行在 ARM（如 Jetson 平台、移动端）或 RISC-V 上时，弱内存模型允许更激进的重排，`volatile` 变量的写入可能永远不被另一个核心观察到。更隐蔽的是，即使在 x86 上，编译器仍然可以在 `volatile` 读写和**非 volatile** 读写之间进行重排——如果 stop 标志旁边还有其他共享数据，这种重排可能导致数据在 stop 信号之后才被看到。

### 抽象不变量：线程通信需要同步关系

线程间通信至少需要下面之一：

1. mutex unlock 与之后同一 mutex lock 建立同步。
2. condition_variable 等待和通知配合受锁保护的谓词。
3. atomic 操作建立原子访问和内存序关系。
4. thread join 建立被 join 线程结束前副作用对 join 后可见。
5. future/promise 的共享状态传递结果和异常。

`volatile` 不在这张表里。
它可以用于告诉编译器“这个内存位置可能被当前程序控制流之外的机制改变”，例如硬件寄存器。
但线程是 C++ 程序内部的并发控制流，语言已经提供 atomic 和锁作为同步工具。

### 规则推导：Java/C# `volatile` 与 C++ `volatile` 不是同一工具

Java 和 C# 的 `volatile` 带有线程可见性和内存屏障语义。
C++ 的 `volatile` 没有这些语义。
跨语言迁移时，不能把写法平移。
...
如果这个标志只表示“是否停止”，不携带其他数据发布关系，`memory_order_relaxed` 通常足够。
如果设置标志之前还写入了其他共享数据，并要求读到标志后看见那些数据，就要考虑 release/acquire 或直接用 mutex。
初学阶段优先使用默认顺序一致内存序，除非能清楚证明 relaxed 的边界。
...
### 代码验证：用 atomic 表达退出标志

下面代码用 `std::atomic<bool>` 表达停止请求。
它不传递复杂数据，只表达一个布尔状态。
...
这个例子里 `ticks` 也用 atomic，因为它被一个线程写、另一个线程在 join 后读。
严格说，`join()` 后读普通 `int` 也能看到线程结束前写入，因为 join 建立同步。
这里使用 atomic 是为了让计数本身在循环中也保持清晰的线程语义。

> ⚠️ **编程陷阱：用 `volatile bool` 作为线程停止标志**
> **错误做法**：`volatile bool stop = false;` 一个线程写 `stop = true`，另一个线程循环读 `stop`。
> **现象**：在某些编译器和优化级别下"看起来能工作"，但 C++ 标准层面这是 data race（未定义行为）。
> **根本原因**：C++ 的 `volatile` 不提供原子性，不建立 happens-before，不保证跨线程可见性。它只告诉编译器"不要优化掉对这个变量的访问"，这是为内存映射 I/O 设计的，不是为线程通信设计的。
> **正确做法**：使用 `std::atomic<bool>`、mutex 保护状态、或 `std::stop_token`。
...
3. **[跨章综合题]**：C++语言进阶/预处理器与宏 的条件编译宏和本节的 `volatile` 都是容易被误用的 C/C++ 特性。总结"看起来能用但语义错误"的共同特征：编译能过、在某些平台看似正常、但标准层面是错误的。
...
3. **[跨章综合题]**：C++语言进阶/预处理器与宏 用 `-E` 和 `nm -C` 验证宏和链接层面的正确性。本节用 TSan 验证并发正确性，用 perf 定位性能瓶颈。整理本课程中所有出现过的"编译/链接/运行时验证工具"，形成一个按问题域分类的工具箱。
