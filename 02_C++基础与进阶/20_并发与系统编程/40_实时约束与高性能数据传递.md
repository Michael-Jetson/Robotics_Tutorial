...
> **难度**：⭐⭐⭐⭐ | **建议用时**：2 周 | **前置要求**：线程管理与互斥同步 线程与同步，原子操作与内存模型 原子与内存模型，并行编程框架 并行框架，C++语言进阶/RAII与智能指针 RAII 与资源管理
...
如果实际只有一个 IMU 回调和一个前端线程，用 MPMC 通常没有必要。
先把拓扑写清楚，才能选择合适的数据通道。

### 抽象不变量：SPSC 环形队列只需要两个所有权方向
...
这样容量为 `N` 的数组最多存 `N-1` 个元素。
这不是浪费，而是简化状态表示。

### 代码验证：固定容量 SPSC ring buffer
...
1. 只适合单生产者单消费者。
2. `T` 的拷贝必须可接受。
3. `std::optional<T>` 返回值可能产生额外构造成本。
4. 如果 `T` 很大，应传递指针、索引或预分配块。

### 工程边界：满队列策略必须显式

生产者 `tryPush` 返回 false 时，不能假装没发生。
常见处理：
...
> ⚠️ **编程陷阱：SPSC 队列被多个生产者使用**
> **错误做法**：多个 IMU 回调线程同时向同一个 `SpscRingBuffer` 写入。
> **现象**：数据顺序混乱，偶发数据覆盖或读到半写状态。
> **根本原因**：SPSC 的正确性依赖于"只有一个写者修改 tail"这个不变量。多个写者同时修改 tail 会破坏环形缓冲的状态一致性。
> **正确做法**：确认拓扑确实是单生产者单消费者。如果有多个生产者，使用 MPSC 队列或先合并再入队。
...
1. **[代码题]** 测试 `SpscRingBuffer` 的边界条件：容量为 4 时最多能存多少元素？为什么？写测试验证满/空/绕回三种状态。
